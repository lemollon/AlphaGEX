...
class TestGEXDataSourcePriority:
    ...
    def test_market_hours_uses_tradier_first(self):
        ...
        import trading.heracles.signals as signals_module
...
class TestCacheLogic:
    ...
    @patch('trading.heracles.signals._persist_gex_cache_to_db')
    @patch('trading.heracles.signals._load_gex_cache_from_db')
    def test_overnight_uses_cached_data(self, mock_load_cache, mock_persist):
        ...
        with patch('trading.heracles.signals.datetime') as mock_datetime:
            mock_datetime.now.return_value = overnight_time
...
def run_all_tests():
    ...
    print("=" * 70)
    print("HERACLES GEX FLOW VERIFICATION TESTS")
    print("=" * 70)
    print()
    ...
    if tests_failed == 0:
        print("✅ ALL TESTS PASSED - HERACLES GEX flow is working correctly!")
        print()
        print("After deploy, HERACLES will:")
