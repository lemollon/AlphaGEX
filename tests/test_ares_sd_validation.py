"""
Test ARES SD-based strike validation fix.

This test verifies that ARES now correctly rejects GEX walls and Oracle strikes
that are less than 1.2 SD from spot, falling back to SD-based calculation.

The bug was that ARES used percentage-based validation (0.5%-5%) which could
...
class TestARESStrikeValidation:
    """Test ARES strike selection enforces minimum 1.2 SD."""

    @pytest.fixture
    def mock_config(self):
        """Create a mock ARES config."""
        config = MagicMock()
    ...
    def signal_generator(self, mock_config):
        ...
        with patch('trading.ares_v2.signals.get_gex_calculator', return_value=None):
            with patch('trading.ares_v2.signals.TRADIER_GEX_AVAILABLE', False):
                with patch('trading.ares_v2.signals.ORACLE_AVAILABLE', False):
                    with patch('trading.ares_v2.signals.ARES_ML_AVAILABLE', False):
