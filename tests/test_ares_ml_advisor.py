"""
ARES ML Advisor Tests

Tests for the ARES iron condor ML advisor.

Run with: pytest tests/test_ares_ml_advisor.py -v
"""
...
class TestAresMLAdvisorImport:
    """Tests for ARES ML advisor import"""

    def test_import_ares_advisor(self):
        """Test that ARES advisor can be imported"""
        try:
            from quant.ares_ml_advisor import AresMLAdvisor
            assert AresMLAdvisor is not None
        except ImportError:
            pytest.skip("ARES ML advisor not available")


class TestAresMLAdvisorInitialization:
    """Tests for ARES ML advisor initialization"""
