"""
Tests for WATCHTOWER Tradier Connection Fix

Verifies that:
1. Tradier credentials are properly loaded from APIConfig
2. WATCHTOWER endpoints return valid data (not data_unavailable)
...
class TestTradierCredentials:
    ...
    def test_tradier_data_fetcher_initializes(self):
        ...
        # Try sandbox credentials first (like WATCHTOWER does)
...
class TestWatchtowerEndpoints:
    ...
    def test_watchtower_data_source_status(self, client):
        """Test /api/watchtower/data-source-status endpoint"""
        response = client.get("/api/watchtower/data-source-status")
...
class TestDiscernmentTradierFix:
    """Test Discernment routes Tradier fix"""

    def test_discernment_get_tradier_exists(self):
        """Verify Discernment has get_tradier helper"""
        from backend.api.routes.discernment_routes import get_tradier
