"""
ICARUS Bot Comprehensive Test Suite
====================================
...
@pytest.fixture
def icarus_config():
    """Create ICARUS config for testing"""
    with patch('database_adapter.get_connection') as mock_conn:
        ...
        return ICARUSConfig(mode=TradingMode.PAPER)
...
def order_executor(mock_db_connection):
    ...
    with patch('database_adapter.get_connection', return_value=mock_db_connection):
        from trading.icarus.executor import OrderExecutor
        from trading.icarus.models import ICARUSConfig, TradingMode
...
class TestICARUSConfig:
    ...
    def test_config_validation_invalid_risk(self, mock_db_connection):
        ...
        with patch('database_adapter.get_connection', return_value=mock_db_connection):
            from trading.icarus.models import ICARUSConfig
            config = ICARUSConfig(risk_per_trade_pct=0)
            is_valid, msg = config.validate()
            assert not is_valid, "Zero risk should be invalid"

            config = ICARUSConfig(risk_per_trade_pct=25)
            is_valid, msg = config.validate()
    ...
    def test_config_validation_invalid_wall_filter(self, mock_db_connection):
        ...
        with patch('database_adapter.get_connection', return_value=mock_db_connection):
            from trading.icarus.models import ICARUSConfig
            config = ICARUSConfig(wall_filter_pct=60)
            is_valid, msg = config.validate()
    ...
    def test_config_validation_invalid_win_probability(self, mock_db_connection):
        ...
        with patch('database_adapter.get_connection', return_value=mock_db_connection):
            from trading.icarus.models import ICARUSConfig
            config = ICARUSConfig(min_win_probability=2.0)
            is_valid, msg = config.validate()
...
class TestSignalGenerator:
    ...
    def test_estimate_spread_pricing(self, signal_generator):
        ...
        from trading.icarus.models import SpreadType
...
class TestOrderExecutor:
    ...
    def test_spread_value_bull_call_max_profit(self, order_executor):
        ...
        from trading.icarus.models import SpreadPosition, SpreadType
    ...
    def test_paper_execution(self, order_executor, mock_db_connection):
        ...
        with patch('database_adapter.get_connection', return_value=mock_db_connection):
            ...
            assert position.position_id.startswith('ICARUS-'), "Position ID should start with ICARUS-"
...
class TestICARUSIntegration:
    """Integration tests for ICARUS trading flow"""

    def test_full_signal_to_execution_flow(self, mock_db_connection):
        """Test complete flow from signal to execution"""
        with patch('database_adapter.get_connection', return_value=mock_db_connection):
            from trading.icarus.models import ICARUSConfig, TradingMode, SpreadType
            from trading.icarus.signals import SignalGenerator
            from trading.icarus.executor import OrderExecutor
            ...
            # Execute trade
            from trading.icarus.models import TradeSignal
            signal = TradeSignal(
                direction=direction,
