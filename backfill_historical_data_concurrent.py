#!/usr/bin/env python3
"""
Historical Data Backfill Script - PostgreSQL Version
...
class HistoricalDataBackfiller:
    ...
    def _ensure_tables_exist(self):
        ...
        # Create gamma_history table if it doesn't exist (PostgreSQL syntax)
        c.execute("""
    ...
    def check_existing_data(self) -> Dict[str, int]:
        ...
        tables = {
            'gex_history': 'SELECT COUNT(*) FROM gex_history WHERE symbol = %s',
            'gamma_history': 'SELECT COUNT(*) FROM gamma_history WHERE symbol = %s',
            'gamma_daily_summary': 'SELECT COUNT(*) FROM gamma_daily_summary WHERE symbol = %s',
    ...
    def get_latest_timestamp(self, table: str) -> Optional[datetime]:
        ...
        try:
            if table == 'regime_signals':
                ...
                c.execute(f'SELECT MAX(timestamp) FROM {table} WHERE symbol = %s', (self.symbol,))

            result = c.fetchone()[0]
            if result:
                if hasattr(result, 'strftime'):
                    return result
                return datetime.strptime(str(result), '%Y-%m-%d %H:%M:%S')
        except Exception:
            pass
        ...
        for bar in price_data:
            ...
            if skip_existing:
                c.execute('''
                    ...
                    WHERE symbol = %s AND DATE(timestamp::timestamp) = %s
            ...
            for hour_offset in [0, 3, 7, 10]:
                ...
                ''', (
                    ...
                    gex['call_wall'],
                    gex['put_wall'],
                    bar['close'] * random.uniform(0.998, 1.002),
                    gex['mm_state'],
                    gex['regime'],
                    'Polygon_Backfill'
                ))
    ...
    def run_backfill(self, days: int = 365, skip_existing: bool = True):
        ...
        print("=" * 70)
        print("HISTORICAL DATA BACKFILL (PostgreSQL)")
        print("=" * 70)
