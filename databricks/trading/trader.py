...
2. Monitor open positions (profit target / stop loss / EOD close)
...
class Trader:
    ...
    def __init__(self, config: Optional[BotConfig] = None):
        ...
        self.db = TradingDatabase(
            bot_name=config.bot_name, dte_mode=config.dte_mode
        )
    ...
    def _recover_orphaned_positions(self):
        ...
        try:
            ...
            for position in open_positions:
                if in_window:
                    logger.info(
                        f"{self.config.bot_name}: Recovered {position.position_id} "
                        "— market open, resuming monitoring"
                    )
                    self.db.log(
                        "RECOVERY",
                        f"Orphaned {position.position_id} found — resuming",
                    )
                else:
                    logger.warning(
                        f"{self.config.bot_name}: Recovered {position.position_id} "
                        "— market closed, force-closing"
                    )
                    self.executor.close_paper_position(
                        position, position.total_credit, "server_restart_recovery"
                    )
                    self.db.log(
                        "RECOVERY",
                        f"Orphaned {position.position_id} force-closed (market closed)",
                    )
        except Exception as e:
            logger.error(f"{self.config.bot_name}: Position recovery failed: {e}")

    def run_cycle(self, close_only: bool = False) -> Dict[str, Any]:
        """Execute one complete trading cycle (main scheduler entry point)."""
        ...
        try:
            ...
            if account.buying_power < 200:
                ...
                result["details"]["reason"] = (
                    f"Buying power ${account.buying_power:.2f} < $200 minimum"
                )
            ...
            if max_contracts < 1:
                result["action"] = "insufficient_bp"
                result["details"]["reason"] = (
                    f"Can't afford 1 contract. "
                    f"BP=${account.buying_power:.2f}, "
                    f"Collateral=${collateral_per:.2f}"
                )
            ...
            result["details"] = {
                ...
                "collateral": collateral_per * max_contracts,
    ...
    def _manage_positions(self, now: datetime) -> Tuple[int, float]:
        ...
        for position in positions:
            # Close stale/expired positions from previous days
            position_date = None
            if position.open_time:
                position_date = (
                    position.open_time.strftime("%Y-%m-%d")
                    if hasattr(position.open_time, "strftime")
                    else str(position.open_time)[:10]
                )
            ...
            if close_price is None:
                ...
                self._mtm_failure_counts[pid] = (
                    self._mtm_failure_counts.get(pid, 0) + 1
                )
    ...
    def can_trade_today(self) -> Tuple[bool, int, str]:
        ...
        if count >= self.config.pdt_max_day_trades:
            return (
                False,
                count,
                f"PDT limit: {count}/{self.config.pdt_max_day_trades} day trades",
            )

        return True, count, f"OK: {count}/{self.config.pdt_max_day_trades} used"
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pdt": {
                "day_trades_rolling_5": max(0, pdt_count),
                "day_trades_remaining": max(
                    0, self.config.pdt_max_day_trades - max(0, pdt_count)
                ),
                "trades_today": trades_today,
                "can_trade": can_trade,
                "reason": pdt_msg,
            ...
            "scheduler": {
                ...
                "last_heartbeat": (
                    heartbeat.get("last_heartbeat") if heartbeat else None
                ),
                "heartbeat_status": (
                    heartbeat.get("status") if heartbeat else "UNKNOWN"
                ),
            },
        }
    ...
    def get_position_monitor(self) -> Optional[Dict[str, Any]]:
        ...
        if current_cost is not None:
            ...
            pnl_pct = (
                ((entry_credit - current_cost) / entry_credit * 100)
                if entry_credit > 0
                else 0
            )
        ...
        return {
            ...
            "pnl_total": (
                round(pnl_per_contract * position.contracts, 2)
                if current_cost
                else None
            ),
            "pnl_pct": round(pnl_pct, 1) if current_cost else None,
            "open_time": (
                position.open_time.isoformat() if position.open_time else None
            ),
...
def create_flame_trader() -> Trader:
    """Create a FLAME (2DTE) trader instance."""
    return Trader(config=flame_config())
