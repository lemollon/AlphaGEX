...
import { query, botTable, t, num, validateBot, dteMode, heartbeatName } from '@/lib/databricks'
import {
  getQuote,
...
function evaluateAdvisor(vix: number, spot: number, expectedMove: number, dteStr: string) {
  const BASE_WP = 0.65
  let winProb = BASE_WP
  ...
  // DTE factor
  if (dteStr === '2DTE') { const a = 0.03; winProb += a; factors.push(['DTE_2DAY_DECAY', a]) }
  ...
  try {
    ...
    const accountRows = await query(
      `SELECT id, current_balance, buying_power FROM ${botTable(bot, 'paper_account')}
       WHERE is_active = TRUE AND dte_mode = '${dte}' ORDER BY id DESC LIMIT 1`,
    )
    ...
    // 10. Insert position (all timestamps explicit for Databricks — no DEFAULT)
    const topFactorsJson = esc(JSON.stringify(adv.topFactors))
    const advReasoning = esc(adv.reasoning)
    ...
    await query(`
      INSERT INTO ${botTable(bot, 'positions')} (
        ...
        status, open_time, open_date, dte_mode, created_at, updated_at
      ) VALUES (
        ...
        '${advReasoning}', '${topFactorsJson}', false,
        false, ${spreadWidth}, ${spreadWidth},
        'PAPER', 'PAPER',
        'open', CURRENT_TIMESTAMP(), CURRENT_DATE(), '${dte}',
    ...
    await query(`
      ...
      VALUES (CURRENT_TIMESTAMP(), 'TRADE_OPEN',
        '${esc(`FORCE TRADE: ${positionId} ${strikes.putLong}/${strikes.putShort}P-${strikes.callShort}/${strikes.callLong}C x${maxContracts} @ $${credits.totalCredit.toFixed(4)}`)}',
        '${logDetails}', '${dte}')
    `)
    ...
    await query(`
      ...
      ) VALUES (CURRENT_DATE(), 'SPY', '${positionId}', CURRENT_TIMESTAMP(),
        ${maxContracts}, ${credits.totalCredit}, '${dte}', CURRENT_TIMESTAMP())
    `)

    // 15. Save equity snapshot
    ...
    await query(`
      ...
      VALUES (CURRENT_TIMESTAMP(), ${bal}, ${cumPnl}, 0, 1,
        '${esc(`force_trade:${positionId}`)}', '${dte}', CURRENT_TIMESTAMP())
    `)

    // 16. Update daily_perf (MERGE INTO for Databricks upsert)
    await query(`
      ...
      WHEN MATCHED THEN UPDATE SET
        trades_executed = target.trades_executed + 1,
        updated_at = CURRENT_TIMESTAMP()
    ...
    // 17. Update heartbeat (MERGE INTO for Databricks upsert)
    const hbDetails = esc(JSON.stringify({ action: 'traded', spot, vix }))
    await query(`
      MERGE INTO ${t('bot_heartbeats')} AS target
      USING (SELECT '${botName}' AS bot_name) AS source
      ON target.bot_name = source.bot_name
      WHEN MATCHED THEN UPDATE SET
      ...
      WHEN NOT MATCHED THEN INSERT (bot_name, last_heartbeat, status, scan_count, details)
        VALUES ('${botName}', CURRENT_TIMESTAMP(), 'active', 1, '${hbDetails}')
    `)
