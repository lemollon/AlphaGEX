"""Callbacks for the FLAME dashboard."""
...
_config = flame_config()
_db = TradingDatabase(bot_name="FLAME", dte_mode="2DTE")


def _get_status() -> dict:
    """Build status dict from database (no live trader needed for webapp)."""
...
def register_flame_callbacks(app):
    """Register all FLAME dashboard callbacks."""

    @app.callback(
        Output("flame-status-card", "children"),
        Input("flame-refresh", "n_intervals"),
    )
    def update_status(_):
        try:
            status = _get_status()
            return bot_status_card(status, "FLAME", color="warning")
        except Exception as e:
            return dbc.Alert(f"Error loading status: {e}", color="danger")

    @app.callback(
        Output("flame-equity-chart", "children"),
        Input("flame-refresh", "n_intervals"),
    )
    def update_equity(_):
        try:
            ...
            fig = create_equity_chart(
                curve, title="FLAME Equity Curve", color="#f59e0b"
            )
    ...
    @app.callback(
        Output("flame-performance-card", "children"),
        Input("flame-refresh", "n_intervals"),
    )
    def update_perf(_):
        try:
    ...
    @app.callback(
        Output("flame-open-positions", "children"),
        Input("flame-refresh", "n_intervals"),
    )
    ...
    @app.callback(
        Output("flame-trade-history", "children"),
        Input("flame-refresh", "n_intervals"),
    )
    def update_trades(_):
        try:
    ...
    @app.callback(
        Output("flame-logs-table", "children"),
        Input("flame-refresh", "n_intervals"),
    )
    def update_logs(_):
        try:
