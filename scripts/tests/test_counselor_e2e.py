#!/usr/bin/env python3
"""
COUNSELOR End-to-End Tests
======================

Tests all COUNSELOR agentic features to ensure they work before merge:
...
def test_knowledge_base():
    """Test that COUNSELOR knowledge base loads correctly"""
    log_section("TEST 1: Knowledge Base Loading")
    results = {"passed": 0, "failed": 0}

    try:
        from ai.counselor_knowledge import (
            DATABASE_TABLES,
            SYSTEM_ARCHITECTURE,
            TRADING_STRATEGIES,
        ...
        log_fail(f"Failed to import counselor_knowledge: {e}")
        results["failed"] += 5
...
def test_agentic_tools():
    """Test that COUNSELOR agentic tools are wired up"""
    ...
    try:
        ...
        log_fail(f"Failed to import counselor_tools: {e}")
        results["failed"] += 10
    except Exception as e:
        log_fail(f"Unexpected error: {e}")
        results["failed"] += 1
...
def test_personality_integration():
    ...
    try:
        ...
        log_fail(f"Failed to import counselor_personality: {e}")
        results["failed"] += 8
    except Exception as e:
        log_fail(f"Unexpected error: {e}")
        results["failed"] += 1
...
def test_api_endpoints():
    ...
    endpoints = [
        ("/api/ai/counselor/info", "GET", "COUNSELOR info"),
        ("/api/ai/counselor/welcome", "GET", "COUNSELOR welcome/briefing"),
    ]
...
def main():
    print(f"\n{'='*60}")
    print(f"{BLUE}COUNSELOR END-TO-END TESTS{RESET}")
    print(f"{'='*60}")
