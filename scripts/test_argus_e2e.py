#!/usr/bin/env python3
"""
ARGUS End-to-End Test Script
============================

Comprehensive testing for ARGUS live data feed, caching, and timezone handling.
...
def test_argus_api(base_url):
    """Test 2: Verify ARGUS API endpoints"""
    print(f"\n{BOLD}TEST 2: ARGUS API Endpoints{RESET}")
    print("=" * 50)
...
def test_cache_behavior(base_url):
    ...
    try:
        ...
        r1 = requests.get(f"{base_url}/api/argus/gamma", timeout=30)
        ...
        r2 = requests.get(f"{base_url}/api/argus/gamma", timeout=30)
...
def main():
    parser = argparse.ArgumentParser(description='ARGUS End-to-End Tests')
    ...
    print(f"\n{'=' * 60}")
    print(f"{BOLD}ARGUS END-TO-END TESTS{RESET}")
    print(f"{'=' * 60}")
