...
def test_db() -> bool:
    ...
    try:
        ...
        if missing:
            print(f"\n  {YELLOW}Tip:{RESET} Tables are auto-created on first VALOR import")
            return False
...
def test_scheduler_imports() -> bool:
    """Test that VALOR can be imported by scheduler."""
    print(f"\n{BOLD}Scheduler Import Check:{RESET}")
    try:
        from trading.valor import ValorTrader, ValorConfig, TradingMode
        print(f"  {GREEN}✓{RESET} ValorTrader imported")
        print(f"  {GREEN}✓{RESET} ValorConfig imported")
...
def main():
    """Run smoke tests."""
    print(f"\n{BOLD}{'='*60}{RESET}")
    print(f"{BOLD}VALOR Smoke Test{RESET}")
    print(f"{'='*60}")
    ...
    if test_api('/status', 'VALOR status'):
        passed += 1
    else:
        failed += 1
    ...
    if failed > 0:
        ...
        print(f"\n{BLUE}VALOR is ready for production!{RESET}")
        sys.exit(0)
