...
1. Prophet returns confidence value
...
def check_signal_accepts_oracle_confidence(bot_name: str, signals_path: str):
    ...
    try:
        ...
        if re.search(r'(IronCondorSignal|TradeSignal)\s*\(', content) and re.search(r'oracle_confidence\s*=\s*prophet', content):
...
def check_position_to_dict_has_oracle(bot_name: str, model_path: str):
    """Check that position.to_dict() includes prophet fields"""
    try:
        with open(model_path, 'r') as f:
            content = f.read()
...
def check_signals_passes_oracle_confidence(bot_name: str, signals_path: str):
    ...
    try:
        ...
        pattern = r'oracle_confidence\s*=\s*prophet.*confidence|oracle_confidence\s*=\s*oracle_confidence'
