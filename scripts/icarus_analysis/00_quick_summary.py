#!/usr/bin/env python3
"""
ICARUS Analysis 00: Quick Summary
=================================
...
r_before = query(f"""
    ...
    FROM icarus_positions WHERE status = 'closed' AND realized_pnl IS NOT NULL AND open_time < '{FIX_DATE}'
""")
r_after = query(f"""
    SELECT COUNT(*), SUM(CASE WHEN realized_pnl > 0 THEN 1 ELSE 0 END), ROUND(SUM(realized_pnl)::numeric, 2)
    FROM icarus_positions WHERE status = 'closed' AND realized_pnl IS NOT NULL AND open_time >= '{FIX_DATE}'
...
r = query("""
    ...
    FROM icarus_positions WHERE status = 'closed' AND oracle_confidence IS NOT NULL
""")
if r and r[0]:
    bad, avg = r[0]
    print(f"\nORACLE CONFIDENCE:")
