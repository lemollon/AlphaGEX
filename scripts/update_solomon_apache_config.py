#!/usr/bin/env python3
"""
Update SOLOMON config in database to match Apache GEX backtest optimal parameters.
...
Usage:
    python scripts/update_solomon_apache_config.py
"""
...
def update_solomon_config():
    """Update SOLOMON config in database with Apache parameters"""

    print("=" * 60)
    print("UPDATING SOLOMON CONFIG TO APACHE BACKTEST PARAMETERS")
    print("=" * 60)
    ...
    try:
        with db_connection() as conn:
            ...
            print("\n" + "=" * 60)
            print("SUCCESS: SOLOMON config updated to Apache parameters")
            print("=" * 60)
...
if __name__ == '__main__':
    ...
    print("\nThis will update the SOLOMON database config.")
    response = input("Proceed? [y/N]: ").strip().lower()

    if response == 'y':
        success = update_solomon_config()
