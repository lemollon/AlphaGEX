#!/usr/bin/env python3
"""
PROMETHEUS Standalone Test Suite
=================================
...
# by directly adding the trading/prometheus path
...
def test_rate_fetcher():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "rate_fetcher",
            os.path.join(os.path.dirname(__file__), "..", "trading", "prometheus", "rate_fetcher.py")
        )
...
def test_models():
    """Test PROMETHEUS models directly."""
    section("MODELS")
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "models",
            os.path.join(os.path.dirname(__file__), "..", "trading", "prometheus", "models.py")
        )
        models = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(models)

        # Test 1: PrometheusConfig scan start time
        ...
        # Test 2: PrometheusConfig scan end time
...
def test_api_endpoints():
    ...
    # Test /api/prometheus-box/status
    try:
        ...
        if resp.status_code == 200:
            ...
            if not missing:
                record_result("/api/prometheus-box/status", True, "All required fields present")
            else:
                record_result("/api/prometheus-box/status", False, f"Missing: {missing}")
            ...
            record_result("/api/prometheus-box/status", False, f"Status {resp.status_code}")

    except Exception as e:
        record_result("/api/prometheus-box/status", False, str(e), is_bug=False)

    # Test /api/prometheus-box/rate-analysis (critical for our changes)
    try:
        ...
        if resp.status_code == 200:
            ...
            record_result("/api/prometheus-box/rate-analysis", False, f"Status {resp.status_code}")

    except Exception as e:
        record_result("/api/prometheus-box/rate-analysis", False, str(e), is_bug=False)
...
def test_frontend_file():
    ...
    frontend_path = os.path.join(
        ...
        "frontend", "src", "app", "prometheus-box", "page.tsx"
    )
...
def test_signal_generator():
    ...
    try:
        ...
        sys.modules['trading.prometheus.rate_fetcher'] = rate_fetcher
        ...
        sys.modules['trading.prometheus.models'] = models
        models_spec.loader.exec_module(models)

        # Now try signals
        sig_spec = importlib.util.spec_from_file_location(
            "signals",
            os.path.join(os.path.dirname(__file__), "..", "trading", "prometheus", "signals.py")
        )
...
def main():
    print("=" * 60)
    print("PROMETHEUS STANDALONE TEST SUITE")
    print("=" * 60)
