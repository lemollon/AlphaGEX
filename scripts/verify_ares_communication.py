#!/usr/bin/env python3
"""
ARES Communication Verification Script

This script verifies the entire ARES data flow:
1. Backend routes are registered and accessible
2. ARES trader is importable and functional
...
def main():
    print_header("ARES COMMUNICATION VERIFICATION")
    ...
    # =========================================================================
    # CHECK 2: ARES Module Import
    # =========================================================================
    print_header("2. ARES MODULE IMPORT")

    try:
        from trading.ares_iron_condor import ARESTrader, TradingMode, ARESConfig
        check("ARESTrader class imported", True)
        check("TradingMode enum imported", True)
        check("ARESConfig dataclass imported", True)
    except ImportError as e:
        check("ARES module import", False, str(e))

    # =========================================================================
    # CHECK 3: ARES Initialization
    # =========================================================================
    print_header("3. ARES INITIALIZATION")

    try:
        ares = ARESTrader(mode=TradingMode.PAPER, initial_capital=200_000)
        check("ARESTrader instantiated", True)
        ...
        check("ARES initialization", False, str(e))
    ...
    try:
        if ares.tradier:
            ...
            # Get account balance
            balance = ares.tradier.get_account_balance()
            if balance:
            ...
            # Get SPY quote
            quote = ares.tradier.get_quote("SPY")
    ...
    try:
        expirations = ares.tradier.get_option_expirations('SPY')
        if expirations:
            ...
            # Get chain
            chain = ares.tradier.get_option_chain('SPY', expirations[0])
    ...
    try:
        ...
        check("BotName.ARES exists", hasattr(BotName, 'ARES'))
        print_info(f"  BotName.ARES = {BotName.ARES.value}")
    ...
    try:
        ...
        ares_status_route = '/bots/ares/status' in routes
        ares_run_route = '/bots/ares/run' in routes

        check("/bots/ares/status route exists", ares_status_route)
        check("/bots/ares/run route exists", ares_run_route)

        if not ares_status_route or not ares_run_route:
            print_info("  Available routes:")
            for r in routes:
                if 'ares' in r.lower() or 'bots' in r.lower():
                    print_info(f"    {r}")
    ...
    try:
        ...
        ares_capital = CAPITAL_ALLOCATION.get('ARES', 0)
        check("ARES capital allocated", ares_capital > 0, f"${ares_capital:,}")
        print_info(f"  ARES: ${ares_capital:,}")
    ...
    try:
        in_window = ares.is_trading_window()
        should_trade = ares.should_trade_today()
        ...
        print_info(f"  Config Window: {ares.config.entry_time_start} - {ares.config.entry_time_end}")
    ...
    if results['failed'] == 0:
        print(f"\n{GREEN}{'='*60}{RESET}")
        print(f"{GREEN}ALL CHECKS PASSED - ARES Communication is working!{RESET}")
