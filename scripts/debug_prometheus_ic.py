#!/usr/bin/env python3
"""
PROMETHEUS IC Trading Diagnostic Script

Run this script to understand why PROMETHEUS (Jubilee) isn't trading today.
...
def main():
    print("\n" + "="*60)
    print("   PROMETHEUS IC Trading Diagnostic")
    ...
    # Check if PROMETHEUS modules can be imported
    try:
        from trading.prometheus.trader import PrometheusICTrader
        from trading.prometheus.models import PrometheusICConfig, TradingMode
        from trading.prometheus.db import PrometheusDatabase
        check_result("PROMETHEUS IC modules import", True)
    except ImportError as e:
        check_result("PROMETHEUS IC modules import", False, str(e))
        print("\nCannot proceed without PROMETHEUS modules!")
        return
    ...
    if not config.require_oracle_approval:
        ...
        try:
            ...
            try:
                ...
                # Get Oracle advice
                prediction = oracle.get_pegasus_advice(
                    context=context,
                    use_gex_walls=True,
    ...
    try:
        # Get recent signals from prometheus_ic_signals table
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("""
            SELECT signal_id, signal_time, is_valid, skip_reason, oracle_confidence
            FROM prometheus_ic_signals
    ...
    if all_checks_passed:
        ...
        print("  PROMETHEUS should be able to trade.")
        ...
        print("  PROMETHEUS is not opening positions.")
        print("\033[0m")
