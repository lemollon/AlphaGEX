#!/usr/bin/env python3
"""
WATCHTOWER Performance Fixes Verification Script
============================================
...
2. WATCHTOWER engine initialization with ML models
...
def test_database_indexes():
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE tablename = 'watchtower_snapshots'
        ...
        expected_indexes = [
            'idx_watchtower_snapshots_snapshot_time_desc',
            'idx_watchtower_snapshots_snapshot_time_asc',
            'idx_watchtower_snapshots_date_time'
        ]
...
def test_watchtower_engine():
    """Test 2: WATCHTOWER engine initialization"""
    logger.info("\n" + "="*60)
    logger.info("TEST 2: WATCHTOWER Engine Initialization")
    logger.info("="*60)

    try:
        from core.watchtower_engine import get_watchtower_engine, initialize_watchtower_engine

        # Test get_watchtower_engine
        engine = get_watchtower_engine()
        check("WATCHTOWER engine singleton", engine is not None)

        # Test initialize function exists
        import inspect
        source = inspect.getsource(initialize_watchtower_engine)
        check("initialize_watchtower_engine has ML loading", "_get_ml_models" in source)

    except ImportError as e:
        check("WATCHTOWER engine import", False, str(e))
    except Exception as e:
...
def test_api_endpoints():
    ...
    try:
        ...
        endpoints = [
            ("/api/watchtower/gamma", "WATCHTOWER gamma"),
...
def test_watchtower_routes_optimization():
    """Test 8: WATCHTOWER routes optimization code"""
    logger.info("\n" + "="*60)
    logger.info("TEST 8: WATCHTOWER Routes Optimization")
    logger.info("="*60)

    try:
        with open('backend/api/routes/watchtower_routes.py', 'r') as f:
            source = f.read()
        ...
        check("WATCHTOWER routes file", False, str(e))
...
def test_frontend_parallel_fetch():
    ...
    try:
        with open('frontend/src/app/watchtower/page.tsx', 'r') as f:
            source = f.read()

        checks = [
            ("Promise.all for parallel fetch", "Promise.all([" in source),
            ("Last updated timestamp", "lastUpdated.toLocaleTimeString()" in source),
            ("Commentary debug logging", "[WATCHTOWER] Fetching commentary" in source),
...
2. Restart backend to pick up changes:
   - WATCHTOWER engine eager loading
   - New caching logic
   - Expected move validation

3. Check logs after restart for:
   - "WATCHTOWER engine initialized with ML models pre-loaded"
   - "FORTRESS Market Data: SPX=$XXXX, VIX=XX.XX, EM=$XX.XX"
   - "SOLOMON Skip Decision: VIX=XX.XX, Expected Move=X.XX%"

4. Test in browser:
   - Open WATCHTOWER page, check Strike Analysis has timestamp
   - Check browser console for [WATCHTOWER] logs
...
def main():
    logger.info("="*60)
    logger.info("WATCHTOWER Performance Fixes Verification")
    ...
    test_api_endpoints()
    test_watchtower_routes_optimization()
