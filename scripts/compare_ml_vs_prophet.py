#!/usr/bin/env python3
"""
Compare FORTRESS ML Advisor vs Prophet Advisor Performance
======================================================
...
2. Prophet Advisor (Multi-factor model with GEX, VIX, Claude AI integration)
...
def simulate_prophet_decisions(trades: List[Dict], prophet) -> Dict[str, Any]:
    """
    Simulate backtest with Prophet advisor filtering.
    Returns metrics for trades Prophet would have allowed.
    ...
    for trade in trades:
        try:
            ...
            prediction = prophet.get_prediction(
            ...
            # Check if Prophet would allow this trade
            ...
            # If Prophet fails, allow the trade (fail-safe)
            allowed_trades.append(trade)
...
def simulate_combined_decisions(trades: List[Dict], ml_advisor, prophet) -> Dict[str, Any]:
    """
    Simulate backtest with ML + Prophet working TOGETHER (how FORTRESS actually operates).

    Decision flow:
    1. VIX-based pre-filter (skip extreme conditions)
    2. ML provides base win probability
    3. Prophet adjusts based on GEX, VIX, market conditions
    ...
    for trade in trades:
        try:
            ...
            # ========== Step 2: Get Prophet Adjustments ==========
            ...
            if prophet:
                try:
                    ...
                    oracle_pred = prophet.get_prediction(
                        bot_name=BotName.FORTRESS,
                        vix=vix,
                        spot_price=spot_price,
                        expected_move=expected_move,
                    ...
                    # Prophet provides adjustments based on conditions
                    oracle_sd_mult = oracle_pred.suggested_sd_multiplier

                    # Calculate adjustment from Prophet's confidence vs baseline
            ...
            # Combined probability = ML base + Prophet adjustment (capped 0-1)
            combined_prob = max(0.0, min(1.0, ml_prob + oracle_adjustment))

            # Use Prophet's SD multiplier if it differs significantly from ML's
...
def run_comparison(start_date: str, end_date: str, initial_capital: float = 1_000_000):
    """
    Run comparison backtest between ML Advisor and Prophet Advisor.
    ...
    print("=" * 80)
    print("ADVISOR COMPARISON: ML Model vs Prophet")
    print("=" * 80)
    ...
    # =========================================================================
    print("\n" + "-" * 80)
    print("PHASE 1: Running Baseline Backtest (CHRONICLES)")
    print("-" * 80)
    ...
    try:
        ...
        print("  Prophet Advisor initialized")
    except Exception as e:
        print(f"  WARNING: Prophet Advisor initialization failed: {e}")

    if not ml_advisor and not prophet:
    ...
    if prophet:
        print("  Simulating Prophet Advisor decisions...")
        oracle_results = simulate_prophet_decisions(all_trades, prophet)
        print(f"    Allowed: {oracle_results['allowed_trades']} | Skipped: {oracle_results['skipped_trades']}")

    # Combined ML + Prophet (how FORTRESS actually operates)
    combined_results = None
    if ml_advisor or prophet:
        print("  Simulating COMBINED ML + Prophet decisions (FORTRESS mode)...")
        combined_results = simulate_combined_decisions(all_trades, ml_advisor, prophet)
    ...
    print(f"\n{'Metric':<25} {'Baseline':>12} {'ML Only':>12} {'Prophet Only':>12} {'COMBINED':>12}")
    print("-" * 85)
    ...
    print(f"    Prophet Only:  {oracle_score}")
    print(f"    COMBINED:     {combined_score}")

    # Determine winner
    scores = [('ML Only', ml_score), ('Prophet Only', oracle_score), ('COMBINED', combined_score)]
    scores = [(n, s) for n, s in scores if s > 0]
    if scores:
        scores.sort(key=lambda x: x[1], reverse=True)
        winner = scores[0][0]
        if winner == 'COMBINED':
            reason = "ML + Prophet together outperforms either alone (synergy effect)"
        elif winner == 'ML Only':
            reason = "Pure ML pattern recognition performed best"
        else:
            reason = "Prophet's multi-factor analysis won"
    else:
        winner = "BASELINE"
    ...
    if combined_results:
        ...
        if combined_score >= max(ml_score, oracle_score):
            ...
            print(f"    Prophet-only mode shows promise - rely more on GEX/VIX signals")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='Compare ML Advisor vs Prophet Advisor')
