#!/usr/bin/env python3
"""
CHRONICLES Full End-to-End Test Suite

Tests all CHRONICLES features including:
...
def test_infrastructure():
    """Test CHRONICLES infrastructure components"""
    log_section("1. INFRASTRUCTURE TESTS")
    results = []

    # Test 1.1: Infrastructure module import
    try:
        from backend.services.chronicles_infrastructure import (
            job_store, connection_pool, orat_cache, get_infrastructure_status
        )
        log_test("Import chronicles_infrastructure", True)
        results.append(True)
    except ImportError as e:
        log_test("Import chronicles_infrastructure", False, str(e))
...
def test_rest_api():
    """Test CHRONICLES REST API endpoints"""
    ...
    try:
        resp = _get("/api/chronicles/infrastructure")
        passed = resp.status_code == 200
        if passed:
            data = resp.json()
...
def test_websocket():
    ...
    if USE_TEST_CLIENT:
        try:
            with _test_client.websocket_connect("/ws/chronicles/job/test_job") as websocket:
                # Send ping
    ...
    async def ws_test():
        ...
        try:
            async with websockets.connect(
                f"{ws_url}/ws/chronicles/job/test_job",
                close_timeout=2
            ) as ws:
                # Send ping
                await ws.send("ping")
...
def main():
    parser = argparse.ArgumentParser(description="CHRONICLES Full Test Suite")
    parser.add_argument("--live", action="store_true", help="Run live backtest tests")
    args = parser.parse_args()

    print(f"\n{Colors.BOLD}{Colors.CYAN}CHRONICLES FULL TEST SUITE{Colors.RESET}")
    if USE_TEST_CLIENT:
