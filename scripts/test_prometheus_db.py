#!/usr/bin/env python
"""
PROMETHEUS Database Verification Script for Render Shell

Run this script in the Render shell to verify all PROMETHEUS database tables and data:
    python scripts/test_prometheus_db.py
...
# Required tables for PROMETHEUS
REQUIRED_TABLES = [
    # Box Spread tables
    ...
    'prometheus_ic_config',
    'prometheus_ic_equity_snapshots',
]

# Required indexes
REQUIRED_INDEXES = [
    'idx_prometheus_ic_positions_status',
    'idx_prometheus_ic_positions_open_time',
    'idx_prometheus_ic_closed_trades_close_time',
    'idx_prometheus_ic_signals_time',
    'idx_prometheus_ic_signals_executed',
    'idx_prometheus_ic_equity_snapshots_time',
    'idx_prometheus_logs_action',
]
...
def check_data_queries(conn) -> Tuple[int, int, List[str]]:
    ...
    queries = [
        ("Get open box positions", """
            SELECT COUNT(*) FROM prometheus_positions WHERE status = 'open'
        """),
        ("Get closed box positions", """
            SELECT COUNT(*) FROM prometheus_positions WHERE status = 'closed'
        """),
        ("Get open IC positions", """
            SELECT COUNT(*) FROM prometheus_ic_positions WHERE status IN ('open', 'pending')
        """),
...
def check_foreign_keys(conn) -> Tuple[int, int, List[str]]:
    ...
    checks = [
        ("IC positions reference valid box positions", """
            SELECT COUNT(*) FROM prometheus_ic_positions p
            WHERE p.source_box_position_id IS NOT NULL
            AND NOT EXISTS (
                SELECT 1 FROM prometheus_positions bp
        ...
        ("IC signals reference valid positions", """
            SELECT COUNT(*) FROM prometheus_ic_signals s
            WHERE s.executed_position_id IS NOT NULL
            AND s.was_executed = TRUE
            AND NOT EXISTS (
                SELECT 1 FROM prometheus_ic_positions p
...
def check_prometheus_config(conn) -> Tuple[int, int, List[str]]:
    """Verify PROMETHEUS configurations are properly stored."""
    cursor = conn.cursor()
    passed = 0
    failed = 0
    failed_configs = []
    ...
    try:
        cursor.execute("""
            SELECT config_data FROM prometheus_config WHERE config_key = 'default'
        """)
        row = cursor.fetchone()
        if row and row[0]:
            config = row[0]
    ...
    try:
        cursor.execute("""
            SELECT config_data FROM prometheus_ic_config WHERE config_key = 'default'
        """)
        row = cursor.fetchone()
        if row and row[0]:
            config = row[0]
...
def main():
    """Main entry point."""
    print(f"\n{BOLD}PROMETHEUS Database Verification Script{RESET}")
    ...
    # Check configs
    passed, failed, failures = check_prometheus_config(conn)
    total_passed += passed
    total_failed += failed
    all_failures.extend(failures)
