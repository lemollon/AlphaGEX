#!/usr/bin/env python3
"""
SOLOMON GEX Directional Strategy - Optimizer & ML Trainer

This script:
1. Trains the MAGNET theory ML model
2. Tests the SOLOMON strategy with different configurations
...
Usage:
    ...
    python scripts/optimize_solomon_strategy.py --train-ml

    # Run backtest with VIX filter + 3-year period
    python scripts/optimize_solomon_strategy.py --vix-filter

    # Run full optimization suite
    python scripts/optimize_solomon_strategy.py --full
"""
...
def run_solomon_backtest(
    ...
    """Run SOLOMON directional backtest with specified parameters"""
...
def main():
    parser = argparse.ArgumentParser(
        description='SOLOMON GEX Directional Strategy - Optimizer & ML Trainer',
...
Examples:
  ...
  python scripts/optimize_solomon_strategy.py --backtest --start 2022-01-01 --end 2024-12-31
  ...
  python scripts/optimize_solomon_strategy.py --full
    ...
    if args.vix_filter:
        print("\n" + "=" * 70)
        print("Running SOLOMON with VIX 15-25 Filter (Best Risk-Adjusted)")
        print("=" * 70)
        run_solomon_backtest(
            start_date=args.start,
            end_date=args.end,
            ticker=args.ticker,
            min_vix=15,
    ...
    if not any([args.train_ml, args.backtest, args.vix_filter, args.full]):
        ...
        print("Quick Start:")
        print("=" * 70)
        print("\n1. Train ML model:     python scripts/optimize_solomon_strategy.py --train-ml")
        print("2. Test VIX filter:    python scripts/optimize_solomon_strategy.py --vix-filter")
        print("3. Full optimization:  python scripts/optimize_solomon_strategy.py --full")
