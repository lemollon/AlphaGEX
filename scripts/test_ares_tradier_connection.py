#!/usr/bin/env python3
"""
ARES-Tradier Connection Test Script
====================================

Run this in Render shell to verify ARES can communicate with Tradier.
...
def test_place_trade(tradier, spy_price):
    ...
    try:
        ...
        print(f"\n  ARES initialized in {trader.config.mode.value} mode")
...
def main():
    parser = argparse.ArgumentParser(description='Test ARES-Tradier connection')
    parser.add_argument('--trade', action='store_true', help='Also test placing a trade (DANGEROUS)')
    args = parser.parse_args()

    print("\n" + "=" * 60)
    print("  ARES-TRADIER CONNECTION TEST")
    ...
    if passed == total:
        print("\n  🎉 All tests passed! ARES-Tradier connection is working.")
    else:
