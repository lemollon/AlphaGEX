#!/usr/bin/env python3
"""
End-to-End Test: ARES SPY Sandbox Trading

This script tests the complete ARES SPY sandbox trading flow:
...
def test_ares_initialization():
    """Test 3: Initialize ARES in sandbox mode."""
    print_header("TEST 3: ARES Trader Initialization (Sandbox Mode)")

    try:
        from trading.ares_iron_condor import ARESTrader, TradingMode, ARESConfig

        # Create config that forces SPY sandbox mode
        config = ARESConfig(paper_trade_spx=False)  # Use SPY instead of SPX

        # Initialize ARES in PAPER mode (which will use sandbox)
        ares = ARESTrader(
        ...
        has_sandbox = ares.tradier_sandbox is not None

        print_result(
            "ARES Initialized",
            True,
            f"Mode: {ares.mode.value}\n"
        ...
        import traceback
        print_result("ARES Initialized", False, f"{e}\n{traceback.format_exc()}")
        return None


def test_market_data(ares):
    """Test 4: Fetch current market data."""
    print_header("TEST 4: Market Data Fetching")

    if not ares:
        print_result("Market Data", False, "ARES not initialized")
        return None
...
def test_iron_condor_strikes(ares, market_data):
    """Test 5: Find Iron Condor strikes."""
    print_header("TEST 5: Iron Condor Strike Finding")

    if not ares or not market_data:
        print_result("Strike Finding", False, "Missing ARES or market data")
        return None
    ...
    try:
        ...
        # Find strikes
        strikes = ares.find_iron_condor_strikes(market_data, expiration)
...
def test_sandbox_order(ares, strikes, expiration, market_data):
    """Test 6: Place sandbox Iron Condor order."""
    print_header("TEST 6: Sandbox Order Placement")

    if not ares or not strikes:
        print_result("Order Placement", False, "Missing ARES or strikes")
        return None

    if not ares.tradier_sandbox:
        print_result(
            "Order Placement",
            False,
            "Tradier sandbox client not available!\n"
            "Check that TRADIER_SANDBOX_API_KEY and TRADIER_SANDBOX_ACCOUNT_ID are set,\n"
            "or that paper_trade_spx=False in ARESConfig."
        )
        return None

    try:
        # Execute the trade (1 contract for testing)
        position = ares.execute_iron_condor(
            ic_strikes=strikes,
            contracts=1,
...
def run_all_tests():
    ...
    print("\n" + "="*60)
    print("  ARES SPY SANDBOX TRADING - END-TO-END TEST")
    print("="*60)
    ...
    # Test 3: ARES Initialization
    ares = test_ares_initialization()
    results.append(("ARES Init", ares is not None))

    if not ares:
        print("\n⚠️  Could not initialize ARES. Check configuration.")
        return results

    # Test 4: Market Data
    market_data = test_market_data(ares)
    ...
    strikes, expiration = test_iron_condor_strikes(ares, market_data)
    ...
    position = test_sandbox_order(ares, strikes, expiration, market_data)
    ...
    if passed == total:
        print("\n  🎉 ALL TESTS PASSED - ARES SPY Sandbox is ready!")
    else:
