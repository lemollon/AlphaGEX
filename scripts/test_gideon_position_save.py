#!/usr/bin/env python3
"""
GIDEON Position Save Test Script
=================================

Tests that GIDEON can:
...
def test_db_schema():
    """Test 1: Verify gideon_positions table has all required columns."""
    print("\n" + "="*60)
    ...
    try:
        ...
        # Get all columns from gideon_positions table
        c.execute("""
            SELECT column_name, data_type
            FROM information_schema.columns
            WHERE table_name = 'gideon_positions'
        ...
        print(f"\nFound {len(columns)} columns in gideon_positions table:")
        ...
        if missing:
            ...
            print("   Run the GIDEON trader once to trigger _ensure_ml_columns migration")
            return False
...
def test_icarus_init():
    ...
    try:
        ...
        print("\n✅ PASS: GIDEON initialized successfully")
        return trader
...
def test_signal_generation(trader):
    """Test 3: Verify GIDEON can generate a signal."""
    print("\n" + "="*60)
...
def test_position_save(trader):
    """Test 4: Verify GIDEON can save a position to database."""
    print("\n" + "="*60)
    ...
    try:
        ...
        test_id = f"TEST-GIDEON-{now.strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
...
def main():
    """Run all GIDEON tests."""
    print("\n" + "="*60)
    print("GIDEON POSITION SAVE TEST SUITE")
    ...
    if failed > 0:
        ...
        print("\n🎉 GIDEON position save is working correctly!")
        return 0
    else:
