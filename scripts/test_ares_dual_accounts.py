#!/usr/bin/env python3
"""
Test script to verify ARES dual sandbox account configuration.
...
def test_environment_variables():
    ...
    vars_to_check = [
        ...
        ("TRADIER_ARES_SANDBOX_API_KEY_2", "Second sandbox API key"),
        ("TRADIER_ARES_SANDBOX_ACCOUNT_ID_2", "Second sandbox account ID"),
    ]
...
def test_both_accounts():
    ...
    # Second account
    key2 = os.getenv('TRADIER_ARES_SANDBOX_API_KEY_2')
    acct2 = os.getenv('TRADIER_ARES_SANDBOX_ACCOUNT_ID_2')

    if key2 and acct2:
        results['secondary'] = test_account_connectivity(key2, acct2, "Second Sandbox Account (ARES Mirror)")
    else:
...
def test_ares_executor_init():
    """Test that ARES OrderExecutor initializes both connections."""
    print("\n" + "=" * 60)
    print("3. TESTING ARES EXECUTOR INITIALIZATION")
    print("=" * 60)

    try:
        from trading.ares_v2.models import ARESConfig, TradingMode
        from trading.ares_v2.executor import OrderExecutor

        # Create config in LIVE mode (which uses sandbox)
        config = ARESConfig(mode=TradingMode.LIVE)
        ...
        print(f"\n  [FAIL] Could not initialize ARES executor: {e}")
        import traceback
        traceback.print_exc()
        return False, False
...
def main():
    print("\n" + "=" * 60)
    print("  ARES DUAL SANDBOX ACCOUNT TEST")
    print("=" * 60)
    ...
    # Test 3: ARES executor
    primary_ok, secondary_ok = test_ares_executor_init()
    ...
    if primary_ok and secondary_ok:
        print("  RESULT: ARES will mirror trades to BOTH accounts")
        ...
        print("  RESULT: Primary account not working - ARES cannot trade")
    print("-" * 60 + "\n")
