"""
GIDEON - Order Executor
========================

Clean order execution via Tradier (paper or live).

GIDEON uses aggressive position sizing:
...
from .models import (
    ...
    TradeSignal, GideonConfig, TradingMode, CENTRAL_TZ
)
...
class OrderExecutor:
    ...
    def __init__(self, config: GideonConfig):
        ...
        if TRADIER_AVAILABLE and config.mode != TradingMode.PAPER:
            try:
                ...
                logger.info("GIDEON OrderExecutor: Tradier initialized for LIVE trading")
            except Exception as e:
                logger.error(f"GIDEON OrderExecutor: Tradier init failed: {e}")
        ...
        if POSITION_MGMT_AVAILABLE:
            try:
                ...
                logger.info("GIDEON OrderExecutor: Position Management Agent initialized")
            except Exception as e:
    ...
    def _execute_paper(self, signal: TradeSignal, thompson_weight: float = 1.0) -> Optional[SpreadPosition]:
        ...
        try:
            ...
            position_id = f"GIDEON-{now.strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
            ...
            logger.info(
                f"GIDEON PAPER TRADE: {signal.spread_type.value} "
    ...
    def _execute_live(self, signal: TradeSignal, thompson_weight: float = 1.0) -> Optional[SpreadPosition]:
        ...
        try:
            ...
            position = SpreadPosition(
                position_id=f"GIDEON-LIVE-{order_id}",
                spread_type=signal.spread_type,
                ticker=self.config.ticker,
                long_strike=signal.long_strike,
                short_strike=signal.short_strike,
            ...
            logger.info(
                f"GIDEON LIVE TRADE: {signal.spread_type.value} "
                f"{signal.long_strike}/{signal.short_strike} "
        ...
        try:
            ...
            try:
                ...
                if mtm.get('success') and mtm.get('current_value') is not None:
                    ...
                    logger.info(f"GIDEON PAPER CLOSE MTM: {position.position_id} close_value=${close_price:.4f}")
            except Exception as e:
            ...
            logger.info(
                f"GIDEON PAPER CLOSE: {position.position_id} "
    ...
    def _close_live(
        ...
        try:
            ...
            logger.info(
                f"GIDEON LIVE CLOSE: {position.position_id} "
                f"@ ${close_price:.2f} P&L: ${realized_pnl:.2f} [{reason}]"
            )
    ...
    def _get_kelly_position_size(self) -> Optional[Dict]:
        """Get Monte Carlo Kelly-based position sizing for GIDEON."""
        ...
        try:
            ...
            # BUG FIX: Query gideon_positions table instead of autonomous_closed_trades
            ...
            logger.info(f"[GIDEON KELLY] Win Rate: {win_rate:.1%}, Safe Kelly: {kelly_result['kelly_safe']:.1f}%")
            return kelly_result
    ...
    def _calculate_position_size(self, max_loss_per_contract: float, thompson_weight: float = 1.0) -> int:
        ...
        GIDEON uses aggressive 4% risk per trade, but Kelly can optimize further.
        """
        capital = 100_000
        ...
        if kelly_result and kelly_result.get('kelly_safe', 0) > 0:
            ...
            sizing_source = "KELLY"
            logger.info(f"[GIDEON] Using Kelly-safe sizing: {kelly_risk_pct:.1f}% risk")
        else:
        ...
        if abs(thompson_weight - 1.0) > 0.05:
            logger.info(f"[GIDEON {sizing_source}] Thompson: weight={thompson_weight:.2f}, base={base_contracts:.1f}, adjusted={adjusted_contracts}")
    ...
    def store_entry_conditions(self, position_id: int, gex_data: Dict) -> bool:
        ...
        try:
            ...
            logger.info(f"[GIDEON] Entry conditions stored for position {position_id}")
            return True
        except Exception as e:
        ...
        try:
            ...
            if result.get('alerts'):
                for alert in result['alerts']:
                    logger.info(f"[GIDEON CONDITION ALERT] {alert['type']}: {alert['message']}")
