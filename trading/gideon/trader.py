"""
GIDEON - Main Trading Orchestrator
====================================
...
from .models import (
    SpreadPosition, PositionStatus, GideonConfig,
...
try:
    ...
    print("✅ GIDEON: ML Data Gatherer loaded")
except ImportError as e:
    ML_GATHERER_AVAILABLE = False
    gather_ml_data = None
    print(f"⚠️ GIDEON: ML Data Gatherer not available: {e}")
...
class GideonTrader(MathOptimizerMixin):
    """
    GIDEON - Aggressive directional spread trader.
    ...
    def __init__(self, config: Optional[GideonConfig] = None):
        """Initialize GIDEON trader."""
        # Initialize database layer FIRST
        self.db = GideonDatabase(bot_name="GIDEON")
        ...
        if not is_valid:
            logger.error(f"GIDEON config validation failed: {error}")
            raise ValueError(f"Invalid GIDEON configuration: {error}")
        ...
        if MATH_OPTIMIZER_AVAILABLE:
            self._init_math_optimizers("GIDEON", enabled=False)
            logger.info("GIDEON: Math optimizers DISABLED - Prophet controls all trading decisions")

        logger.info(
            f"GIDEON initialized: mode={self.config.mode.value}, "
    ...
    def run_cycle(self, close_only: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if close_only:
                logger.info("GIDEON running in CLOSE_ONLY mode - managing positions only")
    ...
    def _record_oracle_outcome(self, pos: SpreadPosition, close_reason: str, pnl: float):
        ...
        try:
            ...
            if success:
                logger.info(f"GIDEON: Recorded outcome to Prophet - {outcome.value}, Dir={direction_predicted}, Correct={direction_correct}, P&L=${pnl:.2f}")

        except Exception as e:
            logger.warning(f"GIDEON: Prophet outcome recording failed: {e}")
    ...
    def _record_proverbs_outcome(
        ...
        try:
            ...
            alerts = enhanced.record_trade_outcome(
                ...
                strategy_type='DIRECTIONAL',  # GIDEON is a Directional bot
            ...
            if alerts:
                for alert in alerts:
                    logger.warning(f"GIDEON Proverbs alert: {alert}")
            logger.debug(f"GIDEON: Recorded outcome to Proverbs - P&L=${pnl:.2f}, Dir={direction_predicted}, Correct={direction_correct}")
        except Exception as e:
            logger.warning(f"GIDEON: Proverbs outcome recording failed: {e}")
    ...
    def _record_thompson_outcome(self, pnl: float):
        ...
        This updates the Beta distribution parameters for GIDEON,
        ...
        try:
            record_bot_outcome('GIDEON', win=(pnl > 0), pnl=pnl)
            logger.debug(f"GIDEON: Recorded outcome to Thompson Sampling - P&L=${pnl:.2f}")
        except Exception as e:
            logger.warning(f"GIDEON: Thompson outcome recording failed: {e}")
    ...
    def _record_learning_memory_prediction(self, pos: SpreadPosition, signal) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"GIDEON: Learning Memory prediction recorded: {prediction_id}")
            return prediction_id

        except Exception as e:
            logger.warning(f"GIDEON: Learning Memory prediction failed: {e}")
            return None
    ...
    def _record_learning_memory_outcome(self, prediction_id: str, pnl: float, close_reason: str):
        ...
        try:
            ...
            logger.info(f"GIDEON: Learning Memory outcome recorded: correct={was_correct}")

        except Exception as e:
            logger.warning(f"GIDEON: Learning Memory outcome recording failed: {e}")
    ...
    def _store_oracle_prediction(self, signal, position: SpreadPosition) -> int | None:
        ...
        try:
            ...
            prediction_id = prophet.store_prediction(
                ...
                strategy_recommendation='DIRECTIONAL'  # Migration 023: GIDEON uses Directional strategy
            )

            if prediction_id and isinstance(prediction_id, int):
                logger.info(f"GIDEON: Prophet prediction stored for {trade_date} (id={prediction_id}, Dir={direction_predicted}, Win Prob: {prediction.win_probability:.0%})")
                ...
                logger.info(f"GIDEON: Prophet prediction stored for {trade_date} (Win Prob: {prediction.win_probability:.0%})")
                return None
            else:
                logger.warning(f"GIDEON: Failed to store Prophet prediction for {trade_date}")
                return None

        except Exception as e:
            logger.warning(f"GIDEON: Prophet prediction storage failed: {e}")
            return None
    ...
    def _get_force_exit_time(self, now: datetime, today: str) -> datetime:
        ...
        if MARKET_CALENDAR_AVAILABLE and MARKET_CALENDAR:
            ...
            if early_close_force < config_force_time:
                logger.info(f"GIDEON: Early close day - adjusting force exit to {early_close_force.strftime('%H:%M')}")
    ...
    def _check_exit_conditions(
        ...
        if current_value is None:
            ...
            logger.warning(f"[GIDEON EXIT] Pricing unavailable for {pos.position_id}")
            ...
            if minutes_to_force <= 30 and pos.expiration == today:
                logger.warning(f"[GIDEON EXIT] Force closing {pos.position_id} - pricing failed but {minutes_to_force:.0f}min to force exit")
    ...
    def _log_scan_activity(
        ...
        try:
            ...
            # Extract Prophet data from context (fetched early for all scans)
            ...
            if prophet_data:
                ...
                oracle_win_probability = oracle_win_probability or prophet_data.get('win_probability', 0)
                oracle_confidence = oracle_confidence or prophet_data.get('confidence', 0)
                ...
                neutral_confidence_val = prophet_data.get('neutral_confidence', 0)
            ...
            if scan_id:
                logger.info(f"[GIDEON] Scan logged: {scan_id}")
            else:
                logger.warning("[GIDEON] Scan logging returned None - possible DB issue")
        except Exception as e:
            logger.error(f"[GIDEON] CRITICAL: Failed to log scan activity: {e}")
            import traceback
            ...
            try:
                ...
                if fallback_scan_id:
                    logger.info(f"[GIDEON] Fallback scan logged: {fallback_scan_id}")
                else:
                    logger.error("[GIDEON] FALLBACK scan logging also failed!")
            except Exception as fallback_err:
                logger.error(f"[GIDEON] FALLBACK logging failed: {fallback_err}")
    ...
    def _log_bot_decision(
        ...
        try:
            ...
            claude_ctx = ClaudeContext(
                prompt="",  # GIDEON doesn't use Claude prompts directly
            ...
            logger.debug(f"Logged GIDEON decision: {action}")
    ...
    def process_expired_positions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not expired_positions:
                logger.info("GIDEON EOD: No expired positions to process")
                return result

            logger.info(f"GIDEON EOD: Processing {len(expired_positions)} expired position(s)")
            ...
            for pos in expired_positions:
                try:
                    ...
                    logger.info(f"GIDEON EOD: Expired {pos.position_id}, P&L: ${final_pnl:.2f}")

                except Exception as e:
                    logger.error(f"GIDEON EOD: Failed to process {pos.position_id}: {e}")
                    result['errors'].append(str(e))

            self.db.log("INFO", f"EOD processed {result['processed_count']} positions")

        except Exception as e:
            logger.error(f"GIDEON EOD processing failed: {e}")
            result['errors'].append(str(e))
...
def run_icarus(config: Optional[GideonConfig] = None) -> GideonTrader:
    """Factory function to create and return GIDEON trader"""
    return GideonTrader(config)
