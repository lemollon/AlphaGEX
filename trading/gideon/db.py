...
from .models import (
    ...
    GideonConfig, TradingMode, DailySummary, CENTRAL_TZ
)
...
class GideonDatabase:
    """
    All GIDEON database operations in one place.

    No SQL scattered throughout the codebase.
    Clear, explicit methods for each operation.
    """

    def __init__(self, bot_name: str = "GIDEON"):
    ...
    def update_daily_performance(self, summary: DailySummary) -> bool:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    INSERT INTO gideon_daily_perf (
                        trade_date, trades_executed, positions_closed, realized_pnl
                    ) VALUES (%s, %s, %s, %s)
                    ON CONFLICT (trade_date) DO UPDATE SET
                        trades_executed = gideon_daily_perf.trades_executed + EXCLUDED.trades_executed,
                        positions_closed = gideon_daily_perf.positions_closed + EXCLUDED.positions_closed,
                        realized_pnl = gideon_daily_perf.realized_pnl + EXCLUDED.realized_pnl,
                        updated_at = NOW()
                """, (
                    summary.date,
    ...
    def get_current_balance(self) -> float:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    SELECT balance FROM gideon_equity_snapshots
        ...
        return 100000.0  # GIDEON default capital
    ...
    def load_config(self) -> GideonConfig:
    ...
    def log(self, level: str, message: str, details: Optional[Dict] = None) -> None:
        ...
        try:
            with db_connection() as conn:
                ...
                import json
                c.execute("""
                    INSERT INTO gideon_logs (level, message, details)
                    VALUES (%s, %s, %s)
