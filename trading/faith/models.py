...
Configuration and position models for the FAITH 2DTE Paper Iron Condor bot.
Cloned from FORTRESS with paper-trading-specific modifications.
"""
...
class TradingMode(Enum):
    """FAITH is paper-only. This enum exists for interface compatibility."""
    PAPER = "paper"
...
class FaithConfig:
    """
    FAITH bot configuration.

    Key differences from FORTRESS:
    - Paper-only mode (no live trading)
    - 2DTE targeting (more premium than 0DTE)
    - $5,000 simulated starting capital
    - Max 1 trade per day
    - 30% profit target (day trade, close same day)
    - 100% stop loss (of credit received)
    ...
    # Strike selection (inherited from FORTRESS)
