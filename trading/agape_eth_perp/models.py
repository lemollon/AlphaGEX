"""
AGAPE-ETH-PERP Models - Data structures for the ETH Perpetual Contract bot.
...
@dataclass
class AgapeEthPerpConfig:
    """Configuration for AGAPE-ETH-PERP bot.

    AGGRESSIVE MODE - perpetual contract trading on ETH.
    Uses quantity-based sizing (float ETH units) instead of integer contracts.
    """

    # Identity
    bot_name: str = "AGAPE_ETH_PERP"
    ticker: str = "ETH"
    ...
    # Risk management
    starting_capital: float = 12500.0    # $12,500 starting capital
    risk_per_trade_pct: float = 5.0      # 5% risk per trade ($625 on $12.5K)
    default_quantity: float = 0.1        # 0.1 ETH default quantity
    min_quantity: float = 0.001          # Minimum tradeable quantity
    max_quantity: float = 10.0           # Maximum quantity per position
    ...
    @classmethod
    def load_from_db(cls, db) -> "AgapeEthPerpConfig":
...
@dataclass
class AgapeEthPerpSignal:
    """Trading signal generated by AGAPE-ETH-PERP signal engine.
    ...
    # Trade parameters - quantity-based (float ETH, not int contracts)
...
@dataclass
class AgapeEthPerpPosition:
    """An open or closed AGAPE-ETH-PERP position.
    ...
    def calculate_pnl(self, current_price: float) -> float:
        ...
        ETH Perpetual: P&L = (current_price - entry_price) * quantity * direction
        No contract multiplier - quantity is directly in ETH units.
        """
