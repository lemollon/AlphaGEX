...
class AgapeEthPerpTrader:
    ...
    def _startup_recovery(self):
        ...
        try:
            ...
            for pos in open_pos:
                ...
                try:
                    ...
                    if age_hours >= stale_threshold_hours:
                        ...
                        stale_closed += 1
                        logger.warning(
                            f"AGAPE-ETH-PERP: Startup recovery closed stale position "
            ...
            logger.error(f"AGAPE-ETH-PERP: Startup recovery failed: {e}")
    ...
    def _manage_positions(self, market_data):
        ...
        if equity <= maintenance_margin:
            logger.warning(f"AGAPE-ETH-PERP: MARGIN LIQUIDATION - equity ${equity:.2f} <= maintenance ${maintenance_margin:.2f}")
            liq_closed = 0
    ...
    def _manage_regime_aware(self, pos, current_price, now):
        ...
        if decision.action == ExitAction.ARM_TRAIL:
            self.db._execute(
                "UPDATE agape_eth_perp_positions SET trailing_active = TRUE, current_stop = %s "
        ...
        if decision.action == ExitAction.UPDATE_TRAIL:
            self.db._execute(
                "UPDATE agape_eth_perp_positions SET current_stop = %s "
                "WHERE position_id = %s AND status = 'open'",
                (round(decision.new_stop, 8), pos["position_id"]),
            )
            return False
    ...
    def _check_entry_conditions(self, now):
        ...
        if balance <= min_required:
            logger.warning(f"AGAPE-ETH-PERP: Insufficient capital ${balance:.2f} (need ${min_required:.2f})")
    ...
    def _check_margin_usage(self, open_pos):
        ...
        try:
            ...
            if blocked:
                logger.warning(f"AGAPE-ETH-PERP: margin {usage:.1f}% >= 70% — refusing new opens")
                return f"MARGIN_OVER_THRESHOLD_{usage:.1f}pct"
        except (ImportError, Exception) as e:
            logger.debug(f"AGAPE-ETH-PERP: margin gate skipped: {e}")
        return None
