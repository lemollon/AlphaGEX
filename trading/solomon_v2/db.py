...
from .models import (
    ...
    SolomonConfig, TradingMode, DailySummary, CENTRAL_TZ
)
...
class SolomonDatabase:
    """
    All SOLOMON database operations in one place.

    No SQL scattered throughout the codebase.
    Clear, explicit methods for each operation.
    """

    def __init__(self, bot_name: str = "SOLOMON"):
    ...
    def _ensure_tables(self) -> None:
        ...
        try:
            with db_connection() as conn:
                ...
                # Signals table
                c.execute("""
                    CREATE TABLE IF NOT EXISTS solomon_signals (
                        id SERIAL PRIMARY KEY,
                ...
                # Daily performance
                c.execute("""
                    CREATE TABLE IF NOT EXISTS solomon_daily_perf (
                        id SERIAL PRIMARY KEY,
                ...
                # Logs table
                c.execute("""
                    CREATE TABLE IF NOT EXISTS solomon_logs (
                        id SERIAL PRIMARY KEY,
    ...
    def _ensure_ml_columns(self, cursor) -> None:
        ...
        columns_to_add = [
            ...
            # Prophet advice (CRITICAL for audit trail - BUG FIX)
        ...
        try:
            with db_connection() as conn:
                ...
                """, (
                    ...
                    pos.oracle_advice or None,  # BUG FIX: Now storing Prophet's trade decision
    ...
    def update_daily_performance(self, summary: DailySummary) -> bool:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    INSERT INTO solomon_daily_perf (
                        trade_date, trades_executed, positions_closed, realized_pnl
                    ) VALUES (%s, %s, %s, %s)
                    ON CONFLICT (trade_date) DO UPDATE SET
                        trades_executed = solomon_daily_perf.trades_executed + EXCLUDED.trades_executed,
                        positions_closed = solomon_daily_perf.positions_closed + EXCLUDED.positions_closed,
                        realized_pnl = solomon_daily_perf.realized_pnl + EXCLUDED.realized_pnl,
                        updated_at = NOW()
                """, (
                    summary.date,
    ...
    def get_current_balance(self) -> float:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    SELECT balance FROM solomon_equity_snapshots
        ...
        return 100000.0  # SOLOMON default capital
    ...
    def load_config(self) -> SolomonConfig:
        """Load config from database, with defaults.

        Uses simple key-value table with prefixed keys like 'SOLOMON_wall_filter_pct'.
        """
        config = SolomonConfig()
        try:
            with db_connection() as conn:
                c = conn.cursor()
                # Query all SOLOMON_ prefixed config keys
                c.execute("""
                    SELECT key, value
                    FROM autonomous_config
                    WHERE key LIKE 'SOLOMON_%'
                """)

                for key, value in c.fetchall():
                    # Strip 'SOLOMON_' prefix to get config attribute name
                    attr_name = key.replace('SOLOMON_', '')
    ...
    def log(self, level: str, message: str, details: Optional[Dict] = None) -> None:
        ...
        try:
            with db_connection() as conn:
                ...
                import json
                c.execute("""
                    INSERT INTO solomon_logs (level, message, details)
                    VALUES (%s, %s, %s)
