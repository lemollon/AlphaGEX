"""
AGAPE-SHIB-FUTURES Trader - Main orchestrator for SHIB Futures Contract trading.

Same logic as AGAPE-DOGE trader but for 1000SHIB-FUT contracts.
...
from trading.agape_shib_futures.db import AgapeShibFuturesDatabase
from trading.agape_shib_futures.signals import (
    AgapeShibFuturesSignalGenerator, get_agape_shib_futures_direction_tracker,
    record_agape_shib_futures_trade_outcome,
)
from trading.agape_shib_futures.executor import AgapeShibFuturesExecutor

logger = logging.getLogger(__name__)
CENTRAL_TZ = ZoneInfo("America/Chicago")

_agape_shib_futures_trader: Optional["AgapeShibFuturesTrader"] = None


def get_agape_shib_futures_trader():
    return _agape_shib_futures_trader


def create_agape_shib_futures_trader(config=None):
    global _agape_shib_futures_trader
    _agape_shib_futures_trader = AgapeShibFuturesTrader(config)
    return _agape_shib_futures_trader


class AgapeShibFuturesTrader:
    """Main AGAPE-SHIB-FUTURES trading bot orchestrator."""

    def __init__(self, config=None):
        self.db = AgapeShibFuturesDatabase()
        self.config = config or AgapeShibFuturesConfig.load_from_db(self.db)
        self.signals = AgapeShibFuturesSignalGenerator(self.config)
        self.executor = AgapeShibFuturesExecutor(self.config, self.db)
        ...
        self._direction_tracker = get_agape_shib_futures_direction_tracker(self.config)
        self._startup_recovery()
        self.db.log("INFO", "INIT", f"AGAPE-SHIB-FUTURES initialized AGGRESSIVE (mode={self.config.mode.value})")
        logger.info(f"AGAPE-SHIB-FUTURES Trader: Initialized AGGRESSIVE (mode={self.config.mode.value})")
    ...
    def _startup_recovery(self):
        ...
        try:
            ...
            for pos in open_pos:
                ...
                try:
                    ...
                    if age_hours >= stale_threshold_hours:
                        ...
                        stale_closed += 1
                        logger.warning(
                            f"AGAPE-SHIB-FUTURES: Startup recovery closed stale position "
            ...
            logger.error(f"AGAPE-SHIB-FUTURES: Startup recovery failed: {e}")
        ...
        try:
            ...
            if position:
                ...
                self.db.log("INFO", "NEW_TRADE",
                    f"{signal.side.upper()} {signal.quantity:.0f}x 1000SHIB-FUT @ ${position.entry_price:.8f}",
            ...
            logger.error(f"AGAPE-SHIB-FUTURES Trader: Cycle failed: {e}", exc_info=True)
    ...
    def _manage_positions(self, market_data):
        ...
        if equity <= maintenance_margin:
            logger.warning(f"AGAPE-SHIB-FUTURES: MARGIN LIQUIDATION - equity ${equity:.2f} <= maintenance ${maintenance_margin:.2f}")
            liq_closed = 0
        ...
        if (self.config.mode == TradingMode.PAPER
            ...
            logger.info("AGAPE-SHIB-FUTURES: Paper mode liquidation recovery — re-enabling bot")
            self._enabled = True
            self._liquidated = False
            self._liquidation_recovery_at = None
        ...
        for pos in open_positions:
            try:
                ...
                logger.error(f"AGAPE-SHIB-FUTURES: Position mgmt error: {e}")
    ...
    def _manage_no_loss_trailing(self, pos, current_price, now):
        ...
        if not trailing_active and max_profit_pct >= self.config.no_loss_activation_pct:
            ...
            self.db._execute("UPDATE agape_shib_futures_positions SET trailing_active = TRUE, current_stop = %s WHERE position_id = %s AND status = 'open'",
        ...
        if trailing_active:
            ...
            if is_long:
                ...
                if new_stop > (current_stop or 0) and new_stop >= entry:
                    self.db._execute("UPDATE agape_shib_futures_positions SET current_stop = %s WHERE position_id = %s AND status = 'open'", (new_stop, pos["position_id"]))
            else:
                new_stop = round(hwm + trail_dist, 8)
    ...
    def _manage_regime_aware(self, pos, current_price, now):
        ...
        if decision.action == ExitAction.ARM_TRAIL:
            self.db._execute(
                "UPDATE agape_shib_futures_positions SET trailing_active = TRUE, current_stop = %s "
        ...
        if decision.action == ExitAction.UPDATE_TRAIL:
            self.db._execute(
                "UPDATE agape_shib_futures_positions SET current_stop = %s "
                "WHERE position_id = %s AND status = 'open'",
                (round(decision.new_stop, 8), pos["position_id"]),
            )
            return False
    ...
    def _check_funding_flip_exit(self, pos):
        ...
        try:
            ...
            logger.debug(f"AGAPE-SHIB-FUTURES: funding-flip check failed: {e}")
        return None
    ...
    def _close_position(self, pos, current_price, reason):
        ...
        if success:
            ...
            record_agape_shib_futures_trade_outcome(pos["side"].upper(), won, self._cycle_count)
    ...
    def _check_entry_conditions(self, now):
        ...
        if balance <= min_required:
            logger.warning(f"AGAPE-SHIB-FUTURES: Insufficient capital ${balance:.2f} (need ${min_required:.2f})")
    ...
    def _check_margin_usage(self, open_pos):
        ...
        try:
            ...
            if blocked:
                logger.warning(f"AGAPE-SHIB-FUTURES: margin {usage:.1f}% >= 70% — refusing new opens")
                return f"MARGIN_OVER_THRESHOLD_{usage:.1f}pct"
        except (ImportError, Exception) as e:
            logger.debug(f"AGAPE-SHIB-FUTURES: margin gate skipped: {e}")
        return None
    ...
    def _save_equity_snapshot(self, market_data):
        try:
            ...
            logger.warning(f"AGAPE-SHIB-FUTURES: Snapshot failed: {e}")
    ...
    def get_market_status(self, now=None):
        ...
        return {
            "market_open": True,
            "status": "OPEN",
            "reason": "SHIB futures contracts trade 24/7/365.",
    ...
    def get_status(self):
        ...
        if ret < -90:
            logger.warning(f"AGAPE-SHIB-FUTURES: Return is {ret:.1f}% — approaching or past liquidation threshold")
        ...
        return {
            "bot_name": "AGAPE_SHIB_FUTURES", "status": status,
            "mode": self.config.mode.value, "ticker": "SHIB", "instrument": "1000SHIB-FUT",
    ...
    def enable(self):
        self._enabled = True
        self.db.log("INFO", "ENABLE", "AGAPE-SHIB-FUTURES enabled")

    def disable(self):
        self._enabled = False
        self.db.log("INFO", "DISABLE", "AGAPE-SHIB-FUTURES disabled")
