"""
AGAPE-SHIB-FUTURES Models - Data structures for the SHIB Futures Contract bot.
...
KEY DIFFERENCES from AGAPE-SHIB-PERP:
- Trades Coinbase Derivatives monthly futures (SHB-29MAY26-CDE, SHB-26JUN26-CDE, etc.)
  rather than the geo-blocked Coinbase International perpetual.
- Sizing is integer contracts (1 contract = 10,000 units of "1000SHIB" = 10M SHIB underlying).
...
@dataclass
class AgapeShibFuturesConfig:
    """Configuration for AGAPE-SHIB-FUTURES bot.
    ...
    # Identity
    bot_name: str = "AGAPE_SHIB_FUTURES"
    ticker: str = "SHIB"
    instrument: str = "1000SHIB-FUT"     # Futures contract
    ...
    max_quantity: float = 100.0         # Max 100 contracts per position — at $60/contract notional that's $6K per trade
    max_open_positions: int = 2         # Conservative for meme coin

    # Position sizing - Futures contract specs (integer contracts only)
    # 1 contract = 10,000 units of "1000SHIB" index = 10M actual SHIB tokens
    contract_size: int = 10_000          # 1 contract = 10,000 units of "1000SHIB" index = 10M SHIB underlying
    default_quantity: float = 1.0        # 1 contract default trade size
    min_quantity: float = 1.0            # Minimum 1 contract (integer contracts only)
    tick_size: float = 0.00001           # Coinbase futures tick — 5-decimal price
    ...
    @classmethod
    def load_from_db(cls, db) -> "AgapeShibFuturesConfig":
...
@dataclass
class AgapeShibFuturesSignal:
    """Trading signal generated by AGAPE-SHIB-FUTURES signal engine."""
    spot_price: float
    timestamp: datetime
