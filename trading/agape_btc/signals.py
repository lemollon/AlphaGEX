"""
AGAPE-BTC Signal Generator - Generates directional /MBT trade signals.

Same logic as AGAPE (ETH) signals but for BTC market microstructure.
...
try:
    ...
    logger.info("AGAPE-BTC Signals: CryptoDataProvider loaded")
except ImportError as e:
    logger.warning(f"AGAPE-BTC Signals: CryptoDataProvider not available: {e}")
...
try:
    ...
    logger.info("AGAPE-BTC Signals: ProphetAdvisor loaded")
except ImportError as e:
    logger.warning(f"AGAPE-BTC Signals: ProphetAdvisor not available: {e}")
...
class AgapeBtcDirectionTracker:
...
_direction_tracker: Optional[AgapeBtcDirectionTracker] = None


def get_agape_btc_direction_tracker(config: Optional[AgapeBtcConfig] = None) -> AgapeBtcDirectionTracker:
    ...
    if _direction_tracker is None:
        ...
        _direction_tracker = AgapeBtcDirectionTracker(
            cooldown_scans=cooldown, win_streak_caution=caution, memory_size=memory,
        )
    return _direction_tracker


def record_agape_btc_trade_outcome(direction: str, is_win: bool, scan_number: int) -> None:
    tracker = get_agape_btc_direction_tracker()
    tracker.record_trade(direction, is_win, scan_number)


class AgapeBtcSignalGenerator:
    """Generates directional trade signals for /MBT based on crypto microstructure."""

    def __init__(self, config: AgapeBtcConfig):
        self.config = config
        ...
        if get_crypto_data_provider:
            try:
                ...
                logger.warning(f"AGAPE-BTC Signals: Crypto provider init failed: {e}")

        if ProphetAdvisor:
            try:
                self._oracle = ProphetAdvisor()
            except Exception as e:
                logger.warning(f"AGAPE-BTC Signals: Prophet init failed: {e}")

    def get_market_data(self) -> Optional[Dict[str, Any]]:
        if not self._crypto_provider:
            logger.error("AGAPE-BTC Signals: No crypto data provider")
            return None
        try:
            ...
            logger.error(f"AGAPE-BTC Signals: Market data fetch failed: {e}")
            return None
    ...
    def get_prophet_advice(self, market_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"AGAPE-BTC Signals: Prophet call failed: {e}")
        return {"advice": "UNAVAILABLE", "win_probability": 0.5, "confidence": 0.0, "top_factors": ["oracle_error"]}

    def generate_signal(self, prophet_data: Optional[Dict] = None) -> AgapeBtcSignal:
        now = datetime.now(CENTRAL_TZ)
        market_data = self.get_market_data()
        if not market_data:
            return AgapeBtcSignal(spot_price=0, timestamp=now, action=SignalAction.WAIT, reasoning="NO_MARKET_DATA")
    ...
    def _determine_action(self, combined_signal, confidence, market_data):
        ...
        direction_tracker = get_agape_btc_direction_tracker(self.config)
    ...
    def _derive_fallback_direction(self, market_data, tracker):
        ...
        # Fix: use actual FundingRate.regime names (EXTREME_SHORT/OVERLEVERAGED_SHORT)
    ...
    def _calculate_position_size(self, spot_price: float, account_equity: float = None) -> Tuple[int, float]:
        ...
        contracts_by_risk = int(max_risk_usd / risk_per_contract)
    ...
    def _calculate_margin_info(self, contracts: int, spot_price: float, side: str, account_equity: float = None) -> Dict:
        ...
        if denominator > 0 and equity_buffer > 0:
            if side == "long":
                liq_price = round(spot_price - (equity_buffer / denominator), 2)
            else:
                liq_price = round(spot_price + (equity_buffer / denominator), 2)
            if liq_price <= 0:
                liq_price = None
