"""
AGAPE-BTC Trader - Main orchestrator for BTC Micro Futures trading.

Same logic as AGAPE (ETH) trader but for /MBT contracts.
"""
...
from trading.agape_btc.db import AgapeBtcDatabase
from trading.agape_btc.signals import (
    AgapeBtcSignalGenerator,
    get_agape_btc_direction_tracker,
    record_agape_btc_trade_outcome,
)
from trading.agape_btc.executor import AgapeBtcExecutor

logger = logging.getLogger(__name__)

CENTRAL_TZ = ZoneInfo("America/Chicago")

_agape_btc_trader: Optional["AgapeBtcTrader"] = None


def get_agape_btc_trader() -> Optional["AgapeBtcTrader"]:
    return _agape_btc_trader


def create_agape_btc_trader(config: Optional[AgapeBtcConfig] = None) -> "AgapeBtcTrader":
    global _agape_btc_trader
    _agape_btc_trader = AgapeBtcTrader(config)
    return _agape_btc_trader


class AgapeBtcTrader:
    """Main AGAPE-BTC trading bot orchestrator."""

    def __init__(self, config: Optional[AgapeBtcConfig] = None):
        self.db = AgapeBtcDatabase()
        if config:
            self.config = config
        else:
            self.config = AgapeBtcConfig.load_from_db(self.db)
        self.signals = AgapeBtcSignalGenerator(self.config)
        self.executor = AgapeBtcExecutor(self.config, self.db)
        self._last_scan_time: Optional[datetime] = None
        self._cycle_count: int = 0
        self._enabled: bool = True
        self.consecutive_losses: int = 0
        self.loss_streak_pause_until: Optional[datetime] = None
        self._direction_tracker = get_agape_btc_direction_tracker(self.config)
        self.db.log("INFO", "INIT", f"AGAPE-BTC trader initialized AGGRESSIVE (mode={self.config.mode.value})")
        logger.info(f"AGAPE-BTC Trader: Initialized AGGRESSIVE (mode={self.config.mode.value})")
        ...
        try:
            ...
            if position:
                ...
                self.db.log("INFO", "NEW_TRADE",
                    f"{signal.side.upper()} {signal.contracts}x /MBT @ ${position.entry_price:.2f}",
            ...
            logger.error(f"AGAPE-BTC Trader: Cycle failed: {e}", exc_info=True)
    ...
    def _manage_positions(self, market_data: Optional[Dict]) -> tuple:
        ...
        for pos_dict in open_positions:
            try:
                ...
                logger.error(f"AGAPE-BTC Trader: Position management error: {e}")
    ...
    def _manage_position_no_loss_trailing(self, pos, current_price, now):
        ...
        if not trailing_active and max_profit_pct >= activation_pct:
            ...
            try:
                self.db._execute(
                    "UPDATE agape_btc_positions SET trailing_active = TRUE, current_stop = %s WHERE position_id = %s AND status = 'open'",
        ...
        if trailing_active:
            ...
            if is_long:
                ...
                if new_stop > (current_stop or 0) and new_stop >= entry_price:
                    try:
                        self.db._execute(
                            "UPDATE agape_btc_positions SET current_stop = %s WHERE position_id = %s AND status = 'open'",
    ...
    def _execute_sar(self, pos, current_price):
        ...
        if reversal_side == "long":
            ...
            stop_loss = current_price * (1 + emergency_pct / 100)
            take_profit = current_price * (1 - emergency_pct / 100)
        reversal_signal = AgapeBtcSignal(
            ...
            stop_loss=stop_loss, take_profit=take_profit, contracts=contracts, max_risk_usd=0,
        )
    ...
    def _close_position(self, pos_dict, current_price, reason):
        ...
        if success:
            ...
            record_agape_btc_trade_outcome(direction=side.upper(), is_win=won, scan_number=self._cycle_count)
    ...
    def get_cme_market_status(self, now=None):
        ...
        if weekday == 5:
            return {"cme_market_open": False, "status": "CLOSED_SATURDAY", "reason": "CME closed Saturday.", "schedule": schedule}
        elif weekday == 6 and hour < 17:
            return {"cme_market_open": False, "status": "CLOSED_SUNDAY_EARLY", "reason": "CME opens at 5:00 PM CT today.", "schedule": schedule}
        elif weekday == 4 and hour >= 16:
            return {"cme_market_open": False, "status": "CLOSED_FRIDAY_LATE", "reason": "CME closed for the weekend.", "schedule": schedule}
        elif 0 <= weekday <= 3 and hour == 16:
            return {"cme_market_open": False, "status": "DAILY_MAINTENANCE", "reason": "CME daily maintenance (4-5 PM CT).", "schedule": schedule}
        else:
            return {"cme_market_open": True, "status": "OPEN", "reason": "CME Micro Bitcoin Futures market is open.", "schedule": schedule}
    ...
    def get_status(self):
        ...
        return {
            "bot_name": "AGAPE_BTC", "status": "ACTIVE" if self._enabled else "DISABLED",
            "mode": self.config.mode.value, "ticker": self.config.ticker,
            "instrument": self.config.instrument, "exchange": "tastytrade",
            "cycle_count": self._cycle_count, "open_positions": len(open_positions),
            "max_positions": None, "current_btc_price": current_price,
    ...
    def enable(self):
        self._enabled = True
        self.db.log("INFO", "ENABLE", "AGAPE-BTC bot enabled")

    def disable(self):
        self._enabled = False
        self.db.log("INFO", "DISABLE", "AGAPE-BTC bot disabled")
