"""
AGAPE-BTC Models - Data structures for the BTC Micro Futures bot.

Mirrors AGAPE (ETH) models pattern with BTC-specific contract specs.
"""
...
class Exchange(Enum):
    TASTYTRADE_CME = "tastytrade"    # CME /MBT futures (Sun 5PM - Fri 4PM CT)
...
@dataclass
class AgapeBtcConfig:
    """Configuration for AGAPE-BTC bot - loaded from autonomous_config table.
    ...
    # Identity
    bot_name: str = "AGAPE_BTC"
    ticker: str = "BTC"
    instrument: str = "/MBT"          # CME Micro Bitcoin Futures
    ...
    # Position sizing - /MBT contract specs
    contract_size: float = 0.1         # /MBT = 0.1 BTC
    ...
    @classmethod
    def load_from_db(cls, db) -> "AgapeBtcConfig":
...
@dataclass
class AgapeBtcSignal:
    """Trading signal generated by AGAPE-BTC signal engine."""
    spot_price: float
    timestamp: datetime
...
@dataclass
class AgapeBtcPosition:
    """An open or closed AGAPE-BTC position."""
    position_id: str
    side: PositionSide
    contracts: int
    entry_price: float
    ...
    def calculate_pnl(self, current_price: float) -> float:
        ...
        /MBT: Each $1 move in BTC = $0.10 per contract.
