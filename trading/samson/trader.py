"""
SAMSON - Main Trading Orchestrator
===================================
...
from .models import (
    IronCondorPosition, PositionStatus, SamsonConfig,
...
try:
    ...
    print("✅ SAMSON: ML Data Gatherer loaded")
except ImportError as e:
    ML_GATHERER_AVAILABLE = False
    gather_ml_data = None
    print(f"⚠️ SAMSON: ML Data Gatherer not available: {e}")
...
class SamsonTrader(MathOptimizerMixin):
    """
    SAMSON - Aggressive SPX Iron Condor Trader
    ...
    def __init__(self, config: Optional[SamsonConfig] = None):
        self.db = SamsonDatabase(bot_name="SAMSON")
        self.config = config or self.db.load_config()

        # Validate configuration at startup
        is_valid, error = self.config.validate()
        if not is_valid:
            logger.error(f"SAMSON config validation failed: {error}")
            raise ValueError(f"Invalid SAMSON configuration: {error}")
        ...
        logger.info(f"SAMSON initialized: mode={self.config.mode.value}, preset={self.config.preset.value}")

    def run_cycle(self, close_only: bool = False) -> Dict[str, Any]:
        """Run trading cycle - SAMSON runs multiple times per day.
        ...
        try:
            ...
            if close_only:
                logger.info("SAMSON running in CLOSE_ONLY mode - managing positions only")
            ...
            if PROVERBS_ENHANCED_AVAILABLE and get_proverbs_enhanced:
                try:
                    ...
                    if enhanced and enhanced.proverbs.is_bot_killed('SAMSON'):
                        logger.warning("[SAMSON] Kill switch ACTIVE — skipping cycle (no new entries)")
                        result['action'] = 'kill_switch_active'
                        self._log_scan_activity(result, scan_context, skip_reason="Kill switch active")
                        return result
                except Exception as e:
                    logger.debug(f"[SAMSON] Kill switch check failed (fail-open): {e}")
    ...
    def _record_proverbs_outcome(
        ...
        try:
            ...
            alerts = enhanced.record_trade_outcome(
                ...
                strategy_type='IRON_CONDOR',  # SAMSON is an Iron Condor bot
                oracle_prediction_id=oracle_prediction_id
            )
            if alerts:
                for alert in alerts:
                    logger.warning(f"SAMSON Proverbs alert: {alert}")
        except Exception as e:
            logger.warning(f"SAMSON: Proverbs outcome recording failed: {e}")
    ...
    def _record_thompson_outcome(self, pnl: float):
        ...
        This updates the Beta distribution parameters for SAMSON,
        ...
        try:
            record_bot_outcome('SAMSON', win=(pnl > 0), pnl=pnl)
            logger.debug(f"SAMSON: Recorded outcome to Thompson Sampling - P&L=${pnl:.2f}")
        except Exception as e:
            logger.warning(f"SAMSON: Thompson outcome recording failed: {e}")
    ...
    def _record_learning_memory_prediction(self, pos: IronCondorPosition, signal) -> Optional[str]:
        ...
        try:
            ...
            context = {
                ...
                "bot": "SAMSON"  # Mark as SAMSON prediction
            }
            ...
            logger.info(f"SAMSON: Learning Memory prediction recorded: {prediction_id}")
            return prediction_id

        except Exception as e:
            logger.warning(f"SAMSON: Learning Memory prediction failed: {e}")
            return None
    ...
    def _record_learning_memory_outcome(self, prediction_id: str, pnl: float, close_reason: str):
        ...
        try:
            ...
            logger.info(f"SAMSON: Learning Memory outcome recorded: correct={was_correct}, P&L=${pnl:.2f}")

        except Exception as e:
            logger.warning(f"SAMSON: Learning Memory outcome recording failed: {e}")
    ...
    def _store_oracle_prediction(self, signal, position: IronCondorPosition) -> int | None:
        ...
        try:
            ...
            prediction = ProphetPrediction(
                ...
                suggested_risk_pct=15.0,  # SAMSON higher risk
            ...
            prediction_id = prophet.store_prediction(
                ...
                strategy_recommendation='IRON_CONDOR'  # Migration 023: SAMSON uses Iron Condor strategy
            )
    ...
    def _get_force_exit_time(self, now: datetime, today: str) -> datetime:
        ...
        if MARKET_CALENDAR_AVAILABLE and MARKET_CALENDAR:
            ...
            if early_close_force < config_force_time:
                logger.info(f"SAMSON: Early close day - adjusting force exit to {early_close_force.strftime('%H:%M')}")
    ...
    def _log_scan_activity(
        ...
        try:
            ...
            if error_msg:
                ...
                decision = "SAMSON SPX Iron Condor opened"
            elif skip_reason:
            ...
            if scan_id:
                print(f"[SAMSON DEBUG] ✅ Scan logged successfully: {scan_id}")
            else:
                print(f"[SAMSON DEBUG] ❌ Scan logging returned None - check database!")
        except Exception as e:
            print(f"[SAMSON DEBUG] ❌ EXCEPTION in _log_scan_activity: {e}")
            import traceback
    ...
    def _log_bot_decision(
        ...
        try:
            ...
            if error_msg:
                ...
                action = "OPEN_IC"
                reason = "SAMSON SPX Iron Condor opened"
            elif skip_reason:
                dec_type = DecisionType.SKIP
                action = "SKIP"
                reason = skip_reason
            ...
            logger.debug(f"Logged SAMSON decision: {action}")
    ...
    def _update_daily_summary(self, today: str, cycle_result: Dict) -> None:
        ...
        try:
            # SAMSON: Count all trades today (not just once per day)
...
def run_titan(config: Optional[SamsonConfig] = None) -> SamsonTrader:
    """Factory function"""
    return SamsonTrader(config)
