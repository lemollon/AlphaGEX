...
from .models import (
    ...
    IronCondorSignal, SamsonConfig, TradingMode, CENTRAL_TZ
)
...
class OrderExecutor:
    """Executes aggressive SPX Iron Condors for SAMSON"""

    def __init__(self, config: SamsonConfig, db=None):
        self.config = config
        self.tradier = None
        ...
        if TRADIER_AVAILABLE and config.mode == TradingMode.LIVE:
            try:
                ...
                logger.info("SAMSON: Tradier initialized (PRODUCTION)")
        ...
        if POSITION_MGMT_AVAILABLE:
            try:
                ...
                logger.info("SAMSON OrderExecutor: Position Management Agent initialized")
            except Exception as e:
    ...
    def _execute_paper(self, signal: IronCondorSignal, thompson_weight: float = 1.0) -> Optional[IronCondorPosition]:
        ...
        try:
            ...
            position_id = f"SAMSON-{now.strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:4].upper()}"
    ...
    def _execute_live(self, signal: IronCondorSignal, thompson_weight: float = 1.0) -> Optional[IronCondorPosition]:
        ...
        try:
            ...
            position = IronCondorPosition(
                position_id=f"SAMSON-LIVE-{put_order_id}",
                ticker="SPX",
                expiration=signal.expiration,
                put_short_strike=signal.put_short,
                put_long_strike=signal.put_long,
    ...
    def _close_paper(self, position: IronCondorPosition, reason: str) -> Tuple[Union[bool, str], float, float]:
        try:
            ...
            if is_expiration:
                ...
                try:
                    ...
                    if mtm.get('success') and mtm.get('current_value') is not None:
                        ...
                        logger.info(f"SAMSON PAPER CLOSE MTM: {position.position_id} close_value=${close_value:.4f}")
                except Exception as e:
            ...
            logger.info(f"SAMSON PAPER CLOSE: {position.position_id} @ ${close_value:.2f}, P&L=${pnl:.2f}")
    ...
    def _close_live(self, position: IronCondorPosition, reason: str) -> Tuple[Union[bool, str], float, float]:
        ...
        try:
            ...
            logger.info(f"SAMSON LIVE CLOSE: {position.position_id} @ ${close_value:.2f}, P&L=${pnl:.2f} [{reason}]")
            return True, close_value, pnl
        except Exception as e:
    ...
    def _get_kelly_position_size(self) -> Optional[Dict]:
        """Get Monte Carlo Kelly-based position sizing for SAMSON."""
        ...
        try:
            ...
            # BUG FIX: Query samson_positions table instead of autonomous_closed_trades
            ...
            logger.info(f"[SAMSON KELLY] Win Rate: {win_rate:.1%}, Safe Kelly: {kelly_result['kelly_safe']:.1f}%")
            return kelly_result
    ...
    def _calculate_position_size(self, max_loss_per_contract: float, thompson_weight: float = 1.0) -> int:
        ...
        if kelly_result and kelly_result.get('kelly_safe', 0) > 0:
            ...
            sizing_source = "KELLY"
            logger.info(f"[SAMSON] Using Kelly-safe sizing: {kelly_risk_pct:.1f}% risk")
        else:
        ...
        if abs(thompson_weight - 1.0) > 0.05:
            logger.info(f"[SAMSON {sizing_source}] Thompson: weight={thompson_weight:.2f}, base={base_contracts:.1f}, adjusted={adjusted_contracts}")
    ...
    def store_entry_conditions(self, position_id: int, gex_data: Dict) -> bool:
        ...
        try:
            ...
            logger.info(f"[SAMSON] Entry conditions stored for position {position_id}")
            return True
        except Exception as e:
        ...
        try:
            ...
            if result.get('alerts'):
                for alert in result['alerts']:
                    logger.info(f"[SAMSON CONDITION ALERT] {alert['type']}: {alert['message']}")
