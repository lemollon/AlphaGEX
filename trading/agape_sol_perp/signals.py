"""
AGAPE-SOL-PERP Signal Generator - Generates directional SOL perpetual trade signals.

Key differences from AGAPE-XRP:
    - Uses CryptoDataProvider with ticker "SOL"
    - _calculate_position_size returns (quantity, max_risk_usd) where quantity is float SOL
...
try:
    ...
    logger.info("AGAPE-SOL-PERP Signals: CryptoDataProvider loaded")
except ImportError as e:
    logger.warning(f"AGAPE-SOL-PERP Signals: CryptoDataProvider not available: {e}")
...
try:
    ...
    logger.info("AGAPE-SOL-PERP Signals: ProphetAdvisor loaded")
except ImportError as e:
    logger.warning(f"AGAPE-SOL-PERP Signals: ProphetAdvisor not available: {e}")


class AgapeSolPerpDirectionTracker:
...
_direction_tracker: Optional[AgapeSolPerpDirectionTracker] = None


def get_agape_sol_perp_direction_tracker(config=None):
    global _direction_tracker
    if _direction_tracker is None:
        _direction_tracker = AgapeSolPerpDirectionTracker(
...
def record_agape_sol_perp_trade_outcome(direction, is_win, scan_number):
    get_agape_sol_perp_direction_tracker().record_trade(direction, is_win, scan_number)


class AgapeSolPerpSignalGenerator:
    """Generates directional trade signals for SOL-PERP based on crypto microstructure."""

    def __init__(self, config: AgapeSolPerpConfig):
        self.config = config
        ...
        if get_crypto_data_provider:
            try:
                ...
                logger.warning(f"AGAPE-SOL-PERP Signals: Crypto provider init failed: {e}")
        if ProphetAdvisor:
            try:
                self._oracle = ProphetAdvisor()
            except Exception as e:
                logger.warning(f"AGAPE-SOL-PERP Signals: Prophet init failed: {e}")
    ...
    def get_market_data(self):
        ...
        try:
            ...
            logger.error(f"AGAPE-SOL-PERP Signals: Market data fetch failed: {e}")
            return None
    ...
    def get_prophet_advice(self, market_data):
        ...
        try:
            ...
            logger.error(f"AGAPE-SOL-PERP Signals: Prophet call failed: {e}")
    ...
    def generate_signal(self, prophet_data=None):
        ...
        if not market_data:
            return AgapeSolPerpSignal(spot_price=0, timestamp=now, action=SignalAction.WAIT, reasoning="NO_MARKET_DATA")
    ...
    def _determine_action(self, combined_signal, confidence, market_data):
        ...
        if confidence_rank.get(confidence, 0) < confidence_rank.get(self.config.min_confidence, 1):
            ...
            logger.info(
                f"AGAPE-SOL-PERP Signals: DEGRADED_NO_COINGLASS override - "
                f"trading {combined_signal} at {confidence} confidence (CoinGlass unavailable)"
            )
        tracker = get_agape_sol_perp_direction_tracker(self.config)
    ...
    def _calculate_position_size(self, spot_price: float) -> Tuple[float, float]:
        """Calculate position size as SOL quantity (float) and max risk in USD.

        Returns:
            (quantity, max_risk_usd) where quantity is float SOL amount.
        """
        capital = self.config.starting_capital
        max_risk_usd = capital * (self.config.risk_per_trade_pct / 100)

        # Risk per unit SOL based on stop distance (2% default)
        stop_distance = spot_price * 0.02 * (self.config.stop_loss_pct / 100)
        if stop_distance <= 0:
            return (self.config.default_quantity, max_risk_usd)

        # quantity = max_risk / risk_per_sol
        ...
        try:
            ...
            if max_qty_by_notional > 0 and max_qty_by_notional < quantity:
                logger.debug(
                    f"AGAPE-SOL-PERP: notional cap reducing size "
                    f"{quantity:.3f} -> {max_qty_by_notional:.3f} SOL "
                    f"(per_pos={per_pos_margin_pct:.1f}% margin, lev={leverage}x)"
                )
                quantity = max(self.config.min_quantity, max_qty_by_notional)
        except Exception as e:
            logger.debug(f"AGAPE-SOL-PERP: notional cap skipped: {e}")

        # Round to 3 decimal places (0.001 SOL precision)
