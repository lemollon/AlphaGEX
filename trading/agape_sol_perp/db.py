"""
AGAPE-SOL-PERP Database Layer - PostgreSQL persistence for the SOL Perpetual bot.

Key differences from AGAPE-XRP:
    - Tables prefixed with agape_sol_perp_ instead of agape_xrp_
    - quantity FLOAT instead of contracts INTEGER in positions table
    - sol_price column instead of xrp_price in snapshots and scan_activity
    - Default starting_capital: 5000.0
    - Config prefix: agape_sol_perp_
"""
...
try:
    ...
    logger.warning("AGAPE-SOL-PERP DB: database_adapter not available")
...
class AgapeSolPerpDatabase:
    """Database operations for AGAPE-SOL-PERP bot."""

    def __init__(self, bot_name: str = "AGAPE_SOL_PERP"):
    ...
    def _execute(self, sql: str, params=None) -> bool:
        ...
        try:
            ...
            logger.error(f"AGAPE-SOL-PERP DB: Execute failed: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def _ensure_tables(self):
        conn = self._get_conn()
        if not conn:
            logger.warning("AGAPE-SOL-PERP DB: No database connection, skipping table creation")
            return
        try:
            cursor = conn.cursor()
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_sol_perp_positions (
            ...
            # Equity snapshots - uses sol_price instead of xrp_price
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_sol_perp_equity_snapshots (
                    id SERIAL PRIMARY KEY,
            ...
            # Scan activity - uses sol_price instead of xrp_price
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_sol_perp_scan_activity (
                    id SERIAL PRIMARY KEY,
                    timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            ...
            # Activity log
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_sol_perp_activity_log (
                    id SERIAL PRIMARY KEY,
                    timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            ...
            for col_sql in [
                "ALTER TABLE agape_sol_perp_positions ADD COLUMN IF NOT EXISTS trailing_active BOOLEAN DEFAULT FALSE",
                "ALTER TABLE agape_sol_perp_positions ADD COLUMN IF NOT EXISTS current_stop FLOAT",
                # Phase 3.1: log new v4 CoinGlass fields per scan
                "ALTER TABLE agape_sol_perp_scan_activity ADD COLUMN IF NOT EXISTS oi_total_usd FLOAT",
                "ALTER TABLE agape_sol_perp_scan_activity ADD COLUMN IF NOT EXISTS ls_long_pct FLOAT",
                "ALTER TABLE agape_sol_perp_scan_activity ADD COLUMN IF NOT EXISTS taker_buy_ratio FLOAT",
                "ALTER TABLE agape_sol_perp_positions ADD COLUMN IF NOT EXISTS regime_at_entry VARCHAR(20)",
                "ALTER TABLE agape_sol_perp_positions ADD COLUMN IF NOT EXISTS accrued_funding_usd FLOAT DEFAULT 0",
                "ALTER TABLE agape_sol_perp_positions ADD COLUMN IF NOT EXISTS last_funding_accrual TIMESTAMP WITH TIME ZONE DEFAULT NOW()",
            ]:
                try:
            ...
            # Indexes
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_sol_perp_positions_status ON agape_sol_perp_positions(status)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_sol_perp_positions_open_time ON agape_sol_perp_positions(open_time DESC)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_sol_perp_equity_snapshots_ts ON agape_sol_perp_equity_snapshots(timestamp DESC)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_sol_perp_scan_activity_ts ON agape_sol_perp_scan_activity(timestamp DESC)")

            conn.commit()
            logger.info("AGAPE-SOL-PERP DB: Tables ensured")
        except Exception as e:
            logger.error(f"AGAPE-SOL-PERP DB: Table creation failed: {e}")
            conn.rollback()
        finally:
            cursor.close()
            conn.close()
    ...
    def load_config(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            logger.debug(f"AGAPE-SOL-PERP DB: Config load failed: {e}")
            return None
        finally:
            cursor.close()
            conn.close()
    ...
    def save_position(self, pos) -> bool:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_sol_perp_positions (
            ...
            logger.error(f"AGAPE-SOL-PERP DB: Failed to save position: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def close_position(self, position_id: str, close_price: float, realized_pnl: float, reason: str) -> bool:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                UPDATE agape_sol_perp_positions
            ...
            logger.error(f"AGAPE-SOL-PERP DB: Failed to close position: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def get_open_positions(self) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"AGAPE-SOL-PERP DB: Failed to get open positions: {e}")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    def get_closed_trades(
        ...
        try:
            ...
            logger.error(f"AGAPE-SOL-PERP DB: Failed to get closed trades: {e}")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    def get_position_count(self) -> int:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM agape_sol_perp_positions WHERE status = 'open'")
    ...
    def accrue_funding(self, position_id: str, delta_usd: float, now=None) -> bool:
        ...
        try:
            ...
            logger.error(f"AGAPE-SOL-PERP DB: Failed to accrue funding for {position_id}: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def update_high_water_mark(self, position_id: str, hwm: float) -> bool:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("UPDATE agape_sol_perp_positions SET high_water_mark = %s WHERE position_id = %s AND status = 'open'", (hwm, position_id))
            conn.commit()
            return True
        except Exception:
            conn.rollback()
    ...
    def save_equity_snapshot(self, equity, unrealized_pnl, realized_cumulative, open_positions, sol_price=None, funding_rate=None):
        conn = self._get_conn()
        if not conn:
            return False
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_sol_perp_equity_snapshots
                (equity, unrealized_pnl, realized_pnl_cumulative, open_positions, sol_price, funding_rate)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (equity, unrealized_pnl, realized_cumulative, open_positions, sol_price, funding_rate))
            conn.commit()
            return True
        except Exception as e:
            logger.error(f"AGAPE-SOL-PERP DB: Failed to save equity snapshot: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def log_scan(self, scan_data: Dict) -> bool:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_sol_perp_scan_activity (
                    outcome, sol_price, funding_rate, funding_regime,
            ...
            """, (
                scan_data.get("outcome"), scan_data.get("sol_price"),
            ...
            logger.error(f"AGAPE-SOL-PERP DB: Failed to log scan: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def log(self, level, action, message, details=None):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_sol_perp_activity_log (level, action, message, details)
                VALUES (%s, %s, %s, %s)
    ...
    def get_logs(self, limit=50):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT timestamp, level, action, message, details FROM agape_sol_perp_activity_log ORDER BY timestamp DESC LIMIT %s", (limit,))
    ...
    def get_scan_activity(self, limit=50):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT timestamp, outcome, sol_price, funding_rate, funding_regime,
                ...
                FROM agape_sol_perp_scan_activity ORDER BY timestamp DESC LIMIT %s
            """, (limit,))
            return [
                {"timestamp": r[0].isoformat() if r[0] else None, "outcome": r[1],
                 "sol_price": float(r[2]) if r[2] else None,
    ...
    def get_scan_count(self) -> int:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM agape_sol_perp_scan_activity")
            row = cursor.fetchone()
