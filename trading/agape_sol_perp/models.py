"""
AGAPE-SOL-PERP Models - Data structures for the SOL Perpetual Contract bot.
...
@dataclass
class AgapeSolPerpConfig:
    """Configuration for AGAPE-SOL-PERP bot.

    AGGRESSIVE MODE - perpetual contract trading on SOL.
    Uses quantity-based sizing (float SOL units) instead of integer contracts.
    """

    # Identity
    bot_name: str = "AGAPE_SOL_PERP"
    ticker: str = "SOL"
    ...
    # Risk management
    starting_capital: float = 5000.0     # $5,000 starting capital (smaller pilot allocation)
    risk_per_trade_pct: float = 1.0      # 5% risk per trade ($250 on $5K)
    default_quantity: float = 5.0        # 5 SOL default quantity (Coinbase SOL perp contract size)
    min_quantity: float = 1.0            # Minimum tradeable quantity (Coinbase increment)
    max_quantity: float = 500.0          # Maximum quantity per position
...
@dataclass
class AgapeSolPerpSignal:
    """Trading signal generated by AGAPE-SOL-PERP signal engine.
    ...
    # Trade parameters - quantity-based (float SOL, not int contracts)
...
@dataclass
class AgapeSolPerpPosition:
    """An open or closed AGAPE-SOL-PERP position.
    ...
    def calculate_pnl(self, current_price: float) -> float:
        ...
        SOL Perpetual: P&L = (current_price - entry_price) * quantity * direction
        No contract multiplier - quantity is directly in SOL units.
        """
