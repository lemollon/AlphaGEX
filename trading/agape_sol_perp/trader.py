"""
AGAPE-SOL-PERP Trader - Main orchestrator for SOL perpetual contract trading.
...
Key differences from AGAPE-XRP:
    ...
    - Status uses sol_price, instrument="SOL-PERP", exchange="perpetual"
    - Equity snapshots use sol_price instead of xrp_price
"""
...
from trading.agape_sol_perp.models import (
    AgapeSolPerpConfig, AgapeSolPerpSignal, AgapeSolPerpPosition,
    PositionSide, PositionStatus, SignalAction, TradingMode,
)
from trading.agape_sol_perp.db import AgapeSolPerpDatabase
from trading.agape_sol_perp.signals import (
    AgapeSolPerpSignalGenerator, get_agape_sol_perp_direction_tracker,
    record_agape_sol_perp_trade_outcome,
)
from trading.agape_sol_perp.executor import AgapeSolPerpExecutor

logger = logging.getLogger(__name__)
CENTRAL_TZ = ZoneInfo("America/Chicago")

_agape_sol_perp_trader: Optional["AgapeSolPerpTrader"] = None


def get_agape_sol_perp_trader():
    return _agape_sol_perp_trader


def create_agape_sol_perp_trader(config=None):
    global _agape_sol_perp_trader
    _agape_sol_perp_trader = AgapeSolPerpTrader(config)
    return _agape_sol_perp_trader


class AgapeSolPerpTrader:
    """Main AGAPE-SOL-PERP trading bot orchestrator.

    Trades SOL perpetual contracts 24/7/365 with no market hours restrictions.
    Uses quantity (float SOL) instead of contracts (int).
    """

    def __init__(self, config=None):
        self.db = AgapeSolPerpDatabase()
        self.config = config or AgapeSolPerpConfig.load_from_db(self.db)
        self.signals = AgapeSolPerpSignalGenerator(self.config)
        self.executor = AgapeSolPerpExecutor(self.config, self.db)
        ...
        self._direction_tracker = get_agape_sol_perp_direction_tracker(self.config)
        self._startup_recovery()
        self.db.log("INFO", "INIT", f"AGAPE-SOL-PERP initialized AGGRESSIVE (mode={self.config.mode.value})")
        logger.info(f"AGAPE-SOL-PERP Trader: Initialized AGGRESSIVE (mode={self.config.mode.value})")
    ...
    def _startup_recovery(self):
        ...
        try:
            ...
            for pos in open_pos:
                ...
                try:
                    ...
                    if age_hours >= stale_threshold_hours:
                        ...
                        stale_closed += 1
                        logger.warning(
                            f"AGAPE-SOL-PERP: Startup recovery closed stale position "
            ...
            logger.error(f"AGAPE-SOL-PERP: Startup recovery failed: {e}")
    ...
    def run_cycle(self, close_only=False):
        ...
        try:
            ...
            if market_data:
                ...
                scan_ctx["sol_price"] = market_data.get("spot_price")
            ...
            if position:
                ...
                self.db.log("INFO", "NEW_TRADE",
                    f"{signal.side.upper()} {signal.quantity} SOL-PERP @ ${position.entry_price:.2f}",
            ...
            logger.error(f"AGAPE-SOL-PERP Trader: Cycle failed: {e}", exc_info=True)
    ...
    def _manage_positions(self, market_data):
        ...
        if equity <= maintenance_margin:
            logger.warning(f"AGAPE-SOL-PERP: MARGIN LIQUIDATION - equity ${equity:.2f} <= maintenance ${maintenance_margin:.2f}")
            liq_closed = 0
        ...
        if (self.config.mode == TradingMode.PAPER
            ...
            logger.info("AGAPE-SOL-PERP: Paper mode liquidation recovery — re-enabling bot")
            self._enabled = True
            self._liquidated = False
            self._liquidation_recovery_at = None
        ...
        for pos in open_positions:
            try:
                ...
                logger.error(f"AGAPE-SOL-PERP: Position mgmt error: {e}")
    ...
    def _manage_no_loss_trailing(self, pos, current_price, now):
        ...
        if not trailing_active and max_profit_pct >= self.config.no_loss_activation_pct:
            ...
            self.db._execute("UPDATE agape_sol_perp_positions SET trailing_active = TRUE, current_stop = %s WHERE position_id = %s AND status = 'open'",
        ...
        if trailing_active:
            ...
            if is_long:
                ...
                if new_stop > (current_stop or 0) and new_stop >= entry:
                    self.db._execute("UPDATE agape_sol_perp_positions SET current_stop = %s WHERE position_id = %s AND status = 'open'", (new_stop, pos["position_id"]))
            else:
                new_stop = round(hwm + trail_dist, 2)
    ...
    def _manage_regime_aware(self, pos, current_price, now):
        ...
        if decision.action == ExitAction.ARM_TRAIL:
            self.db._execute(
                "UPDATE agape_sol_perp_positions SET trailing_active = TRUE, current_stop = %s "
        ...
        if decision.action == ExitAction.UPDATE_TRAIL:
            self.db._execute(
                "UPDATE agape_sol_perp_positions SET current_stop = %s "
                "WHERE position_id = %s AND status = 'open'",
                (round(decision.new_stop, 8), pos["position_id"]),
            )
            return False
    ...
    def _check_funding_flip_exit(self, pos):
        ...
        try:
            ...
            logger.debug(f"AGAPE-SOL-PERP: funding-flip check failed: {e}")
        return None
    ...
    def _close_position(self, pos, current_price, reason):
        ...
        if success:
            ...
            record_agape_sol_perp_trade_outcome(pos["side"].upper(), won, self._cycle_count)
    ...
    def _check_entry_conditions(self, now):
        ...
        if balance <= min_required:
            logger.warning(f"AGAPE-SOL-PERP: Insufficient capital ${balance:.2f} (need ${min_required:.2f})")
    ...
    def _check_margin_usage(self, open_pos):
        ...
        try:
            ...
            if blocked:
                logger.warning(f"AGAPE-SOL-PERP: margin {usage:.1f}% >= 70% — refusing new opens")
                return f"MARGIN_OVER_THRESHOLD_{usage:.1f}pct"
        except (ImportError, Exception) as e:
            logger.debug(f"AGAPE-SOL-PERP: margin gate skipped: {e}")
        return None
    ...
    def _log_scan(self, result, ctx, signal=None):
        ...
        self.db.log_scan({
            "outcome": result.get("outcome"), "sol_price": md.get("spot_price"),
    ...
    def _save_equity_snapshot(self, market_data):
        """Save equity snapshot with sol_price."""
        try:
            open_pos = self.db.get_open_positions()
            cp = self.executor.get_current_price()
            ...
            self.db.save_equity_snapshot(equity=round(equity, 2), unrealized_pnl=round(unrealized, 2),
                                          ...
                                          sol_price=cp, funding_rate=market_data.get("funding_rate") if market_data else None)
        except Exception as e:
            logger.warning(f"AGAPE-SOL-PERP: Snapshot failed: {e}")
    ...
    def get_status(self):
        ...
        if ret < -90:
            logger.warning(f"AGAPE-SOL-PERP: Return is {ret:.1f}% — approaching or past liquidation threshold")
        ...
        return {
            "bot_name": "AGAPE_SOL_PERP", "status": status,
            "mode": self.config.mode.value, "ticker": "SOL", "instrument": "SOL-PERP",
            "exchange": "perpetual", "cycle_count": self._cycle_count or self.db.get_scan_count(),
            "open_positions": len(open_pos), "current_sol_price": cp,
    ...
    def enable(self):
        self._enabled = True
        self.db.log("INFO", "ENABLE", "AGAPE-SOL-PERP enabled")

    def disable(self):
        self._enabled = False
        self.db.log("INFO", "DISABLE", "AGAPE-SOL-PERP disabled")
