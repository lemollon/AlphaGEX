"""
AGAPE-SOL-PERP Executor - Executes SOL perpetual contract trades.
...
Key differences from AGAPE-XRP:
    ...
    - Position ID prefix: "AGAPE-SOL-PERP-"
    - Uses quantity (float SOL) instead of contracts (int)
    - get_current_price() uses CryptoDataProvider "SOL" only
    - Slippage: 0.001 (0.1%)
...
class AgapeSolPerpExecutor:
    """Executes SOL perpetual contract trades.

    SOL-PERP: Perpetual contract, no expiration, quantity in SOL units.
    No tastytrade/CME integration - perpetual contracts trade on crypto exchanges.
    """

    def __init__(self, config: AgapeSolPerpConfig, db=None):
        self.config = config
        self.db = db
    ...
    def _init_crypto_provider(self):
        ...
        try:
            ...
            logger.info("AGAPE-SOL-PERP Executor: CryptoDataProvider initialized")
        except ImportError:
            logger.warning("AGAPE-SOL-PERP Executor: CryptoDataProvider not available")
        except Exception as e:
            logger.warning(f"AGAPE-SOL-PERP Executor: CryptoDataProvider init failed: {e}")

    def execute_trade(self, signal: AgapeSolPerpSignal) -> Optional[AgapeSolPerpPosition]:
        ...
        if not approved:
            ...
            logger.warning(f"AGAPE-SOL-PERP: Trade BLOCKED by margin check: {reason}")
            return None
        ...
        if not margin_ok:
            ...
            logger.warning(
                f"AGAPE-SOL-PERP: Trade BLOCKED by free-margin check: {margin_reason}"
            )
            return None
    ...
    def _execute_paper(self, signal: AgapeSolPerpSignal) -> Optional[AgapeSolPerpPosition]:
        ...
        try:
            ...
            logger.error(f"AGAPE-SOL-PERP Executor: Paper execution failed: {e}")
            return None

    def _execute_live(self, signal: AgapeSolPerpSignal) -> Optional[AgapeSolPerpPosition]:
    ...
    def get_current_price(self) -> Optional[float]:
        ...
        if self._crypto_provider:
            try:
                snapshot = self._crypto_provider.get_snapshot("SOL")
                if snapshot and snapshot.spot_price > 0:
                    return snapshot.spot_price
            except Exception as e:
                logger.warning(f"AGAPE-SOL-PERP Executor: CryptoDataProvider price fetch failed: {e}")

        # Fallback: try to re-initialize and fetch
        try:
            from data.crypto_data_provider import get_crypto_data_provider
            provider = get_crypto_data_provider()
            snapshot = provider.get_snapshot("SOL")
