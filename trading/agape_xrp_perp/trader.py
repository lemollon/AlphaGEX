...
class AgapeXrpPerpTrader:
    ...
    def _startup_recovery(self):
        ...
        try:
            ...
            for pos in open_pos:
                ...
                try:
                    ...
                    if age_hours >= stale_threshold_hours:
                        ...
                        stale_closed += 1
                        logger.warning(
                            f"AGAPE-XRP-PERP: Startup recovery closed stale position "
            ...
            logger.error(f"AGAPE-XRP-PERP: Startup recovery failed: {e}")
    ...
    def _manage_regime_aware(self, pos, current_price, now):
        ...
        if decision.action == ExitAction.UPDATE_TRAIL:
            self.db._execute(
                "UPDATE agape_xrp_perp_positions SET current_stop = %s "
                "WHERE position_id = %s AND status = 'open'",
                (round(decision.new_stop, 8), pos["position_id"]),
            )
            return False
    ...
    def _check_funding_flip_exit(self, pos):
        ...
        try:
            ...
            logger.debug(f"AGAPE-XRP-PERP: funding-flip check failed: {e}")
        return None
    ...
    def _check_entry_conditions(self, now):
        ...
        if balance <= min_required:
            logger.warning(f"AGAPE-XRP-PERP: Insufficient capital ${balance:.2f} (need ${min_required:.2f})")
    ...
    def _check_margin_usage(self, open_pos):
        ...
        Matches the equity definition used by /api/agape-xrp-perp/margin
        (starting_capital + realized_pnl) so trader and dashboard agree.
        Fails open on any error so a transient compute failure can't
        permanently freeze the bot.
        """
        try:
            ...
            if blocked:
                logger.warning(f"AGAPE-XRP-PERP: margin {usage:.1f}% >= 70% — refusing new opens")
                return f"MARGIN_OVER_THRESHOLD_{usage:.1f}pct"
        except (ImportError, Exception) as e:
            logger.debug(f"AGAPE-XRP-PERP: margin gate skipped: {e}")
        return None
