"""
AGAPE-XRP-PERP Models - Data structures for the XRP Perpetual Contract bot.

Mirrors AGAPE-XRP (Futures) models pattern but adapted for perpetual contracts:
...
@dataclass
class AgapeXrpPerpConfig:
    """Configuration for AGAPE-XRP-PERP bot.

    AGGRESSIVE MODE (matching AGAPE/VALOR aggressiveness).
    Perpetual contract specifics: 24/7 trading, quantity-based sizing.
    """

    # Identity
    bot_name: str = "AGAPE_XRP_PERP"
    ticker: str = "XRP"
    instrument: str = "XRP-PERP"       # Perpetual contract
    ...
    # Timing: 24/7/365 for perpetuals (no entry window restrictions)
    # These are kept for config compatibility but not enforced
...
@dataclass
class AgapeXrpPerpSignal:
    """Trading signal generated by AGAPE-XRP-PERP signal engine.
    ...
    # Trade parameters (quantity-based for perpetuals)
    side: Optional[str] = None
    entry_price: Optional[float] = None
    stop_loss: Optional[float] = None
    take_profit: Optional[float] = None
    quantity: float = 0.0               # XRP quantity (float)
...
@dataclass
class AgapeXrpPerpPosition:
    """An open or closed AGAPE-XRP-PERP perpetual position.
    ...
    position_id: str
    side: PositionSide
    quantity: float                     # XRP quantity (float)
    entry_price: float
    stop_loss: float
    take_profit: float
    max_risk_usd: float
    ...
    def calculate_pnl(self, current_price: float) -> float:
        ...
        Perpetual contracts: P&L = (current - entry) * quantity * direction
        No contract multiplier - quantity is in raw XRP units.
        """
