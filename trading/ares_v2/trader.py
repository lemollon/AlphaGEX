...
class ARESTrader(MathOptimizerMixin):
    ...
    def _record_proverbs_outcome(
        ...
        try:
            ...
            if alerts:
                for alert in alerts:
                    logger.warning(f"ARES Proverbs Alert: {alert}")

            logger.debug(f"ARES: Recorded outcome to Proverbs Enhanced - P&L=${pnl:.2f}")

        except Exception as e:
            logger.warning(f"ARES: Proverbs outcome recording failed: {e}")
