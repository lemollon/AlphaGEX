"""
HERACLES ML Advisor - Machine Learning for MES Futures Scalping
================================================================

PURPOSE:
This module creates an ML-enhanced win probability estimator for HERACLES.
...
class HERACLESTrainingMetrics:
    """Metrics from HERACLES model training"""
    accuracy: float
    precision: float
    recall: float
    f1_score: float
...
class HERACLESMLAdvisor:
    """
    ML Advisor for HERACLES MES Futures Scalping.
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            if model_data is None:
                logger.info("No HERACLES ML model found in database")
                return False
            ...
            logger.info(f"Loaded HERACLES ML model v{self.model_version} from database")
            return True

        except Exception as e:
            logger.warning(f"Failed to load HERACLES ML model from database: {e}")
            return False
    ...
    def train(self, min_samples: int = None, use_new_params_only: bool = True) -> Optional[HERACLESTrainingMetrics]:
        ...
        logger.info(f"Training HERACLES ML on {len(df)} trades")
        ...
        # Train XGBoost (matching SAGE's hyperparameters)
        ...
        logger.info(f"HERACLES ML trained successfully:")
...
def get_heracles_ml_advisor() -> HERACLESMLAdvisor:
    """Get or create the HERACLES ML advisor singleton"""
    global _advisor
    if _advisor is None:
