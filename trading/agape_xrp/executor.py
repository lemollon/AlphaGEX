"""
AGAPE-XRP Executor - Executes /XRP trades via tastytrade API.

Same logic as AGAPE (ETH) executor but for XRP Futures.
"""
...
class AgapeXrpExecutor:
    """Executes XRP Futures trades.

    /XRP Contract: 2,500 XRP per contract.
    """

    XRP_CONTRACT_SIZE = 2500.0
    XRP_SYMBOL = "/XRP"

    def __init__(self, config: AgapeXrpConfig, db=None):
        self.config = config
        self.db = db
        self._session = None
        self._account = None
    ...
    def _init_tastytrade(self):
        ...
        if not all([username, password]):
            logger.error("AGAPE-XRP Executor: TASTYTRADE credentials not set")
            return
        try:
            self._session = TastytradeSession(username, password)
            if account_id:
                accounts = TastytradeAccount.get_accounts(self._session)
                self._account = next((a for a in accounts if a.account_number == account_id), accounts[0] if accounts else None)
                if self._account:
                    logger.info(f"AGAPE-XRP Executor: Connected to tastytrade account {account_id}")
        except Exception as e:
            logger.error(f"AGAPE-XRP Executor: tastytrade init failed: {e}")
    ...
    def _execute_paper(self, signal: AgapeXrpSignal) -> Optional[AgapeXrpPosition]:
        try:
            ...
            position_id = f"AGAPE-XRP-{uuid.uuid4().hex[:8].upper()}"
            return AgapeXrpPosition(
                position_id=position_id,
                side=PositionSide.LONG if signal.side == "long" else PositionSide.SHORT,
                contracts=signal.contracts, entry_price=round(fill_price, 4),
    ...
    def _execute_live(self, signal: AgapeXrpSignal) -> Optional[AgapeXrpPosition]:
        ...
        try:
            ...
            xrp_future = self._get_active_xrp_contract()
            if not xrp_future:
                return None
            action = OrderAction.BUY_TO_OPEN if signal.side == "long" else OrderAction.SELL_TO_OPEN
            order = NewOrder(
                time_in_force=OrderTimeInForce.DAY, order_type=OrderType.MARKET,
                legs=[{"instrument_type": "Future", "symbol": xrp_future.symbol, "action": action, "quantity": signal.contracts}],
            )
            ...
            if response and response.order:
                ...
                return AgapeXrpPosition(
                    position_id=f"AGAPE-XRP-{response.order.id}",
                    side=PositionSide.LONG if signal.side == "long" else PositionSide.SHORT,
                    contracts=signal.contracts, entry_price=fill_price,
                    stop_loss=signal.stop_loss, take_profit=signal.take_profit,
                    max_risk_usd=signal.max_risk_usd, underlying_at_entry=signal.spot_price,
                    funding_rate_at_entry=signal.funding_rate,
                    funding_regime_at_entry=signal.funding_regime,
                    ls_ratio_at_entry=signal.ls_ratio, squeeze_risk_at_entry=signal.squeeze_risk,
                    max_pain_at_entry=signal.max_pain, crypto_gex_at_entry=signal.crypto_gex,
                    crypto_gex_regime_at_entry=signal.crypto_gex_regime,
                    oracle_advice=signal.oracle_advice,
                    oracle_win_probability=signal.oracle_win_probability,
                    oracle_confidence=signal.oracle_confidence,
                    oracle_top_factors=signal.oracle_top_factors,
                    signal_action=signal.action.value, signal_confidence=signal.confidence,
                    signal_reasoning=signal.reasoning,
                    status=PositionStatus.OPEN, open_time=datetime.now(CENTRAL_TZ),
                    high_water_mark=fill_price,
                )
            return None
        except Exception as e:
            logger.error(f"AGAPE-XRP Executor: Live execution failed: {e}")
            return None

    def get_current_price(self) -> Optional[float]:
        if self._session:
            try:
                xrp = self._get_active_xrp_contract()
                if xrp:
                    quote = self._account.get_quote(self._session, xrp.symbol)
                    if quote:
    ...
    def _get_active_xrp_contract(self):
        if not self._session:
            return None
        try:
            from tastytrade.instruments import Future
            futures = Future.get_futures(self._session, product_codes=["XRP"])
            if futures:
                active = [f for f in futures if f.is_tradeable]
                if active:
                    return sorted(active, key=lambda f: f.expiration_date)[0]
        except Exception as e:
            logger.error(f"AGAPE-XRP Executor: Failed to find /XRP contract: {e}")
        return None
