"""
AGAPE-LTC-FUTURES Models - Data structures for the LTC Futures Contract bot.
...
LTC: OG large-cap (~$70-100). 50 LTC per contract.

KEY DIFFERENCES from a perpetual contract bot:
- Trades Coinbase Derivatives monthly futures (LTC-29MAY26-CDE, LTC-26JUN26-CDE, etc.)
  rather than the geo-blocked Coinbase International perpetual.
- Sizing is integer contracts (1 contract = 50 units of "LTC" = 50 LTC underlying).
...
@dataclass
class AgapeLtcFuturesConfig:
    """Configuration for AGAPE-LTC-FUTURES bot.

    AGGRESSIVE MODE - Futures contracts, 24/7 trading.
    LTC: OG large-cap, mid-priced coin.
    """

    # Identity
    bot_name: str = "AGAPE_LTC_FUTURES"
    ticker: str = "LTC"
    instrument: str = "LTC-FUT"     # Futures contract
    ...
    # Risk management
    starting_capital: float = 2500.0    # $2.5K starting capital (LTC allocation)
    risk_per_trade_pct: float = 5.0     # 5% risk per trade
    max_quantity: float = 5.0           # Max 5 contracts per position — at ~$4000/contract notional that's ~$20K per trade
    max_open_positions: int = 2         # Conservative for crypto futures

    # Position sizing - Futures contract specs (integer contracts only)
    # 1 contract = 50 LTC underlying (Coinbase Derivatives LTC contract)
    contract_size: int = 50              # 1 contract = 50 LTC underlying
    ...
    no_loss_activation_pct: float = 0.5       # 0.5% activation (LTC has different vol profile)
    no_loss_trail_distance_pct: float = 0.3   # 0.3% trail distance
    no_loss_emergency_stop_pct: float = 5.0
    max_unrealized_loss_pct: float = 1.5      # 1.5% max loss (LTC)
    ...
    @classmethod
    def load_from_db(cls, db) -> "AgapeLtcFuturesConfig":
...
@dataclass
class AgapeLtcFuturesSignal:
    """Trading signal generated by AGAPE-LTC-FUTURES signal engine."""
    spot_price: float
    timestamp: datetime
...
@dataclass
class AgapeLtcFuturesPosition:
    """An open or closed AGAPE-LTC-FUTURES position."""
    position_id: str
    side: PositionSide
    quantity: float
    entry_price: float
