...
class AgapeBtcPerpDatabase:
    ...
    def _ensure_tables(self):
        ...
        try:
            ...
            for col_sql in [
                ...
                "ALTER TABLE agape_btc_perp_positions ADD COLUMN IF NOT EXISTS accrued_funding_usd FLOAT DEFAULT 0",
                "ALTER TABLE agape_btc_perp_positions ADD COLUMN IF NOT EXISTS last_funding_accrual TIMESTAMP WITH TIME ZONE DEFAULT NOW()",
            ]:
                try:
    ...
    def accrue_funding(self, position_id: str, delta_usd: float, now=None) -> bool:
        ...
        try:
            ...
            logger.error(f"AGAPE-BTC-PERP DB: Failed to accrue funding for {position_id}: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def get_scan_count(self) -> int:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM agape_btc_perp_scan_activity")
            row = cursor.fetchone()
