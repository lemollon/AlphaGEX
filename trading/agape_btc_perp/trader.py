...
class AgapeBtcPerpTrader:
    ...
    def __init__(self, config: Optional[AgapeBtcPerpConfig] = None):
        ...
        self._liquidation_recovery_at: Optional[datetime] = None
    ...
    def _manage_positions(self, market_data: Optional[Dict]) -> tuple:
        ...
        if (self.config.mode == TradingMode.PAPER
            ...
            logger.info("AGAPE-BTC-PERP: Paper mode liquidation recovery — re-enabling bot")
            self._enabled = True
    ...
    def _manage_regime_aware(self, pos, current_price, now):
        ...
        if decision.action == ExitAction.ARM_TRAIL:
            self.db._execute(
                "UPDATE agape_btc_perp_positions SET trailing_active = TRUE, current_stop = %s "
        ...
        if decision.action == ExitAction.UPDATE_TRAIL:
            self.db._execute(
                "UPDATE agape_btc_perp_positions SET current_stop = %s "
                "WHERE position_id = %s AND status = 'open'",
                (round(decision.new_stop, 8), pos["position_id"]),
            )
            return False
    ...
    def _check_margin_usage(self, open_pos):
        ...
        try:
            ...
            if blocked:
                logger.warning(f"AGAPE-BTC-PERP: margin {usage:.1f}% >= 70% — refusing new opens")
                return f"MARGIN_OVER_THRESHOLD_{usage:.1f}pct"
        except (ImportError, Exception) as e:
            logger.debug(f"AGAPE-BTC-PERP: margin gate skipped: {e}")
        return None
