...
class OrderExecutor:
    ...
    def __init__(self, config: ATHENAConfig):
        ...
        if POSITION_MGMT_AVAILABLE:
            try:
                ...
                logger.info("ATHENA OrderExecutor: Position Management Agent initialized")
            except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if mtm.get('success') and mtm.get('current_value') is not None:
                    ...
                    logger.info(f"ATHENA PAPER CLOSE MTM: {position.position_id} close_value=${close_price:.4f}")
            except Exception as e:
    ...
    def store_entry_conditions(self, position_id: int, gex_data: Dict) -> bool:
        ...
        try:
            ...
            logger.info(f"[ATHENA] Entry conditions stored for position {position_id}")
            return True
        except Exception as e:
        ...
        try:
            ...
            if result.get('alerts'):
                for alert in result['alerts']:
                    logger.info(f"[ATHENA CONDITION ALERT] {alert['type']}: {alert['message']}")
