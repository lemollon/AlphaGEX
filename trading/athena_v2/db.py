            ...
            with db_connection() as conn:
                ...
                c.execute("""
                    ...
                    ON CONFLICT (trade_date) DO UPDATE SET
                        trades_executed = athena_daily_perf.trades_executed + EXCLUDED.trades_executed,
                        positions_closed = athena_daily_perf.positions_closed + EXCLUDED.positions_closed,
                        realized_pnl = athena_daily_perf.realized_pnl + EXCLUDED.realized_pnl,
                        updated_at = NOW()
                """, (
                    summary.date,
    ...
    def get_current_balance(self) -> float:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    SELECT balance FROM athena_equity_snapshots
        ...
        return 100000.0  # ATHENA default capital
