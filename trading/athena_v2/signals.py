...
class SignalGenerator:
    ...
    def _init_components(self) -> None:
        ...
        if ML_REGIME_AVAILABLE and MLRegimeClassifier:
            try:
                ...
                logger.info("SignalGenerator: ML Regime Classifier initialized")
            except Exception as e:
    ...
    def get_ensemble_boost(self, gex_data: Dict, direction: str, oracle: Dict = None) -> Dict:
        ...
        try:
            ...
            if ensemble:
                logger.info(f"[ATHENA ENSEMBLE] Confidence: {ensemble.confidence:.0f}%, Size: {ensemble.position_size_multiplier:.0%}")
                return {
    ...
    def generate_signal(self) -> Optional[TradeSignal]:
        ...
        if regime_prediction:
            ...
            logger.info(f"[ATHENA ML REGIME] Action: {regime_action}, Confidence: {regime_conf:.1%}, Aligns: {aligns}")
            ...
            if aligns and regime_conf > 0.60:
                ...
                # ML thinks market is rangebound - reduce directional confidence
                penalty = (regime_conf - 0.70) * 0.15
