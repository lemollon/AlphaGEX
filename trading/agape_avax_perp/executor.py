"""
AGAPE-AVAX-PERP Executor - Executes AVAX perpetual contract trades.
...
Key differences from AGAPE-XRP:
    ...
    - Position ID prefix: "AGAPE-AVAX-PERP-"
    - Uses quantity (float AVAX) instead of contracts (int)
    - get_current_price() uses CryptoDataProvider "AVAX" only
    - Slippage: 0.001 (0.1%)
...
class AgapeAvaxPerpExecutor:
    """Executes AVAX perpetual contract trades.

    AVAX-PERP: Perpetual contract, no expiration, quantity in AVAX units.
    No tastytrade/CME integration - perpetual contracts trade on crypto exchanges.
    """

    def __init__(self, config: AgapeAvaxPerpConfig, db=None):
        self.config = config
        self.db = db
    ...
    def _init_crypto_provider(self):
        ...
        try:
            ...
            logger.info("AGAPE-AVAX-PERP Executor: CryptoDataProvider initialized")
        except ImportError:
            logger.warning("AGAPE-AVAX-PERP Executor: CryptoDataProvider not available")
        except Exception as e:
            logger.warning(f"AGAPE-AVAX-PERP Executor: CryptoDataProvider init failed: {e}")

    def execute_trade(self, signal: AgapeAvaxPerpSignal) -> Optional[AgapeAvaxPerpPosition]:
        ...
        if not approved:
            ...
            logger.warning(f"AGAPE-AVAX-PERP: Trade BLOCKED by margin check: {reason}")
            return None
        ...
        if not margin_ok:
            ...
            logger.warning(
                f"AGAPE-AVAX-PERP: Trade BLOCKED by free-margin check: {margin_reason}"
            )
            return None
    ...
    def _execute_paper(self, signal: AgapeAvaxPerpSignal) -> Optional[AgapeAvaxPerpPosition]:
        ...
        try:
            ...
            position_id = f"AGAPE-AVAX-PERP-{uuid.uuid4().hex[:8].upper()}"
            ...
            logger.error(f"AGAPE-AVAX-PERP Executor: Paper execution failed: {e}")
            return None

    def _execute_live(self, signal: AgapeAvaxPerpSignal) -> Optional[AgapeAvaxPerpPosition]:
        ...
        """
        logger.error("AGAPE-AVAX-PERP Executor: LIVE execution is disabled until a real perpetual venue adapter is configured")
        return None

    def get_current_price(self) -> Optional[float]:
        """Get current AVAX price from CryptoDataProvider.

        No tastytrade/CME fallback - perpetual contracts use crypto exchange data only.
        """
        if self._crypto_provider:
            try:
                snapshot = self._crypto_provider.get_snapshot("AVAX")
                if snapshot and snapshot.spot_price > 0:
                    return snapshot.spot_price
            except Exception as e:
                logger.warning(f"AGAPE-AVAX-PERP Executor: CryptoDataProvider price fetch failed: {e}")

        # Fallback: try to re-initialize and fetch
        try:
            from data.crypto_data_provider import get_crypto_data_provider
            provider = get_crypto_data_provider()
            snapshot = provider.get_snapshot("AVAX")
