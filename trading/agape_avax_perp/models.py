"""
AGAPE-AVAX-PERP Models - Data structures for the AVAX Perpetual Contract bot.

Key differences from AGAPE-XRP (futures):
    - Exchange is PERPETUAL (not tastytrade CME)
...
@dataclass
class AgapeAvaxPerpConfig:
    """Configuration for AGAPE-AVAX-PERP bot.

    AGGRESSIVE MODE - perpetual contract trading on AVAX.
    Uses quantity-based sizing (float AVAX units) instead of integer contracts.
    """

    # Identity
    bot_name: str = "AGAPE_AVAX_PERP"
    ticker: str = "AVAX"
    instrument: str = "AVAX-PERP"
    ...
    # No-Loss Trailing Strategy (retuned 2026-05-05; was 1.5/1.25/3.0/24h.
    # Slightly wider activation than ETH/SOL because AVAX is thinner-liquidity
    # and more prone to noise spikes that would otherwise arm too eagerly.)
    use_no_loss_trailing: bool = True
    no_loss_activation_pct: float = 0.8
    no_loss_trail_distance_pct: float = 0.4
    ...
    # Timing - perpetual contracts trade 24/7/365 (no CME hours)
    # These are kept for interface compatibility but not enforced
    ...
    @classmethod
    def load_from_db(cls, db) -> "AgapeAvaxPerpConfig":
...
@dataclass
class AgapeAvaxPerpSignal:
    """Trading signal generated by AGAPE-AVAX-PERP signal engine.
    ...
    # Trade parameters - quantity-based (float AVAX, not int contracts)
...
@dataclass
class AgapeAvaxPerpPosition:
    """An open or closed AGAPE-AVAX-PERP position.
    ...
    def calculate_pnl(self, current_price: float) -> float:
        ...
        AVAX Perpetual: P&L = (current_price - entry_price) * quantity * direction
        No contract multiplier - quantity is directly in AVAX units.
        """
