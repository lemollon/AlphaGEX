"""
AGAPE-AVAX-PERP Signal Generator - Generates directional AVAX perpetual trade signals.

Key differences from AGAPE-XRP:
    - Uses CryptoDataProvider with ticker "AVAX"
    - _calculate_position_size returns (quantity, max_risk_usd) where quantity is float AVAX
...
try:
    ...
    logger.info("AGAPE-AVAX-PERP Signals: CryptoDataProvider loaded")
except ImportError as e:
    logger.warning(f"AGAPE-AVAX-PERP Signals: CryptoDataProvider not available: {e}")
...
try:
    ...
    logger.info("AGAPE-AVAX-PERP Signals: ProphetAdvisor loaded")
except ImportError as e:
    logger.warning(f"AGAPE-AVAX-PERP Signals: ProphetAdvisor not available: {e}")


class AgapeAvaxPerpDirectionTracker:
...
_direction_tracker: Optional[AgapeAvaxPerpDirectionTracker] = None


def get_agape_avax_perp_direction_tracker(config=None):
    global _direction_tracker
    if _direction_tracker is None:
        _direction_tracker = AgapeAvaxPerpDirectionTracker(
...
def record_agape_avax_perp_trade_outcome(direction, is_win, scan_number):
    get_agape_avax_perp_direction_tracker().record_trade(direction, is_win, scan_number)


class AgapeAvaxPerpSignalGenerator:
    """Generates directional trade signals for AVAX-PERP based on crypto microstructure."""

    def __init__(self, config: AgapeAvaxPerpConfig):
        self.config = config
        ...
        if get_crypto_data_provider:
            try:
                ...
                logger.warning(f"AGAPE-AVAX-PERP Signals: Crypto provider init failed: {e}")
        if ProphetAdvisor:
            try:
                self._oracle = ProphetAdvisor()
            except Exception as e:
                logger.warning(f"AGAPE-AVAX-PERP Signals: Prophet init failed: {e}")
    ...
    def get_market_data(self):
        ...
        try:
            ...
            return {
                ...
                "ls_long_pct": snapshot.ls_ratio.long_pct if snapshot.ls_ratio else None,
                "oi_total_usd": snapshot.oi_snapshot.total_usd if snapshot.oi_snapshot else None,
                "taker_buy_ratio": snapshot.taker_volume.buy_ratio if snapshot.taker_volume else None,
            ...
            logger.error(f"AGAPE-AVAX-PERP Signals: Market data fetch failed: {e}")
            return None
    ...
    def get_prophet_advice(self, market_data):
        ...
        try:
            ...
            logger.error(f"AGAPE-AVAX-PERP Signals: Prophet call failed: {e}")
    ...
    def generate_signal(self, prophet_data=None):
        ...
        if not market_data:
            return AgapeAvaxPerpSignal(spot_price=0, timestamp=now, action=SignalAction.WAIT, reasoning="NO_MARKET_DATA")
    ...
    def _determine_action(self, combined_signal, confidence, market_data):
        ...
        if confidence_rank.get(confidence, 0) < confidence_rank.get(self.config.min_confidence, 1):
            ...
            logger.info(
                f"AGAPE-AVAX-PERP Signals: DEGRADED_NO_COINGLASS override - "
                f"trading {combined_signal} at {confidence} confidence (CoinGlass unavailable)"
            )
        tracker = get_agape_avax_perp_direction_tracker(self.config)
    ...
    def _calculate_position_size(self, spot_price: float) -> Tuple[float, float]:
        """Calculate position size as AVAX quantity (float) and max risk in USD.

        Returns:
            (quantity, max_risk_usd) where quantity is float AVAX amount.
        """
        capital = self.config.starting_capital
        max_risk_usd = capital * (self.config.risk_per_trade_pct / 100)

        # Risk per unit AVAX based on stop distance (2% default)
        stop_distance = spot_price * 0.02 * (self.config.stop_loss_pct / 100)
        if stop_distance <= 0:
            return (self.config.default_quantity, max_risk_usd)

        # quantity = max_risk / risk_per_avax
        ...
        try:
            ...
            if max_qty_by_notional > 0 and max_qty_by_notional < quantity:
                logger.debug(
                    f"AGAPE-AVAX-PERP: notional cap reducing size "
                    f"{quantity:.3f} -> {max_qty_by_notional:.3f} AVAX "
                    f"(per_pos={per_pos_margin_pct:.1f}% margin, lev={leverage}x)"
                )
                quantity = max(self.config.min_quantity, max_qty_by_notional)
        except Exception as e:
            logger.debug(f"AGAPE-AVAX-PERP: notional cap skipped: {e}")

        # Round to 3 decimal places (0.001 AVAX precision)
