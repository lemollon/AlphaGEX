"""
AGAPE-DOGE-PERP Trader - Main orchestrator for DOGE Perpetual contract trading.
...
from trading.agape_doge_perp.db import AgapeDogePerpDatabase
from trading.agape_doge_perp.signals import (
    AgapeDogePerpSignalGenerator, get_agape_doge_perp_direction_tracker,
    record_agape_doge_perp_trade_outcome,
)
from trading.agape_doge_perp.executor import AgapeDogePerpExecutor

logger = logging.getLogger(__name__)
CENTRAL_TZ = ZoneInfo("America/Chicago")

_agape_doge_perp_trader: Optional["AgapeDogePerpTrader"] = None


def get_agape_doge_perp_trader():
    return _agape_doge_perp_trader


def create_agape_doge_perp_trader(config=None):
    global _agape_doge_perp_trader
    _agape_doge_perp_trader = AgapeDogePerpTrader(config)
    return _agape_doge_perp_trader


class AgapeDogePerpTrader:
    """Main AGAPE-DOGE-PERP trading bot orchestrator."""

    def __init__(self, config=None):
        self.db = AgapeDogePerpDatabase()
        self.config = config or AgapeDogePerpConfig.load_from_db(self.db)
        self.signals = AgapeDogePerpSignalGenerator(self.config)
        self.executor = AgapeDogePerpExecutor(self.config, self.db)
        ...
        self._direction_tracker = get_agape_doge_perp_direction_tracker(self.config)
        self._startup_recovery()
        self.db.log("INFO", "INIT", f"AGAPE-DOGE-PERP initialized AGGRESSIVE (mode={self.config.mode.value})")
        logger.info(f"AGAPE-DOGE-PERP Trader: Initialized AGGRESSIVE (mode={self.config.mode.value})")
    ...
    def _startup_recovery(self):
        ...
        try:
            ...
            for pos in open_pos:
                ...
                try:
                    ...
                    if age_hours >= stale_threshold_hours:
                        ...
                        stale_closed += 1
                        logger.warning(
                            f"AGAPE-DOGE-PERP: Startup recovery closed stale position "
            ...
            logger.error(f"AGAPE-DOGE-PERP: Startup recovery failed: {e}")
    ...
    def run_cycle(self, close_only=False):
        ...
        try:
            ...
            logger.error(f"AGAPE-DOGE-PERP Trader: Cycle failed: {e}", exc_info=True)
    ...
    def _manage_positions(self, market_data):
        ...
        if (self.config.mode == TradingMode.PAPER
            ...
            logger.info("AGAPE-DOGE-PERP: Paper mode liquidation recovery — re-enabling bot")
            self._enabled = True
            self._liquidated = False
            self._liquidation_recovery_at = None
        ...
        for pos in open_positions:
            try:
                ...
                logger.error(f"AGAPE-DOGE-PERP: Position mgmt error: {e}")
    ...
    def _manage_no_loss_trailing(self, pos, current_price, now):
        ...
        if not trailing_active and max_profit_pct >= self.config.no_loss_activation_pct:
            ...
            self.db._execute("UPDATE agape_doge_perp_positions SET trailing_active = TRUE, current_stop = %s WHERE position_id = %s AND status = 'open'",
        ...
        if trailing_active:
            ...
            if is_long:
                ...
                if new_stop > (current_stop or 0) and new_stop >= entry:
                    self.db._execute("UPDATE agape_doge_perp_positions SET current_stop = %s WHERE position_id = %s AND status = 'open'", (new_stop, pos["position_id"]))
            else:
                new_stop = round(hwm + trail_dist, 6)
    ...
    def _manage_regime_aware(self, pos, current_price, now):
        ...
        if decision.action == ExitAction.ARM_TRAIL:
            self.db._execute(
                "UPDATE agape_doge_perp_positions SET trailing_active = TRUE, current_stop = %s "
        ...
        if decision.action == ExitAction.UPDATE_TRAIL:
            self.db._execute(
                "UPDATE agape_doge_perp_positions SET current_stop = %s "
                "WHERE position_id = %s AND status = 'open'",
                (round(decision.new_stop, 8), pos["position_id"]),
            )
            return False
    ...
    def _check_funding_flip_exit(self, pos):
        ...
        try:
            ...
            logger.debug(f"AGAPE-DOGE-PERP: funding-flip check failed: {e}")
        return None
    ...
    def _close_position(self, pos, current_price, reason):
        ...
        if success:
            ...
            record_agape_doge_perp_trade_outcome(pos["side"].upper(), won, self._cycle_count)
    ...
    def _save_equity_snapshot(self, market_data):
        try:
            ...
            logger.warning(f"AGAPE-DOGE-PERP: Snapshot failed: {e}")
    ...
    def get_status(self):
        ...
        if ret < -90:
            logger.warning(f"AGAPE-DOGE-PERP: Return is {ret:.1f}% — approaching or past liquidation threshold")
        ...
        return {
            "bot_name": "AGAPE_DOGE_PERP", "status": status,
    ...
    def enable(self):
        self._enabled = True
        self.db.log("INFO", "ENABLE", "AGAPE-DOGE-PERP enabled")

    def disable(self):
        self._enabled = False
        self.db.log("INFO", "DISABLE", "AGAPE-DOGE-PERP disabled")
