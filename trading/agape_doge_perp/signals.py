"""
AGAPE-DOGE-PERP Signal Generator - Generates directional DOGE perpetual trade signals.

Same logic as AGAPE-XRP signals but for DOGE perpetual contract market microstructure.
"""
...
try:
    ...
    logger.info("AGAPE-DOGE-PERP Signals: CryptoDataProvider loaded")
except ImportError as e:
    logger.warning(f"AGAPE-DOGE-PERP Signals: CryptoDataProvider not available: {e}")
...
try:
    ...
    logger.info("AGAPE-DOGE-PERP Signals: ProphetAdvisor loaded")
except ImportError as e:
    logger.warning(f"AGAPE-DOGE-PERP Signals: ProphetAdvisor not available: {e}")


class AgapeDogePerpDirectionTracker:
...
_direction_tracker: Optional[AgapeDogePerpDirectionTracker] = None


def get_agape_doge_perp_direction_tracker(config=None):
    global _direction_tracker
    if _direction_tracker is None:
        _direction_tracker = AgapeDogePerpDirectionTracker(
...
def record_agape_doge_perp_trade_outcome(direction, is_win, scan_number):
    get_agape_doge_perp_direction_tracker().record_trade(direction, is_win, scan_number)


class AgapeDogePerpSignalGenerator:
    """Generates directional trade signals for DOGE-PERP based on crypto microstructure."""

    def __init__(self, config: AgapeDogePerpConfig):
        self.config = config
        ...
        if get_crypto_data_provider:
            try:
                ...
                logger.warning(f"AGAPE-DOGE-PERP Signals: Crypto provider init failed: {e}")
        if ProphetAdvisor:
            try:
                self._oracle = ProphetAdvisor()
            except Exception as e:
                logger.warning(f"AGAPE-DOGE-PERP Signals: Prophet init failed: {e}")
    ...
    def get_market_data(self):
        ...
        try:
            ...
            logger.error(f"AGAPE-DOGE-PERP Signals: Market data fetch failed: {e}")
            return None
    ...
    def get_prophet_advice(self, market_data):
        ...
        try:
            ...
            logger.error(f"AGAPE-DOGE-PERP Signals: Prophet call failed: {e}")
    ...
    def generate_signal(self, prophet_data=None):
        ...
        if not market_data:
            return AgapeDogePerpSignal(spot_price=0, timestamp=now, action=SignalAction.WAIT, reasoning="NO_MARKET_DATA")
    ...
    def _calculate_position_size(self, spot_price):
        ...
        quantity = max(self.config.min_quantity, min(max_risk_usd / risk_per_unit, self.config.max_quantity))
        ...
        try:
            ...
            if max_qty_by_notional > 0 and max_qty_by_notional < quantity:
                logger.debug(
                    f"AGAPE-DOGE-PERP: notional cap reducing size "
                    f"{quantity:.0f} -> {max_qty_by_notional:.0f} DOGE "
                    f"(per_pos={per_pos_margin_pct:.1f}% margin, lev={leverage}x)"
                )
                quantity = max(self.config.min_quantity, max_qty_by_notional)
        except Exception as e:
            logger.debug(f"AGAPE-DOGE-PERP: notional cap skipped: {e}")

        return (round(quantity, 2), round(quantity * risk_per_unit, 2))
