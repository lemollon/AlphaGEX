"""
AGAPE-DOGE-PERP Models - Data structures for the DOGE Perpetual Contract bot.

Mirrors AGAPE-XRP models pattern with perpetual contract specs.
...
@dataclass
class AgapeDogePerpConfig:
    """Configuration for AGAPE-DOGE-PERP bot.

    AGGRESSIVE MODE - Perpetual contracts, 24/7 trading.
    DOGE: High volatility meme coin, large quantity trades.
    """

    # Identity
    bot_name: str = "AGAPE_DOGE_PERP"
    ticker: str = "DOGE"
    instrument: str = "DOGE-PERP"     # Perpetual contract
    ...
    default_quantity: float = 500.0     # 500 DOGE default trade size
    min_quantity: float = 1.0           # Minimum order size
    ...
    no_loss_activation_pct: float = 0.3      # 0.3% activation (meme coin - tighter)
    no_loss_trail_distance_pct: float = 0.25  # 0.25% trail distance
    no_loss_emergency_stop_pct: float = 5.0
    max_unrealized_loss_pct: float = 0.75     # 0.75% max loss (meme coin)
...
@dataclass
class AgapeDogePerpSignal:
    """Trading signal generated by AGAPE-DOGE-PERP signal engine."""
    spot_price: float
    timestamp: datetime
...
@dataclass
class AgapeDogePerpPosition:
    """An open or closed AGAPE-DOGE-PERP position."""
    position_id: str
    side: PositionSide
    quantity: float
    entry_price: float
