"""
AGAPE-SHIB-PERP Signal Generator - Generates directional SHIB-PERP trade signals.
...
try:
    ...
    logger.info("AGAPE-SHIB-PERP Signals: CryptoDataProvider loaded")
except ImportError as e:
    logger.warning(f"AGAPE-SHIB-PERP Signals: CryptoDataProvider not available: {e}")
...
try:
    ...
    logger.info("AGAPE-SHIB-PERP Signals: ProphetAdvisor loaded")
except ImportError as e:
    logger.warning(f"AGAPE-SHIB-PERP Signals: ProphetAdvisor not available: {e}")


class AgapeShibPerpDirectionTracker:
...
_direction_tracker: Optional[AgapeShibPerpDirectionTracker] = None


def get_agape_shib_perp_direction_tracker(config=None):
    global _direction_tracker
    if _direction_tracker is None:
        _direction_tracker = AgapeShibPerpDirectionTracker(
...
def record_agape_shib_perp_trade_outcome(direction, is_win, scan_number):
    get_agape_shib_perp_direction_tracker().record_trade(direction, is_win, scan_number)


class AgapeShibPerpSignalGenerator:
    """Generates directional trade signals for SHIB-PERP based on crypto microstructure."""

    def __init__(self, config: AgapeShibPerpConfig):
        self.config = config
        ...
        if get_crypto_data_provider:
            try:
                ...
                logger.warning(f"AGAPE-SHIB-PERP Signals: Crypto provider init failed: {e}")
        if ProphetAdvisor:
            try:
                self._oracle = ProphetAdvisor()
            except Exception as e:
                logger.warning(f"AGAPE-SHIB-PERP Signals: Prophet init failed: {e}")
    ...
    def get_market_data(self):
        ...
        try:
            ...
            logger.error(f"AGAPE-SHIB-PERP Signals: Market data fetch failed: {e}")
            return None
    ...
    def get_prophet_advice(self, market_data):
        ...
        try:
            ...
            logger.error(f"AGAPE-SHIB-PERP Signals: Prophet call failed: {e}")
    ...
    def generate_signal(self, prophet_data=None):
        ...
        if not market_data:
            return AgapeShibPerpSignal(spot_price=0, timestamp=now, action=SignalAction.WAIT, reasoning="NO_MARKET_DATA")
    ...
    def _determine_action(self, combined_signal, confidence, market_data):
        ...
        if confidence_rank.get(confidence, 0) < confidence_rank.get(self.config.min_confidence, 1):
            ...
            logger.info(
                f"AGAPE-SHIB-PERP Signals: DEGRADED_NO_COINGLASS override - "
                f"trading {combined_signal} at {confidence} confidence (CoinGlass unavailable)"
            )
        tracker = get_agape_shib_perp_direction_tracker(self.config)
