"""
AGAPE-SHIB-PERP Database Layer - PostgreSQL persistence for the SHIB bot.
...
try:
    ...
    logger.warning("AGAPE-SHIB-PERP DB: database_adapter not available")
...
class AgapeShibPerpDatabase:
    """Database operations for AGAPE-SHIB-PERP bot."""

    def __init__(self, bot_name: str = "AGAPE_SHIB_PERP"):
    ...
    def _execute(self, sql: str, params=None) -> bool:
        ...
        try:
            ...
            logger.error(f"AGAPE-SHIB-PERP DB: Execute failed: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def _ensure_tables(self):
        conn = self._get_conn()
        if not conn:
            logger.warning("AGAPE-SHIB-PERP DB: No database connection, skipping table creation")
            return
        try:
            cursor = conn.cursor()

            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_shib_perp_positions (
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_shib_perp_equity_snapshots (
                    id SERIAL PRIMARY KEY,
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_shib_perp_scan_activity (
                    id SERIAL PRIMARY KEY,
                    timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS agape_shib_perp_activity_log (
                    id SERIAL PRIMARY KEY,
                    timestamp TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_shib_perp_positions_status ON agape_shib_perp_positions(status)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_shib_perp_positions_open_time ON agape_shib_perp_positions(open_time DESC)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_shib_perp_equity_snapshots_ts ON agape_shib_perp_equity_snapshots(timestamp DESC)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_agape_shib_perp_scan_activity_ts ON agape_shib_perp_scan_activity(timestamp DESC)")

            conn.commit()
            logger.info("AGAPE-SHIB-PERP DB: Tables ensured")
        except Exception as e:
            logger.error(f"AGAPE-SHIB-PERP DB: Table creation failed: {e}")
            conn.rollback()
        finally:
            cursor.close()
            conn.close()
    ...
    def load_config(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            logger.debug(f"AGAPE-SHIB-PERP DB: Config load failed: {e}")
            return None
        finally:
            cursor.close()
            conn.close()
    ...
    def save_position(self, pos) -> bool:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_shib_perp_positions (
            ...
            logger.error(f"AGAPE-SHIB-PERP DB: Failed to save position: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def close_position(self, position_id: str, close_price: float, realized_pnl: float, reason: str) -> bool:
        ...
        try:
            ...
            logger.error(f"AGAPE-SHIB-PERP DB: Failed to close position: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def get_open_positions(self) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"AGAPE-SHIB-PERP DB: Failed to get open positions: {e}")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    def get_closed_trades(self, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"AGAPE-SHIB-PERP DB: Failed to get closed trades: {e}")
            return []
        finally:
            cursor.close()
            conn.close()
    ...
    def get_position_count(self) -> int:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM agape_shib_perp_positions WHERE status = 'open'")
    ...
    def update_high_water_mark(self, position_id: str, hwm: float) -> bool:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("UPDATE agape_shib_perp_positions SET high_water_mark = %s WHERE position_id = %s AND status = 'open'", (hwm, position_id))
            conn.commit()
            return True
        except Exception:
            conn.rollback()
    ...
    def save_equity_snapshot(self, equity, unrealized_pnl, realized_cumulative, open_positions, shib_price=None, funding_rate=None):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_shib_perp_equity_snapshots
            ...
            logger.error(f"AGAPE-SHIB-PERP DB: Failed to save equity snapshot: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def log_scan(self, scan_data: Dict) -> bool:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_shib_perp_scan_activity (
            ...
            logger.error(f"AGAPE-SHIB-PERP DB: Failed to log scan: {e}")
            conn.rollback()
            return False
        finally:
            cursor.close()
    ...
    def log(self, level, action, message, details=None):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO agape_shib_perp_activity_log (level, action, message, details)
                VALUES (%s, %s, %s, %s)
    ...
    def get_logs(self, limit=50):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT timestamp, level, action, message, details FROM agape_shib_perp_activity_log ORDER BY timestamp DESC LIMIT %s", (limit,))
    ...
    def get_scan_activity(self, limit=50):
        ...
        try:
            ...
            cursor.execute("""
                ...
                FROM agape_shib_perp_scan_activity ORDER BY timestamp DESC LIMIT %s
            """, (limit,))
            return [
    ...
    def get_scan_count(self) -> int:
        ...
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM agape_shib_perp_scan_activity")
            row = cursor.fetchone()
