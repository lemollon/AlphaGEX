...
class ValorDatabase:
    ...
    def __init__(self, bot_name: str = "VALOR"):
    ...
    def _ensure_tables(self) -> None:
        ...
        try:
            with db_connection() as conn:
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS valor_closed_trades (
                        id SERIAL PRIMARY KEY,
                        position_id VARCHAR(50) UNIQUE NOT NULL,
                        symbol VARCHAR(20) NOT NULL,
                        direction VARCHAR(10) NOT NULL,
                ...
                # Add missing columns to valor_closed_trades if they don't exist
                ...
                # Signals history
                c.execute("""
                    CREATE TABLE IF NOT EXISTS valor_signals (
                        id SERIAL PRIMARY KEY,
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS valor_equity_snapshots (
                        id SERIAL PRIMARY KEY,
                ...
                # Activity logs
                c.execute("""
                    CREATE TABLE IF NOT EXISTS valor_logs (
                        id SERIAL PRIMARY KEY,
                ...
                # Daily performance
                c.execute("""
                    CREATE TABLE IF NOT EXISTS valor_daily_perf (
                        id SERIAL PRIMARY KEY,
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS valor_scan_activity (
                        id SERIAL PRIMARY KEY,
                ...
                c.execute("CREATE INDEX IF NOT EXISTS idx_valor_scan_activity_time ON valor_scan_activity(scan_time)")
                c.execute("CREATE INDEX IF NOT EXISTS idx_valor_scan_activity_outcome ON valor_scan_activity(outcome)")
                c.execute("CREATE INDEX IF NOT EXISTS idx_valor_scan_activity_regime ON valor_scan_activity(gamma_regime)")
    ...
    def get_position_by_id(self, position_id: str) -> Optional[FuturesPosition]:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    SELECT * FROM valor_positions WHERE position_id = %s
                """, (position_id,))
    ...
    def get_position_count(self) -> int:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("SELECT COUNT(*) FROM valor_positions WHERE status = 'open'")
                result = c.fetchone()
    ...
    def get_config(self) -> ValorConfig:
        """Load configuration from database"""
        config = ValorConfig()
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("SELECT config_key, config_value FROM valor_config")
                rows = c.fetchall()
                ...
                config = ValorConfig.from_dict(config_dict)
    ...
    def save_config(self, config: ValorConfig) -> bool:
    ...
    def get_diagnostics(self) -> Dict[str, Any]:
        """
        DIAGNOSTIC: Get raw counts and sample data from all VALOR tables.
        ...
        try:
            with db_connection() as conn:
                ...
                # Closed trades
                c.execute("SELECT COUNT(*) FROM valor_closed_trades")
                ...
                c.execute("SELECT COALESCE(SUM(realized_pnl), 0) FROM valor_closed_trades")
                ...
                # Equity snapshots
                c.execute("SELECT COUNT(*) FROM valor_equity_snapshots")
                snap_count = c.fetchone()[0]
                result["tables"]["equity_snapshots"] = {"count": snap_count}

                # Scan activity
                c.execute("SELECT COUNT(*) FROM valor_scan_activity")
                scan_count = c.fetchone()[0]

                c.execute("SELECT COUNT(*) FROM valor_scan_activity WHERE outcome = 'TRADED'")
                traded_count = c.fetchone()[0]

                c.execute("SELECT COUNT(*) FROM valor_scan_activity WHERE trade_outcome IS NOT NULL")
    ...
    def cleanup_orphaned_positions(self) -> Dict[str, int]:
        """
        Fix orphaned positions that exist in valor_positions as 'open'
        but also have records in valor_closed_trades.
        ...
        try:
            with db_connection() as conn:
                ...
                c.execute("""
                    ...
                    FROM valor_positions p
                    INNER JOIN valor_closed_trades ct ON p.position_id = ct.position_id
                    WHERE p.status = 'open'
                """)
    ...
    def save_scan_activity(
        ...
        try:
            with db_connection() as conn:
                ...
                c.execute("""
                    INSERT INTO valor_scan_activity (
    ...
    def get_scan_activity(
        ...
        try:
            with db_connection() as conn:
                ...
                query = "SELECT * FROM valor_scan_activity WHERE 1=1"
                params = []
    ...
    def get_ml_training_data(self, min_trades: int = 50) -> List[Dict]:
        ...
        try:
            with db_connection() as conn:
                ...
                logger.info(f"Retrieved {len(data)} training samples for VALOR ML")
