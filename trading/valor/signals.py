...
- Use /api/valor/ml/approve to activate ML after reviewing training results
...
def _get_ml_advisor():
    ...
    try:
        ...
        if _ml_advisor.model is not None:
            ...
            logger.info("VALOR ML Advisor initialized but no model trained yet")
    except Exception as e:
        logger.warning(f"Could not load VALOR ML Advisor: {e}")
        _ml_advisor = None
...
def _get_config_value(key: str, default: Any = None) -> Any:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT config_value FROM valor_config WHERE config_key = %s",
            (key,)
        )
        row = cursor.fetchone()
        conn.close()
        ...
        try:
            ...
            if flip_point <= 0:
                logger.warning(
                    ...
                    f"VALOR requires real GEX data from TradingVolatilityAPI. "
