"""
GRACE - Paper Trade Executor
============================

Executes paper trades using real Tradier market data.
Clone of FAITH's executor for 1DTE Iron Condors.
"""
...
from .models import (
    IronCondorPosition, IronCondorSignal, GraceConfig,
...
class GraceExecutor:
    ...
    def calculate_collateral(self, spread_width: float, net_credit: float) -> float:
        ...
        if spread_width <= 0:
            logger.error("GRACE: Invalid spread width for collateral calculation")
            return 0
    ...
    def calculate_max_contracts(self, buying_power: float, collateral_per_contract: float) -> int:
        ...
        if collateral_per_contract <= 0:
            logger.error("GRACE: Collateral per contract is zero or negative")
            return 0
    ...
    def open_paper_position(self, signal: IronCondorSignal, contracts: int) -> Optional[IronCondorPosition]:
        ...
        if contracts < 1:
            logger.error("GRACE: Cannot open position with 0 contracts")
            return None

        try:
            now = datetime.now(CENTRAL_TZ)
            position_id = f"GRACE-{now.strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
            ...
            if not self.db.save_position(position):
                logger.error(f"GRACE: Failed to save position {position_id}")
                return None
            ...
            logger.info(
                f"GRACE PAPER OPEN: {signal.put_long}/{signal.put_short}P-"
            ...
            logger.error(f"GRACE: Paper open failed: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def close_paper_position(
        ...
        try:
            ...
            if not self.db.close_position(
                ...
                logger.error(f"GRACE: Failed to close position {position.position_id} in DB")
                return False, 0
            ...
            logger.info(
                f"GRACE PAPER CLOSE: {position.position_id} @ ${close_price:.4f} "
            ...
            logger.error(f"GRACE: Paper close failed: {e}")
            import traceback
            traceback.print_exc()
            return False, 0
