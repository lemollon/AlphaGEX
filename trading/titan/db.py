...
from .models import (
    ...
    TITANConfig, TradingMode, StrategyPreset,
...
class TITANDatabase:
    """Database operations for TITAN aggressive SPX trading"""

    def __init__(self, bot_name: str = "TITAN"):
    ...
    def _ensure_tables(self) -> None:
        ...
        try:
            with db_connection() as conn:
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS titan_signals (
                        id SERIAL PRIMARY KEY,
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS titan_daily_perf (
                        id SERIAL PRIMARY KEY,
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS titan_logs (
                        id SERIAL PRIMARY KEY,
    ...
    def get_position_count(self) -> int:
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("SELECT COUNT(*) FROM titan_positions WHERE status = 'open'")
                return c.fetchone()[0]
        except Exception:
            return 0
    ...
    def log(self, level: str, message: str, details: Optional[Dict] = None) -> None:
        try:
            with db_connection() as conn:
                c = conn.cursor()
                import json
                c.execute("INSERT INTO titan_logs (level, message, details) VALUES (%s, %s, %s)",
    ...
    def log_orphaned_order(
        ...
        try:
            with db_connection() as conn:
                ...
                # Ensure table exists (shared with ARES/PEGASUS)
                c.execute("""
    ...
    def load_config(self) -> TITANConfig:
        """Load config from DB"""
        config = TITANConfig()
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("SELECT config_key, config_value FROM autonomous_config WHERE bot_name = 'TITAN'")
    ...
    def update_daily_performance(self, summary) -> bool:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    INSERT INTO titan_daily_perf (
                        trade_date, trades_executed, positions_closed, realized_pnl
                    ) VALUES (%s, %s, %s, %s)
                    ON CONFLICT (trade_date) DO UPDATE SET
                        trades_executed = titan_daily_perf.trades_executed + EXCLUDED.trades_executed,
                        positions_closed = titan_daily_perf.positions_closed + EXCLUDED.positions_closed,
                        realized_pnl = titan_daily_perf.realized_pnl + EXCLUDED.realized_pnl,
                        updated_at = NOW()
                """, (
                    summary.date,
    ...
    def get_current_balance(self) -> float:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    SELECT balance FROM titan_equity_snapshots
        ...
        return 200000.0  # TITAN default capital
