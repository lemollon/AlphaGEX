...
class SignalGenerator:
    ...
    def _init_tradier(self) -> None:
        ...
        try:
            ...
            if test_quote and test_quote.get('last', 0) > 0:
                logger.info(f"TITAN: Tradier production API connected (SPX=${test_quote.get('last', 0):.2f})")
            else:
                logger.warning("TITAN: Tradier connected but SPX quote unavailable")
        except Exception as e:
            logger.warning(f"TITAN: Tradier init failed, using estimated credits: {e}")
            self.tradier = None
    ...
    def get_real_credits(
        ...
        """
        if not self.tradier:
            logger.debug("TITAN: Tradier not available, cannot get real credits")
            return None
        ...
        try:
            ...
            if not all([put_short_quote, put_long_quote, call_short_quote, call_long_quote]):
                logger.warning(f"TITAN: Missing option quotes for {expiration}")
                return None
            ...
            if put_credit <= 0 or call_credit <= 0:
                logger.warning(f"TITAN: Invalid credits - put=${put_credit:.2f}, call=${call_credit:.2f}")
            ...
            logger.info(f"TITAN: REAL QUOTES - Put spread ${put_credit:.2f}, Call spread ${call_credit:.2f}, Total ${total:.2f}")
            ...
            logger.warning(f"TITAN: Failed to get real credits: {e}")
            return None
    ...
    def generate_signal(self, oracle_data: Optional[Dict[str, Any]] = None) -> Optional[IronCondorSignal]:
        ...
        if not pricing:
            logger.info("TITAN: Using estimated credits (Tradier unavailable)")
