"""
AGAPE-BCH-FUTURES Models - Data structures for the BCH Futures Contract bot.
...
BCH: BTC-fork high-beta (~$200-400). 25 BCH per contract.

KEY DIFFERENCES from AGAPE-BCH-PERP:
- Trades Coinbase Derivatives monthly futures (BCH-29MAY26-CDE, BCH-26JUN26-CDE, etc.)
  rather than the geo-blocked Coinbase International perpetual.
- Sizing is integer contracts (1 contract = 25 units of "BCH" = 25 BCH underlying).
...
@dataclass
class AgapeBchFuturesConfig:
    """Configuration for AGAPE-BCH-FUTURES bot.

    AGGRESSIVE MODE - Futures contracts, 24/7 trading.
    BCH: Very low price meme coin, very large quantity trades (millions).
    """

    # Identity
    bot_name: str = "AGAPE_BCH_FUTURES"
    ticker: str = "BCH"
    instrument: str = "BCH-FUT"     # Futures contract
    ...
    # Risk management
    starting_capital: float = 2500.0    # $1K starting capital (meme coin allocation)
    risk_per_trade_pct: float = 5.0     # 5% risk per trade
    max_quantity: float = 3.0         # Max 3 contracts per position — at ~$7500/contract notional that's ~$22K per trade
    max_open_positions: int = 2         # Conservative for meme coin

    # Position sizing - Futures contract specs (integer contracts only)
    # 1 contract = 25 units of "BCH" index = 25 BCH actual BCH tokens
    contract_size: int = 25          # 1 contract = 25 units of "BCH" index = 25 BCH underlying
    default_quantity: float = 1.0        # 1 contract default trade size
    min_quantity: float = 1.0            # Minimum 1 contract (integer contracts only)
    tick_size: float = 0.05           # Coinbase futures tick — 2-decimal price (0.05 increments)
    ...
    no_loss_activation_pct: float = 0.6       # 0.3% activation (BCH - tighter)
    no_loss_trail_distance_pct: float = 0.4   # 0.2% trail distance
    no_loss_emergency_stop_pct: float = 5.0
    max_unrealized_loss_pct: float = 1.5      # 0.5% max loss (BCH)
    ...
    @classmethod
    def load_from_db(cls, db) -> "AgapeBchFuturesConfig":
...
@dataclass
class AgapeBchFuturesSignal:
    """Trading signal generated by AGAPE-BCH-FUTURES signal engine."""
    spot_price: float
    timestamp: datetime
...
@dataclass
class AgapeBchFuturesPosition:
    """An open or closed AGAPE-BCH-FUTURES position."""
    position_id: str
    side: PositionSide
    quantity: float
    entry_price: float
