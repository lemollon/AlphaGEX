"""
AGAPE-BCH-FUTURES Signal Generator - Generates directional BCH-FUT trade signals.

Same logic as AGAPE-DOGE signals but for BCH market microstructure.
...
from trading.agape_bch_futures.models import (
    AgapeBchFuturesConfig,
    AgapeBchFuturesSignal,
    SignalAction,
    PositionSide,
)
...
try:
    ...
    logger.info("AGAPE-BCH-FUTURES Signals: CryptoDataProvider loaded")
except ImportError as e:
    logger.warning(f"AGAPE-BCH-FUTURES Signals: CryptoDataProvider not available: {e}")
...
try:
    ...
    logger.info("AGAPE-BCH-FUTURES Signals: ProphetAdvisor loaded")
except ImportError as e:
    logger.warning(f"AGAPE-BCH-FUTURES Signals: ProphetAdvisor not available: {e}")


class AgapeBchFuturesDirectionTracker:
...
_direction_tracker: Optional[AgapeBchFuturesDirectionTracker] = None


def get_agape_bch_futures_direction_tracker(config=None):
    global _direction_tracker
    if _direction_tracker is None:
        _direction_tracker = AgapeBchFuturesDirectionTracker(
...
def record_agape_bch_futures_trade_outcome(direction, is_win, scan_number):
    get_agape_bch_futures_direction_tracker().record_trade(direction, is_win, scan_number)


class AgapeBchFuturesSignalGenerator:
    """Generates directional trade signals for BCH-FUT based on crypto microstructure."""

    def __init__(self, config: AgapeBchFuturesConfig):
        self.config = config
        ...
        if get_crypto_data_provider:
            try:
                ...
                logger.warning(f"AGAPE-BCH-FUTURES Signals: Crypto provider init failed: {e}")
        if ProphetAdvisor:
            try:
                self._oracle = ProphetAdvisor()
            except Exception as e:
                logger.warning(f"AGAPE-BCH-FUTURES Signals: Prophet init failed: {e}")
    ...
    def get_market_data(self):
        ...
        try:
            ...
            logger.error(f"AGAPE-BCH-FUTURES Signals: Market data fetch failed: {e}")
            return None
    ...
    def get_prophet_advice(self, market_data):
        ...
        try:
            ...
            logger.error(f"AGAPE-BCH-FUTURES Signals: Prophet call failed: {e}")
    ...
    def generate_signal(self, prophet_data=None):
        ...
        if not market_data:
            return AgapeBchFuturesSignal(spot_price=0, timestamp=now, action=SignalAction.WAIT, reasoning="NO_MARKET_DATA")
    ...
    def _determine_action(self, combined_signal, confidence, market_data):
        ...
        tracker = get_agape_bch_futures_direction_tracker(self.config)
    ...
    def _calculate_position_size(self, spot_price):
        """Calculate position size in BCH quantity.
        ...
        try:
            ...
            if max_qty_by_notional > 0 and max_qty_by_notional < quantity:
                logger.debug(
                    f"AGAPE-BCH-FUTURES: notional cap reducing size "
                    f"{quantity:.0f} -> {max_qty_by_notional:.0f} BCH "
                    f"(per_pos={per_pos_margin_pct:.1f}% margin, lev={leverage}x)"
                )
                quantity = max(self.config.min_quantity, max_qty_by_notional)
        except Exception as e:
            logger.debug(f"AGAPE-BCH-FUTURES: notional cap skipped: {e}")
