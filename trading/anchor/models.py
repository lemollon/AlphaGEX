...
Data models for ANCHOR SPX Iron Condor trading bot.

ANCHOR trades SPX Iron Condors:
...
class AnchorConfig:
    """
    ANCHOR configuration for SPX trading.
    """
    # Strategy
    ...
    def validate(self) -> tuple[bool, str]:
        ...
        if self.capital > 2_000_000:
            logger.warning(f"ANCHOR: capital=${self.capital:,.0f} exceeds safe limit $2M, clamping")
        ...
        if self.max_contracts > 100:
            logger.warning(f"ANCHOR: max_contracts={self.max_contracts} exceeds safe limit 100, clamping")
...
class IronCondorSignal:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            self.total_credit >= 0.75 and  # Match AnchorConfig.min_credit
            self.put_short > self.put_long > 0 and
            self.call_short < self.call_long and
            self.call_short > self.put_short
        )
