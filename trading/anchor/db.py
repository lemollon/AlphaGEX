"""
ANCHOR - Database Layer
=========================

Database operations for SPX Iron Condor trading.
Same structure as FORTRESS but with ANCHOR-specific tables.
"""
...
from .models import (
    ...
    AnchorConfig, TradingMode, StrategyPreset,
...
class AnchorDatabase:
    """Database operations for ANCHOR SPX trading"""

    def __init__(self, bot_name: str = "ANCHOR"):
        self.bot_name = bot_name
        self._ensure_tables()

    def _migrate_from_pegasus(self, cursor) -> None:
        """Migrate old pegasus_* tables to anchor_* after the PEGASUS->ANCHOR rename."""
        for old_name, new_name in [
            ('pegasus_positions', 'anchor_positions'),
            ('pegasus_signals', 'anchor_signals'),
            ('pegasus_daily_perf', 'anchor_daily_perf'),
            ('pegasus_logs', 'anchor_logs'),
            ('pegasus_equity_snapshots', 'anchor_equity_snapshots'),
        ]:
            try:
                cursor.execute(
        ...
        try:
            cursor.execute("""
                ...
                SET key = 'anchor_' || SUBSTRING(key FROM 9)
                WHERE key LIKE 'pegasus_%'
                  AND NOT EXISTS (
                      SELECT 1 FROM autonomous_config ac2
                      WHERE ac2.key = 'anchor_' || SUBSTRING(autonomous_config.key FROM 9)
                  )
            """)
            if cursor.rowcount > 0:
                logger.info(f"{self.bot_name}: Migrated {cursor.rowcount} config keys pegasus_* -> anchor_*")
        except Exception as e:
    ...
    def _ensure_tables(self) -> None:
        ...
        try:
            with db_connection() as conn:
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS anchor_signals (
                        id SERIAL PRIMARY KEY,
                ...
                c.execute("""
                    CREATE TABLE IF NOT EXISTS anchor_daily_perf (
                        id SERIAL PRIMARY KEY,
    ...
    def has_traded_today(self, date: str) -> bool:
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    SELECT COUNT(*) FROM anchor_positions
    ...
    def log(self, level: str, message: str, details: Optional[Dict] = None) -> None:
        try:
            with db_connection() as conn:
                c = conn.cursor()
                import json
                c.execute("INSERT INTO anchor_logs (level, message, details) VALUES (%s, %s, %s)",
    ...
    def load_config(self) -> AnchorConfig:
        """Load config from DB.

        The autonomous_config table has columns: key TEXT, value TEXT.
        ANCHOR config keys use the 'anchor_' prefix (e.g., 'anchor_mode', 'anchor_ticker').
        """
        config = AnchorConfig()
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("SELECT key, value FROM autonomous_config WHERE key LIKE 'anchor_%'")
                for key, value in c.fetchall():
                    field_name = key.replace('anchor_', '', 1)
    ...
    def update_daily_performance(self, summary) -> bool:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    INSERT INTO anchor_daily_perf (
                        trade_date, trades_executed, positions_closed, realized_pnl
                    ) VALUES (%s, %s, %s, %s)
                    ON CONFLICT (trade_date) DO UPDATE SET
                        trades_executed = anchor_daily_perf.trades_executed + EXCLUDED.trades_executed,
                        positions_closed = anchor_daily_perf.positions_closed + EXCLUDED.positions_closed,
                        realized_pnl = anchor_daily_perf.realized_pnl + EXCLUDED.realized_pnl,
                        updated_at = NOW()
                """, (
                    summary.date,
    ...
    def get_current_balance(self) -> float:
        ...
        try:
            with db_connection() as conn:
                c = conn.cursor()
                c.execute("""
                    SELECT balance FROM anchor_equity_snapshots
        ...
        return 200000.0  # ANCHOR default capital
