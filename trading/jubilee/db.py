"""
JUBILEE Database Layer - Full Transparency Storage
...
class JubileeDatabase:
    """
    Database operations for JUBILEE box spread system.
    ...
    def __init__(self, bot_name: str = "JUBILEE"):
    ...
    def _ensure_tables(self):
        ...
        try:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_positions (
                    ...
                    cash_deployed_to_fortress DECIMAL(15, 2) DEFAULT 0,
                    cash_deployed_to_samson DECIMAL(15, 2) DEFAULT 0,
                    cash_deployed_to_anchor DECIMAL(15, 2) DEFAULT 0,
                    cash_held_in_reserve DECIMAL(15, 2) DEFAULT 0,
                    total_cash_deployed DECIMAL(15, 2) DEFAULT 0,

                    -- Returns tracking
                    returns_from_fortress DECIMAL(15, 2) DEFAULT 0,
                    returns_from_samson DECIMAL(15, 2) DEFAULT 0,
                    returns_from_anchor DECIMAL(15, 2) DEFAULT 0,
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_capital_deployments (
                    ...
                    anchor_allocation_pct DECIMAL(6, 2),
                    anchor_allocation_reasoning TEXT,
                    anchor_returns_to_date DECIMAL(15, 2) DEFAULT 0,
            ...
            # Configuration table
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_config (
                    id SERIAL PRIMARY KEY,
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_equity_snapshots (
                    id SERIAL PRIMARY KEY,
            ...
            # IC TRADING TABLES
            # ==================================================================
            # These tables support JUBILEE's own Iron Condor trading
            # using capital borrowed via box spreads.
            # ==================================================================

            # IC Positions table - Active Iron Condor positions
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_ic_positions (
                    id SERIAL PRIMARY KEY,
                    position_id VARCHAR(50) UNIQUE NOT NULL,
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_ic_signals (
                    id SERIAL PRIMARY KEY,
                    signal_id VARCHAR(50) UNIQUE NOT NULL,
                    signal_time TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
                    source_box_position_id VARCHAR(50),
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_ic_config (
                    id SERIAL PRIMARY KEY,
                    config_key VARCHAR(50) UNIQUE DEFAULT 'default',
                    config_data JSONB NOT NULL,
                    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS jubilee_ic_equity_snapshots (
                    id SERIAL PRIMARY KEY,
                    snapshot_time TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
                    total_equity DECIMAL(15, 2),
            ...
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_positions_status
                ON jubilee_positions (status)
            """)
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_positions_close_time
                ON jubilee_positions (close_time DESC NULLS LAST)
            """)

            # Box Signals - query by time and execution status
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_signals_time
                ON jubilee_signals (signal_time DESC)
            """)
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_signals_executed
                ON jubilee_signals (was_executed, signal_time DESC)
            """)

            # Box Equity Snapshots - query by time for intraday
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_equity_snapshots_time
                ON jubilee_equity_snapshots (snapshot_time DESC)
            """)
            ...
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_ic_positions_status
                ON jubilee_ic_positions (status)
            """)
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_ic_positions_open_time
                ON jubilee_ic_positions (open_time DESC)
            """)
            ...
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_ic_signals_time
                ON jubilee_ic_signals (signal_time DESC)
            """)
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_ic_signals_executed
                ON jubilee_ic_signals (was_executed, signal_time DESC)
            """)

            # IC Equity Snapshots - query by time for intraday
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_ic_equity_snapshots_time
                ON jubilee_ic_equity_snapshots (snapshot_time DESC)
            """)

            # Logs - query by action type for IC logs
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_jubilee_logs_action
                ON jubilee_logs (action, log_time DESC)
            """)

            # Add new columns if they don't exist (for schema migration)
            # These columns were added for enhanced transparency tracking
            migration_columns = [
                ("jubilee_equity_snapshots", "quote_source", "VARCHAR(50)"),
                ("jubilee_equity_snapshots", "calculation_method", "VARCHAR(50)"),
                ("jubilee_equity_snapshots", "total_mtm_unrealized", "DECIMAL(15, 2)"),
                ("jubilee_equity_snapshots", "total_ic_returns", "DECIMAL(15, 2)"),
                ("jubilee_equity_snapshots", "total_costs_accrued", "DECIMAL(15, 2)"),
                # Fix for logs table schema mismatch
                ("jubilee_logs", "log_time", "TIMESTAMP WITH TIME ZONE DEFAULT NOW()"),
                ("jubilee_logs", "level", "VARCHAR(20)"),
                ("jubilee_logs", "action", "VARCHAR(100)"),
                ("jubilee_logs", "message", "TEXT"),
                ("jubilee_logs", "details", "JSONB"),
                ("jubilee_logs", "position_id", "VARCHAR(50)"),
                ("jubilee_logs", "signal_id", "VARCHAR(50)"),
            ]
            ...
            default_value_columns = [
                # jubilee_positions - cost tracking
                ("jubilee_positions", "borrowing_cost", "0"),
                ("jubilee_positions", "implied_annual_rate", "0"),
                ("jubilee_positions", "daily_cost", "0"),
                ("jubilee_positions", "cost_accrued_to_date", "0"),
                ("jubilee_positions", "fed_funds_at_entry", "0"),
                ("jubilee_positions", "margin_rate_at_entry", "0"),
                ("jubilee_positions", "savings_vs_margin", "0"),
                ("jubilee_positions", "spot_at_entry", "0"),
                ("jubilee_positions", "vix_at_entry", "0"),
                ("jubilee_positions", "current_margin_used", "0"),
                ("jubilee_positions", "margin_cushion", "0"),
                ("jubilee_positions", "contracts", "1"),
                ("jubilee_positions", "entry_credit", "0"),
                ("jubilee_positions", "total_credit_received", "0"),
                ("jubilee_positions", "dte_at_entry", "0"),
                ("jubilee_positions", "current_dte", "0"),
                # jubilee_ic_positions - IC trading
                ("jubilee_ic_positions", "contracts", "1"),
                ("jubilee_ic_positions", "entry_credit", "0"),
                ("jubilee_ic_positions", "total_credit_received", "0"),
                ("jubilee_ic_positions", "current_value", "0"),
                ("jubilee_ic_positions", "unrealized_pnl", "0"),
                ("jubilee_ic_positions", "entry_delta", "0"),
                ("jubilee_ic_positions", "entry_vega", "0"),
                ("jubilee_ic_positions", "entry_theta", "0"),
                ("jubilee_ic_positions", "current_dte", "0"),
                ("jubilee_ic_positions", "max_profit", "0"),
                ("jubilee_ic_positions", "max_loss", "0"),
            ...
            conn.commit()
            cursor.close()
            logger.info("JUBILEE tables initialized successfully")
    ...
    def load_config(self) -> JubileeConfig:
        """Load configuration from database or return defaults"""
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                SELECT config_data FROM jubilee_config
            ...
            if row and row[0]:
                return JubileeConfig.from_dict(row[0])
            return JubileeConfig()

        except Exception as e:
            logger.error(f"Error loading config: {e}")
            return JubileeConfig()

    def save_config(self, config: JubileeConfig) -> bool:
        """Save configuration to database"""
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO jubilee_config (config_key, config_data, updated_at)
        ...
        try:
            ...
            cursor.execute("""
                ...
                ON CONFLICT (position_id) DO UPDATE SET
                    ...
                    returns_from_fortress = EXCLUDED.returns_from_fortress,
                    returns_from_samson = EXCLUDED.returns_from_samson,
                    returns_from_anchor = EXCLUDED.returns_from_anchor,
            ...
            """, (
                ...
                position.cash_deployed_to_fortress, position.cash_deployed_to_samson,
                position.cash_deployed_to_anchor, position.cash_held_in_reserve,
                position.total_cash_deployed,
                position.returns_from_fortress, position.returns_from_samson,
                position.returns_from_anchor, position.total_ic_returns, position.net_profit,
    ...
    def _row_to_position(self, data: Dict[str, Any]) -> BoxSpreadPosition:
        ...
        return BoxSpreadPosition(
            ...
            cash_deployed_to_fortress=float(data['cash_deployed_to_fortress'] or 0),
            cash_deployed_to_samson=float(data['cash_deployed_to_samson'] or 0),
            cash_deployed_to_anchor=float(data['cash_deployed_to_anchor'] or 0),
            cash_held_in_reserve=float(data['cash_held_in_reserve'] or 0),
            total_cash_deployed=float(data['total_cash_deployed'] or 0),
            returns_from_fortress=float(data['returns_from_fortress'] or 0),
            returns_from_samson=float(data['returns_from_samson'] or 0),
            returns_from_anchor=float(data['returns_from_anchor'] or 0),
    ...
    def record_equity_snapshot(self, use_real_quotes: bool = True) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO jubilee_equity_snapshots (
    ...
    def get_intraday_equity(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT * FROM jubilee_equity_snapshots
    ...
    # ==========================================================================
    # These methods support JUBILEE's own Iron Condor trading using
    ...
    def save_ic_config(self, config: JubileeICConfig) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO jubilee_ic_config (config_key, config_data, updated_at)
                VALUES ('default', %s, NOW())
                ON CONFLICT (config_key)
                DO UPDATE SET config_data = %s, updated_at = NOW()
            """, (json.dumps(config.to_dict()), json.dumps(config.to_dict())))
    ...
    def get_open_ic_positions(self) -> List[JubileeICPosition]:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT * FROM jubilee_ic_positions
    ...
    def save_ic_position(self, position: JubileeICPosition) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO jubilee_ic_positions (
    ...
    def log_ic_signal(
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO jubilee_ic_signals (
    ...
    def get_recent_ic_signals(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT * FROM jubilee_ic_signals
                ORDER BY signal_time DESC
                LIMIT %s
            """, (limit,))
            rows = cursor.fetchall()
    ...
    def get_last_ic_trade_time(self) -> Optional[datetime]:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT open_time FROM jubilee_ic_positions
                ORDER BY open_time DESC
                LIMIT 1
            """)
            row = cursor.fetchone()
    ...
    def record_ic_equity_snapshot(self) -> bool:
        ...
        - Uses jubilee_ic_equity_snapshots table
        """
        try:
            conn = self._get_connection()
            cursor = conn.cursor()
            ...
            # Insert snapshot
            cursor.execute("""
                INSERT INTO jubilee_ic_equity_snapshots (
