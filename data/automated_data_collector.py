...
def run_discernment_outcome_tracking():
    ...
    FIX: This was missing! Discernment performance metrics weren't being populated
    ...
    3. Records direction/magnitude accuracy in discernment_outcomes
    """
    if not is_after_market_close():
        return

    print(f"🎯 Discernment Outcome Tracking - {datetime.now(CENTRAL_TZ).strftime('%H:%M:%S')}")

    try:
        from core.discernment_outcome_tracker import track_discernment_outcomes

        results = track_discernment_outcomes(min_age_hours=24, max_age_days=7)
        ...
        if outcomes_recorded > 0:
            print(f"  ✅ discernment_outcomes updated: {outcomes_recorded} outcomes recorded")
            print(f"     Direction accuracy: {direction_accuracy}%")
            log_collection('discernment_outcome_tracking', 'discernment_outcomes', True,
                          f"Recorded {outcomes_recorded}/{predictions_found}, accuracy: {direction_accuracy}%")
        elif predictions_found == 0:
            print(f"  ✅ discernment_outcomes - no untracked predictions")
            log_collection('discernment_outcome_tracking', 'discernment_outcomes', True, "No predictions to track")
        else:
            print(f"  ⚠️ discernment_outcomes - found {predictions_found} predictions but recorded 0 outcomes")
            log_collection('discernment_outcome_tracking', 'discernment_outcomes', False,
                          f"Found {predictions_found} but recorded 0, errors: {errors}")
    except ImportError as e:
        print(f"  ⚠️ discernment_outcome_tracking - module not available: {e}")
        log_collection('discernment_outcome_tracking', 'discernment_outcomes', False, f"Module not available: {e}")
    except Exception as e:
        import traceback
        tb = traceback.format_exc()
        print(f"  ❌ discernment_outcome_tracking failed: {e}\n{tb}")
        log_collection('discernment_outcome_tracking', 'discernment_outcomes', False, str(e), tb)
