...
def test_bot_table_naming():
    assert bot_table("flow", "config") == "flow_config"
...
def test_create_bot_tables_does_not_overwrite_existing_config():
    ...
    with engine.begin() as conn:
        conn.execute(text(
            "UPDATE flow_config SET pt_pct = 0.99, max_contracts = 99"
        ))
    # Run migration a second time
    create_bot_tables(engine)
    with engine.begin() as conn:
        row = conn.execute(
            text("SELECT pt_pct, max_contracts FROM flow_config")
        ).fetchone()
