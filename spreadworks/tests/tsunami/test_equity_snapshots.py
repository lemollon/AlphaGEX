"""Tests for backend.bots.tsunami.equity_snapshots."""
...
from backend.bots.tsunami import equity_snapshots  # noqa: E402
...
from backend.bots.tsunami.management.state import Position, PositionState  # noqa: E402


def _open_pos(defined_max=0.20, current_call=0.45, current_short=0.20, current_long=0.05) -> Position:
    return Position(
        position_id="p1", instance_name="TSUNAMI-MSTU",
...
class SafeFallbackWithoutDB(unittest.TestCase):
    def test_returns_zero_without_db(self):
        with patch("backend.bots.tsunami.equity_snapshots._connect",
...
class StartingCapitalLookup(unittest.TestCase):
    def test_known_instance_returns_allocation_cap(self):
        self.assertEqual(equity_snapshots._instance_starting_capital("TSUNAMI-MSTU"), 200.0)
        self.assertEqual(equity_snapshots._instance_starting_capital("TSUNAMI-CONL"), 150.0)

    def test_unknown_instance_returns_zero(self):
        self.assertEqual(equity_snapshots._instance_starting_capital("TSUNAMI-NOPE"), 0.0)
