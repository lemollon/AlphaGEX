...
class _InMemoryAuditStore:
    """In-memory replacement for tsunami_trade_audit during tests."""
...
class FullLifecycle(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._patches = [
            patch("backend.bots.tsunami.audit.recorder.store.insert",
                  side_effect=self.store.insert),
            patch("backend.bots.tsunami.audit.replayer.store.query_by_position",
                  side_effect=self.store.query_by_position),
            patch("backend.bots.tsunami.instance.is_killed", return_value=False),
        ]
    ...
    def test_lifecycle_entry_through_thursday_close(self):
        ...
        instance = TsunamiInstance(config=get("TSUNAMI-TSLL"))
