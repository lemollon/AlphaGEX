"""Tests for backend.bots.tsunami.monitoring.heartbeat."""
...
from backend.bots.tsunami.monitoring import heartbeat  # noqa: E402
...
class SafeFallbackWithoutDB(unittest.TestCase):
    def test_record_returns_false_without_db(self):
        with patch("backend.bots.tsunami.monitoring.heartbeat._connect",
                   return_value=(None, False)):
            self.assertFalse(heartbeat.record_heartbeat("TSUNAMI-MSTU"))

    def test_read_returns_none_without_db(self):
        with patch("backend.bots.tsunami.monitoring.heartbeat._connect",
                   return_value=(None, False)):
            self.assertIsNone(heartbeat.read_heartbeat("TSUNAMI-MSTU"))

    def test_is_stale_returns_true_without_db(self):
        with patch("backend.bots.tsunami.monitoring.heartbeat._connect",
                   return_value=(None, False)):
            self.assertTrue(heartbeat.is_stale("TSUNAMI-MSTU"))
...
class RecordHeartbeatUpsert(unittest.TestCase):
    def test_upsert_runs_with_expected_params(self):
        ...
        self.assertEqual(params[0], "TSUNAMI-MSTU")
    ...
    def test_failure_returns_false_no_raise(self):
        ...
        with patch("backend.bots.tsunami.monitoring.heartbeat._connect",
                   return_value=(conn, True)):
            ok = heartbeat.record_heartbeat("TSUNAMI-MSTU")
        self.assertFalse(ok)
...
class IsStaleLogic(unittest.TestCase):
    ...
    def test_returns_true_when_old_heartbeat(self):
        ...
        row = ("TSUNAMI-MSTU", old, "OK", 100, 0, None, None)
        conn = _FakeConn(fetchone_result=row)
        with patch("backend.bots.tsunami.monitoring.heartbeat._connect",
                   return_value=(conn, True)):
            self.assertTrue(heartbeat.is_stale("TSUNAMI-MSTU", max_age_seconds=300))

    def test_returns_false_when_recent_heartbeat(self):
        recent = datetime.now(timezone.utc) - timedelta(seconds=60)
        row = ("TSUNAMI-MSTU", recent, "OK", 100, 0, None, None)
        conn = _FakeConn(fetchone_result=row)
        with patch("backend.bots.tsunami.monitoring.heartbeat._connect",
                   return_value=(conn, True)):
            self.assertFalse(heartbeat.is_stale("TSUNAMI-MSTU", max_age_seconds=300))
...
class ReadHeartbeat(unittest.TestCase):
    def test_returns_dict_with_iso_timestamps(self):
        ...
        row = ("TSUNAMI-MSTU", ts, "OK", 42, 1, ts, {"k": "v"})
        conn = _FakeConn(fetchone_result=row)
        with patch("backend.bots.tsunami.monitoring.heartbeat._connect",
                   return_value=(conn, True)):
            result = heartbeat.read_heartbeat("TSUNAMI-MSTU")
        self.assertEqual(result["bot_name"], "TSUNAMI-MSTU")
