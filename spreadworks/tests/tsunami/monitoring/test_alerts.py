"""Tests for backend.bots.tsunami.monitoring.alerts."""
...
from backend.bots.tsunami.monitoring import alerts  # noqa: E402
...
class HeartbeatCheckDelegates(unittest.TestCase):
    def test_check_heartbeat_stale_calls_module(self):
        with patch("backend.bots.tsunami.monitoring.alerts.heartbeat.is_stale",
                   return_value=True) as mock_stale:
            self.assertTrue(alerts.check_heartbeat_stale("TSUNAMI-MSTU"))
        mock_stale.assert_called_once_with("TSUNAMI-MSTU", max_age_seconds=300)
...
class AlertComposersCallDiscord(unittest.TestCase):
    ...
    def test_alert_heartbeat_stale(self):
        with patch("backend.bots.tsunami.monitoring.alerts.discord.post_embed",
                   return_value=True) as mock_post:
            self.assertTrue(alerts.alert_heartbeat_stale("TSUNAMI-MSTU"))
        kwargs = mock_post.call_args.kwargs
        self.assertIn("Heartbeat stale", kwargs["title"])
        self.assertIn("TSUNAMI-MSTU", kwargs["description"])
    ...
    def test_alert_kill_switch(self):
        with patch("backend.bots.tsunami.monitoring.alerts.discord.post_embed",
            ...
            alerts.alert_kill_switch(
                scope="INSTANCE", instance="TSUNAMI-MSTU",
    ...
    def test_alert_exit_filled(self):
        with patch("backend.bots.tsunami.monitoring.alerts.discord.post_embed",
            ...
            alerts.alert_exit_filled(
                instance="TSUNAMI-MSTU", trigger_id="T7",
