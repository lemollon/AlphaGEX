"""Tests for backend.bots.tsunami.kill_switch.instance_triggers."""
...
from backend.bots.tsunami.kill_switch import KillScope  # noqa: E402
from backend.bots.tsunami.kill_switch.instance_triggers import (  # noqa: E402
...
class IK1Drawdown(unittest.TestCase):
    def test_fires_above_30pct(self):
        evt = evaluate_drawdown("TSUNAMI-MSTU", drawdown_pct=0.35)
        self.assertIsNotNone(evt)
        self.assertEqual(evt.trigger_id, "I-K1")
        self.assertEqual(evt.scope, KillScope.INSTANCE)
        self.assertEqual(evt.instance_name, "TSUNAMI-MSTU")

    def test_does_not_fire_at_exactly_30pct(self):
        # Spec uses strict >, not >=.
        evt = evaluate_drawdown("TSUNAMI-MSTU", drawdown_pct=DRAWDOWN_THRESHOLD)
        self.assertIsNone(evt)

    def test_does_not_fire_below_30pct(self):
        self.assertIsNone(evaluate_drawdown("TSUNAMI-MSTU", drawdown_pct=0.20))

    def test_context_carries_pct(self):
        evt = evaluate_drawdown("TSUNAMI-TSLL", drawdown_pct=0.40)
...
class IK2ConsecutiveLosses(unittest.TestCase):
    def test_fires_at_5(self):
        evt = evaluate_consecutive_losses("TSUNAMI-NVDL", consecutive_losses=5)
        self.assertIsNotNone(evt)
        self.assertEqual(evt.trigger_id, "I-K2")

    def test_fires_above_5(self):
        evt = evaluate_consecutive_losses("TSUNAMI-NVDL", consecutive_losses=10)
        self.assertIsNotNone(evt)

    def test_does_not_fire_at_4(self):
        self.assertIsNone(
            evaluate_consecutive_losses("TSUNAMI-NVDL", consecutive_losses=4)
        )

    def test_does_not_fire_at_zero(self):
        self.assertIsNone(
            evaluate_consecutive_losses("TSUNAMI-NVDL", consecutive_losses=0)
        )
...
class IK3TradesWithoutUpside(unittest.TestCase):
    def test_fires_at_20(self):
        evt = evaluate_trades_without_upside("TSUNAMI-CONL", trades_without_upside=20)
        self.assertIsNotNone(evt)
        self.assertEqual(evt.trigger_id, "I-K3")

    def test_fires_above_20(self):
        evt = evaluate_trades_without_upside("TSUNAMI-CONL", trades_without_upside=25)
        self.assertIsNotNone(evt)

    def test_does_not_fire_at_19(self):
        self.assertIsNone(
            evaluate_trades_without_upside("TSUNAMI-CONL", trades_without_upside=19)
        )
