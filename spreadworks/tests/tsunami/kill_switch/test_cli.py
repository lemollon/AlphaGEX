"""Tests for backend.bots.tsunami.kill_switch.cli."""
...
from backend.bots.tsunami.kill_switch import cli  # noqa: E402


class CLIList(unittest.TestCase):
    def test_list_empty(self):
        with patch("backend.bots.tsunami.kill_switch.cli.list_active_kills", return_value=[]), \
    ...
    def test_list_renders_rows(self):
        rows = [{"id": 1, "scope": "INSTANCE", "instance_name": "TSUNAMI-MSTU",
                 "trigger_id": "I-K1", "reason": "x", "killed_at": "2026-05-04T15:00:00"}]
        with patch("backend.bots.tsunami.kill_switch.cli.list_active_kills", return_value=rows), \
             patch("sys.stdout", new_callable=io.StringIO) as out:
            rc = cli.main(["list-kills"])
        self.assertEqual(rc, 0)
        self.assertIn("TSUNAMI-MSTU", out.getvalue())
...
class CLIOverrideRefusedWithoutFlag(unittest.TestCase):
    ...
    def test_refuses_without_confirmation_flag(self):
        with patch("backend.bots.tsunami.kill_switch.cli.clear_kill") as clear, \
             patch("sys.stderr", new_callable=io.StringIO) as err:
            rc = cli.main([
                "override-kill", "--scope", "INSTANCE",
                "--instance", "TSUNAMI-MSTU", "--by", "leron",
            ])
...
class CLIOverrideHappyPath(unittest.TestCase):
    def test_instance_override_calls_clear(self):
        with patch("backend.bots.tsunami.kill_switch.cli.clear_kill", return_value=True) as clear, \
             patch("sys.stdout", new_callable=io.StringIO) as out:
            rc = cli.main([
                "override-kill", "--scope", "INSTANCE",
                "--instance", "tsunami-mstu",  # lowercase to test uppercasing
                "--by", "leron", "--confirm-leron-override",
            ])
        self.assertEqual(rc, 0)
        clear.assert_called_once_with("INSTANCE", "TSUNAMI-MSTU", cleared_by="leron")
        self.assertIn("OK cleared INSTANCE/TSUNAMI-MSTU", out.getvalue())
    ...
    def test_override_noop_when_no_active_kill(self):
        with patch("backend.bots.tsunami.kill_switch.cli.clear_kill", return_value=False), \
             patch("sys.stdout", new_callable=io.StringIO) as out:
            rc = cli.main([
                "override-kill", "--scope", "PLATFORM",
                "--by", "leron", "--confirm-leron-override",
