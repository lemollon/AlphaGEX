"""Tests for backend.bots.tsunami.kill_switch.state.
...
from backend.bots.tsunami.kill_switch import (  # noqa: E402
    KillEvent,
    KillScope,
    clear_kill,
    is_killed,
...
class SafeFallbackWithoutDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._patch = patch(
            "backend.bots.tsunami.kill_switch.state._connect",
    ...
    def test_is_killed_returns_false(self):
        self.assertFalse(is_killed(KillScope.INSTANCE, "TSUNAMI-MSTU"))
    ...
    def test_record_then_check_uses_db_connection(self):
        ...
        with patch(
            ...
            ok = record_kill(KillEvent(
                scope=KillScope.INSTANCE, instance_name="TSUNAMI-MSTU",
                trigger_id="I-K1", reason="drawdown",
                context={"dd_pct": 0.35},
            ))
            killed = is_killed(KillScope.INSTANCE, "TSUNAMI-MSTU")

        self.assertTrue(ok)
        self.assertTrue(killed)
        # Verify INSERT and SELECT were both issued.
        self.assertTrue(any("INSERT INTO tsunami_kill_state" in r[0] for r in rows))
        self.assertTrue(any("SELECT 1 FROM tsunami_kill_state" in r[0] for r in rows))
