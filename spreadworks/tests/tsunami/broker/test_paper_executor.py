"""Tests for backend.bots.tsunami.broker.paper_executor."""
...
from backend.bots.tsunami.broker.paper_executor import (  # noqa: E402
    _next_friday,
    paper_broker_executor,
)
from backend.bots.tsunami.configs import get  # noqa: E402
from backend.bots.tsunami.engine import EngineEntryDecision  # noqa: E402
from backend.bots.tsunami.instance import TsunamiInstance  # noqa: E402
from backend.bots.tsunami.strike_mapping.engine import OptionLeg, TradeStructure  # noqa: E402
from backend.bots.tsunami.strike_mapping.letf_mapper import LETFTarget  # noqa: E402
from backend.bots.tsunami.strike_mapping.wall_finder import Wall  # noqa: E402
...
class PaperBrokerExecutorReturnsNoneWithoutDB(unittest.TestCase):
    def test_returns_none_when_db_unavailable(self):
        with patch("backend.bots.tsunami.broker.paper_executor._connect",
                   return_value=(None, False)):
            inst = TsunamiInstance(config=get("TSUNAMI-MSTU"))
...
class PaperBrokerExecutorPersistsAndReturnsId(unittest.TestCase):
    def test_returns_uuid_position_id_and_appends_position(self):
        ...
        with patch("backend.bots.tsunami.broker.paper_executor._connect",
             ...
             patch("backend.bots.tsunami.broker.paper_executor.audit_recorder") as ar:
            ar.record_entry_filled.return_value = True
            inst = TsunamiInstance(config=get("TSUNAMI-MSTU"))
            result = paper_broker_executor(inst, _approved_decision())

        self.assertIsNotNone(result)
        self.assertTrue(result.startswith("tsunami-paper-"))
