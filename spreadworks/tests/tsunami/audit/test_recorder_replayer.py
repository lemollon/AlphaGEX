"""Tests for backend.bots.tsunami.audit.recorder + replayer."""
...
from backend.bots.tsunami.audit import recorder, replayer  # noqa: E402
...
class RecorderDispatchesEventTypes(unittest.TestCase):
    ...
    def test_entry_eval_uses_correct_event_type(self):
        with patch("backend.bots.tsunami.audit.recorder.store.insert", return_value=True) as ins:
    ...
    def test_exit_filled_uses_correct_event_type(self):
        with patch("backend.bots.tsunami.audit.recorder.store.insert", return_value=True) as ins:
            recorder.record_exit_filled(
                "TSUNAMI-MSTU", position_id="p1", trigger_id="T7",
...
class ReplayerReconstructsTimeline(unittest.TestCase):
    def test_replay_assembles_full_chain(self):
        rows = [
            {"id": 1, "timestamp": "t0", "instance": "TSUNAMI-MSTU",
             "event_type": "ENTRY_EVAL", "data": {"decision": "STRUCTURE_RETURNED"},
             "position_id": "p1"},
            {"id": 2, "timestamp": "t1", "instance": "TSUNAMI-MSTU",
             "event_type": "ENTRY_FILLED", "data": {"contracts": 1},
             "position_id": "p1"},
            {"id": 3, "timestamp": "t2", "instance": "TSUNAMI-MSTU",
             "event_type": "MANAGEMENT_EVAL", "data": {"fired_action": None},
             "position_id": "p1"},
            {"id": 4, "timestamp": "t3", "instance": "TSUNAMI-MSTU",
             "event_type": "EXIT_FILLED",
             "data": {"trigger_id": "T7", "realized_pnl": 12.5},
             "position_id": "p1"},
        ]
        with patch("backend.bots.tsunami.audit.replayer.store.query_by_position", return_value=rows):
            timeline = replayer.replay_position("p1")

        self.assertEqual(timeline.instance, "TSUNAMI-MSTU")
    ...
    def test_replay_empty_when_no_rows(self):
        with patch("backend.bots.tsunami.audit.replayer.store.query_by_position", return_value=[]):
