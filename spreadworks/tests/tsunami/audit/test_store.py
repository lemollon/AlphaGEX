"""Tests for backend.bots.tsunami.audit.store."""
...
from backend.bots.tsunami.audit import store  # noqa: E402
...
class EventTypeValidation(unittest.TestCase):
    ...
    def test_insert_rejects_invalid_event_type(self):
        with patch("backend.bots.tsunami.audit.store._connect") as conn:
            ok = store.insert("TSUNAMI-MSTU", "BOGUS", {}, position_id="p1")
        self.assertFalse(ok)
...
class SafeFallbackWithoutDB(unittest.TestCase):
    def setUp(self):
        self._patch = patch(
            "backend.bots.tsunami.audit.store._connect", return_value=(None, False)
        )
        self._patch.start()
    ...
    def test_insert_returns_false_without_db(self):
        self.assertFalse(
            store.insert("TSUNAMI-MSTU", "ENTRY_EVAL", {"k": 1}, position_id="p1")
        )
    ...
    def test_query_recent_returns_empty(self):
        self.assertEqual(store.query_recent("TSUNAMI-MSTU"), [])
...
class InsertExecutesValidSQL(unittest.TestCase):
    def test_insert_calls_execute_with_payload(self):
        ...
        with patch(
            "backend.bots.tsunami.audit.store._connect", return_value=(_Conn(), True)
        ):
            ok = store.insert(
                "TSUNAMI-MSTU", "ENTRY_FILLED",
                {"strike": 9.0, "n": 1}, position_id="p-uuid-1",
            )
        self.assertTrue(ok)
        self.assertEqual(len(seen), 1)
        sql, args = seen[0]
        self.assertIn("INSERT INTO tsunami_trade_audit", sql)
        self.assertEqual(args[0], "TSUNAMI-MSTU")
