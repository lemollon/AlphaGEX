"""Tests for backend.bots.tsunami.instance."""
...
from backend.bots.tsunami.configs import InstanceConfig, get  # noqa: E402
...
def _position(*, position_id="p1", defined_max=0.20, contracts=1) -> Position:
    p = Position(
        position_id=position_id, instance_name="TSUNAMI-MSTU",
...
class IdentityProperties(unittest.TestCase):
    def test_name_letf_underlying(self):
        ...
        self.assertEqual(inst.name, "TSUNAMI-MSTU")
...
class CapacityCheck(unittest.TestCase):
    def test_has_capacity_when_empty_and_not_killed(self):
        with patch("backend.bots.tsunami.instance.is_killed", return_value=False):
            inst = TsunamiInstance(config=get("TSUNAMI-MSTU"))
