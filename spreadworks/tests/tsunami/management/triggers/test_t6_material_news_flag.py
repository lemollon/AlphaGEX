"""Tests for backend.bots.tsunami.management.triggers.t6_material_news_flag.
...
from backend.bots.tsunami.management import Position, PositionState  # noqa: E402
from backend.bots.tsunami.management.triggers.t6_material_news_flag import (  # noqa: E402
    evaluate,
)
...
def _position(*, letf="TSLL", underlying="TSLA") -> Position:
    return Position(
        position_id="p", instance_name=f"TSUNAMI-{letf}",
...
class T6Fire(unittest.TestCase):
    ...
    def test_fires_for_each_underlying_independently(self):
        # Flag on MSTR fires for TSUNAMI-MSTU, not for TSUNAMI-TSLL.
        flagged = {"MSTR"}
