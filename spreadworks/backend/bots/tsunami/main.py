"""TSUNAMI runner -- entry point that cycles through all 5 instances.

Per master spec section 9.2 + kickoff prompt:
    "spreadworks/backend/bots/tsunami/main.py -- entry point: run all 5 instances"
...
Usage:
    python -m backend.bots.tsunami.main --cycle entry
    python -m backend.bots.tsunami.main --cycle management
    python -m backend.bots.tsunami.main --cycle both
    python -m backend.bots.tsunami.main --dry-run   (logs only; no audit / no broker)
"""
...
# spreadworks/ root (4 levels up: tsunami -> bots -> backend -> spreadworks)
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)

from backend.bots.tsunami.audit import recorder as audit_recorder  # noqa: E402
from backend.bots.tsunami.configs import all_instances  # noqa: E402
...
from backend.bots.tsunami.monitoring import alerts as monitoring_alerts  # noqa: E402
from backend.bots.tsunami.monitoring import heartbeat as monitoring_heartbeat  # noqa: E402
from backend.bots.tsunami import equity_snapshots as equity_snapshots_module  # noqa: E402

logger = logging.getLogger(__name__)


SnapshotFetcher = Callable[[TsunamiInstance], MarketSnapshot]
PlatformFetcher = Callable[[dict[str, TsunamiInstance]], PlatformContext]
BrokerExecutor = Callable[[TsunamiInstance, EngineEntryDecision], Optional[str]]


def _default_snapshot_fetcher(_inst: TsunamiInstance) -> MarketSnapshot:
...
def _default_platform_fetcher(_instances: dict[str, TsunamiInstance]) -> PlatformContext:
    return PlatformContext(open_position_count=0, open_dollars_at_risk=0.0)


def _default_broker_executor(
    _inst: TsunamiInstance, _decision: EngineEntryDecision
) -> Optional[str]:
    raise NotImplementedError(
...
@dataclass
class Runner:
    """Top-level orchestrator that cycles through every TSUNAMI instance."""

    engine: TsunamiEngine = field(default_factory=TsunamiEngine)
    ...
    instances: dict[str, TsunamiInstance] = field(
...
def _record_entry_eval(instance: TsunamiInstance, decision: EngineEntryDecision) -> None:
    chain_payload = [
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="backend.bots.tsunami.main")
