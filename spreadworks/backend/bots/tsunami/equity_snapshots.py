"""TSUNAMI equity snapshot writer.
...
from .configs import GLOBAL, TSUNAMI_INSTANCES
...
def _instance_starting_capital(instance_name: str) -> float:
    ...
    cfg = TSUNAMI_INSTANCES.get(instance_name)
    if cfg is None:
        return 0.0
...
def _instance_realized_pnl(conn, instance_name: str) -> float:
    ...
    try:
        cur.execute(
            "SELECT COALESCE(SUM(realized_pnl), 0) FROM tsunami_paper_positions "
...
def _instance_unrealized_pnl(instance: TsunamiInstance) -> float:
...
def write_snapshots(instances: dict[str, TsunamiInstance]) -> int:
