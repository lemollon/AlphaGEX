"""TSUNAMI kill-switch persistent state.
...
def is_killed(scope: KillScope | str, instance_name: Optional[str] = None) -> bool:
    ...
    try:
        ...
        try:
            if name is None:
                cur.execute(
                    "SELECT 1 FROM tsunami_kill_state "
...
def record_kill(event: KillEvent) -> bool:
    ...
    try:
        ...
        try:
            cur.execute(
                """
                INSERT INTO tsunami_kill_state
...
def clear_kill(
    ...
    try:
        ...
        try:
            if name is None:
                cur.execute(
                    "UPDATE tsunami_kill_state SET active = FALSE, "
    ...
    try:
        ...
        try:
            cur.execute(
                ...
                "FROM tsunami_kill_state WHERE active = TRUE ORDER BY killed_at DESC"
            )
