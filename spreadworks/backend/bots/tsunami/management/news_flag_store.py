...
action. Flag persists in tsunami_news_flags (migration 029) until
...
def is_ticker_flagged(ticker: str) -> bool:
    """True when ticker has an active row in tsunami_news_flags."""
    conn, ok = _connect()
    ...
    try:
        ...
        try:
            cur.execute(
                "SELECT 1 FROM tsunami_news_flags WHERE ticker = %s LIMIT 1",
                (ticker,),
            )
...
def flag_ticker(ticker: str, reason: str = "", flagged_by: str = "cli") -> bool:
    ...
    try:
        ...
        try:
            cur.execute(
                """
                INSERT INTO tsunami_news_flags (ticker, reason, flagged_by)
                VALUES (%s, %s, %s)
...
def unflag_ticker(ticker: str) -> bool:
    ...
    try:
        cur = conn.cursor()
        try:
            cur.execute("DELETE FROM tsunami_news_flags WHERE ticker = %s", (ticker,))
...
def list_flagged_tickers() -> List[dict]:
    ...
    try:
        ...
        try:
            cur.execute(
                "SELECT ticker, reason, flagged_at, flagged_by FROM tsunami_news_flags "
