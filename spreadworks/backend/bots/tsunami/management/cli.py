"""CLI for TSUNAMI management actions (T6 material-news flag).
...
Usage:
    python -m backend.bots.tsunami.management.cli flag-news TSLA --reason "FDA news"
    python -m backend.bots.tsunami.management.cli unflag-news TSLA
    python -m backend.bots.tsunami.management.cli list-flags
...
# spreadworks/ root (5 levels up from this subpackage module)
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))))
if _REPO_ROOT not in sys.path:
    sys.path.insert(0, _REPO_ROOT)

from backend.bots.tsunami.management import news_flag_store  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="tsunami.management.cli",
        description="TSUNAMI management CLI (material-news flag).",
    )
