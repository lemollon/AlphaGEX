...
from backend.bots.tsunami.strike_mapping.engine import (
...
def evaluate(
    ...
    # Epsilon absorbs float noise at the exact spec boundary (net cost
    # == 30% of long-call mid must PASS per the <= in the spec).
    if net_cost > threshold + 1e-9:
        return GateResult(
            gate="G08",
            outcome=GateOutcome.FAIL,
            reason=(
