"""TSUNAMI gate orchestrator -- runs G02..G10 in order, persists failures.
...
``tsunami_gate_failures`` (migration 028), and returns the chain so the
...
from backend.bots.tsunami.strike_mapping.engine import TradeStructure
...
def orchestrate_entry(inputs: GateInputs) -> EntryDecision:
    ...
    if failed is not None:
        try:
            ...
            logger.warning("tsunami_gate_failures persist failed: %r", exc)
...
def _persist_failure(inputs: GateInputs, chain: list[GateResult], failed: GateResult) -> None:
    """Best-effort write to tsunami_gate_failures. Swallows DB errors."""
    try:
    ...
    try:
        ...
        try:
            cur.execute(
                """
                INSERT INTO tsunami_gate_failures (
