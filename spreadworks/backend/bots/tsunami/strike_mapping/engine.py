...
from backend.bots.tsunami.strike_mapping.leg_builder import (
    ThreeLegStructure,
    build_legs,
)
from backend.bots.tsunami.strike_mapping.letf_mapper import LETFTarget, map_to_letf
from backend.bots.tsunami.strike_mapping.wall_finder import (
    GammaStrike,
    Wall,
    find_wall,
)
...
def passes_bid_ask(leg: OptionLeg) -> bool:
    ...
    if mid <= 0:
        return False
    # Epsilon absorbs float noise at the exact boundary: e.g. bid 0.90 /
    # ask 1.10 gives (1.10-0.90)/1.00 = 0.20000000000000007, which must
    # count as "spread == 20%" and pass per the spec's <=.
    return (leg.ask - leg.bid) / mid <= MAX_BID_ASK_RATIO + 1e-9
...
def build_trade_structure(
    ...
    if long_call_mid <= 0:
        return None
    # Epsilon: exact-30% boundary must pass (same as gates/g08).
    if net_cost > MAX_NET_COST_TO_CALL_RATIO * long_call_mid + 1e-9:
        return None
