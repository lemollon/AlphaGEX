"""TSUNAMI paper-trading broker executor.
...
persists positions to tsunami_paper_positions (migration 032).
...
Lifecycle:
    ...
    is called by Runner.run_entry_cycle. We:
      ...
      2. INSERT a tsunami_paper_positions row with all entry economics
...
from backend.bots.tsunami.audit import recorder as audit_recorder
from backend.bots.tsunami.engine import EngineEntryDecision
from backend.bots.tsunami.instance import TsunamiInstance
from backend.bots.tsunami.management.state import Position, PositionState
...
def _persist_position(
    ...
    """INSERT row into tsunami_paper_positions. Returns True on success."""
    ...
    try:
        ...
        try:
            cur.execute(
                """
                INSERT INTO tsunami_paper_positions (
...
def paper_broker_executor(
    ...
    position_id = f"tsunami-paper-{uuid.uuid4().hex[:12]}"
