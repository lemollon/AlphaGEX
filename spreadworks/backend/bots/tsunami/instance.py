"""TsunamiInstance -- stateful per-LETF wrapper.

Per master spec section 9.2:
    "TsunamiInstance: Stateful per-LETF wrapper. Holds config +
...
from backend.bots.tsunami.kill_switch import KillScope, is_killed
from backend.bots.tsunami.management.state import Position
...
class TsunamiInstance:
    ...
    @property
    def name(self) -> str:
        """Bot-guard tag, e.g. 'TSUNAMI-MSTU'."""
...
) -> dict[str, TsunamiInstance]:
    """Construct empty TsunamiInstance objects for each config (no open positions)."""
    return {cfg.bot_guard_tag: TsunamiInstance(config=cfg) for cfg in configs}
