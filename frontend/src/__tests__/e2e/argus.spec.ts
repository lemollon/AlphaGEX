/**
 * E2E Tests for ARGUS (0DTE Gamma Live)
 *
 * These tests verify:
 * 1. ARGUS page loads correctly
 ...
 *
 * Run with: npx playwright test argus.spec.ts
 */
...
test.describe('ARGUS - 0DTE Gamma Live', () => {
  ...
  test('ARGUS page loads successfully', async ({ page }) => {
  ...
  test('AI Commentary panel is visible', async ({ page }) => {
    ...
    await expect(page.locator('text=ARGUS AI INTEL')).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Commentary panel can be expanded', async ({ page }) => {
    ...
    // Find expand button near ARGUS AI INTEL
    const expandButton = page.locator('text=ARGUS AI INTEL').locator('..').locator('button').first()
...
test.describe('ARGUS - Navigation Integration', () => {

  test('ARGUS is accessible from navigation menu', async ({ page }) => {
    await page.goto(BASE_URL)
    await page.waitForTimeout(2000)

    // Find and click ARGUS in navigation
    const argusLink = page.locator('a[href="/argus"]').or(page.locator('text=ARGUS (0DTE Gamma)'))

    if (await argusLink.isVisible()) {
      await argusLink.click()
      await page.waitForURL('**/argus')

      // Should be on ARGUS page
      await expect(page.locator('h1:has-text("ARGUS")')).toBeVisible({ timeout: 10000 })
    }
  })

  test('Navigation from ARGUS to other pages works', async ({ page }) => {
    await page.goto(BASE_URL + '/argus')
    await page.waitForTimeout(2000)
  ...
  test('Returning to ARGUS maintains state', async ({ page }) => {
    await page.goto(BASE_URL + '/argus')
    await page.waitForTimeout(3000)
...
test.describe('ARGUS - Gamma Flip Detection', () => {
...
test.describe('ARGUS - Error Handling', () => {
  ...
  test('Handles API error gracefully', async ({ page }) => {
    // Block ARGUS API endpoints
    await page.route('**/api/argus/**', route => route.abort())
...
test.describe('ARGUS - Performance', () => {
  ...
  test('Page loads within acceptable time', async ({ page }) => {
    ...
    console.log(`ARGUS page load time: ${loadTime}ms`)
