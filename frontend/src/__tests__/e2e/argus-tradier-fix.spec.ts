/**
 * E2E Tests for ARGUS Tradier Connection Fix
 *
 ...
 *
 * Run with: npx playwright test argus-tradier-fix.spec.ts
 */
...
test.describe('ARGUS - Tradier Connection Fix', () => {
  ...
  test('Page loads without React error #310 crash', async ({ page }) => {
    ...
    // Navigate to ARGUS
    await page.goto(BASE_URL + '/argus')

    // Wait for page to fully load
    await page.waitForTimeout(5000)

    // Verify page header loaded (not crashed)
    await expect(page.locator('h1:has-text("ARGUS")')).toBeVisible({ timeout: 15000 })
  ...
  test('Page survives multiple rapid data updates', async ({ page }) => {
    ...
    // Verify initial load
    await expect(page.locator('h1:has-text("ARGUS")')).toBeVisible({ timeout: 10000 })
    ...
    if (await refreshButton.isVisible()) {
      ...
      await expect(page.locator('h1:has-text("ARGUS")')).toBeVisible()
  ...
  test('Data source status endpoint returns valid response', async ({ page }) => {
    ...
    const response = await page.request.get(API_URL + '/api/argus/data-source-status')
  ...
  test('Test Tradier connection endpoint works', async ({ page }) => {
    const response = await page.request.get(API_URL + '/api/argus/test-tradier-connection')
  ...
  test('ARGUS gamma endpoint returns valid structure', async ({ page }) => {
    const response = await page.request.get(API_URL + '/api/argus/gamma?symbol=SPY')
  ...
  test('Page handles empty strikes array without crash', async ({ page, request }) => {
    ...
    await page.route('**/api/argus/gamma**', async route => {
      await route.fulfill({
        status: 200,
  ...
  test('Commentary section loads without causing re-render loop', async ({ page }) => {
    ...
    // Check for ARGUS AI INTEL section
    await expect(page.locator('text=ARGUS AI INTEL')).toBeVisible({ timeout: 15000 })
...
test.describe('ARGUS - Data Integrity', () => {
...
test.describe('ARGUS - HYPERION Parity', () => {

  test('HYPERION page also loads without crash', async ({ page }) => {
    const errors: string[] = []
    page.on('pageerror', err => errors.push(err.message))
    page.on('console', msg => {
      if (msg.type() === 'error' && msg.text().includes('re-render')) {
    ...
    await page.goto(BASE_URL + '/hyperion')
    await page.waitForTimeout(5000)

    // HYPERION should load
    await expect(page.locator('h1:has-text("HYPERION")')).toBeVisible({ timeout: 15000 })
  ...
  test('HYPERION data source status works', async ({ page }) => {
    const response = await page.request.get(API_URL + '/api/hyperion/data-source-status')

    // Endpoint might not exist yet on HYPERION
    if (response.status() === 200) {
      const data = await response.json()
      expect(data).toHaveProperty('success')
    } else if (response.status() === 404) {
      // Expected if endpoint not added to HYPERION
      console.log('HYPERION data-source-status endpoint not yet implemented')
    }
  })
})
