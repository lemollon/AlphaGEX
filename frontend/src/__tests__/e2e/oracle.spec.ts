/**
 * E2E Tests for Oracle Knowledge Base
 *
 * These tests verify:
 * 1. Oracle page loads correctly
 ...
 *
 * Run with: npx playwright test oracle.spec.ts
 */
...
test.describe('Oracle Knowledge Base - Page Load', () => {
  ...
  test('Oracle page loads successfully', async ({ page }) => {
    await page.goto(BASE_URL + '/oracle')

    // Verify page header
    await expect(page.locator('h1:has-text("Oracle Knowledge Base")')).toBeVisible({ timeout: 15000 })
    await expect(page.locator('text=Centralized intelligence hub')).toBeVisible({ timeout: 5000 })
  })

  test('Oracle page shows status cards', async ({ page }) => {
    await page.goto(BASE_URL + '/oracle')
...
test.describe('Oracle Knowledge Base - Tab Navigation', () => {
  ...
  test('Performance tab switches correctly', async ({ page }) => {
    ...
    await expect(page.locator('text=Oracle Prediction Performance')).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Live Logs tab switches correctly', async ({ page }) => {
    ...
    await expect(page.locator('text=Live Oracle Logs')).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Decision Log tab switches correctly', async ({ page }) => {
    ...
    await expect(page.locator('text=ORACLE Decision Log')).toBeVisible({ timeout: 10000 })
  })
  ...
  test('Data Flow tab switches correctly', async ({ page }) => {
    ...
    await expect(page.locator('text=Oracle Data Flow - Full Transparency')).toBeVisible({ timeout: 10000 })
  })
})

test.describe('Oracle Knowledge Base - Bot Interactions Tab', () => {
...
test.describe('Oracle Knowledge Base - Performance Tab', () => {
...
test.describe('Oracle Knowledge Base - Training Tab', () => {
...
test.describe('Oracle Knowledge Base - Live Logs Tab', () => {
...
test.describe('Oracle Knowledge Base - Decision Log Tab', () => {
  ...
  test('DecisionLogViewer component loads', async ({ page }) => {
    ...
    await expect(page.locator('text=ORACLE Decision Log')).toBeVisible({ timeout: 5000 })
  })
})

test.describe('Oracle Knowledge Base - Data Flow Tab', () => {
...
test.describe('Oracle Knowledge Base - Info Section', () => {
...
test.describe('Oracle Knowledge Base - Error Handling', () => {
  ...
  test('Shows loading state initially', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Oracle Knowledge Base")')).toBeVisible({ timeout: 10000 })
  })

  test('Handles API errors gracefully', async ({ page }) => {
    // Block Oracle API endpoints
    await page.route('**/api/zero-dte/oracle/**', route => route.abort())
  ...
  test('Error boundary catches component errors', async ({ page }) => {
    ...
    for (const tab of tabs) {
      ...
      await expect(page.locator('h1:has-text("Oracle Knowledge Base")')).toBeVisible()
    }
  })

  test('Page handles missing data gracefully', async ({ page }) => {
    // Mock empty API responses
    await page.route('**/api/zero-dte/oracle/status', route =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ success: true, oracle: null, bot_heartbeats: {} })
      })
    )
...
test.describe('Oracle Knowledge Base - Navigation Integration', () => {

  test('Oracle is accessible from navigation menu', async ({ page }) => {
    await page.goto(BASE_URL)
    await page.waitForTimeout(2000)

    // Find and click Oracle in navigation
    const oracleLink = page.locator('a[href="/oracle"]').or(page.locator('text=Oracle'))

    if (await oracleLink.first().isVisible()) {
      await oracleLink.first().click()
      await page.waitForURL('**/oracle')

      // Should be on Oracle page
      await expect(page.locator('h1:has-text("Oracle Knowledge Base")')).toBeVisible({ timeout: 10000 })
    }
  })

  test('Navigation from Oracle to other pages works', async ({ page }) => {
    await page.goto(BASE_URL + '/oracle')
    await page.waitForTimeout(2000)
  ...
  test('Returning to Oracle maintains functionality', async ({ page }) => {
    await page.goto(BASE_URL + '/oracle')
    await page.waitForTimeout(3000)
...
test.describe('Oracle Knowledge Base - Performance Tests', () => {
  ...
  test('Page loads within acceptable time', async ({ page }) => {
    ...
    console.log(`Oracle page load time: ${loadTime}ms`)
  ...
  test('No memory leaks on tab cycling', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Oracle Knowledge Base")')).toBeVisible({ timeout: 5000 })
  })
})

test.describe('Oracle Knowledge Base - Data Display Verification', () => {
...
test.describe('Oracle Knowledge Base - Interactive Features', () => {
