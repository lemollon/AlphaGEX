...
/**
 * STARS ML Status Badge
 *
 * A compact status indicator showing whether STARS ML models are trained
 * and providing quick access to the STARS dashboard.
 *
 * Used on: WATCHTOWER, GLORY pages
 */
...
export default function StarsStatusBadge() {
  const [status, setStatus] = useState<StarsStatus | null>(null)
  ...
  const getTooltip = () => {
    if (loading) return 'Checking STARS ML status...'
    if (error) return 'Failed to load STARS status'
    if (!status?.is_trained) return 'STARS ML models not trained. Click to train.'
  ...
  return (
    ...
    >
      ...
      <span className="font-medium text-gray-300">STARS</span>
      {getStatusIcon()}
