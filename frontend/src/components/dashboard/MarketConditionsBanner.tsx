...
// Oracle recommendation display
const getOracleRecommendation = (data: OracleData): { label: string; color: string; bgColor: string; icon: typeof Shield } => {
...
export default function MarketConditionsBanner() {
  ...
  const { data: oracleData } = useSWR<OracleData>('/api/oracle/status', fetcher, { refreshInterval: 30000 })
  ...
  const oracle = getOracleRecommendation(oracleData || {})
    ...
    <div className="bg-gradient-to-r from-gray-900 via-gray-900 to-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className={`flex items-center gap-3 px-4 py-2 rounded-lg ${oracle.bgColor}`}>
            <OracleIcon className={`w-5 h-5 ${oracle.color}`} />
            <div>
              <div className="text-xs text-gray-500">Oracle Says</div>
              <div className={`font-bold ${oracle.color}`}>{oracle.label}</div>
