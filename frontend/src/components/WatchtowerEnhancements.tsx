...
/**
 * WATCHTOWER Enhanced Features Components
 *
 * Additional visualization components for WATCHTOWER (0DTE):
...
export function TradeSetupDetector({ symbol = 'SPY' }: { symbol?: string }) {
  ...
  useEffect(() => {
    const fetchSetup = async () => {
      try {
        const response = await api.get(`/api/watchtower/trade-setups?symbol=${symbol}`)
...
export function OptimalStrikes({ symbol = 'SPY' }: { symbol?: string }) {
  ...
  useEffect(() => {
    const fetchStrikes = async () => {
      try {
        const response = await api.get(`/api/watchtower/optimal-strikes?symbol=${symbol}`)
        if (response.data?.success) {
...
export function PatternOutcomes({ symbol = 'SPY' }: { symbol?: string }) {
  ...
  useEffect(() => {
    const fetchPatterns = async () => {
      try {
        const response = await api.get(`/api/watchtower/pattern-outcomes?symbol=${symbol}`)
...
export function PinAccuracyTracker({ symbol = 'SPY' }: { symbol?: string }) {
  ...
  useEffect(() => {
    const fetchAccuracy = async () => {
      try {
        const response = await api.get(`/api/watchtower/pin-accuracy?symbol=${symbol}`)
...
export function GammaDecayVisualization({ symbol = 'SPY' }: { symbol?: string }) {
  ...
  useEffect(() => {
    const fetchDecay = async () => {
      try {
        const response = await api.get(`/api/watchtower/gamma-decay?symbol=${symbol}`)
        if (response.data?.success) {
