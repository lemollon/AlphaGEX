...
function useBotLiveData(): { liveData: AllBotsLiveData, isLoading: boolean } {
  ...
  const liveData = useMemo<AllBotsLiveData>(() => {
    ...
    if (aresData) {
      ...
      data['fortress'] = {
        // Map to PROPHET system planets (where FORTRESS performance is shown)
        'prediction': {
          pnl: perf.total_pnl,
    ...
    if (botsStatus?.data) {
      ...
      // PROPHET
      if (bots.PROPHET) {
        const perf = extractPerf(bots.PROPHET)
        data['prophet'] = {
          'prediction': {
            pnl: perf.total_pnl,
            winRate: perf.win_rate,
...
class Canvas3DErrorBoundary extends Component<{ children: ReactNode, fallback: ReactNode }, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    console.error('Covenant3D Error:', error, errorInfo)
  }
...
function getSystemCategory(systemId: string): SystemCategory {
  ...
  const innerRing = ['gideon', 'anchor', 'samson', 'wisdom', 'quant']
...
const BOT_NODES = [
  { id: 'prophet', name: 'PROPHET', angle: 0 },
...
function InterstellarNetwork({ onNavigate, paused }: { onNavigate: (systemId: string, position: [number, number, number]) => void, paused: boolean }) {
  ...
  const wormholes = useMemo(() => [
    ...
    { position: [12, 0, -21] as [number, number, number], color: '#06b6d4', targetSystem: 'WATCHTOWER' },
    { position: [-8, -3, -23] as [number, number, number], color: '#8b5cf6', targetSystem: 'PROPHET' },
    { position: [8, -10, -22] as [number, number, number], color: '#ef4444', targetSystem: 'CHRONICLES' },
  ], [])
...
const useStockPrices = () => {
  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      try {
        const response = await fetch(`${API_URL}/api/discernment/batch-quotes`)
...
// Inner Ring: 5 Bot Systems (GIDEON, ANCHOR, SAMSON, WISDOM, QUANT) - radius ~45
// Outer Ring: 7 Original Systems - radius ~80
// =============================================================================

const SOLAR_SYSTEMS = [
  // =============================================================================
  // MANNA - THE CENTER OF THE COVENANT UNIVERSE (Christian Theme)
  // =============================================================================
  {
    id: 'manna',
    name: 'MANNA',
...
function SunFlareEffect({ flareType, color, paused }: { flareType: FlareType, color: string, paused: boolean }) {
  ...
  // Radiant: Brilliant solar corona with god rays (for DISCERNMENT)
  const RadiantFlare = () => {
  ...
  // Fractal: Mathematical geometric patterns (for GLORY)
  const FractalFlare = () => {
...
// =============================================================================
// MANNA EFFECTS - The Most Divine, Extravagant System in the Covenant
...
function MannaEffects({ color, sunColor, paused }: { color: string, sunColor: string, paused: boolean }) {
  ...
  // ==================== STAR DUST PARTICLES ====================
  // Golden sparkles surrounding the system
  ...
  // ==================== HALOS ====================
  const halos = useMemo(() => [
    { radius: 3, opacity: 0.5, speed: 0.3 },
    { radius: 5, opacity: 0.4, speed: -0.25 },
    { radius: 7, opacity: 0.3, speed: 0.2 },
    { radius: 9, opacity: 0.25, speed: -0.15 },
    { radius: 11, opacity: 0.2, speed: 0.1 },
  ], [])

  // ==================== FALLING MANNA ====================
  const mannaCount = 150
  ...
  useFrame((state) => {
    ...
    if (starDustRef.current) {
      ...
      for (let i = 0; i < starDustCount; i++) {
        ...
        positions[i * 3 + 1] += Math.cos(t * 1.5 + i * 0.5) * 0.005
        positions[i * 3 + 2] += Math.sin(t * 1.8 + i * 0.3) * 0.005
      }
      starDustRef.current.geometry.attributes.position.needsUpdate = true
      starDustRef.current.rotation.y = t * 0.01
    }
    ...
    // Divine rays pulsing from center
    if (raysRef.current) {
      raysRef.current.children.forEach((ray, i) => {
        const pulse = 0.5 + Math.abs(Math.sin(t * divineRays[i].speed + divineRays[i].phase)) * 0.5
        ray.scale.y = pulse
      })
    }

    // Verse rings rotation
    if (verseRingsRef.current) {
      verseRingsRef.current.children.forEach((ring, i) => {
        ring.rotation.y = t * (0.1 + i * 0.03) * (i % 2 === 0 ? 1 : -1)
    ...
    if (mannaRef.current) {
      const positions = mannaRef.current.geometry.attributes.position.array as Float32Array
      for (let i = 0; i < mannaCount; i++) {
        positions[i * 3 + 1] -= 0.06 + Math.random() * 0.02
        positions[i * 3] += Math.sin(t + i) * 0.008
        positions[i * 3 + 2] += Math.cos(t + i) * 0.008
      ...
      mannaRef.current.geometry.attributes.position.needsUpdate = true
    }
  ...
  return (
    <group ref={groupRef}>
      {/* ==================== BREATHING DIVINE CORE ==================== */}
      {/* All effects originate from this central point at (0,0,0) */}
      ...
      {/* ==================== GOLDEN CROSS - Centered at origin ==================== */}
      <group ref={crossRef}>
        {/* Main vertical beam */}
        <mesh position={[0, 0, 0]}>
      ...
      {/* ==================== STAR DUST - Twinkling golden particles ==================== */}
      ...
      {/* ==================== HALOS - Centered rings ==================== */}
      <group ref={haloRef}>
        {halos.map((halo, i) => (
          <mesh key={i} rotation={[Math.PI / 2, 0, 0]}>
            <torusGeometry args={[halo.radius, 0.08, 16, 128]} />
            <meshBasicMaterial color="#fbbf24" transparent opacity={halo.opacity} side={THREE.DoubleSide} />
          </mesh>
        ))}
      </group>

      {/* ==================== FALLING MANNA - Heavenly Bread ==================== */}
      <points ref={mannaRef}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" count={mannaCount} array={mannaPositions} itemSize={3} />
        </bufferGeometry>
        <pointsMaterial color="#fef3c7" size={0.15} transparent opacity={0.9} sizeAttenuation />
      </points>
      ...
      <group ref={verseRingsRef}>
        ...
        <group rotation={[Math.PI / 5, 0, 0]}>
          <mesh rotation={[Math.PI / 2, 0, 0]}>
            <torusGeometry args={[13, 0.03, 8, 128]} />
            <meshBasicMaterial color="#fbbf24" transparent opacity={0.5} side={THREE.DoubleSide} />
          </mesh>
          {MANNA_VERSES.slice(0, 4).map((verse, i) => (
            <Html key={i} position={[Math.cos((i / 4) * Math.PI * 2) * 13, 0, Math.sin((i / 4) * Math.PI * 2) * 13]} center style={{ pointerEvents: 'none' }}>
              <div className="text-[8px] text-amber-200 whitespace-nowrap font-serif italic px-2 py-0.5 rounded bg-black/30 backdrop-blur-sm">
                {verse}
              </div>
            </Html>
          ))}
        ...
        <group rotation={[-Math.PI / 6, 0, Math.PI / 5]}>
          <mesh rotation={[Math.PI / 2, 0, 0]}>
            <torusGeometry args={[17, 0.03, 8, 128]} />
            <meshBasicMaterial color="#f59e0b" transparent opacity={0.4} side={THREE.DoubleSide} />
          </mesh>
          {MANNA_VERSES.slice(4, 8).map((verse, i) => (
            <Html key={i} position={[Math.cos((i / 4) * Math.PI * 2) * 17, 0, Math.sin((i / 4) * Math.PI * 2) * 17]} center style={{ pointerEvents: 'none' }}>
              <div className="text-[7px] text-yellow-300/80 whitespace-nowrap font-serif italic px-2 py-0.5 rounded bg-black/30 backdrop-blur-sm">
                {verse}
              </div>
            </Html>
          ))}
      ...
      {/* ==================== CONNECTION BEAMS TO ALL SYSTEMS ==================== */}
      <group>
...
// WATCHTOWER - Scanning beams, surveillance drones, all-seeing eyes
...
// DISCERNMENT - Sun God with radiant beams, solar prominences, ML scanner effects
function DiscernmentEffects({ color, sunColor, paused }: { color: string, sunColor: string, paused: boolean }) {
  const groupRef = useRef<THREE.Group>(null)
  ...
  useFrame((state) => {
    ...
    // Discernment's chariot circling
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Discernment's Chariot - golden rays following a path */}
...
// WATCHTOWER: Comprehensive surveillance system with radar, drones, data streams, and heat maps
...
// PROPHET: Swirling crystal ball with visions
...
// CHRONICLES: Giant swinging pendulum
  ...
  return (
    <group>
      ...
      <AlwaysVisibleGroup>
        ...
        {/* WATCHTOWER - Surveillance System */}
        <ArgusSurveillanceSystem
          position={SOLAR_SYSTEMS.find(s => s.id === 'watchtower')?.position || [24, 5, -18]}
          color="#22d3ee"
          paused={paused}
        />

        {/* PROPHET - Crystal Ball Visions */}
        <CrystalBallVisions
          position={SOLAR_SYSTEMS.find(s => s.id === 'prophet')?.position || [0, 15, -25]}
          color="#a855f7"
          paused={paused}
        />

        {/* CHRONICLES - Giant Pendulum */}
        <GiantPendulum
          position={SOLAR_SYSTEMS.find(s => s.id === 'chronicles')?.position || [-18, -8, -22]}
          color="#ef4444"
          paused={paused}
        />
  ...
  return (
    <div className="w-full h-full bg-[#030712] flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-cyan-400">Initializing COVENANT...</p>
      </div>
    </div>
  )
}
    ...
    <div className="absolute top-4 right-4 z-10">
      ...
      {expanded && (
        <div className="mt-2 bg-black/80 border border-cyan-500/30 rounded-lg p-4 backdrop-blur min-w-[200px]">
          <h3 className="text-cyan-400 font-bold mb-3 text-sm">COVENANT Controls</h3>
...
export default function Covenant3D({
  botStatus = {},
  onNodeClick,
  className = '',
  gexValue = 0,
  ...
  const handleScreenshot = useCallback(() => {
    ...
    if (canvas) {
      ...
      link.download = `covenant-${Date.now()}.png`
