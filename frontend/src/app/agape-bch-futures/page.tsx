...
import {
  useAGAPEBchFuturesStatus,
  useAGAPEBchFuturesPerformance,
  useAGAPEBchFuturesPositions,
  useAGAPEBchFuturesScanActivity,
  useAGAPEBchFuturesClosedTrades,
  useAGAPEBchFuturesSnapshot,
  useAGAPEBchFuturesGexMapping,
  useAGAPEBchFuturesChartData,
  useAGAPEBchFuturesBrief,
...
type AgapeBchFuturesTabId = typeof AGAPE_BCH_FUTURES_TABS[number]['id']

// ==============================================================================
// MAIN COMPONENT
// ==============================================================================

export default function AgapeBchFuturesPage() {
  const [activeTab, setActiveTab] = useState<AgapeBchFuturesTabId>('portfolio')
  const [equityTimeframe, setEquityTimeframe] = useState('intraday')
  const sidebarPadding = useSidebarPadding()

  // Brand from centralized system
  const brand = BOT_BRANDS.AGAPE_BCH_FUTURES

  // Data hooks (matching FORTRESS/SAMSON pattern)
  const { data: statusData, isLoading: statusLoading, mutate: refreshStatus } = useAGAPEBchFuturesStatus()
  const { data: perfData } = useAGAPEBchFuturesPerformance()
  const { data: positionsData, isLoading: posLoading } = useAGAPEBchFuturesPositions()
  const { data: snapshotData, isLoading: snapLoading } = useAGAPEBchFuturesSnapshot({ enabled: activeTab === 'snapshot' })
  const { data: scansData } = useAGAPEBchFuturesScanActivity(30, { enabled: activeTab === 'activity' })
  const { data: closedData } = useAGAPEBchFuturesClosedTrades(50, { enabled: activeTab === 'history' })
  const { data: mappingData } = useAGAPEBchFuturesGexMapping({ enabled: activeTab === 'config' })
  const { data: chartData, isLoading: chartLoading } = useAGAPEBchFuturesChartData({ enabled: activeTab === 'charts' })
  const { data: briefData, isLoading: briefLoading } = useAGAPEBchFuturesBrief({ enabled: activeTab === 'charts' })
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE-BCH-FUTURES..." />
        </div>
      </>
    )
  }
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="rounded-lg p-3 border bg-amber-900/20 border-amber-500/30">
            <div className="flex items-center gap-3 text-sm">
              ...
              <span className="text-amber-300">
                BCH Futures (BCH-29MAY26-CDE) - Monthly Coinbase Derivatives via FCM
              </span>
              <span className="text-gray-500 ml-auto text-xs">1 contract = 25 units of BCH index = 25 BCH</span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    AGAPE-BCH-FUTURES paused after {status.aggressive_features.consecutive_losses} consecutive losses. Will resume automatically.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <StatCard
              label="BCH Price"
              value={status?.current_bch_price ? `$${status.current_bch_price.toFixed(2)}` : '---'}
          ...
          <div className="flex gap-2 border-b border-gray-800 pb-2">
            {AGAPE_BCH_FUTURES_TABS.map((tab) => (
              <button
                key={tab.id}
...
function PortfolioTab({
  ...
  brand: typeof BOT_BRANDS.AGAPE_BCH_FUTURES
  paperAccount: any
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_BCH_FUTURES" marketType="crypto_perp" marginEndpoint="/api/agape-bch-futures/margin" />

      {/* Equity Curve */}
      <BotCard title="Equity Curve" botName="AGAPE_BCH_FUTURES" icon={<TrendingUp className="w-5 h-5" />}
        headerRight={
      ...
      >
        <EquityCurveChart
          title=""
          botFilter="AGAPE_BCH_FUTURES"
          showIntradayOption={equityTimeframe === 'intraday'}
        />
      </BotCard>

      {/* Open Positions */}
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE_BCH_FUTURES" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          <LoadingState />
          ...
          <EmptyState
            ...
            description="AGAPE-BCH-FUTURES is scanning for opportunities"
          />
        ) : (
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-mono font-semibold">
                      {pos.quantity?.toLocaleString()}x BCH-FUT @ ${pos.entry_price?.toFixed(2)}
                    </span>
                    {pos.trailing_active && (
      ...
      {(perf?.total_trades > 0 || (paperAccount?.total_trades ?? 0) > 0) && (
        <BotCard title="Performance" botName="AGAPE_BCH_FUTURES" icon={<BarChart3 className="w-5 h-5" />}>
...
function OverviewTab({
  ...
  return (
    <>
      ...
      <BotCard title="Aggressive Features" botName="AGAPE_BCH_FUTURES" icon={<Zap className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_BCH_FUTURES" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-500">Instrument</span>
            <p className="text-white font-mono">{status?.instrument || 'BCH-FUT'}</p>
          </div>
          <div>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE_BCH_FUTURES }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Funding Rate */}
        <BotCard title="Funding Rate" subtitle="Replaces: Gamma Regime (POSITIVE/NEGATIVE)" botName="AGAPE_BCH_FUTURES">
        ...
        {/* Long/Short Ratio */}
        <BotCard title="Long/Short Ratio" subtitle="Replaces: GEX Directional Bias" botName="AGAPE_BCH_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Liquidations */}
        <BotCard title="Liquidation Clusters" subtitle="Replaces: Gamma Walls / Price Magnets" botName="AGAPE_BCH_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Crypto GEX */}
        <BotCard title="Crypto GEX (Deribit)" subtitle="Direct equivalent of Net GEX" botName="AGAPE_BCH_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_BCH_FUTURES }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE-BCH-FUTURES scans will appear here as they run"
      />
    )
  }

  return (
    <BotCard title={`Scan Activity (${scans.length})`} botName="AGAPE_BCH_FUTURES" icon={<Activity className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">Time</th>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">BCH</th>
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_BCH_FUTURES }) {
  const trades = data || []
  ...
  return (
    <BotCard title={`Trade History (${trades.length})`} botName="AGAPE_BCH_FUTURES" icon={<History className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE_BCH_FUTURES }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_BCH_FUTURES" icon={<Settings className="w-5 h-5" />}>
      ...
      {/* GEX Mapping */}
      {mappingData && (
        <BotCard title={mappingData.title || 'GEX Mapping'} subtitle={mappingData.description} botName="AGAPE_BCH_FUTURES" icon={<Globe className="w-5 h-5" />}>
      ...
      {mappingData?.trade_instrument && (
        <BotCard title="Trade Instrument" botName="AGAPE_BCH_FUTURES" icon={<Target className="w-5 h-5" />}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
