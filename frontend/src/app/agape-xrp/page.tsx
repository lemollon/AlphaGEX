...
import {
  useAGAPEXRPStatus,
  useAGAPEXRPPerformance,
  useAGAPEXRPPositions,
  useAGAPEXRPScanActivity,
  useAGAPEXRPClosedTrades,
  useAGAPEXRPSnapshot,
  useAGAPEXRPGexMapping,
...
type AgapeXrpTabId = typeof AGAPE_XRP_TABS[number]['id']

// ==============================================================================
// MAIN COMPONENT
// ==============================================================================

export default function AgapeXrpPage() {
  const [activeTab, setActiveTab] = useState<AgapeXrpTabId>('portfolio')
  const [equityTimeframe, setEquityTimeframe] = useState('intraday')
  const sidebarPadding = useSidebarPadding()

  // Brand from centralized system
  const brand = BOT_BRANDS.AGAPE_XRP

  // Data hooks (matching FORTRESS/SAMSON pattern)
  const { data: statusData, isLoading: statusLoading, mutate: refreshStatus } = useAGAPEXRPStatus()
  const { data: perfData } = useAGAPEXRPPerformance()
  const { data: positionsData, isLoading: posLoading } = useAGAPEXRPPositions()
  const { data: snapshotData, isLoading: snapLoading } = useAGAPEXRPSnapshot({ enabled: activeTab === 'snapshot' })
  const { data: scansData } = useAGAPEXRPScanActivity(30, { enabled: activeTab === 'activity' })
  const { data: closedData } = useAGAPEXRPClosedTrades(50, { enabled: activeTab === 'history' })
  const { data: mappingData } = useAGAPEXRPGexMapping({ enabled: activeTab === 'config' })
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE-XRP..." />
        </div>
      </>
    )
  }
            ...
            <div className={`rounded-lg p-3 border ${
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className={status.market.cme_market_open ? 'text-green-300' : 'text-yellow-300'}>
                  CME /XRP: {status.market.cme_market_open ? 'Market Open' : status.market.reason}
                </span>
          ...
          <div className={`${brand.lightBg} border ${brand.lightBorder} rounded-lg p-4`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className={`${brand.primaryText} font-semibold`}>Aggressive Mode - Crypto Directional Trading</h3>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    AGAPE-XRP paused after {status.aggressive_features.consecutive_losses} consecutive losses. Will resume automatically.
                  </p>
                </div>
              </div>
            </div>
...
function PortfolioTab({
  ...
  brand: typeof BOT_BRANDS.AGAPE_XRP
  paperAccount: any
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_XRP" marketType="crypto_futures" marginEndpoint="/api/agape-xrp/margin" />
      ...
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE_XRP" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          ...
          <EmptyState
            ...
            description="AGAPE-XRP is scanning for opportunities"
          />
        ) : (
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-mono font-semibold">
                      {pos.contracts}x /XRP @ ${pos.entry_price?.toFixed(4)}
                    </span>
                    {pos.trailing_active && (
      ...
      <BotCard title="Bot Configuration" botName="AGAPE_XRP" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className="text-white font-mono">{status?.instrument || '/XRP'}</p>
          </div>
          <div>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE_XRP }) {
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_XRP }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE-XRP scans will appear here as they run"
      />
    )
  }
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_XRP }) {
  const trades = data || []
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE_XRP }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_XRP" icon={<Settings className="w-5 h-5" />}>
