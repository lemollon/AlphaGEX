...
import {
  ...
  useAGAPEPerformance,
  useAGAPEPositions,
  useAGAPEScanActivity,
  useAGAPEClosedTrades,
  useAGAPESnapshot,
  useAGAPEGexMapping,
...
type AgapeTabId = typeof AGAPE_TABS[number]['id']
...
export default function AgapePage() {
  const [activeTab, setActiveTab] = useState<AgapeTabId>('portfolio')
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE..." />
        </div>
      </>
    )
  }
      ...
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          ...
          <EmptyState
            ...
            description="AGAPE is scanning for opportunities"
          />
        ) : (
      ...
      <BotCard title="Bot Configuration" botName="AGAPE" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className="text-white font-mono">{status?.instrument || '/MET'}</p>
          </div>
          <div>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE }) {
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE scans will appear here as they run"
      />
    )
  }
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE }) {
  const trades = data || []
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE" icon={<Settings className="w-5 h-5" />}>
      ...
      {/* GEX Mapping */}
      {mappingData && (
        <BotCard title={mappingData.title || 'GEX Mapping'} subtitle={mappingData.description} botName="AGAPE" icon={<Globe className="w-5 h-5" />}>
