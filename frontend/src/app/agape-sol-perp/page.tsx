...
import {
  useAGAPESolPerpStatus,
  useAGAPESolPerpPerformance,
  useAGAPESolPerpPositions,
  useAGAPESolPerpScanActivity,
  useAGAPESolPerpClosedTrades,
  useAGAPESolPerpSnapshot,
  useAGAPESolPerpGexMapping,
  useAGAPESolPerpChartData,
  useAGAPESolPerpBrief,
...
type AgapeSolPerpTabId = typeof AGAPE_SOL_PERP_TABS[number]['id']

// ==============================================================================
// MAIN COMPONENT
// ==============================================================================

export default function AgapeSolPerpPage() {
  const [activeTab, setActiveTab] = useState<AgapeSolPerpTabId>('portfolio')
  const [equityTimeframe, setEquityTimeframe] = useState('intraday')
  const sidebarPadding = useSidebarPadding()

  // Brand from centralized system
  const brand = BOT_BRANDS.AGAPE_SOL_PERP

  // Data hooks (matching FORTRESS/SAMSON pattern)
  const { data: statusData, isLoading: statusLoading, mutate: refreshStatus } = useAGAPESolPerpStatus()
  const { data: perfData } = useAGAPESolPerpPerformance()
  const { data: positionsData, isLoading: posLoading } = useAGAPESolPerpPositions()
  const { data: snapshotData, isLoading: snapLoading } = useAGAPESolPerpSnapshot({ enabled: activeTab === 'snapshot' })
  const { data: scansData } = useAGAPESolPerpScanActivity(30, { enabled: activeTab === 'activity' })
  const { data: closedData } = useAGAPESolPerpClosedTrades(50, { enabled: activeTab === 'history' })
  const { data: mappingData } = useAGAPESolPerpGexMapping({ enabled: activeTab === 'config' })
  const { data: chartData, isLoading: chartLoading } = useAGAPESolPerpChartData({ enabled: activeTab === 'charts' })
  const { data: briefData, isLoading: briefLoading } = useAGAPESolPerpBrief({ enabled: activeTab === 'charts' })
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE-SOL-PERP..." />
        </div>
      </>
    )
  }
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    AGAPE-SOL-PERP paused after {status.aggressive_features.consecutive_losses} consecutive losses. Will resume automatically.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <StatCard
              label="SOL Price"
              value={status?.current_sol_price ? `$${status.current_sol_price.toFixed(2)}` : '---'}
...
function PortfolioTab({
  ...
  brand: typeof BOT_BRANDS.AGAPE_SOL_PERP
  paperAccount: any
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_SOL_PERP" marketType="crypto_perp" marginEndpoint="/api/agape-sol-perp/margin" />

      {/* Equity Curve */}
      <BotCard title="Equity Curve" botName="AGAPE_SOL_PERP" icon={<TrendingUp className="w-5 h-5" />}
        headerRight={
      ...
      >
        <EquityCurveChart
          title=""
          botFilter="AGAPE_SOL_PERP"
          showIntradayOption={equityTimeframe === 'intraday'}
        />
      </BotCard>

      {/* Open Positions */}
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE_SOL_PERP" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          <LoadingState />
          ...
          <EmptyState
            ...
            description="AGAPE-SOL-PERP is scanning for opportunities"
          />
        ) : (
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-mono font-semibold">
                      {pos.quantity}x SOL-PERP @ ${pos.entry_price?.toFixed(2)}
                    </span>
                    {pos.trailing_active && (
      ...
      {(perf?.total_trades > 0 || (paperAccount?.total_trades ?? 0) > 0) && (
        <BotCard title="Performance" botName="AGAPE_SOL_PERP" icon={<BarChart3 className="w-5 h-5" />}>
...
function OverviewTab({
  ...
  return (
    <>
      ...
      <BotCard title="Aggressive Features" botName="AGAPE_SOL_PERP" icon={<Zap className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_SOL_PERP" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-500">Instrument</span>
            <p className="text-white font-mono">{status?.instrument || 'SOL-PERP'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-white font-mono">{status?.max_contracts || 500}</p>
          </div>
          <div>
            <span className="text-gray-500">Cooldown</span>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE_SOL_PERP }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Funding Rate */}
        <BotCard title="Funding Rate" subtitle="Replaces: Gamma Regime (POSITIVE/NEGATIVE)" botName="AGAPE_SOL_PERP">
        ...
        {/* Long/Short Ratio */}
        <BotCard title="Long/Short Ratio" subtitle="Replaces: GEX Directional Bias" botName="AGAPE_SOL_PERP">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Liquidations */}
        <BotCard title="Liquidation Clusters" subtitle="Replaces: Gamma Walls / Price Magnets" botName="AGAPE_SOL_PERP">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Crypto GEX */}
        <BotCard title="Crypto GEX (Deribit)" subtitle="Direct equivalent of Net GEX" botName="AGAPE_SOL_PERP">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_SOL_PERP }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE-SOL-PERP scans will appear here as they run"
      />
    )
  }

  return (
    <BotCard title={`Scan Activity (${scans.length})`} botName="AGAPE_SOL_PERP" icon={<Activity className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">Time</th>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">SOL</th>
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_SOL_PERP }) {
  const trades = data || []
  ...
  return (
    <BotCard title={`Trade History (${trades.length})`} botName="AGAPE_SOL_PERP" icon={<History className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE_SOL_PERP }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_SOL_PERP" icon={<Settings className="w-5 h-5" />}>
      ...
      {/* GEX Mapping */}
      {mappingData && (
        <BotCard title={mappingData.title || 'GEX Mapping'} subtitle={mappingData.description} botName="AGAPE_SOL_PERP" icon={<Globe className="w-5 h-5" />}>
      ...
      {mappingData?.trade_instrument && (
        <BotCard title="Trade Instrument" botName="AGAPE_SOL_PERP" icon={<Target className="w-5 h-5" />}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
