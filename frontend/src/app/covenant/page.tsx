...
import type { BotStatus } from '@/components/Covenant3D'

// Dynamically import Covenant3D with SSR disabled - Three.js requires browser APIs
const Covenant3D = dynamic(() => import('@/components/Covenant3D'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full flex items-center justify-center bg-[#030712]">
      <div className="text-center">
        <div className="w-16 h-16 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin mx-auto mb-4" />
        <p className="text-blue-400 text-sm">Initializing COVENANT...</p>
      </div>
    </div>
  )
})
...
const TRADING_TIPS = [
  ...
  "PROPHET's ML models predict direction with 65%+ accuracy",
...
export default function CovenantPage() {
  ...
  useEffect(() => {
    const fetchBotStatus = async () => {
      try {
        ...
        setBotStatus(prev => ({
          ...prev,
          fortress: aresRes?.data?.status === 'running' ? 'trading' :
  ...
  return (
    <div className="fixed inset-0 bg-[#050810]">
      {/* Full-screen 3D COVENANT Visualization */}
      <Covenant3D
        botStatus={botStatus}
