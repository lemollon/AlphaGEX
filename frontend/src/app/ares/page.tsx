...
export default function AresPage() {
  ...
  // Filter to SPY only (ARES V2 only trades SPY)
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading ARES..." />
        </div>
      </>
    )
  }
  ...
  return (
    <>
      <Navigation />
      <main className="min-h-screen bg-black text-white p-4 md:p-6 pt-24">
                ...
                {closedPositions.length > 0 && (
                  <div className="flex justify-end mb-4">
                    <button
                      onClick={() => {
                        const today = new Date().toISOString().split('T')[0]
                        exportTradesToCSV(closedPositions, `ares-trades-${today}.csv`)
