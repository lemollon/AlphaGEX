...
import {
  ...
  useAGAPEEthPerpChartData,
  useAGAPEEthPerpBrief,
...
export default function AgapeEthPerpPage() {
  ...
  const { data: chartData, isLoading: chartLoading } = useAGAPEEthPerpChartData({ enabled: activeTab === 'charts' })
  const { data: briefData, isLoading: briefLoading } = useAGAPEEthPerpBrief({ enabled: activeTab === 'charts' })
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'history' && (
              <TradeHistoryTable bots={['eth']} showBotColumn={false} defaultRange="30d" title="ETH-PERP Trade History" />
            )}
...
function PortfolioTab({
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_ETH_PERP" marketType="crypto_perp" marginEndpoint="/api/agape-eth-perp/margin" />
