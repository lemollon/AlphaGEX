...
type AnchorTabId = typeof ANCHOR_TABS[number]['id']
...
function PositionCard({ position, isOpen }: { position: IronCondorPosition; isOpen: boolean }) {
  ...
  return (
    <div className={`bg-[#0a0a0a] rounded-lg border ${isOpen ? 'border-blue-500/30' : isPositive ? 'border-green-500/30' : 'border-red-500/30'} overflow-hidden`}>
          ...
          <div className="mt-4 bg-purple-500/10 border border-purple-500/30 rounded-lg p-3">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              ...
              <div>
                ...
                <span className={`font-bold ${(position.oracle_win_probability || 0) >= 0.55 ? 'text-green-400' : 'text-red-400'}`}>
...
export default function AnchorPage() {
  ...
  const [activeTab, setActiveTab] = useState<AnchorTabId>('portfolio')
  const { addToast } = useToast()

  // Data hooks
  const { data: statusData, error: statusError, isLoading: statusLoading, mutate: refreshStatus } = useANCHORStatus()
  const { data: positionsData, error: positionsError, isLoading: positionsLoading } = useANCHORPositions()
  // Equity curve data is now fetched by the shared EquityCurveChart component
  const { data: configData } = useAnchorConfig()
  const { data: livePnLData, isLoading: livePnLLoading, isValidating: livePnLValidating } = useANCHORLivePnL()
  const { data: scanData, isLoading: scansLoading } = useScanActivityAnchor(50)

  // UNIFIED METRICS: Single source of truth for all stats
  const { data: unifiedData, mutate: refreshUnified } = useUnifiedBotSummary('ANCHOR')
  const unifiedMetrics = unifiedData?.data

  // Extract data
  const status: ANCHORStatus | null = statusData?.data || null
  ...
  // Brand info
  const brand = BOT_BRANDS.ANCHOR

  const handleRefresh = async () => {
    await Promise.all([refreshStatus(), refreshUnified()])
    addToast({ type: 'success', title: 'Refreshed', message: 'ANCHOR data refreshed' })
  }
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading ANCHOR..." />
        </div>
      </>
    )
  }
    ...
    <>
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-300 text-sm mt-1">
                    ANCHOR is using the default starting capital ($200,000). For accurate P&L and return calculations,
                    configure your actual starting capital via the API.
                  </p>
                  <p className="text-gray-500 text-xs mt-2">
                    POST /api/metrics/ANCHOR/capital with your actual starting capital
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-900/30 border border-blue-500/50 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-300 text-sm mt-1">
                    {status.message || 'ANCHOR is paper trading with $200k simulated capital.'}
                  </p>
          ...
          {/* ANCHOR vs SAMSON Comparison Banner */}
          <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
            <div className="flex items-start gap-3">
              <Shield className="w-5 h-5 text-blue-400 mt-0.5 flex-shrink-0" />
              <div className="flex-1">
                <h3 className="text-blue-400 font-semibold">ANCHOR vs SAMSON: Conservative Parameters</h3>
                <p className="text-gray-400 text-sm mt-1">ANCHOR uses standard SPX Iron Condor settings for consistent, steady returns.</p>
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'portfolio' && (
              <>
                ...
                <DriftStatusCard botName="ANCHOR" />

                {/* Equity Curve - Using Shared Component */}
                <EquityCurveChart
                  title="ANCHOR Equity Curve"
                  botFilter="ANCHOR"
              ...
              <BotCard title="Bot Status" icon={<Server className="h-5 w-5" />}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    ...
                    <span className={`text-xl font-bold ${status?.traded_today ? 'text-green-400' : 'text-gray-400'}`}>
                ...
                {closedPositions.length > 0 && (
                  <div className="flex justify-end mb-4">
                    <button
                      onClick={() => {
                        const today = new Date().toISOString().split('T')[0]
                        exportTradesToCSV(closedPositions, `anchor-trades-${today}.csv`)
              ...
              <BotCard title="Configuration" icon={<Settings className="h-5 w-5" />}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    ...
                    <span className="text-xl font-bold text-white">${config?.spread_width || 10}</span>
                  </div>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    <span className="text-gray-500 text-sm block">Risk Per Trade</span>
                    <span className="text-xl font-bold text-white">{config?.risk_per_trade_pct || 10}%</span>
                  </div>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    <span className="text-gray-500 text-sm block">SD Multiplier</span>
                    <span className="text-xl font-bold text-white">{config?.sd_multiplier || 1.0}x</span>
                  </div>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    <span className="text-gray-500 text-sm block">Min Credit</span>
                    <span className="text-xl font-bold text-white">${config?.min_credit || 1.50}</span>
                  </div>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    <span className="text-gray-500 text-sm block">Profit Target</span>
                    <span className="text-xl font-bold text-green-400">{config?.profit_target_pct || 50}%</span>
                  </div>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    <span className="text-gray-500 text-sm block">Use Stop Loss</span>
                    <span className={`text-xl font-bold ${config?.use_stop_loss ? 'text-green-400' : 'text-gray-400'}`}>
                      {config?.use_stop_loss ? 'YES' : 'NO'}
                    </span>
                  </div>
