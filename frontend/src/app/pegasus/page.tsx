...
type PegasusTabId = typeof PEGASUS_TABS[number]['id']
...
export default function PegasusPage() {
  ...
  const [activeTab, setActiveTab] = useState<PegasusTabId>('portfolio')
  ...
  const handleReset = async () => {
    const response = await apiClient.resetPEGASUSData(true)
    if (response.data?.success) {
      addToast({ type: 'success', title: 'Reset Complete', message: 'PEGASUS data has been reset successfully' })
      refreshStatus()
    } else {
      addToast({ type: 'error', title: 'Reset Failed', message: response.data?.message || 'Failed to reset PEGASUS data' })
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading PEGASUS..." />
        </div>
      </>
    )
  }
    ...
    <>
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {/* PEGASUS vs SAMSON Comparison Banner */}
          <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
            <div className="flex items-start gap-3">
              <Shield className="w-5 h-5 text-blue-400 mt-0.5 flex-shrink-0" />
              <div className="flex-1">
                <h3 className="text-blue-400 font-semibold">PEGASUS vs SAMSON: Conservative Parameters</h3>
