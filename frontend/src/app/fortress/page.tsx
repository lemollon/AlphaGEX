...
function PositionCard({ position, isOpen }: { position: IronCondorPosition; isOpen: boolean }) {
  ...
  return (
    <div className={`bg-[#0a0a0a] rounded-lg border ${isOpen ? 'border-red-500/30' : isPositive ? 'border-green-500/30' : 'border-red-500/30'} overflow-hidden`}>
      {/* Header - Always visible */}
      <div className="p-4 cursor-pointer hover:bg-gray-800/30 transition-colors" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-white font-bold">SPY Iron Condor</span>
              {isOpen && (
        ...
        <div className="px-4 pb-4 border-t border-gray-800 space-y-4">
          ...
          <div className="bg-gray-800/50 rounded-lg p-3">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              ...
              <div>
                <span className="text-gray-500 block">SPY at Entry</span>
                <span className="text-white font-bold">${position.underlying_at_entry?.toFixed(2)}</span>
              </div>
              <div>
...
export default function FortressPage() {
  ...
  const config = configData?.data || configData || null

  // Filter to SPY only (FORTRESS V2 only trades SPY)
  ...
  // These come from database aggregates via /api/metrics/fortress/summary
  ...
  const handleRefresh = async () => {
    ...
    addToast({ type: 'success', title: 'Refreshed', message: 'FORTRESS data refreshed' })
  }
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading FORTRESS..." />
        </div>
      </>
    )
  }
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-300 text-sm mt-1">
                    FORTRESS is using default starting capital ({formatCurrency(currentEquity - totalPnL)}).
                    Configure your actual starting capital for accurate returns.
                  </p>
                  <p className="text-gray-400 text-xs mt-2">
                    Set via: POST /api/metrics/fortress/capital or connect Tradier to auto-detect.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-red-900/30 border border-red-500/50 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-300 text-sm mt-1">
                    FORTRESS requires a Tradier connection for live trading. Capital source: {capitalSource}.
                  </p>
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'portfolio' && (
              <>
                ...
                <DriftStatusCard botName="FORTRESS" />

                {/* Equity Curve - Using Shared Component */}
                <EquityCurveChart
                  title="FORTRESS Equity Curve"
                  botFilter="FORTRESS"
              ...
              <BotCard title="Bot Status" icon={<Server className="h-5 w-5" />}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    ...
                    <span className="text-xl font-bold text-red-400">SPY</span>
                  </div>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                ...
                {closedPositions.length > 0 && (
                  <div className="flex justify-end mb-4">
                    <button
                      onClick={() => {
                        const today = new Date().toISOString().split('T')[0]
                        exportTradesToCSV(closedPositions, `fortress-trades-${today}.csv`)
            ...
            {activeTab === 'config' && (
              ...
              <BotCard title="Configuration" icon={<Settings className="h-5 w-5" />}>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-gray-800/50 rounded-lg p-4">
                    <span className="text-gray-500 text-sm block">VIX Skip</span>
                    <span className="text-xl font-bold text-white">{config?.vix_skip || 30}</span>
                  </div>
                  <div className="bg-gray-800/50 rounded-lg p-4">
                ...
                {config?.preset && (
                  <div className="mt-4 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
                    <p className="text-gray-300 text-sm">
                      <span className="text-red-400 font-bold">Strategy Preset:</span> {config.preset}
                    </p>
                  </div>
                )}
              </BotCard>
