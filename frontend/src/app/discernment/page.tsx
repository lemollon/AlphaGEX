...
interface DiscernmentPrediction {
  direction: string
...
interface DiscernmentScanResult {
  symbol: string
  timestamp: string
  scan_id: string
  features: DiscernmentFeatures | null
  prediction: DiscernmentPrediction | null
  strategies: DiscernmentStrategy[]
  market_regime: string
  gex_regime: string
...
export default function DiscernmentPage() {
  ...
  const [scanResults, setScanResults] = useState<DiscernmentScanResult[]>([])
  ...
  const [selectedResult, setSelectedResult] = useState<DiscernmentScanResult | null>(null)
  ...
  const fetchPerformance = async () => {
    try {
      const response = await apiClient.getDiscernmentPerformance()
  ...
  const fetchTrackingStatus = async () => {
    try {
      const response = await apiClient.getDiscernmentTrackingStatus()
      if (response.data?.success) {
  ...
  const checkPinRisk = async (symbol?: string) => {
    ...
    try {
      const response = await apiClient.getDiscernmentPinRisk(sym)
      if (response.data?.success) {
  ...
  const performScan = async () => {
    ...
    try {
      ...
      const response = await apiClient.discernmentScan(symbols, true)
          ...
          {!scanning && scanResults.length === 0 && !error && (
            <div className="bg-gradient-to-br from-primary/10 via-background-card to-yellow-500/5 rounded-xl border border-primary/30 p-8">
              <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-bold mb-2">Welcome to DISCERNMENT</h2>
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400">
                  Add symbols above (like SPY, QQQ, TSLA) and click "Scan with DISCERNMENT" to analyze
                </p>
              </div>
            </div>
          )}
          ...
          {!scanning && scanResults.length === 0 && (
            <div className="bg-background-card rounded-xl p-12 text-center border border-gray-800">
              ...
              <p className="text-gray-400 max-w-md mx-auto">
                Add symbols above and click "Scan with DISCERNMENT" to get ML-powered predictions
