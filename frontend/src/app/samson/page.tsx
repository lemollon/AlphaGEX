...
export default function TitanPage() {
  ...
  // Brand info
  const brand = BOT_BRANDS.SAMSON

  const handleRefresh = async () => {
    await refreshStatus()
    addToast({ type: 'success', title: 'Refreshed', message: 'SAMSON data refreshed' })
  }
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading SAMSON..." />
        </div>
      </>
    )
  }
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-300 text-sm mt-1">
                    {status.message || 'SAMSON is paper trading with $200k simulated capital. Multiple trades per day with 30-min cooldown.'}
                  </p>
          ...
          <div className="space-y-6">
            ...
            {activeTab === 'portfolio' && (
              <>
                ...
                <DriftStatusCard botName="SAMSON" />

                {/* Equity Curve - Using Shared Component */}
                <EquityCurveChart
                  title="SAMSON Equity Curve"
                  botFilter="SAMSON"
                ...
                {closedPositions.length > 0 && (
                  <div className="flex justify-end mb-4">
                    <button
                      onClick={() => {
                        const today = new Date().toISOString().split('T')[0]
                        exportTradesToCSV(closedPositions, `samson-trades-${today}.csv`)
