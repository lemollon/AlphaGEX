...
import {
  useAGAPELinkFuturesStatus,
  useAGAPELinkFuturesPerformance,
  useAGAPELinkFuturesPositions,
  useAGAPELinkFuturesScanActivity,
  useAGAPELinkFuturesClosedTrades,
  useAGAPELinkFuturesSnapshot,
  useAGAPELinkFuturesGexMapping,
  useAGAPELinkFuturesChartData,
  useAGAPELinkFuturesBrief,
...
type AgapeLinkFuturesTabId = typeof AGAPE_LINK_FUTURES_TABS[number]['id']

// ==============================================================================
// MAIN COMPONENT
// ==============================================================================

export default function AgapeLinkFuturesPage() {
  const [activeTab, setActiveTab] = useState<AgapeLinkFuturesTabId>('portfolio')
  const [equityTimeframe, setEquityTimeframe] = useState('intraday')
  const sidebarPadding = useSidebarPadding()

  // Brand from centralized system
  const brand = BOT_BRANDS.AGAPE_LINK_FUTURES

  // Data hooks (matching FORTRESS/SAMSON pattern)
  const { data: statusData, isLoading: statusLoading, mutate: refreshStatus } = useAGAPELinkFuturesStatus()
  const { data: perfData } = useAGAPELinkFuturesPerformance()
  const { data: positionsData, isLoading: posLoading } = useAGAPELinkFuturesPositions()
  const { data: snapshotData, isLoading: snapLoading } = useAGAPELinkFuturesSnapshot({ enabled: activeTab === 'snapshot' })
  const { data: scansData } = useAGAPELinkFuturesScanActivity(30, { enabled: activeTab === 'activity' })
  const { data: closedData } = useAGAPELinkFuturesClosedTrades(50, { enabled: activeTab === 'history' })
  const { data: mappingData } = useAGAPELinkFuturesGexMapping({ enabled: activeTab === 'config' })
  const { data: chartData, isLoading: chartLoading } = useAGAPELinkFuturesChartData({ enabled: activeTab === 'charts' })
  const { data: briefData, isLoading: briefLoading } = useAGAPELinkFuturesBrief({ enabled: activeTab === 'charts' })
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE-LINK-FUTURES..." />
        </div>
      </>
    )
  }
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="rounded-lg p-3 border bg-amber-900/20 border-amber-500/30">
            <div className="flex items-center gap-3 text-sm">
              ...
              <span className="text-amber-300">
                LINK Futures (LNK-29MAY26-CDE) - Monthly Coinbase Derivatives via FCM
              </span>
              <span className="text-gray-500 ml-auto text-xs">1 contract = 100 LINK</span>
            </div>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    AGAPE-LINK-FUTURES paused after {status.aggressive_features.consecutive_losses} consecutive losses. Will resume automatically.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <StatCard
              label="LINK Price"
              value={status?.current_link_price ? `$${status.current_link_price.toFixed(2)}` : '---'}
          ...
          <div className="flex gap-2 border-b border-gray-800 pb-2">
            {AGAPE_LINK_FUTURES_TABS.map((tab) => (
              <button
                key={tab.id}
...
function PortfolioTab({
  ...
  brand: typeof BOT_BRANDS.AGAPE_LINK_FUTURES
  paperAccount: any
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_LINK_FUTURES" marketType="crypto_perp" marginEndpoint="/api/agape-link-futures/margin" />

      {/* Equity Curve */}
      <BotCard title="Equity Curve" botName="AGAPE_LINK_FUTURES" icon={<TrendingUp className="w-5 h-5" />}
        headerRight={
      ...
      >
        <EquityCurveChart
          title=""
          botFilter="AGAPE_LINK_FUTURES"
          showIntradayOption={equityTimeframe === 'intraday'}
        />
      </BotCard>

      {/* Open Positions */}
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE_LINK_FUTURES" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          <LoadingState />
          ...
          <EmptyState
            ...
            description="AGAPE-LINK-FUTURES is scanning for opportunities"
          />
        ) : (
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-mono font-semibold">
                      {pos.quantity?.toLocaleString()}x LINK-FUT @ ${pos.entry_price?.toFixed(2)}
                    </span>
                    {pos.trailing_active && (
      ...
      {(perf?.total_trades > 0 || (paperAccount?.total_trades ?? 0) > 0) && (
        <BotCard title="Performance" botName="AGAPE_LINK_FUTURES" icon={<BarChart3 className="w-5 h-5" />}>
...
function OverviewTab({
  ...
  return (
    <>
      ...
      <BotCard title="Aggressive Features" botName="AGAPE_LINK_FUTURES" icon={<Zap className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_LINK_FUTURES" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-500">Instrument</span>
            <p className="text-white font-mono">{status?.instrument || 'LINK-FUT'}</p>
          </div>
          <div>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE_LINK_FUTURES }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Funding Rate */}
        <BotCard title="Funding Rate" subtitle="Replaces: Gamma Regime (POSITIVE/NEGATIVE)" botName="AGAPE_LINK_FUTURES">
        ...
        {/* Long/Short Ratio */}
        <BotCard title="Long/Short Ratio" subtitle="Replaces: GEX Directional Bias" botName="AGAPE_LINK_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Liquidations */}
        <BotCard title="Liquidation Clusters" subtitle="Replaces: Gamma Walls / Price Magnets" botName="AGAPE_LINK_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Crypto GEX */}
        <BotCard title="Crypto GEX (Deribit)" subtitle="Direct equivalent of Net GEX" botName="AGAPE_LINK_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_LINK_FUTURES }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE-LINK-FUTURES scans will appear here as they run"
      />
    )
  }

  return (
    <BotCard title={`Scan Activity (${scans.length})`} botName="AGAPE_LINK_FUTURES" icon={<Activity className="w-5 h-5" />}>
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_LINK_FUTURES }) {
  const trades = data || []
  ...
  return (
    <BotCard title={`Trade History (${trades.length})`} botName="AGAPE_LINK_FUTURES" icon={<History className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE_LINK_FUTURES }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_LINK_FUTURES" icon={<Settings className="w-5 h-5" />}>
      ...
      {/* GEX Mapping */}
      {mappingData && (
        <BotCard title={mappingData.title || 'GEX Mapping'} subtitle={mappingData.description} botName="AGAPE_LINK_FUTURES" icon={<Globe className="w-5 h-5" />}>
      ...
      {mappingData?.trade_instrument && (
        <BotCard title="Trade Instrument" botName="AGAPE_LINK_FUTURES" icon={<Target className="w-5 h-5" />}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
