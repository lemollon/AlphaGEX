...
import {
  useAGAPEShibFuturesStatus,
  useAGAPEShibFuturesPerformance,
  useAGAPEShibFuturesPositions,
  useAGAPEShibFuturesScanActivity,
  useAGAPEShibFuturesClosedTrades,
  useAGAPEShibFuturesSnapshot,
  useAGAPEShibFuturesGexMapping,
  useAGAPEShibFuturesChartData,
  useAGAPEShibFuturesBrief,
...
type AgapeShibFuturesTabId = typeof AGAPE_SHIB_FUTURES_TABS[number]['id']

// ==============================================================================
// MAIN COMPONENT
// ==============================================================================

export default function AgapeShibFuturesPage() {
  const [activeTab, setActiveTab] = useState<AgapeShibFuturesTabId>('portfolio')
  const [equityTimeframe, setEquityTimeframe] = useState('intraday')
  const sidebarPadding = useSidebarPadding()

  // Brand from centralized system
  const brand = BOT_BRANDS.AGAPE_SHIB_FUTURES

  // Data hooks (matching FORTRESS/SAMSON pattern)
  const { data: statusData, isLoading: statusLoading, mutate: refreshStatus } = useAGAPEShibFuturesStatus()
  const { data: perfData } = useAGAPEShibFuturesPerformance()
  const { data: positionsData, isLoading: posLoading } = useAGAPEShibFuturesPositions()
  const { data: snapshotData, isLoading: snapLoading } = useAGAPEShibFuturesSnapshot({ enabled: activeTab === 'snapshot' })
  const { data: scansData } = useAGAPEShibFuturesScanActivity(30, { enabled: activeTab === 'activity' })
  const { data: closedData } = useAGAPEShibFuturesClosedTrades(50, { enabled: activeTab === 'history' })
  const { data: mappingData } = useAGAPEShibFuturesGexMapping({ enabled: activeTab === 'config' })
  const { data: chartData, isLoading: chartLoading } = useAGAPEShibFuturesChartData({ enabled: activeTab === 'charts' })
  const { data: briefData, isLoading: briefLoading } = useAGAPEShibFuturesBrief({ enabled: activeTab === 'charts' })

  const status = statusData?.data
  const perf = perfData?.data
  const paperAccount = status?.paper_account || null
  const startingCapital = paperAccount?.starting_capital ?? status?.starting_capital ?? 1000
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE-SHIB-FUTURES..." />
        </div>
      </>
    )
  }
    ...
    <>
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          {/* 1k SHIB Futures Banner */}
          <div className="rounded-lg p-3 border bg-amber-900/20 border-amber-500/30">
            <div className="flex items-center gap-3 text-sm">
              <div className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-pulse" />
              <span className="text-amber-300">
                1k SHIB Futures (SHB-29MAY26-CDE) - Monthly Coinbase Derivatives via FCM
              </span>
              <span className="text-gray-500 ml-auto text-xs">1 contract = 10K units of 1000SHIB index = 10M SHIB</span>
            </div>
          </div>
          ...
          <div className={`${brand.lightBg} border ${brand.lightBorder} rounded-lg p-4`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className={`${brand.primaryText} font-semibold`}>Aggressive Mode - Futures Contract Trading</h3>
              ...
              <span className="px-2 py-0.5 text-xs rounded font-mono bg-amber-900/50 text-amber-300">
                FUTURE
              </span>
              {status?.mode && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    AGAPE-SHIB-FUTURES paused after {status.aggressive_features.consecutive_losses} consecutive losses. Will resume automatically.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <StatCard
              label="SHIB Price"
              value={status?.current_shib_price ? `$${status.current_shib_price.toFixed(8)}` : '---'}
          ...
          <div className="flex gap-2 border-b border-gray-800 pb-2">
            {AGAPE_SHIB_FUTURES_TABS.map((tab) => (
              <button
                key={tab.id}
...
function PortfolioTab({
  ...
  brand: typeof BOT_BRANDS.AGAPE_SHIB_FUTURES
  paperAccount: any
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_SHIB_FUTURES" marketType="crypto_perp" marginEndpoint="/api/agape-shib-futures/margin" />

      {/* Equity Curve */}
      <BotCard title="Equity Curve" botName="AGAPE_SHIB_FUTURES" icon={<TrendingUp className="w-5 h-5" />}
        headerRight={
      ...
      >
        <EquityCurveChart
          title=""
          botFilter="AGAPE_SHIB_FUTURES"
          showIntradayOption={equityTimeframe === 'intraday'}
        />
      </BotCard>

      {/* Open Positions */}
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE_SHIB_FUTURES" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          <LoadingState />
          ...
          <EmptyState
            ...
            description="AGAPE-SHIB-FUTURES is scanning for opportunities"
          />
        ) : (
              ...
              <div key={pos.position_id} className={`rounded-lg p-4 border ${brand.positionBorder} ${brand.positionBg}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-mono font-semibold">
                      {pos.quantity?.toLocaleString()}x 1000SHIB-FUT @ ${pos.entry_price?.toFixed(8)}
                    </span>
                    {pos.trailing_active && (
                      <span className={`px-2 py-0.5 ${brand.badgeBg} ${brand.badgeText} text-xs rounded font-mono`}>
                        TRAILING @ ${pos.current_stop?.toFixed(8)}
                      </span>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
                  <div>
                    ...
                    <p className="text-red-400 font-mono">{pos.stop_loss ? `$${pos.stop_loss.toFixed(8)}` : 'Trailing'}</p>
                  </div>
                  <div>
                    <span className="text-gray-500">Take Profit</span>
                    <p className="text-green-400 font-mono">{pos.take_profit ? `$${pos.take_profit.toFixed(8)}` : 'No-Loss Trail'}</p>
                  </div>
                  <div>
      ...
      {(perf?.total_trades > 0 || (paperAccount?.total_trades ?? 0) > 0) && (
        <BotCard title="Performance" botName="AGAPE_SHIB_FUTURES" icon={<BarChart3 className="w-5 h-5" />}>
...
function OverviewTab({
  ...
  return (
    <>
      ...
      <BotCard title="Aggressive Features" botName="AGAPE_SHIB_FUTURES" icon={<Zap className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_SHIB_FUTURES" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-500">Instrument</span>
            <p className="text-white font-mono">{status?.instrument || '1000SHIB-FUT'}</p>
          </div>
          <div>
            <span className="text-gray-500">Starting Capital</span>
            <p className="text-white font-mono">${status?.starting_capital?.toLocaleString() || '1,000'}</p>
          </div>
          <div>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE_SHIB_FUTURES }) {
  ...
  return (
    <>
      ...
      <BotCard
        title={`${data.symbol} $${data.spot_price?.toFixed(8) || '---'}`}
        botName="AGAPE_SHIB_FUTURES"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Funding Rate */}
        <BotCard title="Funding Rate" subtitle="Replaces: Gamma Regime (POSITIVE/NEGATIVE)" botName="AGAPE_SHIB_FUTURES">
        ...
        {/* Long/Short Ratio */}
        <BotCard title="Long/Short Ratio" subtitle="Replaces: GEX Directional Bias" botName="AGAPE_SHIB_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Liquidations */}
        <BotCard title="Liquidation Clusters" subtitle="Replaces: Gamma Walls / Price Magnets" botName="AGAPE_SHIB_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500">Nearest Long Liq</span>
              <span className="text-red-400 font-mono">{data.liquidations?.nearest_long_liq ? `$${data.liquidations.nearest_long_liq.toFixed(8)}` : '---'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Nearest Short Liq</span>
              <span className="text-green-400 font-mono">{data.liquidations?.nearest_short_liq ? `$${data.liquidations.nearest_short_liq.toFixed(8)}` : '---'}</span>
            </div>
            <div className="flex justify-between">
              ...
              <div className="space-y-1">
                {data.liquidations.top_clusters.slice(0, 5).map((c: any, i: number) => (
                  <div key={i} className="flex justify-between text-xs">
                    <span className="text-gray-400 font-mono">${c.price?.toFixed(8)}</span>
        ...
        {/* Crypto GEX */}
        <BotCard title="Crypto GEX (Deribit)" subtitle="Direct equivalent of Net GEX" botName="AGAPE_SHIB_FUTURES">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white font-mono">{data.crypto_gex?.flip_point ? `$${data.crypto_gex.flip_point.toFixed(8)}` : '---'}</span>
            </div>
          </div>
        </BotCard>
      </div>
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_SHIB_FUTURES }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE-SHIB-FUTURES scans will appear here as they run"
      />
    )
  }

  return (
    <BotCard title={`Scan Activity (${scans.length})`} botName="AGAPE_SHIB_FUTURES" icon={<Activity className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">Time</th>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">SHIB</th>
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_SHIB_FUTURES }) {
  const trades = data || []
  ...
  return (
    <BotCard title={`Trade History (${trades.length})`} botName="AGAPE_SHIB_FUTURES" icon={<History className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
          ...
          <tbody className="divide-y divide-gray-800">
            {trades.map((trade: any, i: number) => (
              <tr key={i} className="hover:bg-gray-800/30">
                ...
                <td className="px-4 py-2 text-white font-mono">${trade.entry_price?.toFixed(8)}</td>
                <td className="px-4 py-2 text-white font-mono">${trade.close_price?.toFixed(8) || '---'}</td>
                <td className="px-4 py-2">
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE_SHIB_FUTURES }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_SHIB_FUTURES" icon={<Settings className="w-5 h-5" />}>
      ...
      {/* GEX Mapping */}
      {mappingData && (
        <BotCard title={mappingData.title || 'GEX Mapping'} subtitle={mappingData.description} botName="AGAPE_SHIB_FUTURES" icon={<Globe className="w-5 h-5" />}>
      ...
      {mappingData?.trade_instrument && (
        <BotCard title="Trade Instrument" botName="AGAPE_SHIB_FUTURES" icon={<Target className="w-5 h-5" />}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
