...
import {
  useAGAPEBTCStatus,
  useAGAPEBTCPerformance,
  useAGAPEBTCPositions,
  useAGAPEBTCScanActivity,
  useAGAPEBTCClosedTrades,
  useAGAPEBTCSnapshot,
  useAGAPEBTCGexMapping,
...
type AgapeBtcTabId = typeof AGAPE_BTC_TABS[number]['id']

// ==============================================================================
// MAIN COMPONENT
// ==============================================================================

export default function AgapeBtcPage() {
  const [activeTab, setActiveTab] = useState<AgapeBtcTabId>('portfolio')
  const [equityTimeframe, setEquityTimeframe] = useState('intraday')
  const sidebarPadding = useSidebarPadding()

  // Brand from centralized system
  const brand = BOT_BRANDS.AGAPE_BTC
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE-BTC..." />
        </div>
      </>
    )
  }
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className={`rounded-lg p-3 border ${
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <span className={status.market.cme_market_open ? 'text-green-300' : 'text-yellow-300'}>
                  CME /MBT: {status.market.cme_market_open ? 'Market Open' : status.market.reason}
                </span>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    AGAPE-BTC paused after {status.aggressive_features.consecutive_losses} consecutive losses. Will resume automatically.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <StatCard
              label="BTC Price"
              value={status?.current_btc_price ? `$${status.current_btc_price.toFixed(2)}` : '---'}
...
function PortfolioTab({
  ...
  brand: typeof BOT_BRANDS.AGAPE_BTC
  paperAccount: any
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_BTC" marketType="crypto_futures" marginEndpoint="/api/agape-btc/margin" />

      {/* Equity Curve */}
      <BotCard title="Equity Curve" botName="AGAPE_BTC" icon={<TrendingUp className="w-5 h-5" />}
        headerRight={
      ...
      {/* Open Positions */}
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE_BTC" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          <LoadingState />
          ...
          <EmptyState
            ...
            description="AGAPE-BTC is scanning for opportunities"
          />
        ) : (
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-mono font-semibold">
                      {pos.contracts}x /MBT @ ${pos.entry_price?.toFixed(2)}
                    </span>
                    {pos.trailing_active && (
      ...
      {(perf?.total_trades > 0 || (paperAccount?.total_trades ?? 0) > 0) && (
        <BotCard title="Performance" botName="AGAPE_BTC" icon={<BarChart3 className="w-5 h-5" />}>
...
function OverviewTab({
  ...
  return (
    <>
      ...
      <BotCard title="Aggressive Features" botName="AGAPE_BTC" icon={<Zap className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_BTC" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-500">Instrument</span>
            <p className="text-white font-mono">{status?.instrument || '/MBT'}</p>
          </div>
          <div>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE_BTC }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Funding Rate */}
        <BotCard title="Funding Rate" subtitle="Replaces: Gamma Regime (POSITIVE/NEGATIVE)" botName="AGAPE_BTC">
        ...
        {/* Long/Short Ratio */}
        <BotCard title="Long/Short Ratio" subtitle="Replaces: GEX Directional Bias" botName="AGAPE_BTC">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Liquidations */}
        <BotCard title="Liquidation Clusters" subtitle="Replaces: Gamma Walls / Price Magnets" botName="AGAPE_BTC">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Crypto GEX */}
        <BotCard title="Crypto GEX (Deribit)" subtitle="Direct equivalent of Net GEX" botName="AGAPE_BTC">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_BTC }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE-BTC scans will appear here as they run"
      />
    )
  }

  return (
    <BotCard title={`Scan Activity (${scans.length})`} botName="AGAPE_BTC" icon={<Activity className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">Time</th>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">BTC</th>
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_BTC }) {
  const trades = data || []
  ...
  return (
    <BotCard title={`Trade History (${trades.length})`} botName="AGAPE_BTC" icon={<History className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE_BTC }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_BTC" icon={<Settings className="w-5 h-5" />}>
      ...
      {/* GEX Mapping */}
      {mappingData && (
        <BotCard title={mappingData.title || 'GEX Mapping'} subtitle={mappingData.description} botName="AGAPE_BTC" icon={<Globe className="w-5 h-5" />}>
      ...
      {mappingData?.trade_instrument && (
        <BotCard title="Trade Instrument" botName="AGAPE_BTC" icon={<Target className="w-5 h-5" />}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
