...
import {
  useAGAPEXrpPerpStatus,
  useAGAPEXrpPerpPerformance,
  useAGAPEXrpPerpPositions,
  useAGAPEXrpPerpScanActivity,
  useAGAPEXrpPerpClosedTrades,
  useAGAPEXrpPerpSnapshot,
  useAGAPEXrpPerpGexMapping,
  useAGAPEXrpPerpChartData,
  useAGAPEXrpPerpBrief,
...
type AgapeXrpPerpTabId = typeof AGAPE_XRP_PERP_TABS[number]['id']

// ==============================================================================
// MAIN COMPONENT
// ==============================================================================

export default function AgapeXrpPerpPage() {
  const [activeTab, setActiveTab] = useState<AgapeXrpPerpTabId>('portfolio')
  const [equityTimeframe, setEquityTimeframe] = useState('intraday')
  const sidebarPadding = useSidebarPadding()

  // Brand from centralized system
  const brand = BOT_BRANDS.AGAPE_XRP_PERP
  ...
  const { data: chartData, isLoading: chartLoading } = useAGAPEXrpPerpChartData({ enabled: activeTab === 'charts' })
  const { data: briefData, isLoading: briefLoading } = useAGAPEXrpPerpBrief({ enabled: activeTab === 'charts' })

  const status = statusData?.data
  const perf = perfData?.data
  const paperAccount = status?.paper_account || null
  const startingCapital = paperAccount?.starting_capital ?? status?.starting_capital ?? 9000
    ...
    return (
      <>
        ...
        <div className="flex items-center justify-center h-screen">
          <LoadingState message="Loading AGAPE-XRP-PERP..." />
        </div>
      </>
    )
  }
      ...
      <main className={`min-h-screen bg-black text-white px-4 pb-4 md:px-6 md:pb-6 pt-24 transition-all duration-300 ${sidebarPadding}`}>
        <div className="max-w-7xl mx-auto space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    AGAPE-XRP-PERP paused after {status.aggressive_features.consecutive_losses} consecutive losses. Will resume automatically.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <StatCard
              label="XRP Price"
              value={status?.current_xrp_price ? `$${status.current_xrp_price.toFixed(4)}` : '---'}
...
function PortfolioTab({
  ...
  brand: typeof BOT_BRANDS.AGAPE_XRP_PERP
  paperAccount: any
  ...
  return (
    <>
      ...
      {/* Margin Analysis */}
      <MarginAnalysis botName="AGAPE_XRP_PERP" marketType="crypto_perp" marginEndpoint="/api/agape-xrp-perp/margin" />

      {/* Equity Curve */}
      <BotCard title="Equity Curve" botName="AGAPE_XRP_PERP" icon={<TrendingUp className="w-5 h-5" />}
        headerRight={
      ...
      >
        <EquityCurveChart
          title=""
          botFilter="AGAPE_XRP_PERP"
          showIntradayOption={equityTimeframe === 'intraday'}
        />
      </BotCard>

      {/* Open Positions */}
      <BotCard title={`Open Positions (${openPositions.length})`} botName="AGAPE_XRP_PERP" icon={<Wallet className="w-5 h-5" />}>
        {posLoading ? (
          <LoadingState />
          ...
          <EmptyState
            ...
            description="AGAPE-XRP-PERP is scanning for opportunities"
          />
        ) : (
              ...
              <div key={pos.position_id} className={`rounded-lg p-4 border ${brand.positionBorder} ${brand.positionBg}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white font-mono font-semibold">
                      {pos.quantity}x XRP-PERP @ ${pos.entry_price?.toFixed(4)}
                    </span>
                    {pos.trailing_active && (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
                  <div>
                    ...
                    <p className="text-red-400 font-mono">{pos.stop_loss ? `$${pos.stop_loss.toFixed(4)}` : 'Trailing'}</p>
                  </div>
                  <div>
                    <span className="text-gray-500">Take Profit</span>
                    <p className="text-green-400 font-mono">{pos.take_profit ? `$${pos.take_profit.toFixed(4)}` : 'No-Loss Trail'}</p>
                  </div>
                  <div>
      ...
      {(perf?.total_trades > 0 || (paperAccount?.total_trades ?? 0) > 0) && (
        <BotCard title="Performance" botName="AGAPE_XRP_PERP" icon={<BarChart3 className="w-5 h-5" />}>
...
function OverviewTab({
  ...
  return (
    <>
      ...
      <BotCard title="Aggressive Features" botName="AGAPE_XRP_PERP" icon={<Zap className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_XRP_PERP" icon={<Settings className="w-5 h-5" />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-500">Instrument</span>
            <p className="text-white font-mono">{status?.instrument || 'XRP-PERP'}</p>
          </div>
          <div>
            <span className="text-gray-500">Starting Capital</span>
            <p className="text-white font-mono">${status?.starting_capital?.toLocaleString() || '9,000'}</p>
          </div>
          <div>
...
function SnapshotTab({ data, loading, brand }: { data: any; loading: boolean; brand: typeof BOT_BRANDS.AGAPE_XRP_PERP }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Funding Rate */}
        <BotCard title="Funding Rate" subtitle="Replaces: Gamma Regime (POSITIVE/NEGATIVE)" botName="AGAPE_XRP_PERP">
        ...
        {/* Long/Short Ratio */}
        <BotCard title="Long/Short Ratio" subtitle="Replaces: GEX Directional Bias" botName="AGAPE_XRP_PERP">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
        ...
        {/* Liquidations */}
        <BotCard title="Liquidation Clusters" subtitle="Replaces: Gamma Walls / Price Magnets" botName="AGAPE_XRP_PERP">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500">Nearest Long Liq</span>
              <span className="text-red-400 font-mono">{data.liquidations?.nearest_long_liq ? `$${data.liquidations.nearest_long_liq.toFixed(4)}` : '---'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Nearest Short Liq</span>
              <span className="text-green-400 font-mono">{data.liquidations?.nearest_short_liq ? `$${data.liquidations.nearest_short_liq.toFixed(4)}` : '---'}</span>
            </div>
            <div className="flex justify-between">
              ...
              <div className="space-y-1">
                {data.liquidations.top_clusters.slice(0, 5).map((c: any, i: number) => (
                  <div key={i} className="flex justify-between text-xs">
                    <span className="text-gray-400 font-mono">${c.price?.toFixed(4)}</span>
        ...
        {/* Crypto GEX */}
        <BotCard title="Crypto GEX (Deribit)" subtitle="Direct equivalent of Net GEX" botName="AGAPE_XRP_PERP">
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-white font-mono">{data.crypto_gex?.flip_point ? `$${data.crypto_gex.flip_point.toFixed(4)}` : '---'}</span>
            </div>
          </div>
        </BotCard>
      </div>
...
function ActivityTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_XRP_PERP }) {
  const scans = data || []
  ...
  if (scans.length === 0) {
    return (
      <EmptyState
        ...
        description="AGAPE-XRP-PERP scans will appear here as they run"
      />
    )
  }

  return (
    <BotCard title={`Scan Activity (${scans.length})`} botName="AGAPE_XRP_PERP" icon={<Activity className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">Time</th>
              <th className="text-left px-4 py-3 text-gray-500 font-medium">XRP</th>
...
function HistoryTab({ data, brand }: { data: any[]; brand: typeof BOT_BRANDS.AGAPE_XRP_PERP }) {
  const trades = data || []
  ...
  return (
    <BotCard title={`Trade History (${trades.length})`} botName="AGAPE_XRP_PERP" icon={<History className="w-5 h-5" />}>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-800/50">
            <tr>
          ...
          <tbody className="divide-y divide-gray-800">
            {trades.map((trade: any, i: number) => (
              <tr key={i} className="hover:bg-gray-800/30">
                ...
                <td className="px-4 py-2 text-white font-mono">${trade.entry_price?.toFixed(4)}</td>
                <td className="px-4 py-2 text-white font-mono">${trade.close_price?.toFixed(4) || '---'}</td>
                <td className="px-4 py-2">
...
function ConfigTab({ status, mappingData, brand }: { status: any; mappingData: any; brand: typeof BOT_BRANDS.AGAPE_XRP_PERP }) {
  return (
    <>
      {/* Configuration */}
      <BotCard title="Bot Configuration" botName="AGAPE_XRP_PERP" icon={<Settings className="w-5 h-5" />}>
      ...
      {/* GEX Mapping */}
      {mappingData && (
        <BotCard title={mappingData.title || 'GEX Mapping'} subtitle={mappingData.description} botName="AGAPE_XRP_PERP" icon={<Globe className="w-5 h-5" />}>
      ...
      {mappingData?.trade_instrument && (
        <BotCard title="Trade Instrument" botName="AGAPE_XRP_PERP" icon={<Target className="w-5 h-5" />}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
