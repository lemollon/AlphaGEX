...
const COINS: CoinId[] = ['ALL', 'ETH', 'BTC', 'XRP']
...
const COIN_META: Record<CoinId, {
  ...
  apiPrefix: string; priceKey: string; priceDecimals: number;
}> = {
  'ALL': {
    symbol: 'ALL', label: 'All Futures', instrument: '', hexColor: '#06B6D4',
    ...
    apiPrefix: '', priceKey: '', priceDecimals: 2,
  },
  'ETH': {
    symbol: 'ETH', label: 'Ethereum', instrument: '/MET (0.1 ETH)', hexColor: '#06B6D4',
    bgActive: 'bg-cyan-600', borderActive: 'border-cyan-500', textActive: 'text-cyan-400',
    bgCard: 'bg-cyan-950/30', borderCard: 'border-cyan-700/40',
    apiPrefix: '/api/agape', priceKey: 'current_eth_price', priceDecimals: 2,
  },
  'BTC': {
    symbol: 'BTC', label: 'Bitcoin', instrument: '/MBT (0.1 BTC)', hexColor: '#F59E0B',
    bgActive: 'bg-amber-600', borderActive: 'border-amber-500', textActive: 'text-amber-400',
    bgCard: 'bg-amber-950/30', borderCard: 'border-amber-700/40',
    apiPrefix: '/api/agape-btc', priceKey: 'current_btc_price', priceDecimals: 2,
  },
  'XRP': {
    symbol: 'XRP', label: 'Ripple', instrument: '/XRP (2,500 XRP)', hexColor: '#3B82F6',
    ...
    apiPrefix: '/api/agape-xrp', priceKey: 'current_xrp_price', priceDecimals: 4,
  },
}
...
const TOTAL_CAPITAL = 15000 // $5000 x 3 bots
...
function useFuturesStatus(coin: CoinId) {
...
export default function FuturesCryptoPage() {
  const [selectedCoin, setSelectedCoin] = useState<CoinId>('ALL')
  const [activeTab, setActiveTab] = useState<SectionTabId>('overview')
  const sidebarPadding = useSidebarPadding()

  // Fetch all 3 bots status for the combined view and price ticker
  const { data: ethStatusData, isLoading: ethLoading, mutate: refreshEth } = useSWR('/api/agape/status', fetcher, { refreshInterval: 10_000 })
  const { data: btcStatusData, isLoading: btcLoading, mutate: refreshBtc } = useSWR('/api/agape-btc/status', fetcher, { refreshInterval: 10_000 })
  const { data: xrpStatusData, isLoading: xrpLoading, mutate: refreshXrp } = useSWR('/api/agape-xrp/status', fetcher, { refreshInterval: 10_000 })

  // Fetch all 3 performance for combined stats
  const { data: ethPerfData } = useSWR('/api/agape/performance', fetcher, { refreshInterval: 30_000 })
  const { data: btcPerfData } = useSWR('/api/agape-btc/performance', fetcher, { refreshInterval: 30_000 })
  const { data: xrpPerfData } = useSWR('/api/agape-xrp/performance', fetcher, { refreshInterval: 30_000 })

  const isAllView = selectedCoin === 'ALL'
  const allLoading = ethLoading && btcLoading && xrpLoading

  const refreshAll = () => { refreshEth(); refreshBtc(); refreshXrp() }
  ...
  const coinSummaries = useMemo(() => {
    const build = (statusData: any, perfData: any, coin: CoinId) => {
      ...
      return {
        coin,
        price: status?.[COIN_META[coin].priceKey] ?? null,
        ...
        startingCapital: pa?.starting_capital ?? status?.starting_capital ?? 5000,
        currentBalance: pa?.current_balance ?? 5000,
  ...
  }, [ethStatusData, btcStatusData, xrpStatusData, ethPerfData, btcPerfData, xrpPerfData])

  // Loading state
  if (allLoading && !ethStatusData && !btcStatusData && !xrpStatusData) {
    return (
      <>
        <Navigation />
        <div className="flex items-center justify-center h-screen bg-gray-950">
          <div className="text-center space-y-3">
            ...
            <p className="text-gray-400 text-sm">Loading Futures Crypto...</p>
          </div>
        </div>
      </>
    )
        ...
        <div className="max-w-7xl mx-auto space-y-5">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl md:text-3xl font-bold text-white">
                  Futures Crypto <span className="text-cyan-400">Multi-Coin</span>
                </h1>
              ...
              <p className="text-gray-500 text-sm mt-1">
                Directional trading via CME Micro Futures: /MET (ETH), /MBT (BTC), /XRP (XRP)
              </p>
            </div>
            <button
              onClick={refreshAll}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {COINS.map((coin) => {
              ...
              const summary = coin !== 'ALL' ? coinSummaries[coin] : null
              return (
                <button
                  key={coin}
...
function AllCoinsDashboard({ summaries }: { summaries: Record<'ETH' | 'BTC' | 'XRP', any> }) {
  const [eqTimeFrame, setEqTimeFrame] = useState<TimeFrameId>('30d')

  // Aggregate totals
  const coins = ['ETH', 'BTC', 'XRP'] as const
  const totalPnl = coins.reduce((s, c) => s + (summaries[c]?.totalPnl ?? 0), 0)
  const totalReturn = TOTAL_CAPITAL > 0 ? (totalPnl / TOTAL_CAPITAL) * 100 : 0
  const totalUnrealized = coins.reduce((s, c) => s + (summaries[c]?.unrealizedPnl ?? 0), 0)
  const totalTrades = coins.reduce((s, c) => s + (summaries[c]?.totalTrades ?? 0), 0)
  const totalPositions = coins.reduce((s, c) => s + (summaries[c]?.openPositions ?? 0), 0)
...
function AllCoinsRecentTrades() {
  const { data: ethClosed } = useSWR('/api/agape/closed-trades?limit=10', fetcher, { refreshInterval: 60_000 })
  const { data: btcClosed } = useSWR('/api/agape-btc/closed-trades?limit=10', fetcher, { refreshInterval: 60_000 })
  const { data: xrpClosed } = useSWR('/api/agape-xrp/closed-trades?limit=10', fetcher, { refreshInterval: 60_000 })
  ...
  return (
    <div className={`rounded-xl border p-4 ${meta.bgCard} ${meta.borderCard}`}>
      ...
      <p className="text-gray-500 text-xs">
        CME Micro Futures: {meta.instrument} | Directional trading (Long & Short) | $5,000 starting capital
      </p>
    </div>
  )
}
...
function OverviewTab({ coin }: { coin: CoinId }) {
  const { data: statusData } = useFuturesStatus(coin)
  const { data: perfData } = useFuturesPerformance(coin)
  const { data: snapshotData } = useFuturesSnapshot(coin)
  ...
  const startingCapital = pa?.starting_capital ?? status?.starting_capital ?? 5000
      ...
      <SectionCard title="Configuration" icon={<Settings className={`w-5 h-5 ${meta.textActive}`} />}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className={`font-mono ${meta.textActive}`}>{status?.instrument || meta.instrument.split(' ')[0]}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-cyan-400 font-mono">CME Futures</p>
          </div>
        </div>
      </SectionCard>
    </div>
...
function PositionsTab({ coin }: { coin: CoinId }) {
  const { data: posData, isLoading } = useFuturesPositions(coin)
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-white font-mono font-semibold">
                {pos.contracts}x {meta.instrument.split(' ')[0]} @ {fmtPrice(pos.entry_price, meta.priceDecimals)}
              </span>
              {pos.trailing_active && (
...
function PerformanceTab({ coin }: { coin: CoinId }) {
  const { data: perfData, isLoading } = useFuturesPerformance(coin)
...
function ClosedTradesTable({ coin }: { coin: CoinId }) {
  const { data: closedData } = useFuturesClosedTrades(coin, 50)
...
function EquityCurveTab({ coin }: { coin: CoinId }) {
  ...
  const { data: equityData, isLoading: histLoading } = useFuturesEquityCurve(coin, eqDays)
  const { data: intradayData, isLoading: intradayLoading } = useFuturesIntradayEquity(coin)
  const meta = COIN_META[coin]
  const points = isIntraday
    ? (intradayData?.data_points || [])
    : (equityData?.data?.equity_curve || [])
  const gradientId = `eqFill-futures-${coin}`
  const isLoading = isIntraday ? intradayLoading : histLoading

  const startCap = equityData?.data?.starting_capital ?? 5000
...
function ActivityTab({ coin }: { coin: CoinId }) {
  const { data: scanData, isLoading } = useFuturesScanActivity(coin, 40)
  ...
  // Determine price field key based on coin
  const priceField = coin === 'ETH' ? 'eth_price' : coin === 'BTC' ? 'btc_price' : 'xrp_price'
...
function HistoryTab({ coin }: { coin: CoinId }) {
  const { data: closedData, isLoading } = useFuturesClosedTrades(coin, 50)
  const trades = closedData?.data || []
  const meta = COIN_META[coin]
      ...
      {coins.map(coin => {
        ...
        return (
          <div key={coin} className="flex items-center gap-2 whitespace-nowrap">
            ...
            {coin !== 'XRP' && <span className="text-gray-700 mx-1">|</span>}
          </div>
        )
      })}
    </div>
    ...
    <div className={`rounded-xl border p-4 ${meta.bgCard} ${meta.borderCard} transition-colors`}>
      ...
      <div className="grid grid-cols-2 gap-y-2 text-sm">
        ...
        <div>
          ...
          <p className={`font-mono text-xs ${meta.textActive}`}>{meta.instrument.split(' ')[0]}</p>
        </div>
      </div>
      {/* Status badge */}
      ...
      <div className="h-20">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={points} margin={{ top: 0, right: 10, left: 0, bottom: 0 }}>
            <defs>
              <linearGradient id="drawdownFillFutures" x1="0" y1="0" x2="0" y2="1">
