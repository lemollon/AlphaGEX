-- PROPHET Tables Migration
-- Creates tables for Prophet ML Advisor, GEX data, and saved strategies
-- Run: psql $DATABASE_URL -f db/migrations/001_oracle_tables.sql

-- ============================================================================
-- PROPHET PREDICTIONS TABLE
-- Stores predictions made by Prophet for feedback loop and analysis
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_prophet_predictions_date ON prophet_predictions(trade_date);
CREATE INDEX IF NOT EXISTS idx_prophet_predictions_bot ON prophet_predictions(bot_name);
CREATE INDEX IF NOT EXISTS idx_prophet_predictions_outcome ON prophet_predictions(actual_outcome);
...
CREATE INDEX IF NOT EXISTS idx_training_outcomes_date ON prophet_training_outcomes(trade_date);
CREATE INDEX IF NOT EXISTS idx_training_outcomes_bot ON prophet_training_outcomes(bot_name);
CREATE INDEX IF NOT EXISTS idx_training_outcomes_win ON prophet_training_outcomes(is_win);
...
-- VERIFICATION
-- ============================================================================
SELECT 'Prophet tables created successfully' as status;
SELECT table_name FROM information_schema.tables
WHERE table_schema = 'public'
AND table_name IN ('prophet_predictions', 'gex_daily', 'saved_strategies', 'prophet_training_outcomes')
ORDER BY table_name;
