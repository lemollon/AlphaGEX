# WATCHTOWER (0DTE Gamma Live) - Feature Requirements

## Overview

**WATCHTOWER** - Named after the "all-seeing" giant with 100 eyes from Greek mythology. A real-time visualization of net gamma by strike with ML-powered probability predictions, momentum indicators, Claude AI commentary, and a feedback loop for continuous bot improvement.
...
| Specification | Value |
|---------------|-------|
| **Name** | WATCHTOWER (0DTE Gamma Live) |
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│ 🧠 WATCHTOWER AI COMMENTARY                                        [X Close] │
├─────────────────────────────────────────────────────────────────────────┤
...
```
You are WATCHTOWER, the all-seeing gamma analyst for AlphaGEX. Generate a 5-minute market update based on the current 0DTE gamma structure.
...
| **Bot Decisions** | What bots did based on WATCHTOWER data | Correlate decisions with outcomes |
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  WATCHTOWER (0DTE Gamma Live) - SPY                                    [🔔 3] [⚙️]   │
...
|----------|--------|-------------|
| `GET /api/watchtower/gamma` | GET | Current net gamma by strike |
| `GET /api/watchtower/history` | GET | Historical gamma (last 30 min) |
| `GET /api/watchtower/probability` | GET | ML-powered probability per strike |
| `GET /api/watchtower/alerts` | GET | Active alerts |
| `GET /api/watchtower/commentary` | GET | Latest Claude AI commentary |
| `POST /api/watchtower/commentary/generate` | POST | Trigger new commentary |
| `GET /api/watchtower/bots` | GET | Active bot positions |
| `GET /api/watchtower/accuracy` | GET | Prediction accuracy metrics |
| `GET /api/watchtower/patterns` | GET | Pattern match analysis |
| `GET /api/watchtower/export` | GET | Export data to Excel |
| `WS /api/watchtower/stream` | WS | Real-time updates |
| `GET /api/watchtower/replay` | GET | Historical replay data for date/time |
| `GET /api/watchtower/replay/dates` | GET | Available dates for replay |
...
CREATE TABLE watchtower_commentary (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_watchtower_snapshots_time ON watchtower_snapshots(snapshot_time);
CREATE INDEX idx_watchtower_snapshots_expiration ON watchtower_snapshots(expiration_date);
CREATE INDEX idx_argus_strikes_snapshot ON argus_strikes(snapshot_id);
CREATE INDEX idx_watchtower_commentary_time ON watchtower_commentary(created_at);
CREATE INDEX idx_watchtower_alerts_time ON watchtower_alerts(triggered_at);
...
| File | Purpose |
|------|---------|
| `backend/api/routes/watchtower_routes.py` | All WATCHTOWER API endpoints |
| `core/watchtower_engine.py` | Core WATCHTOWER logic and calculations |
| `core/watchtower_commentary.py` | Claude AI commentary generator |
| `core/argus_feedback.py` | Feedback loop processor |
| `frontend/src/app/watchtower/page.tsx` | Main WATCHTOWER page |
| `frontend/src/components/watchtower/*` | All WATCHTOWER components |
...
| Endpoint | Tests |
|----------|-------|
| `GET /api/watchtower/gamma` | Returns correct structure, handles missing data |
| `GET /api/watchtower/probability` | Returns probabilities sum to 100% |
| `GET /api/watchtower/commentary` | Returns valid commentary |
| `GET /api/watchtower/alerts` | Returns alerts in correct priority order |
| `GET /api/watchtower/export` | Returns valid Excel file |
...
| Scenario | Steps |
|----------|-------|
| **Full Page Load** | Navigate to WATCHTOWER, verify all components render |
...
# API tests
pytest tests/api/test_watchtower_routes.py -v

# Frontend component tests
cd frontend && npm run test:watchtower

# E2E tests
cd frontend && npm run test:e2e:watchtower
...
```yaml
# .github/workflows/watchtower-tests.yml
...
on:
  push:
    paths:
      ...
      - 'frontend/src/app/watchtower/**'
      - 'frontend/src/components/watchtower/**'
jobs:
  test:
    ...
    steps:
      ...
      - name: Run Frontend Tests
        run: cd frontend && npm test -- --grep watchtower
      - name: Run E2E Tests
        run: cd frontend && npm run test:e2e:watchtower
```
...
- **Feature Name:** WATCHTOWER (0DTE Gamma Live)
- **Created:** 2024-12-21
- **Updated:** 2024-12-21
