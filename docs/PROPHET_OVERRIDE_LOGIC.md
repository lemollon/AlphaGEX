# Prophet Override Logic - Technical Architecture

## Overview

Prophet is the central AI advisory system that aggregates multiple signals (GEX, ML predictions, VIX regime, market conditions) and provides curated recommendations to each trading bot (ARES, SOLOMON, ATLAS). The override logic allows the system to adjust or completely override ML model predictions based on additional market context.
...
Prophet provides a recommended position size based on win probability using a modified Kelly Criterion:
...
When `use_gex_walls=True`, Prophet provides specific strikes based on gamma exposure walls:
...
3. Query Prophet for direction confirmation
...
Prophet enforces strict anti-hallucination rules in Claude prompts:
...
STRATEGY_PRESETS = {
    StrategyPreset.MODERATE: {
        ...
        "risk_per_trade_pct": 10.0,  # Max risk (Prophet can suggest less)
        "sd_multiplier": 1.0,        # Default (Prophet can override)
    }
}
```
...
- [Circuit Breaker Integration](./CIRCUIT_BREAKER.md) - How circuit breakers interact with Prophet
