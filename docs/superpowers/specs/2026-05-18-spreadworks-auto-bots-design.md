...
spreadworks/
  backend/
    bots/                          # NEW
      ...
      db.py                        # Per-bot table helpers (bot_table('breeze', 'positions'))
      strategies/
        __init__.py
        iron_butterfly.py          # BREEZE entry
...
|----------|---------|------------------|-----------|----------|-----------|------------|--------|
| breeze     | BREEZE    | iron_butterfly   | 0         | —        | 30%       | 200% credit| 14:45  |
...
CREATE TABLE {bot}_config (
  ...
  sd_mult           NUMERIC(4,2) NOT NULL DEFAULT 1.0,   -- BREEZE: wing distance
  front_dte         INTEGER NOT NULL DEFAULT 0,
  back_dte          INTEGER,                              -- NULL for BREEZE
  ...
  use_gex_walls     BOOLEAN NOT NULL DEFAULT false,       -- BREEZE only: clip wings to walls
...
CREATE TABLE {bot}_positions (
  position_id     TEXT PRIMARY KEY,         -- e.g. 'breeze-2026-05-18-001'
...
### 6.1 BREEZE — Iron Butterfly, 0DTE SPY
...
5. **Open:** insert into `breeze_positions` with
...
3. Compute signed P&L:
   - **Credit strats (BREEZE):** `mtm_pnl = (entry_price − mtm_value) × contracts × 100` (we received `entry_price` credit; profit when buy-back is cheaper).
...
5. Decide:
   ...
   - **Time-of-day PT ladder** (BREEZE only): override `pt_target_pnl` each scan using `pt_pct_morning=0.30`, `pt_pct_midday=0.40`, `pt_pct_afternoon=0.50` based on CT clock. (Ported from IronForge SPARK fix.) TIDE/DRIFT use static `pt_pct`.
   - **EOD trigger:**
     - BREEZE: if CT now ≥ `eod_close_ct` → close with reason `EOD`.
...
5. `/api/spreadworks/bots/breeze/equity-curve/intraday` returns at least 2
