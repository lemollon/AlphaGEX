...
# ============================================================================
# SPX DEBUG ENDPOINTS - Real-time debugging and diagnostics
# ============================================================================

@app.get("/api/spx/debug/logs")
async def get_spx_debug_logs(limit: int = 100, category: str = None, session_id: str = None):
    ...
    - category: Filter by category (DATA_FETCH, REGIME, SIZING, TRADE, POSITION, SCAN)
...
@app.get("/api/spx/debug/scan-cycles")
async def get_spx_scan_cycles(limit: int = 20):
    ...
    try:
        ...
        # Get scan cycle starts and ends
        c.execute('''
            SELECT
                session_id,
                scan_cycle,
...
@app.get("/api/spx/debug/regime-history")
...
@app.get("/api/spx/debug/kelly-history")
    ...
    try:
        ...
        c.execute('''
            SELECT
                ...
                data
            FROM spx_debug_logs
            WHERE category = 'SIZING'
            AND subcategory = 'kelly_calculation'
            ORDER BY timestamp DESC
            LIMIT %s
        ''', (limit,))
        ...
        for row in c.fetchall():
            ...
            kelly_calcs.append({
                ...
                'avg_win': kelly_data.get('avg_win'),
                'avg_loss': kelly_data.get('avg_loss'),
...
@app.get("/api/spx/debug/data-fetches")
...
@app.post("/api/spx/debug/run-diagnostic")
