"""
AGAPE-BTC-PERP API Routes - BTC Perpetual Contract Trading Bot endpoints.

AGAPE-BTC-PERP trades BTC perpetual contracts using crypto market
...
router = APIRouter(prefix="/api/agape-btc-perp", tags=["AGAPE-BTC-PERP"])
...
get_agape_btc_perp_trader = None
try:
    from trading.agape_btc_perp.trader import AgapeBtcPerpTrader, get_agape_btc_perp_trader, create_agape_btc_perp_trader
    AGAPE_BTC_PERP_AVAILABLE = True
    logger.info("AGAPE-BTC-PERP Routes: AgapeBtcPerpTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-BTC-PERP Routes: AgapeBtcPerpTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-BTC-PERP trader instance."""
    if not AGAPE_BTC_PERP_AVAILABLE:
        return None
    trader = get_agape_btc_perp_trader()
    if trader is None:
        try:
            trader = create_agape_btc_perp_trader()
        except Exception as e:
            logger.error(f"AGAPE-BTC-PERP Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-BTC-PERP bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-BTC-PERP trader not initialized",
            "message": "AGAPE-BTC-PERP module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-BTC-PERP positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-BTC-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_btc_perp_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        logger.error(f"AGAPE-BTC-PERP equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_btc_perp_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            SELECT timestamp, equity, unrealized_pnl,
                   realized_pnl_cumulative, open_positions, btc_price
            FROM agape_btc_perp_equity_snapshots
        ...
        if open_rows:
            # Get current BTC price from latest snapshot or trader
            current_btc_price = None
            if snapshots:
                current_btc_price = float(snapshots[-1][5]) if snapshots[-1][5] else None
            ...
            if not current_btc_price:
                ...
                if trader:
                    try:
                        current_btc_price = trader.executor.get_current_price()
                    except Exception:
                        pass
            ...
            if current_btc_price:
                for pos_row in open_rows:
                    ...
                    pnl = (current_btc_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        for snapshot in snapshots:
            ts, balance, snap_unrealized, snap_realized_cum, open_count, btc_price = snapshot
        ...
        logger.error(f"AGAPE-BTC-PERP intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-BTC-PERP performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-BTC-PERP activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-BTC-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-BTC-PERP scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    ...
    """Get current crypto market microstructure snapshot for BTC.
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-BTC-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC-PERP signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for BTC perpetuals.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-BTC-PERP.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-BTC-PERP: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-BTC-PERP uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        "Actual gamma exposure calculated from Deribit BTC options. "
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-BTC-PERP uses contrarian logic."
                    ),
...
async def get_margin_analysis():
    ...
    try:
        ...
        summary["spec"] = {
            "name": spec.get("name", "BTC Perpetual"),
        ...
        logger.error(f"AGAPE-BTC-PERP margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-BTC-PERP trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-BTC-PERP not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-BTC-PERP enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-BTC-PERP trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-BTC-PERP not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-BTC-PERP disabled"}


@router.post("/force-close-all")
async def force_close_all(reason: str = "MANUAL_CLOSE"):
    """Force-close every open AGAPE-BTC-PERP position at current mark."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-BTC-PERP not available")
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for BTC charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("BTC"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-BTC-PERP chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for BTC.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("BTC")
        if not snap:
        ...
        logger.error(f"AGAPE-BTC-PERP brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
