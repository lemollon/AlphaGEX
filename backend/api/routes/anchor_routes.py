"""
ANCHOR SPX Iron Condor Bot API Routes
========================================

API endpoints for the ANCHOR SPX Iron Condor trading bot.
...
try:
    ...
    MTM_AVAILABLE = True
    logger.info("Mark-to-market utility loaded for ANCHOR")
...
try:
    ...
    logger.info("TradierDataFetcher loaded for ANCHOR")
except ImportError as e:
...
if not TRADIER_AVAILABLE:
    try:
        ...
        if tradier_path.exists():
            ...
            TRADIER_AVAILABLE = True
            logger.info(f"TradierDataFetcher loaded via direct import for ANCHOR")
    except Exception as e:
        logger.warning(f"Direct Tradier import failed for ANCHOR: {e}")
...
def _calculate_anchor_unrealized_pnl(positions: list) -> dict:
    """
    Calculate unrealized P&L for ANCHOR positions using mark-to-market pricing.
    ...
    for pos in positions:
        ...
        if spx_price and spx_price > 0:
            ...
            if put_short < spx_price < call_short:
                ...
                # value for SPX European-style cash settlement.
...
# Try to import ANCHOR trader
anchor_trader = None
try:
    from trading.anchor import AnchorTrader, AnchorConfig, TradingMode, StrategyPreset
    ANCHOR_AVAILABLE = True
except ImportError as e:
    ANCHOR_AVAILABLE = False
    AnchorConfig = None
    StrategyPreset = None
    logger.warning(f"ANCHOR module not available: {e}")


def get_anchor_instance():
    """Get the ANCHOR trader instance from scheduler if available"""
    global anchor_trader
    if anchor_trader:
        return anchor_trader

    try:
        from scheduler.trader_scheduler import get_anchor_trader
        anchor_trader = get_anchor_trader()
        return anchor_trader
    except ImportError as e:
        logger.debug(f"Could not import trader_scheduler: {e}")
        return None
    except Exception as e:
        logger.debug(f"Could not get ANCHOR trader: {e}")
        return None


def _get_tradier_account_balance() -> dict:
    """
    Get account balance from Tradier API for ANCHOR.

    ANCHOR always uses PRODUCTION Tradier because:
    - Sandbox doesn't support SPX quotes
    - ANCHOR needs SPX prices for trading
    """
    ...
    try:
        ...
        # ANCHOR always uses PRODUCTION Tradier for SPX quotes (sandbox doesn't have SPX)
        api_key = (
        ...
        logger.info(f"ANCHOR Tradier: mode=PRODUCTION (SPX requires prod), api_key={'SET' if api_key else 'NOT SET'}, account_id={account_id}")

        if not api_key or not account_id:
            return {'connected': False, 'total_equity': 0, 'sandbox': False, 'error': 'No credentials configured'}

        # ANCHOR uses production (sandbox=False) for SPX quotes
        ...
        logger.error(f"ANCHOR Tradier balance fetch ERROR: {e}")
...
async def get_anchor_status():
    """
    Get current ANCHOR bot status.

    Returns mode, capital, P&L, positions, configuration, and heartbeat.
    """
    anchor = get_anchor_instance()
    heartbeat = _get_heartbeat('ANCHOR')

    if not anchor:
        # ANCHOR not running - read stats from database
        starting_capital = 200000  # Default for ANCHOR (SPX bot)
        total_pnl = 0
        ...
        try:
            ...
            try:
                cursor.execute("SELECT value FROM autonomous_config WHERE key = 'anchor_starting_capital'")
            ...
            logger.debug(f"Could not read ANCHOR stats from database: {db_err}")

        win_rate = round((win_count / closed_count) * 100, 1) if closed_count > 0 else 0

        # ANCHOR paper trades with $200k - Tradier is only for SPX prices, not required
        tradier_balance = _get_tradier_account_balance()
        tradier_connected = tradier_balance.get('connected', False)

        # ANCHOR always uses paper capital - Tradier is optional for price data
        ...
        # ANCHOR trading window: 8:30 AM - 2:45 PM CT (market closes at 3:00 PM CT)
        entry_start = "08:30"
        entry_end = "14:45"
        ...
        # Determine if ANCHOR is actually active based on heartbeat
        scan_interval = 5
    ...
    try:
        ...
        db_open_count = 0
        db_closed_count = 0
        db_starting_capital = 200000  # Default for ANCHOR (SPX bot)
...
async def get_anchor_positions():
    """
    Get ANCHOR open and recently closed positions.
    ...
    if not anchor:
        # ANCHOR not running - read from database directly
        try:
            conn = get_connection()
            cursor = conn.cursor()
    ...
    try:
        positions = anchor.get_positions()
        open_positions = []
        for pos in positions:
            dte = 0
            if pos.expiration:
        ...
        logger.error(f"Error getting ANCHOR positions: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/equity-curve")
async def get_anchor_equity_curve(days: int = 30):
    """
    Get ANCHOR equity curve data including unrealized P&L from open positions.
    ...
    try:
        ...
        if open_positions:
            mtm_result = _calculate_anchor_unrealized_pnl(open_positions)
            unrealized_pnl = mtm_result['total_unrealized_pnl']
            logger.debug(f"ANCHOR equity-curve: unrealized=${unrealized_pnl:.2f} via {mtm_result['method']}")
...
async def get_anchor_intraday_equity(date: str = None):
    """
    Get ANCHOR intraday equity curve with 5-minute interval snapshots.
    ...
    try:
        ...
        cursor.execute("""
            SELECT value FROM autonomous_config WHERE key = 'anchor_starting_capital'
        """)
        row = cursor.fetchone()
        if row and row[0]:
            try:
        ...
        try:
            ...
            if open_rows:
                ...
                mtm_result = _calculate_anchor_unrealized_pnl(open_rows)
                unrealized_pnl = mtm_result['total_unrealized_pnl']
                open_positions = mtm_result['positions']
                pricing_method = mtm_result['method']
                logger.debug(f"ANCHOR intraday: unrealized=${unrealized_pnl:.2f} via {pricing_method}")
        except Exception as e:
        ...
        # IMPORTANT: Without live pricing from ANCHOR worker, unrealized_pnl = 0
        ...
        return {
            ...
            "open_positions_count": len(open_positions)  # Number of currently open positions
        }

    except Exception as e:
        logger.error(f"Error getting ANCHOR intraday equity: {e}")
        import traceback
        traceback.print_exc()
        return {
            "success": False,
...
async def save_anchor_equity_snapshot():
    """
    ...
    try:
        ...
        cursor.execute("""
            SELECT position_id, total_credit, contracts, spread_width,
                   put_short_strike, put_long_strike, call_short_strike, call_long_strike,
                   expiration
            FROM anchor_positions WHERE status = 'open'
        """)
        open_positions = cursor.fetchall()
        open_count = len(open_positions)
        ...
        if open_positions:
            ...
            unrealized_pnl = mtm_result['total_unrealized_pnl']
            pricing_method = mtm_result['method']
            logger.debug(f"ANCHOR snapshot: unrealized=${unrealized_pnl:.2f} via {pricing_method}")
        ...
        # Insert snapshot
        cursor.execute('''
            INSERT INTO anchor_equity_snapshots
        ...
        logger.error(f"Error saving ANCHOR equity snapshot: {e}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def run_anchor_cycle(
    ...
    if not anchor:
        raise HTTPException(
            status_code=503,
            detail="ANCHOR not initialized. Wait for scheduled startup."
        )

    try:
        result = anchor.run_cycle()

        return {
            "success": True,
            "data": result,
            "message": "ANCHOR cycle completed"
        }
    except Exception as e:
        logger.error(f"Error running ANCHOR cycle: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_anchor_config():
    """
    Get ANCHOR configuration parameters.
    """
    anchor = get_anchor_instance()
    ...
    default_config = {
        ...
        "description": "ANCHOR trades SPX Iron Condors with $10 spread widths using SPXW weekly options."
    }

    if anchor and hasattr(anchor, 'config'):
...
async def force_close_anchor_positions(
    request: Request,
    auth: AuthInfo = Depends(require_admin) if AUTH_AVAILABLE and require_admin else None
):
    """
    Force close all open ANCHOR positions.
    ...
    if not anchor:
        raise HTTPException(
            status_code=503,
            detail="ANCHOR not initialized."
        )

    try:
        result = anchor.force_close_all("MANUAL")
        ...
        logger.error(f"Error force closing ANCHOR positions: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/live-pnl")
async def get_anchor_live_pnl():
    """
    Get real-time unrealized P&L for all open ANCHOR positions.
    """
    anchor = get_anchor_instance()
    today = datetime.now(ZoneInfo("America/Chicago")).strftime('%Y-%m-%d')
    ...
    if not anchor:
        try:
            ...
            for row in open_rows:
                ...
                if MTM_AVAILABLE and exp and put_short and put_long and call_short and call_long:
                    try:
                        ...
                        logger.debug(f"ANCHOR live-pnl MTM failed for {pos_id}: {e}")
            ...
            logger.warning(f"Could not read ANCHOR live P&L from database: {db_err}")
        ...
        return {
            ...
            "data": {
                ...
                "positions": [],
                "position_count": 0,
                "message": "ANCHOR not initialized"
            }
        }
    ...
    try:
        ...
        logger.error(f"Error getting ANCHOR live P&L: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/logs")
async def get_anchor_logs(
    ...
    """
    Get ANCHOR logs for debugging and monitoring.
    """
    ...
    try:
        ...
        # anchor_logs table has columns: log_time, level (not created_at, log_level)
        # and does NOT have bot_name column (it's ANCHOR-specific)
        ...
        # If anchor_logs table doesn't exist, try bot_logs
        try:
            conn = get_connection()
            c = conn.cursor()

            where_clause = "WHERE bot_name = 'ANCHOR'"
            params = []
            if level:
                where_clause += " AND level = %s"
                params.append(level)
        ...
        logger.error(f"Error getting ANCHOR logs: {e}")
        return {
            "success": True,
            "data": [],
            "count": 0,
...
async def get_anchor_performance(
    days: int = Query(30, description="Number of days to include")
):
    """
    Get ANCHOR performance metrics over time.
    """
    # Resolve Query objects for direct function calls (E2E tests)
    ...
    try:
        ...
        logger.error(f"Error getting ANCHOR performance: {e}")
        return {
            "success": True,
            "data": {
                "summary": {
...
@router.post("/reset")
async def reset_anchor_data(confirm: bool = False):
    """
    Reset ANCHOR trading data - delete all positions and start fresh.

    Args:
        confirm: Must be True to actually delete data (safety check)

    WARNING: This will permanently delete ALL ANCHOR trading history.
    """
    if not confirm:
        ...
        try:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM anchor_positions")
            total = cursor.fetchone()[0]
            cursor.execute("SELECT COUNT(*) FROM anchor_positions WHERE status = 'open'")
            open_count = cursor.fetchone()[0]
            cursor.execute("SELECT COUNT(*) FROM anchor_positions WHERE status IN ('closed', 'expired', 'partial_close')")
            closed_count = cursor.fetchone()[0]
            conn.close()

            return {
                "success": False,
                "message": "Set confirm=true to reset ANCHOR data. This action cannot be undone.",
                "preview": {
    ...
    try:
        ...
        # Delete all ANCHOR positions
        cursor.execute("DELETE FROM anchor_positions")
        deleted_positions = cursor.rowcount

        # Also delete ANCHOR daily performance
        deleted_performance = 0
        try:
            cursor.execute("DELETE FROM anchor_daily_perf")
            deleted_performance = cursor.rowcount
        except Exception:
            pass

        # Also delete ANCHOR scan activity logs if table exists
        deleted_scans = 0
        try:
            cursor.execute("DELETE FROM anchor_scan_activity")
        ...
        try:
            cursor.execute("DELETE FROM bot_scan_activity WHERE bot_name = 'ANCHOR'")
            deleted_scans += cursor.rowcount
        except Exception:
            pass

        # Reset ANCHOR config to defaults
        deleted_config = 0
        try:
            cursor.execute("DELETE FROM autonomous_config WHERE key LIKE 'anchor_%'")
        ...
        logger.info(f"ANCHOR reset complete: {deleted_positions} positions, {deleted_performance} performance records, {deleted_scans} scan logs, {deleted_config} config entries deleted")

        return {
            "success": True,
            "message": "ANCHOR data has been reset successfully",
            "deleted": {
        ...
        logger.error(f"Error resetting ANCHOR data: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def cleanup_open_positions(confirm: bool = False):
    """
    Clean up open ANCHOR positions without affecting closed trade history.
    ...
    try:
        ...
        cursor.execute("DELETE FROM anchor_positions WHERE status = 'open'")
        deleted_count = cursor.rowcount

        # Also clear any equity snapshots from today (they include the bad unrealized P&L)
        today = datetime.now(ZoneInfo("America/Chicago")).strftime('%Y-%m-%d')
        cursor.execute("""
            DELETE FROM anchor_equity_snapshots
            WHERE DATE(timestamp::timestamptz AT TIME ZONE 'America/Chicago') = %s
        """, (today,))
        ...
        logger.info(f"ANCHOR cleanup: Deleted {deleted_count} open positions and {deleted_snapshots} today's snapshots")
        ...
        logger.error(f"Error cleaning up ANCHOR positions: {e}")
        raise HTTPException(status_code=500, detail=str(e))
