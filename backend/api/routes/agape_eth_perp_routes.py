"""
AGAPE-ETH-PERP API Routes - ETH Perpetual Contract Trading Bot endpoints.

AGAPE-ETH-PERP trades ETH perpetual contracts using crypto market
...
router = APIRouter(prefix="/api/agape-eth-perp", tags=["AGAPE-ETH-PERP"])
...
get_agape_eth_perp_trader = None
try:
    from trading.agape_eth_perp.trader import AgapeEthPerpTrader, get_agape_eth_perp_trader, create_agape_eth_perp_trader
    AGAPE_ETH_PERP_AVAILABLE = True
    logger.info("AGAPE-ETH-PERP Routes: AgapeEthPerpTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-ETH-PERP Routes: AgapeEthPerpTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-ETH-PERP trader instance."""
    if not AGAPE_ETH_PERP_AVAILABLE:
        return None
    trader = get_agape_eth_perp_trader()
    if trader is None:
        try:
            trader = create_agape_eth_perp_trader()
        except Exception as e:
            logger.error(f"AGAPE-ETH-PERP Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-ETH-PERP bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-ETH-PERP trader not initialized",
            "message": "AGAPE-ETH-PERP module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-ETH-PERP positions with unrealized P&L.
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_performance():
    """Get AGAPE-ETH-PERP performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-ETH-PERP activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-ETH-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-ETH-PERP scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    ...
    """Get current crypto market microstructure snapshot for ETH.
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    try:
        ...
        logger.error(f"AGAPE-ETH-PERP signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for ETH perpetuals.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-ETH-PERP.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-ETH-PERP: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-ETH-PERP uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-ETH-PERP uses contrarian logic."
                    ),
...
async def get_margin_analysis():
    ...
    try:
        ...
        summary["spec"] = {
            "name": spec.get("name", "ETH Perpetual"),
        ...
        logger.error(f"AGAPE-ETH-PERP margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-ETH-PERP trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-ETH-PERP not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-ETH-PERP enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-ETH-PERP trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-ETH-PERP not available")
    trader.disable()
...
async def force_close_all(reason: str = "MANUAL_CLOSE", limit: int = 0):
    """Force-close open AGAPE-ETH-PERP positions at current mark.
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for ETH charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("ETH"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-ETH-PERP chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for ETH.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("ETH")
        if not snap:
        ...
        logger.error(f"AGAPE-ETH-PERP brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
