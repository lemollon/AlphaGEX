"""
FORTRESS Iron Condor Bot API Routes
================================

API endpoints for the FORTRESS aggressive Iron Condor trading bot.
Provides status, positions, equity curve, and trade management.

FORTRESS targets 10% monthly returns through daily 0DTE Iron Condors.
"""
...
try:
    ...
    MTM_AVAILABLE = True
    logger.info("Mark-to-market utility loaded for FORTRESS")
...
if not TRADIER_AVAILABLE:
    try:
        ...
        # backend/api/routes/fortress_routes.py -> go up 3 levels to project root
...
try:
    from trading.fortress_v2 import FortressTrader, TradingMode, StrategyPreset, STRATEGY_PRESETS
    # Note: FORTRESS trader is initialized by scheduler, we query its state
    ARES_AVAILABLE = True
except ImportError:
...
def get_ares_instance():
    ...
    if fortress_trader:
        return fortress_trader

    try:
        from scheduler.trader_scheduler import get_fortress_trader
        fortress_trader = get_fortress_trader()
        return fortress_trader
    except ImportError as e:
        logger.debug(f"Could not import trader_scheduler: {e}")
        return None
    except Exception as e:
        logger.debug(f"Could not get FORTRESS trader: {e}")
        return None
        ...
        if MTM_AVAILABLE and expiration:
            try:
                ...
                mtm = calculate_ic_mark_to_market(
                    underlying='SPY',  # FORTRESS trades SPY
                    expiration=exp_str,
...
def _is_ares_actually_active(heartbeat: dict, scan_interval_minutes: int = 5) -> tuple[bool, str]:
    ...
    # These statuses indicate FORTRESS is NOT active/healthy
    inactive_statuses = {
        'UNAVAILABLE': 'FORTRESS trader not initialized',
        'ERROR': 'FORTRESS encountered an error',
        'KILLED': 'FORTRESS stopped by kill switch',
        'NEVER_RUN': 'FORTRESS has never run',
        'UNKNOWN': 'FORTRESS status unknown'
    }
...
def _get_tradier_account_balance() -> dict:
    ...
    FORTRESS uses SANDBOX Tradier account for trading.
    ...
    try:
        ...
        # FORTRESS uses SANDBOX Tradier account
        use_sandbox = True  # FORTRESS uses sandbox account
        ...
        logger.info(f"Tradier balance fetch: mode=SANDBOX (FORTRESS), api_key={'SET' if api_key else 'NOT SET'}, account_id={account_id}")
...
def _get_tradier_positions() -> dict:
    ...
    FORTRESS uses SANDBOX Tradier - this shows actual positions in the broker.
    ...
    try:
        ...
        tradier = TradierDataFetcher(
            api_key=api_key,
            account_id=account_id,
            sandbox=True  # FORTRESS uses SANDBOX
        )
...
async def get_tradier_positions():
    ...
    FORTRESS database should match these positions.
    """
...
async def sync_with_tradier():
    """
    Sync FORTRESS database with Tradier SANDBOX account.

    Compares positions in Tradier with FORTRESS database and reconciles:
    ...
    # Get FORTRESS database positions
    try:
        conn = get_connection()
        cursor = conn.cursor()
        ...
        if result['tradier_positions'] == 0 and result['db_positions'] > 0:
            result['discrepancies'].append({
                ...
                'message': f"FORTRESS DB has {result['db_positions']} open positions but Tradier has none",
        ...
        if result['tradier_positions'] > 0 and result['db_positions'] == 0:
            result['discrepancies'].append({
                ...
                'message': f"Tradier has {result['tradier_positions']} positions but FORTRESS DB has none",
...
async def diagnose_tradier_connection():
    ...
    # Step 3: Check SANDBOX credentials (FORTRESS uses sandbox account)
    ...
    results["steps"].append({
        "step": 3,
        "name": "Check SANDBOX credentials (FORTRESS uses sandbox)",
    ...
    # FORTRESS uses SANDBOX credentials
...
async def diagnose_database_state():
    """
    Diagnostic endpoint to check fortress_positions database state.
...
async def get_fortress_status():
    """
    Get current FORTRESS bot status.
    ...
    # FORTRESS trading window: 8:30 AM - 2:45 PM CT (market closes at 3:00 PM CT)
    entry_start = "08:30"
    ...
    if not fortress:
        # FORTRESS not running in this process - read stats from database
        total_pnl = 0
        ...
        try:
            ...
            logger.debug(f"Could not read FORTRESS stats from database: {db_err}")
        ...
        try:
            ...
            logger.debug(f"Could not read FORTRESS config from database: {e}")

        # Get actual Tradier account balance and positions - FORTRESS MUST be connected to Tradier
        tradier_balance = _get_tradier_account_balance()
        ...
        if tradier_balance.get('connected') and tradier_balance.get('total_equity', 0) > 0:
            ...
            logger.error(f"FORTRESS Tradier connection failed: {tradier_error}")
        ...
        starting_capital = 100000  # Default for FORTRESS (SPY bot)
        try:
            conn = get_connection()
            cursor = conn.cursor()
    ...
    try:
        ...
        if tradier_balance.get('connected') and tradier_balance.get('total_equity', 0) > 0:
            ...
            logger.warning(f"FORTRESS Tradier connection failed: {status['tradier_error']}")
...
async def get_fortress_positions():
    """
    ...
    try:
        ...
        logger.error(f"Error getting FORTRESS positions: {e}")
        import traceback
        traceback.print_exc()
        return {
            "success": True,
...
async def get_fortress_equity_curve(days: int = 30):
    """
    Get FORTRESS equity curve data including unrealized P&L from open positions.
            ...
            if open_positions and MTM_AVAILABLE:
                for pos in open_positions:
                    ...
                    try:
                        ...
                        logger.debug(f"FORTRESS MTM calculation failed for {pos_id}: {e}")

            # Build equity curve from database positions - one point per DATE for consistent daily P&L
    ...
    try:
        ...
        unrealized_pnl = 0.0
        open_positions_count = len(fortress.open_positions) if hasattr(fortress, 'open_positions') else 0

        if hasattr(fortress, 'open_positions') and fortress.open_positions and MTM_AVAILABLE:
            for pos in fortress.open_positions:
                try:
                    ...
                    logger.debug(f"FORTRESS MTM calculation failed for position: {e}")

        # Sort positions by close time for equity curve
        # Check if fortress has closed_positions attribute, otherwise use empty list
        closed_positions_list = getattr(fortress, 'closed_positions', []) or []
        sorted_positions = sorted(
            closed_positions_list,
            key=lambda p: p.close_date or p.expiration or ""
        )

        # Build one point per DATE for consistent daily P&L display
        # CRITICAL FIX: Aggregate trades by date so daily_pnl shows the day's total,
        # not just the last trade's P&L. This ensures:
        # equity[today] - equity[yesterday] = daily_pnl[today]
        if sorted_positions:
            ...
            for pos in sorted_positions:
                ...
                if date_str not in trades_by_date:
                    trades_by_date[date_str] = []
                trades_by_date[date_str].append(pos)
            ...
            # Create one data point per DATE with aggregated daily P&L
            for date_str in sorted_dates:
                day_positions = trades_by_date[date_str]
                day_pnl = sum(float(pos.realized_pnl or 0) for pos in day_positions)
                day_trade_count = len(day_positions)
        ...
        # Calculate today's realized P&L from in-memory closed positions
        today_str = now.strftime('%Y-%m-%d')
        today_realized = sum(
            float(pos.realized_pnl or 0)
            for pos in closed_positions_list
            if str(pos.close_date or pos.expiration)[:10] == today_str
        )
        ...
        logger.error(f"Error getting FORTRESS equity curve: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_ares_intraday_equity(date: str = None):
    ...
    starting_capital = 100000  # Default starting capital for FORTRESS
    ...
    try:
        ...
        logger.error(f"Error getting FORTRESS intraday equity: {e}")
        import traceback
        traceback.print_exc()
        return {
            "success": False,
...
async def save_equity_snapshot():
    ...
    try:
        ...
        for col in ['unrealized_pnl', 'realized_pnl']:
            try:
                cursor.execute(f"ALTER TABLE fortress_equity_snapshots ADD COLUMN IF NOT EXISTS {col} DECIMAL(12, 2)")
            except Exception:
                pass
...
async def get_fortress_performance(
    ...
    try:
        ...
        # Get daily performance data (same format as SAMSON)
        ...
        logger.error(f"Error getting FORTRESS performance: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_fortress_logs(
    ...
    """
    Get FORTRESS logs for debugging and monitoring.
    """
    ...
    try:
        ...
        logger.error(f"Error getting FORTRESS logs: {e}")
        return {
            "success": True,
            "data": [],
            "count": 0,
...
async def get_ares_decisions(limit: int = 50):
    ...
    Returns decision logs filtered for FORTRESS bot.
    """
    try:
        ...
        logger.error(f"Error getting FORTRESS decisions: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        if has_explicit_prod:
            ...
            try:
                ...
                using_sandbox = False
                logger.info("FORTRESS API: Using Tradier PRODUCTION API (TRADIER_PROD_* credentials)")
            except Exception as e:
                logger.warning(f"FORTRESS API: Failed to initialize production client: {e}")
        ...
        if not tradier and sandbox_key and sandbox_account:
            try:
                ...
                using_sandbox = True
                logger.info("FORTRESS API: Using Tradier SANDBOX API (SPX/VIX will be estimated)")
            except Exception as e:
                logger.error(f"FORTRESS API: Failed to initialize sandbox client: {e}")
        ...
        if using_sandbox:
            ...
            logger.info(f"FORTRESS API: Sandbox mode - SPX estimated from SPY*10: ${spx_price}")
        else:
            ...
            if spx_quote and spx_quote.get('last'):
                ...
                spx_price = spy_price * 10
                logger.warning("FORTRESS API: $SPX.X failed, using SPY*10 fallback")
        ...
        if vix < 8 or vix > 100:
            logger.warning(f"FORTRESS API: VIX {vix} outside normal range, clamping")
        ...
        if spx_price and spx_price > 0:
            ...
            if spx_expected_move <= 0:
                logger.error(f"FORTRESS API: SPX expected move calculation failed, using fallback")
                spx_expected_move = spx_price * (vix / 100) * 0.063
        if spy_price and spy_price > 0:
            spy_expected_move = spy_price * (vix / 100) * math.sqrt(1/252)
            # Validate expected move is reasonable
            if spy_expected_move <= 0:
                logger.error(f"FORTRESS API: SPY expected move calculation failed, using fallback")
        ...
        logger.info(f"FORTRESS API: SPX=${spx_price}, SPY=${spy_price}, VIX={vix}, SPX_EM=${spx_expected_move:.2f}, SPY_EM={spy_expected_move:.2f}, Source={api_source}")
...
async def run_ares_cycle(
    ...
    """
    Manually trigger an FORTRESS V2 trading cycle.
    ...
    if not fortress:
        raise HTTPException(
            status_code=503,
            detail="FORTRESS not initialized. Wait for scheduled startup."
        )
    ...
    try:
        ...
        return {
            "success": True,
            "data": result,
            "message": "FORTRESS V2 cycle completed"
        }
    except Exception as e:
...
@router.get("/config")
async def get_fortress_config():
    """
    Get FORTRESS configuration parameters.
    ...
    default_config = {
        ...
        "description": "FORTRESS (Aggressive Iron Condor) trades daily 0DTE Iron Condors targeting 10% monthly returns through premium collection."
    }

    if fortress and fortress.config:
...
async def sync_tradier_positions(
    ...
    if not fortress:
        return {
            "success": False,
            "error": "FORTRESS not initialized",
            "data": None
        }

    try:
        result = fortress.sync_positions_from_tradier()
    ...
    if not fortress:
        return {
            ...
            "data": {
                ...
                "positions": [],
                "orders": [],
                "errors": ["FORTRESS not initialized"]
            }
        }

    try:
        result = fortress.get_tradier_account_status()
...
async def check_tradier_connection():
    ...
    This endpoint checks if FORTRESS can connect to Tradier API directly,
    without needing the FORTRESS trading bot to be running.
...
async def get_ares_live_pnl():
    ...
    if not fortress:
        # FORTRESS not running - read from database
        try:
            conn = get_connection()
            cursor = conn.cursor()
            ...
            for row in open_rows:
                ...
                if MTM_AVAILABLE and exp and put_short and put_long and call_short and call_long:
                    try:
                        ...
                        logger.debug(f"FORTRESS live-pnl MTM failed for {pos_id}: {e}")
        ...
        return {
            ...
            "data": {
                ...
                "positions": [],
                "position_count": 0,
                "message": "FORTRESS not initialized"
            }
        }

    # Check if fortress has get_live_pnl method
    if not hasattr(fortress, 'get_live_pnl'):
        # Method not available on this trader version - fall back to database
        logger.debug("FORTRESS trader doesn't have get_live_pnl method, using database fallback")
        try:
            conn = get_connection()
            cursor = conn.cursor()
    ...
    try:
        live_pnl = fortress.get_live_pnl()
        ...
        logger.warning(f"FORTRESS get_live_pnl attribute error: {e}")
        return {
            "success": True,
            "data": {
                "total_unrealized_pnl": None,
        ...
        logger.error(f"Error getting FORTRESS live P&L: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def process_expired_positions(
    ...
    try:
        result = fortress.process_expired_positions()
...
async def skip_ares_today(
    ...
    This will prevent FORTRESS from opening any new positions until tomorrow.
    ...
    try:
        ...
        today = datetime.now(ZoneInfo("America/Chicago")).date()
        fortress.skip_date = today

        return {
            "success": True,
            "message": f"FORTRESS will skip trading for {today.isoformat()}",
            "data": {
...
@router.post("/config")
async def update_fortress_config(
    updates: FortressConfigUpdate,
    request: Request,
    auth: AuthInfo = Depends(require_admin) if AUTH_AVAILABLE and require_admin else None
):
    """
    Update FORTRESS configuration parameters.
    ...
    try:
        ...
        for field, value in update_data.items():
            if hasattr(fortress.config, field):
                setattr(fortress.config, field, value)
                updated[field] = value
        ...
        return {
            ...
            "data": {
                "updated": updated,
                "current_config": {
                    "risk_per_trade_pct": fortress.config.risk_per_trade_pct,
                    "sd_multiplier": fortress.config.sd_multiplier,
                    "spread_width": fortress.config.spread_width,
                    "min_credit_per_spread": fortress.config.min_credit_per_spread,
                    "max_contracts": fortress.config.max_contracts,
                    "use_stop_loss": fortress.config.use_stop_loss,
                    "profit_target_pct": fortress.config.profit_target_pct
                }
            }
        }
        ...
        logger.error(f"Error updating FORTRESS config: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def set_strategy_preset(
    ...
    try:
        ...
        fortress.config.apply_strategy_preset(preset_id)
        ...
        return {
            ...
            "data": {
                ...
                "current_config": {
                    "vix_hard_skip": fortress.config.vix_hard_skip,
                    "vix_monday_friday_skip": fortress.config.vix_monday_friday_skip,
                    "vix_streak_skip": fortress.config.vix_streak_skip,
                    "risk_per_trade_pct": fortress.config.risk_per_trade_pct,
                    "sd_multiplier": fortress.config.sd_multiplier
                }
            }
...
async def get_fortress_scan_activity(
    date: str = None,
    outcome: str = None,
    limit: int = 50
):
    """
    Get FORTRESS scan activity with full decision context.
    ...
    This is the key endpoint for understanding FORTRESS behavior.
    """
    try:
        ...
        logger.error(f"Error getting FORTRESS scan activity: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/scan-activity/today")
async def get_fortress_scan_activity_today():
    """Get all FORTRESS scans from today with summary."""
    today = datetime.now(ZoneInfo("America/Chicago")).strftime('%Y-%m-%d')
    return await get_fortress_scan_activity(date=today, limit=200)


@router.post("/reset")
async def reset_fortress_data(confirm: bool = False):
    """
    Reset FORTRESS trading data - delete all positions and start fresh.

    Args:
        confirm: Must be True to actually delete data (safety check)

    WARNING: This will permanently delete ALL FORTRESS trading history.
    """
    if not confirm:
        ...
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM fortress_positions WHERE status IN ('closed', 'expired', 'partial_close')")
            closed_count = cursor.fetchone()[0]
            conn.close()

            return {
                "success": False,
                "message": "Set confirm=true to reset FORTRESS data. This action cannot be undone.",
                "preview": {
    ...
    try:
        ...
        # Delete all FORTRESS positions
        cursor.execute("DELETE FROM fortress_positions")
        deleted_positions = cursor.rowcount

        # Also delete FORTRESS daily performance
        deleted_performance = 0
        try:
            cursor.execute("DELETE FROM fortress_daily_perf")
            deleted_performance = cursor.rowcount
        except Exception:
            pass

        # Also delete FORTRESS scan activity logs
        deleted_scans = 0
        try:
            cursor.execute("DELETE FROM fortress_scan_activity")
        ...
        logger.info(f"FORTRESS reset complete: {deleted_positions} positions, {deleted_performance} performance records, {deleted_scans} scan logs, {deleted_config} config entries deleted")

        return {
            "success": True,
            "message": "FORTRESS data has been reset successfully",
            "deleted": {
        ...
        logger.error(f"Error resetting FORTRESS data: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_ares_diagnostics():
    ...
    This endpoint checks if FORTRESS can actually execute trades in Tradier sandbox.
    ...
    # Get execution status from FORTRESS instance if running
    execution_status = None
    if fortress and hasattr(fortress, 'executor') and hasattr(fortress.executor, 'get_execution_status'):
        execution_status = fortress.executor.get_execution_status()
    ...
    if execution_status:
        if not execution_status.get('can_execute'):
            ...
            can_execute = True
    elif fortress is None:
        issues.append("FORTRESS trader not running in this process - cannot verify execution capability")
    ...
    return {
        ...
        "data": {
            ...
            "message": "FORTRESS is ready to execute trades in Tradier sandbox" if can_execute and not issues else f"FORTRESS cannot execute trades: {'; '.join(issues)}"
        }
    }
...
async def get_ares_intraday_diagnostics():
    ...
    Checks:
    - If fortress_equity_snapshots table exists
    - Number of snapshots today and total
    - Most recent snapshot data
    - fortress_positions table columns
    - Comparison with SAMSON snapshots
    """
    CENTRAL_TZ = ZoneInfo("America/Chicago")
    now = datetime.now(CENTRAL_TZ)
    today = now.strftime('%Y-%m-%d')
    ...
    try:
        ...
        # Check if fortress_equity_snapshots table exists
        cursor.execute("""
            SELECT EXISTS (
        ...
        if not ares_table_exists:
            ...
            """)
            result["fortress"]["columns"] = [{"name": r[0], "type": r[1]} for r in cursor.fetchall()]

            # Count total snapshots
            cursor.execute("SELECT COUNT(*) FROM fortress_equity_snapshots")
            result["fortress"]["total_snapshots"] = cursor.fetchone()[0]
            ...
            cursor.execute("""
                ...
                WHERE DATE(timestamp::timestamptz AT TIME ZONE 'America/Chicago') = %s
            """, (today,))
            result["fortress"]["today_snapshots"] = cursor.fetchone()[0]

            if result["fortress"]["today_snapshots"] == 0:
                result["issues"].append("No FORTRESS snapshots saved today")
            ...
            row = cursor.fetchone()
            if row:
                result["fortress"]["latest_snapshot"] = {
                ...
                result["fortress"]["latest_snapshot"] = None
                result["issues"].append("No snapshots ever saved for FORTRESS")

        # Check fortress_positions table
        cursor.execute("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables
        ...
        if ares_pos_exists:
            ...
            if missing_cols:
                result["issues"].append(f"fortress_positions missing columns: {missing_cols}")

            # Count positions
            cursor.execute("SELECT COUNT(*) FROM fortress_positions WHERE status = 'open'")
            result["fortress"]["open_positions"] = cursor.fetchone()[0]

            cursor.execute("SELECT COUNT(*) FROM fortress_positions WHERE status IN ('closed', 'expired', 'partial_close')")
            result["fortress"]["closed_positions"] = cursor.fetchone()[0]

            cursor.execute("SELECT COALESCE(SUM(realized_pnl), 0) FROM fortress_positions WHERE status IN ('closed', 'expired', 'partial_close')")
            result["fortress"]["total_realized_pnl"] = float(cursor.fetchone()[0] or 0)
        ...
        result["samson"]["table_exists"] = titan_table_exists

        if titan_table_exists:
            cursor.execute("SELECT COUNT(*) FROM samson_equity_snapshots")
            result["samson"]["total_snapshots"] = cursor.fetchone()[0]

            cursor.execute("""
                SELECT COUNT(*) FROM samson_equity_snapshots
                WHERE DATE(timestamp::timestamptz AT TIME ZONE 'America/Chicago') = %s
            """, (today,))
            result["samson"]["today_snapshots"] = cursor.fetchone()[0]

            cursor.execute("""
                SELECT timestamp, balance FROM samson_equity_snapshots
                ORDER BY timestamp DESC LIMIT 1
            """)
            row = cursor.fetchone()
            if row:
                result["samson"]["latest_snapshot"] = {
                    "timestamp": row[0].isoformat() if row[0] else None,
                    "balance": float(row[1]) if row[1] else 0
                }
