"""
AGAPE-DOGE-PERP API Routes - DOGE Perpetual Contract Trading Bot endpoints.

AGAPE-DOGE-PERP trades DOGE perpetual contracts using crypto market
...
router = APIRouter(prefix="/api/agape-doge-perp", tags=["AGAPE-DOGE-PERP"])
...
get_agape_doge_perp_trader = None
try:
    from trading.agape_doge_perp.trader import AgapeDogePerpTrader, get_agape_doge_perp_trader, create_agape_doge_perp_trader
    AGAPE_DOGE_PERP_AVAILABLE = True
    logger.info("AGAPE-DOGE-PERP Routes: AgapeDogePerpTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-DOGE-PERP Routes: AgapeDogePerpTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-DOGE-PERP trader instance."""
    if not AGAPE_DOGE_PERP_AVAILABLE:
        return None
    trader = get_agape_doge_perp_trader()
    if trader is None:
        try:
            trader = create_agape_doge_perp_trader()
        except Exception as e:
            logger.error(f"AGAPE-DOGE-PERP Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-DOGE-PERP bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-DOGE-PERP trader not initialized",
            "message": "AGAPE-DOGE-PERP module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-DOGE-PERP status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-DOGE-PERP positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-DOGE-PERP not available"}
    ...
    try:
        ...
        return {
            ...
            "data": positions,
            "count": len(positions),
            "current_doge_price": current_price,
            "fetched_at": _format_ct(),
        }
    except Exception as e:
        logger.error(f"AGAPE-DOGE-PERP positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-DOGE-PERP closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_doge_perp_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        cursor.execute("""
            SELECT
                ...
                realized_pnl,
                position_id
            FROM agape_doge_perp_positions
        ...
        logger.error(f"AGAPE-DOGE-PERP equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_doge_perp_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            SELECT timestamp, equity, unrealized_pnl,
                   realized_pnl_cumulative, open_positions, doge_price
            FROM agape_doge_perp_equity_snapshots
        ...
        if open_rows:
            # Get current DOGE price from latest snapshot or trader
            current_doge_price = None
            if snapshots:
                current_doge_price = float(snapshots[-1][5]) if snapshots[-1][5] else None

            if not current_doge_price:
                # Try from trader
                trader = _get_trader()
                if trader:
                    try:
                        current_doge_price = trader.executor.get_current_price()
                    except Exception:
                        pass

            if current_doge_price:
                for pos_row in open_rows:
                    side = pos_row[1]
                    quantity = float(pos_row[2])
                    entry_price = float(pos_row[3])
                    direction = 1 if side == 'long' else -1
                    pnl = (current_doge_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        for snapshot in snapshots:
            ts, balance, snap_unrealized, snap_realized_cum, open_count, doge_price = snapshot
        ...
        logger.error(f"AGAPE-DOGE-PERP intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-DOGE-PERP performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-DOGE-PERP performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-DOGE-PERP activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-DOGE-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-DOGE-PERP logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-DOGE-PERP scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-DOGE-PERP scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    symbol: str = Query(default="DOGE", description="Crypto symbol"),
):
    """Get current crypto market microstructure snapshot for DOGE.
    ...
    try:
        ...
        logger.error(f"AGAPE-DOGE-PERP snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-DOGE-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-DOGE-PERP signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for DOGE perpetuals.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-DOGE-PERP.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-DOGE-PERP: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-DOGE-PERP uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        "Actual gamma exposure calculated from Deribit DOGE options. "
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-DOGE-PERP uses contrarian logic."
                    ),
...
async def get_margin_analysis():
    """Margin analysis for AGAPE-DOGE-PERP open positions."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-DOGE-PERP not available"}
    ...
    try:
        ...
        spec = PERPETUAL_MARGIN_SPECS.get("DOGE-PERP", {})
        positions = trader.db.get_open_positions()
        try:
            current_price = trader.executor.get_current_price()
        except Exception:
        ...
        for pos in positions:
            ...
            if not pos_price:
                continue
            leverage = pos.get("leverage_at_entry") or spec.get("default_leverage", 5)
            result = MarginCalculator.calculate_perpetual_margin(
                ...
                maintenance_margin_rate=spec.get("maintenance_margin_rate", 0.01),
        ...
        summary["spec"] = {
            "name": spec.get("name", "DOGE Perpetual"),
            "default_leverage": spec.get("default_leverage", 5),
            "max_leverage": spec.get("max_leverage", 20),
            "maintenance_margin_rate": spec.get("maintenance_margin_rate", 0.01),
            "funding_interval_hours": spec.get("funding_interval_hours", 8),
        }
        ...
        logger.error(f"AGAPE-DOGE-PERP margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-DOGE-PERP trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-DOGE-PERP not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-DOGE-PERP enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-DOGE-PERP trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-DOGE-PERP not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-DOGE-PERP disabled"}
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for DOGE charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("DOGE"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-DOGE-PERP chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for DOGE.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("DOGE")
        if not snap:
        ...
        logger.error(f"AGAPE-DOGE-PERP brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
