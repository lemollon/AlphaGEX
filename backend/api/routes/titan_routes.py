...
async def get_titan_intraday_equity(date: str = None):
    ...
    try:
        ...
        if is_today and is_weekday and is_market_hours:
            cursor.execute("""
                SELECT MAX(timestamp) FROM titan_equity_snapshots
            ...
            if not last_snap_time:
                ...
                logger.info("TITAN intraday: No snapshots today, auto-saving first snapshot")
            else:
                ...
                if minutes_since_last >= 4:
                    should_auto_save = True
                    logger.info(f"TITAN intraday: Last snapshot {minutes_since_last:.1f} min ago, auto-saving")

            if should_auto_save:
                try:
                    cursor.execute("""
                        SELECT COALESCE(SUM(realized_pnl), 0)
                        FROM titan_positions WHERE status IN ('closed', 'expired')
                    """)
                    auto_realized = float(cursor.fetchone()[0] or 0)

                    # Get open positions for MTM
                    cursor.execute("""
                        SELECT position_id, total_credit, contracts, spread_width,
                               put_short_strike, put_long_strike, call_short_strike, call_long_strike, expiration
                        FROM titan_positions WHERE status = 'open'
                    """)
                    ...
                    if auto_open_positions and MTM_AVAILABLE:
                        mtm_result = _calculate_titan_unrealized_pnl(auto_open_positions)
                    ...
                    conn.commit()
                    logger.info(f"TITAN intraday: Auto-saved snapshot equity=${auto_equity:.2f}")
                except Exception as auto_err:
                    logger.warning(f"TITAN intraday: Auto-save failed: {auto_err}")
