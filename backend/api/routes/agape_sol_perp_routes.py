"""
AGAPE-SOL-PERP API Routes - SOL Perpetual Contract Trading Bot endpoints.

AGAPE-SOL-PERP trades SOL perpetual contracts using crypto market
...
router = APIRouter(prefix="/api/agape-sol-perp", tags=["AGAPE-SOL-PERP"])
...
get_agape_sol_perp_trader = None
try:
    from trading.agape_sol_perp.trader import AgapeSolPerpTrader, get_agape_sol_perp_trader, create_agape_sol_perp_trader
    AGAPE_SOL_PERP_AVAILABLE = True
    logger.info("AGAPE-SOL-PERP Routes: AgapeSolPerpTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-SOL-PERP Routes: AgapeSolPerpTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-SOL-PERP trader instance."""
    if not AGAPE_SOL_PERP_AVAILABLE:
        return None
    trader = get_agape_sol_perp_trader()
    if trader is None:
        try:
            trader = create_agape_sol_perp_trader()
        except Exception as e:
            logger.error(f"AGAPE-SOL-PERP Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-SOL-PERP bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-SOL-PERP trader not initialized",
            "message": "AGAPE-SOL-PERP module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-SOL-PERP positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-SOL-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_sol_perp_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        logger.error(f"AGAPE-SOL-PERP equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_sol_perp_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            SELECT timestamp, equity, unrealized_pnl,
                   realized_pnl_cumulative, open_positions, sol_price
            FROM agape_sol_perp_equity_snapshots
        ...
        if open_rows:
            # Get current SOL price from latest snapshot or trader
            current_sol_price = None
            if snapshots:
                current_sol_price = float(snapshots[-1][5]) if snapshots[-1][5] else None

            if not current_sol_price:
                # Try from trader
                trader = _get_trader()
                if trader:
                    try:
                        current_sol_price = trader.executor.get_current_price()
                    except Exception:
                        pass
            ...
            if current_sol_price:
                for pos_row in open_rows:
                    ...
                    pnl = (current_sol_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        for snapshot in snapshots:
            ts, balance, snap_unrealized, snap_realized_cum, open_count, sol_price = snapshot
        ...
        logger.error(f"AGAPE-SOL-PERP intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-SOL-PERP performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-SOL-PERP activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-SOL-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-SOL-PERP scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    symbol: str = Query(default="SOL", description="Crypto symbol"),
):
    """Get current crypto market microstructure snapshot for SOL.
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-SOL-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-SOL-PERP signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for SOL perpetuals.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-SOL-PERP.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-SOL-PERP: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-SOL-PERP uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        "Actual gamma exposure calculated from Deribit SOL options. "
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-SOL-PERP uses contrarian logic."
                    ),
...
async def get_margin_analysis():
    ...
    try:
        ...
        spec = PERPETUAL_MARGIN_SPECS.get("SOL-PERP", {})
        positions = trader.db.get_open_positions()
        try:
            current_price = trader.executor.get_current_price()
        except Exception:
        ...
        starting_capital = getattr(trader.config, "starting_capital", 5000.0)
        ...
        summary["spec"] = {
            "name": spec.get("name", "SOL Perpetual"),
        ...
        logger.error(f"AGAPE-SOL-PERP margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-SOL-PERP trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-SOL-PERP not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-SOL-PERP enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-SOL-PERP trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-SOL-PERP not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-SOL-PERP disabled"}


@router.post("/force-close-all")
async def force_close_all(reason: str = "MANUAL_CLOSE", limit: int = 0):
    """Force-close open AGAPE-SOL-PERP positions at current mark.
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for SOL charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("SOL"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-SOL-PERP chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for SOL.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("SOL")
        if not snap:
        ...
        logger.error(f"AGAPE-SOL-PERP brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
