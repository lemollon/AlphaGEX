"""
ORACLE Dashboard API Routes
============================

API endpoints for the Oracle ML Advisory System.
...
try:
    ...
    logger.info("Oracle advisor system loaded")
...
class TrainRequest(BaseModel):
    """Request to trigger Oracle training"""
...
async def oracle_health():
    ...
    if not ORACLE_AVAILABLE:
        return {
            "status": "unavailable",
            "message": "Oracle system not loaded"
        }
...
async def oracle_status():
    """
    Get detailed Oracle status including training metrics.
    """
    if not ORACLE_AVAILABLE:
    ...
    try:
        ...
        if oracle.training_metrics:
            result["training_metrics"] = {
                "accuracy": oracle.training_metrics.accuracy,
                "precision": oracle.training_metrics.precision,
                "recall": oracle.training_metrics.recall,
                "f1_score": oracle.training_metrics.f1_score,
                "auc_roc": oracle.training_metrics.auc_roc,
                "total_samples": oracle.training_metrics.total_samples,
                "training_date": oracle.training_metrics.training_date
            }
...
async def get_strategy_performance(
    ...
    try:
        oracle = get_oracle()
        results = oracle.analyze_strategy_performance(days=days)
...
async def trigger_training(request: TrainRequest):
    """
    Trigger Oracle model training.

    Training uses:
    1. Live outcomes from oracle_training_outcomes table (preferred)
