...
API endpoints for the GRACE 1DTE Paper Iron Condor bot.
Clone of FAITH routes targeting 1DTE for side-by-side comparison.

Endpoints:
- GET  /api/grace/status              - Bot status and configuration
- GET  /api/grace/positions           - Current open positions
- GET  /api/grace/trades              - Trade history (closed positions)
- GET  /api/grace/performance         - P&L, win rate, statistics
- GET  /api/grace/pdt-status          - PDT dashboard data
- GET  /api/grace/paper-account       - Paper account balance/collateral
- GET  /api/grace/position-monitor    - Live position monitoring data
- GET  /api/grace/equity-curve        - Historical equity curve
- GET  /api/grace/equity-curve/intraday - Today's intraday equity curve
- GET  /api/grace/logs                - Activity logs
- POST /api/grace/toggle              - Enable/disable bot
- POST /api/grace/run-cycle           - Manually trigger a scan cycle
"""
...
def _get_trader():
    """Get or create the GRACE trader singleton."""
    global _grace_trader
    if _grace_trader is None:
        try:
            from trading.grace.trader import GraceTrader
            _grace_trader = GraceTrader()
            logger.info("GRACE: Trader initialized via API")
        except Exception as e:
            logger.error(f"GRACE: Failed to initialize trader: {e}")
            raise HTTPException(
                status_code=503,
                detail=f"GRACE bot unavailable: {e}"
            )
    return _grace_trader


@router.get("/api/grace/status")
async def get_grace_status():
    """Get comprehensive GRACE bot status."""
    try:
        trader = _get_trader()
        ...
        logger.error(f"GRACE status error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/positions")
async def get_grace_positions():
    ...
    try:
        ...
        logger.error(f"GRACE positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/trades")
async def get_grace_trades(limit: int = 50):
    ...
    try:
        ...
        logger.error(f"GRACE trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/performance")
async def get_grace_performance():
    ...
    try:
        ...
        logger.error(f"GRACE performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/pdt-status")
async def get_grace_pdt_status():
    ...
    try:
        ...
        from trading.grace.models import CENTRAL_TZ
        ...
        logger.error(f"GRACE PDT status error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/paper-account")
async def get_grace_paper_account():
    ...
    try:
        ...
        logger.error(f"GRACE paper account error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/position-monitor")
async def get_grace_position_monitor():
    ...
    try:
        ...
        logger.error(f"GRACE position monitor error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/equity-curve")
async def get_grace_equity_curve():
    ...
    try:
        ...
        logger.error(f"GRACE equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/api/grace/equity-curve/intraday")
async def get_grace_intraday_equity(date: str = None):
    """Get GRACE intraday equity curve with snapshots."""
    ...
    try:
        ...
        cursor.execute("""
            SELECT value FROM autonomous_config WHERE key = 'grace_starting_capital'
        """)
        row = cursor.fetchone()
        if row and row[0]:
            try:
        ...
        # Get today's closed trades with timestamps
        cursor.execute("""
        ...
        logger.error(f"GRACE intraday equity error: {e}")
        import traceback
        traceback.print_exc()
        return {
            "success": False,
...
@router.get("/api/grace/logs")
async def get_grace_logs(limit: int = 100):
    """Get activity logs."""
    try:
        trader = _get_trader()
        ...
        logger.error(f"GRACE logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/api/grace/toggle")
async def toggle_grace(active: bool = True):
    """Enable or disable the GRACE bot."""
    try:
        trader = _get_trader()
        ...
        logger.error(f"GRACE toggle error: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/api/grace/run-cycle")
async def run_grace_cycle(close_only: bool = False):
    """Manually trigger a GRACE scan/trade cycle."""
    try:
        trader = _get_trader()
        ...
        logger.error(f"GRACE run cycle error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
