...
async def get_pegasus_intraday_equity(date: str = None):
    ...
    try:
        ...
        try:
            ...
            for row in open_rows:
                pos_id, total_credit, contracts, spread_width, put_short, call_short = row
                ...
                if spx_price and spx_price > 0 and put_short and call_short:
                    ...
                    if put_short < spx_price < call_short:
                        ...
                        current_value = credit_val * max(0.1, 0.5 - factor * 0.3)
                    elif spx_price <= put_short:
                        # Below put short - losing money
                        intrinsic = put_short - spx_price
                        current_value = min(spread_w, intrinsic + credit_val * 0.2)
                    else:
                        # Above call short - losing money
                        intrinsic = spx_price - call_short
                        current_value = min(spread_w, intrinsic + credit_val * 0.2)

                    current_unrealized = (credit_val - current_value) * 100 * num_contracts
