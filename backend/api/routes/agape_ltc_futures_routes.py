"""
AGAPE-LTC-FUTURES API Routes - LTC Futures Contract Trading Bot endpoints.

AGAPE-LTC-FUTURES trades LTC-FUT futures contracts using crypto market
...
router = APIRouter(prefix="/api/agape-ltc-futures", tags=["AGAPE-LTC-FUTURES"])
...
get_agape_ltc_futures_trader = None
try:
    from trading.agape_ltc_futures.trader import AgapeLtcFuturesTrader, get_agape_ltc_futures_trader, create_agape_ltc_futures_trader
    AGAPE_LTC_FUTURES_AVAILABLE = True
    logger.info("AGAPE-LTC-FUTURES Routes: AgapeLtcFuturesTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-LTC-FUTURES Routes: AgapeLtcFuturesTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-LTC-FUTURES trader instance."""
    if not AGAPE_LTC_FUTURES_AVAILABLE:
        return None
    trader = get_agape_ltc_futures_trader()
    if trader is None:
        try:
            trader = create_agape_ltc_futures_trader()
        except Exception as e:
            logger.error(f"AGAPE-LTC-FUTURES Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-LTC-FUTURES bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-LTC-FUTURES trader not initialized",
            "message": "AGAPE-LTC-FUTURES module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-LTC-FUTURES status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-LTC-FUTURES positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-LTC-FUTURES not available"}
    ...
    try:
        ...
        return {
            ...
            "data": positions,
            "count": len(positions),
            "current_ltc_price": current_price,
            "fetched_at": _format_ct(),
        }
    except Exception as e:
        logger.error(f"AGAPE-LTC-FUTURES positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-LTC-FUTURES closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_ltc_futures_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        cursor.execute("""
            SELECT
                ...
                realized_pnl,
                position_id
            FROM agape_ltc_futures_positions
        ...
        logger.error(f"AGAPE-LTC-FUTURES equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_ltc_futures_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            SELECT timestamp, equity, unrealized_pnl,
                   realized_pnl_cumulative, open_positions, ltc_price
            FROM agape_ltc_futures_equity_snapshots
        ...
        if open_rows:
            # Get current LTC price from latest snapshot or trader
            current_ltc_price = None
            if snapshots:
                current_ltc_price = float(snapshots[-1][5]) if snapshots[-1][5] else None

            if not current_ltc_price:
                # Try from trader
                trader = _get_trader()
                if trader:
                    try:
                        current_ltc_price = trader.executor.get_current_price()
                    except Exception:
                        pass

            if current_ltc_price:
                for pos_row in open_rows:
                    side = pos_row[1]
                    quantity = float(pos_row[2])
                    entry_price = float(pos_row[3])
                    direction = 1 if side == 'long' else -1
                    pnl = (current_ltc_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        for snapshot in snapshots:
            ts, balance, snap_unrealized, snap_realized_cum, open_count, ltc_price = snapshot
        ...
        logger.error(f"AGAPE-LTC-FUTURES intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-LTC-FUTURES performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-LTC-FUTURES performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-LTC-FUTURES activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-LTC-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-LTC-FUTURES logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-LTC-FUTURES scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-LTC-FUTURES scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    symbol: str = Query(default="LTC", description="Crypto symbol"),
):
    """Get current crypto market microstructure snapshot for LTC.
    ...
    try:
        ...
        logger.error(f"AGAPE-LTC-FUTURES snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-LTC-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-LTC-FUTURES signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for LTC futures.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-LTC-FUTURES.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-LTC-FUTURES: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-LTC-FUTURES uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        "Actual gamma exposure calculated from Deribit LTC options. "
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-LTC-FUTURES uses contrarian logic."
                    ),
            ...
            "trade_instrument": {
                "symbol": "LTC-FUT (Futures)",
                "exchange": "Coinbase Derivatives FCM",
                "type": "Monthly Futures Contract",
                "contract_size": "1 contract = 50 LTC underlying",
...
async def get_margin_analysis():
    """Margin analysis for AGAPE-LTC-FUTURES open positions."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-LTC-FUTURES not available"}
    ...
    try:
        ...
        spec = PERPETUAL_MARGIN_SPECS.get("LTC-FUT", {})
        positions = trader.db.get_open_positions()
        try:
            current_price = trader.executor.get_current_price()
        except Exception:
        ...
        summary["spec"] = {
            "name": spec.get("name", "LTC Futures"),
        ...
        logger.error(f"AGAPE-LTC-FUTURES margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-LTC-FUTURES trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-LTC-FUTURES not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-LTC-FUTURES enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-LTC-FUTURES trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-LTC-FUTURES not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-LTC-FUTURES disabled"}


@router.post("/force-close-all")
async def force_close_all(reason: str = "MANUAL_CLOSE", limit: int = 0):
    """Force-close open AGAPE-LTC-FUTURES positions at current mark.
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for LTC charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("LTC"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-LTC-FUTURES chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for LTC.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("LTC")
        if not snap:
        ...
        logger.error(f"AGAPE-LTC-FUTURES brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
