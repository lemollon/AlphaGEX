...
async def get_athena_intraday_equity(date: str = None):
    ...
    try:
        ...
        if is_today and is_weekday and is_market_hours:
            cursor.execute("""
                SELECT MAX(timestamp) FROM athena_equity_snapshots
            ...
            if not last_snap_time:
                ...
                logger.info("ATHENA intraday: No snapshots today, auto-saving first snapshot")
            else:
                ...
                if minutes_since_last >= 4:
                    should_auto_save = True
                    logger.info(f"ATHENA intraday: Last snapshot {minutes_since_last:.1f} min ago, auto-saving")

            if should_auto_save:
                try:
                    cursor.execute("""
                        SELECT COALESCE(SUM(realized_pnl), 0)
                        FROM athena_positions WHERE status IN ('closed', 'expired')
                    """)
                    ...
                    cursor.execute("""
                        SELECT position_id, spread_type, entry_debit, contracts,
                               long_strike, short_strike, max_profit, max_loss, expiration
                        FROM athena_positions WHERE status = 'open'
                    """)
                    ...
                    if auto_open_positions:
                        pnl_result = _calculate_athena_unrealized_pnl(auto_open_positions, None)
                    ...
                    conn.commit()
                    logger.info(f"ATHENA intraday: Auto-saved snapshot equity=${auto_equity:.2f}")
                except Exception as auto_err:
                    logger.warning(f"ATHENA intraday: Auto-save failed: {auto_err}")
