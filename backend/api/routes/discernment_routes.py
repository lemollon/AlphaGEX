"""
DISCERNMENT API Routes - AI-Powered Live Options Scanner

Endpoints:
- POST /api/discernment/scan - Scan symbols with ML predictions
- GET /api/discernment/scan/{scan_id} - Get scan results
- GET /api/discernment/history - Get scan history
- POST /api/discernment/feedback - Record outcome for learning
- GET /api/discernment/performance - Get model performance stats
- GET /api/discernment/live-quote/{symbol} - Get live Tradier quote
- GET /api/discernment/options-chain/{symbol} - Get live options chain
"""
...
_discernment_tradier_instance = None
...
def get_tradier():
    ...
    """
    global _discernment_tradier_instance

    if _discernment_tradier_instance is not None:
        return _discernment_tradier_instance
    ...
    try:
        ...
        if api_key and account_id:
            _discernment_tradier_instance = TradierDataFetcher(
                api_key=api_key,
                account_id=account_id,
                sandbox=True
            )
            logger.info("DISCERNMENT: Tradier initialized with credentials")
            return _discernment_tradier_instance

        logger.error("DISCERNMENT: No Tradier credentials configured")
        return None
    except Exception as e:
        logger.error(f"DISCERNMENT: Failed to initialize Tradier: {e}")
        return None
...
def ensure_discernment_tables():
    """Create DISCERNMENT tables if they don't exist"""
    ...
    try:
        ...
        # DISCERNMENT Outcomes table (for learning)
        c.execute('''
        ...
        # DISCERNMENT Model Performance table
        c.execute('''
        ...
        # DISCERNMENT Live Quotes cache
        c.execute('''
            CREATE TABLE IF NOT EXISTS discernment_live_quotes (
                id SERIAL PRIMARY KEY,
                symbol VARCHAR(20) NOT NULL,
                timestamp TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
                bid REAL,
        ...
        # DISCERNMENT Pin Risk History table
        c.execute('''
            CREATE TABLE IF NOT EXISTS discernment_pin_risk_history (
                id SERIAL PRIMARY KEY,
                symbol VARCHAR(20) NOT NULL,
                timestamp TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
                spot_price REAL,
        ...
        # Indexes
        c.execute("CREATE INDEX IF NOT EXISTS idx_discernment_scans_timestamp ON discernment_scans(timestamp)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_discernment_predictions_symbol ON discernment_predictions(symbol)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_discernment_pin_risk_symbol ON discernment_pin_risk_history(symbol)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_discernment_pin_risk_timestamp ON discernment_pin_risk_history(timestamp)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_discernment_predictions_scan ON discernment_predictions(scan_id)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_discernment_outcomes_symbol ON discernment_outcomes(symbol)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_discernment_outcomes_prediction ON discernment_outcomes(prediction_id)")

        conn.commit()
        logger.info("✅ DISCERNMENT tables created/verified")

    except Exception as e:
        logger.error(f"Failed to create DISCERNMENT tables: {e}")
    finally:
        conn.close()


# Initialize tables on import
ensure_discernment_tables()
...
def save_scan_result(scan_id: str, symbols: List[str], results: List[Dict], vix: float, duration_ms: int):
    ...
    try:
        ...
        c.execute('''
            INSERT INTO discernment_scans (scan_id, symbols, results, market_regime, vix_at_scan, scan_duration_ms)
            VALUES (%s, %s, %s, %s, %s, %s)
...
def save_pin_risk_analysis(analysis_data: Dict):
    ...
    try:
        ...
        c.execute('''
            INSERT INTO discernment_pin_risk_history (
...
def save_prediction(prediction_data: Dict, scan_id: str):
    ...
    try:
        ...
        c.execute('''
            INSERT INTO discernment_predictions (
...
@router.post("/scan")
async def discernment_scan(request: ScanRequest):
    """
    Perform DISCERNMENT scan on symbols.
    ...
    try:
        ...
        logger.error(f"DISCERNMENT engine not available: {e}")
        raise HTTPException(status_code=500, detail="DISCERNMENT ML engine not available")
    except Exception as e:
...
async def get_scan_results(scan_id: str):
    ...
    try:
        ...
        c.execute('''
            ...
            FROM discernment_scans WHERE scan_id = %s
        ''', (scan_id,))
...
async def record_feedback(request: FeedbackRequest):
    ...
    try:
        ...
        c.execute('''
            INSERT INTO discernment_outcomes (
...
async def get_model_performance():
    """Get DISCERNMENT model performance statistics"""
    conn = get_db_connection()
    if not conn:
        raise HTTPException(status_code=500, detail="Database unavailable")
    ...
    try:
        ...
        c.execute("SELECT COUNT(*) FROM discernment_predictions WHERE timestamp > NOW() - INTERVAL '30 days'")
        total_predictions = c.fetchone()[0]

        # Get outcomes count
        c.execute("SELECT COUNT(*) FROM discernment_outcomes WHERE timestamp > NOW() - INTERVAL '30 days'")
...
async def get_pin_risk_history(
    ...
    try:
        ...
        c.execute('''
            ...
            FROM discernment_pin_risk_history
            WHERE symbol = %s
...
async def trigger_outcome_tracking(
    ...
    """
    Manually trigger Discernment outcome tracking.
    ...
    This endpoint was added to fix the issue where Discernment performance 30-day
    metrics weren't showing any data because outcomes were never tracked.
    """
    try:
        from core.discernment_outcome_tracker import track_discernment_outcomes

        results = track_discernment_outcomes(
...
async def get_tracking_status():
    """
    Get Discernment outcome tracking status.
    ...
    try:
        ...
        # Total predictions
        c.execute("SELECT COUNT(*) FROM discernment_predictions")
        total_predictions = c.fetchone()[0]
        ...
        c.execute("""
            SELECT COUNT(*)
            FROM discernment_predictions p
            LEFT JOIN discernment_outcomes o ON p.prediction_id = o.prediction_id
            WHERE o.id IS NULL
