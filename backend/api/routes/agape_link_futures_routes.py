"""
AGAPE-LINK-FUTURES API Routes - LINK Futures Contract Trading Bot endpoints.

AGAPE-LINK-FUTURES trades LINK-FUT futures contracts using crypto market
...
router = APIRouter(prefix="/api/agape-link-futures", tags=["AGAPE-LINK-FUTURES"])
...
get_agape_link_futures_trader = None
try:
    from trading.agape_link_futures.trader import AgapeLinkFuturesTrader, get_agape_link_futures_trader, create_agape_link_futures_trader
    AGAPE_LINK_FUTURES_AVAILABLE = True
    logger.info("AGAPE-LINK-FUTURES Routes: AgapeLinkFuturesTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-LINK-FUTURES Routes: AgapeLinkFuturesTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-LINK-FUTURES trader instance."""
    if not AGAPE_LINK_FUTURES_AVAILABLE:
        return None
    trader = get_agape_link_futures_trader()
    if trader is None:
        try:
            trader = create_agape_link_futures_trader()
        except Exception as e:
            logger.error(f"AGAPE-LINK-FUTURES Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-LINK-FUTURES bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-LINK-FUTURES trader not initialized",
            "message": "AGAPE-LINK-FUTURES module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-LINK-FUTURES status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-LINK-FUTURES positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-LINK-FUTURES not available"}
    ...
    try:
        ...
        return {
            ...
            "data": positions,
            "count": len(positions),
            "current_link_price": current_price,
            "fetched_at": _format_ct(),
        }
    except Exception as e:
        logger.error(f"AGAPE-LINK-FUTURES positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-LINK-FUTURES closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_link_futures_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        cursor.execute("""
            SELECT
                ...
                realized_pnl,
                position_id
            FROM agape_link_futures_positions
        ...
        logger.error(f"AGAPE-LINK-FUTURES equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_link_futures_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            SELECT timestamp, equity, unrealized_pnl,
                   realized_pnl_cumulative, open_positions, link_price
            FROM agape_link_futures_equity_snapshots
        ...
        if open_rows:
            # Get current LINK price from latest snapshot or trader
            current_link_price = None
            if snapshots:
                current_link_price = float(snapshots[-1][5]) if snapshots[-1][5] else None

            if not current_link_price:
                # Try from trader
                trader = _get_trader()
                if trader:
                    try:
                        current_link_price = trader.executor.get_current_price()
                    except Exception:
                        pass

            if current_link_price:
                for pos_row in open_rows:
                    side = pos_row[1]
                    quantity = float(pos_row[2])
                    entry_price = float(pos_row[3])
                    direction = 1 if side == 'long' else -1
                    pnl = (current_link_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        # Add market open point (futures trade 24/7, use 00:00 for "day start")
        ...
        for snapshot in snapshots:
            ts, balance, snap_unrealized, snap_realized_cum, open_count, link_price = snapshot
        ...
        logger.error(f"AGAPE-LINK-FUTURES intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-LINK-FUTURES performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-LINK-FUTURES performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-LINK-FUTURES activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-LINK-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-LINK-FUTURES logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-LINK-FUTURES scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-LINK-FUTURES scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    symbol: str = Query(default="LINK", description="Crypto symbol"),
):
    """Get current crypto market microstructure snapshot for LINK.
    ...
    try:
        ...
        logger.error(f"AGAPE-LINK-FUTURES snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-LINK-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-LINK-FUTURES signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for LINK futures.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-LINK-FUTURES.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-LINK-FUTURES: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-LINK-FUTURES uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        "Actual gamma exposure calculated from Deribit LINK options. "
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-LINK-FUTURES uses contrarian logic."
                    ),
            ...
            "trade_instrument": {
                "symbol": "LINK-FUT (Futures)",
                "exchange": "Coinbase Derivatives FCM",
                "type": "Monthly Futures Contract",
                "contract_size": "1 contract = 100 LINK underlying",
...
async def get_margin_analysis():
    """Margin analysis for AGAPE-LINK-FUTURES open positions."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-LINK-FUTURES not available"}
    ...
    try:
        ...
        spec = PERPETUAL_MARGIN_SPECS.get("LINK-FUT", {})
        positions = trader.db.get_open_positions()
        try:
            current_price = trader.executor.get_current_price()
        except Exception:
        ...
        summary["spec"] = {
            "name": spec.get("name", "LINK Futures"),
        ...
        logger.error(f"AGAPE-LINK-FUTURES margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-LINK-FUTURES trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-LINK-FUTURES not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-LINK-FUTURES enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-LINK-FUTURES trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-LINK-FUTURES not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-LINK-FUTURES disabled"}


@router.post("/force-close-all")
async def force_close_all(reason: str = "MANUAL_CLOSE", limit: int = 0):
    """Force-close open AGAPE-LINK-FUTURES positions at current mark.
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for LINK charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("LINK"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-LINK-FUTURES chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for LINK.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("LINK")
        if not snap:
        ...
        logger.error(f"AGAPE-LINK-FUTURES brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
