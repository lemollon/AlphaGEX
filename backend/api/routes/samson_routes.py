"""
SAMSON Aggressive SPX Iron Condor Bot API Routes
=================================================
...
router = APIRouter(prefix="/api/samson", tags=["SAMSON"])
...
try:
    ...
    MTM_AVAILABLE = True
    logger.info("Mark-to-market utility loaded for SAMSON")
...
try:
    ...
    logger.info("TradierDataFetcher loaded for SAMSON")
except ImportError as e:
...
if not TRADIER_AVAILABLE:
    try:
        ...
        if tradier_path.exists():
            ...
            TRADIER_AVAILABLE = True
            logger.info(f"TradierDataFetcher loaded via direct import for SAMSON")
    except Exception as e:
        logger.warning(f"Direct Tradier import failed for SAMSON: {e}")
...
# Try to import SAMSON trader
samson_trader = None
try:
    from trading.samson import SamsonTrader, SamsonConfig, TradingMode, StrategyPreset
...
def get_samson_instance():
    ...
    if samson_trader:
        return samson_trader

    try:
        from scheduler.trader_scheduler import get_samson_trader
        samson_trader = get_samson_trader()
        return samson_trader
    except ImportError as e:
        logger.debug(f"Could not import trader_scheduler: {e}")
        return None
    except Exception as e:
        logger.debug(f"Could not get SAMSON trader: {e}")
        return None


def _get_tradier_account_balance() -> dict:
    """Get account balance from Tradier API for SAMSON."""
    ...
    try:
        ...
        logger.info(f"SAMSON Tradier: mode=PRODUCTION (SPX requires prod)")
        ...
        logger.error(f"SAMSON Tradier balance fetch ERROR: {e}")
...
@router.get("/status")
async def get_samson_status():
    """
    Get current SAMSON bot status.
    ...
    if not samson:
        # SAMSON not running - read stats from database
        # Get starting capital from config table (NOT hardcoded)
        starting_capital = 200000  # Default for SAMSON (SPX bot)
        try:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT value FROM autonomous_config WHERE key = 'samson_starting_capital'")
        ...
        try:
            ...
            cursor.execute('''
                SELECT
                    ...
                    SUM(CASE WHEN DATE(open_time::timestamptz AT TIME ZONE 'America/Chicago') = %s THEN 1 ELSE 0 END) as trades_today
            ...
            logger.debug(f"Could not read SAMSON stats from database: {db_err}")
        ...
        # SAMSON trading window: 8:30 AM - 2:45 PM CT (market closes at 3:00 PM CT)
        entry_start = "08:30"
        ...
        return {
            ...
            "data": {
                ...
                "config": {
                    "risk_per_trade": 15.0,  # SAMSON: higher risk
                    "spread_width": 12.0,     # SAMSON: wider spreads
                    "sd_multiplier": 0.8,     # SAMSON: closer strikes
                    "ticker": "SPX",
                    "profit_target_pct": 30,  # SAMSON: faster exit
    ...
    try:
        ...
        logger.error(f"Error getting SAMSON status: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/positions")
async def get_samson_positions():
    """
    Get SAMSON open and recently closed positions.
    ...
    try:
        positions = samson.get_positions()
        open_positions = []
        for pos in positions:
            dte = 0
            if pos.expiration:
        ...
        logger.error(f"Error getting SAMSON positions: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/equity-curve")
async def get_samson_equity_curve(days: int = 30):
    """
    Get SAMSON equity curve data including unrealized P&L from open positions.
...
async def get_samson_intraday_equity(date: str = None):
    ...
    try:
        ...
        cursor.execute("""
            SELECT value FROM autonomous_config WHERE key = 'samson_starting_capital'
        """)
        row = cursor.fetchone()
        if row and row[0]:
            try:
        ...
        try:
            ...
            if open_rows:
                ...
                logger.debug(f"SAMSON intraday: unrealized=${unrealized_pnl:.2f} via {pricing_method}")
        except Exception as e:
        ...
        logger.error(f"Error getting SAMSON intraday equity: {e}")
        import traceback
        traceback.print_exc()
        return {
            "success": False,
...
async def save_samson_equity_snapshot():
    """
    ...
    try:
        ...
        logger.error(f"Error saving SAMSON equity snapshot: {e}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def run_samson_cycle(
    ...
    """
    Manually trigger a SAMSON trading cycle.
    ...
    if not samson:
        raise HTTPException(
            status_code=503,
            detail="SAMSON not initialized. Wait for scheduled startup."
        )

    try:
        result = samson.run_cycle()

        return {
            "success": True,
            "data": result,
            "message": "SAMSON cycle completed"
        }
    except Exception as e:
        logger.error(f"Error running SAMSON cycle: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_samson_config():
    ...
    default_config = {
        ...
        "description": "SAMSON is an aggressive SPX Iron Condor bot with $12 spreads, multiple trades per day."
    }

    if samson and hasattr(samson, 'config'):
        config = samson.config
        return {
            "success": True,
            "data": {
                "ticker": "SPX",
...
async def force_close_samson_positions(
    request: Request,
    auth: AuthInfo = Depends(require_admin) if AUTH_AVAILABLE and require_admin else None
):
    """
    Force close all open SAMSON positions.
    ...
    if not samson:
        raise HTTPException(
            status_code=503,
            detail="SAMSON not initialized."
        )

    try:
        result = samson.force_close_all("MANUAL")
        ...
        logger.error(f"Error force closing SAMSON positions: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_samson_live_pnl():
    ...
    if not samson:
        try:
            ...
            for row in open_rows:
                ...
                if MTM_AVAILABLE and exp and put_short and put_long and call_short and call_long:
                    try:
                        ...
                        logger.debug(f"SAMSON live-pnl MTM failed for {pos_id}: {e}")
            ...
            logger.warning(f"Could not read SAMSON live P&L from database: {db_err}")
        ...
        return {
            ...
            "data": {
                ...
                "positions": [],
                "position_count": 0,
                "message": "SAMSON not initialized"
            }
        }
    ...
    try:
        ...
        logger.error(f"Error getting SAMSON live P&L: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/logs")
async def get_samson_logs(
    ...
    """Get SAMSON logs for debugging and monitoring."""
    ...
    try:
        ...
        logger.error(f"Error getting SAMSON logs: {e}")
        return {
            "success": True,
            "data": [],
            "count": 0,
...
async def get_samson_performance(
    days: int = Query(30, description="Number of days to include")
):
    """Get SAMSON performance metrics over time."""
    ...
    try:
        ...
        logger.error(f"Error getting SAMSON performance: {e}")
        return {
            "success": True,
            "data": {
                "summary": {
...
@router.post("/reset")
async def reset_samson_data(
    request: Request,
    auth: AuthInfo = Depends(require_admin) if AUTH_AVAILABLE and require_admin else None
):
    """Reset all SAMSON data (positions, signals, logs)."""
    try:
        conn = get_connection()
        c = conn.cursor()

        # Delete all SAMSON data
        c.execute("DELETE FROM samson_positions")
        c.execute("DELETE FROM samson_signals")
        c.execute("DELETE FROM samson_logs")
        c.execute("DELETE FROM samson_daily_perf")
        ...
        return {
            "success": True,
            "message": "All SAMSON data has been reset"
        }

    except Exception as e:
        logger.error(f"Error resetting SAMSON data: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/cleanup-open-positions")
async def cleanup_samson_open_positions(confirm: bool = False):
    """
    Clean up open SAMSON positions without affecting closed trade history.
    ...
    try:
        ...
        cursor.execute("DELETE FROM samson_positions WHERE status = 'open'")
        ...
        try:
            cursor.execute("""
                DELETE FROM samson_equity_snapshots
        ...
        logger.info(f"SAMSON cleanup: Deleted {deleted_count} open positions and {deleted_snapshots} today's snapshots")
        ...
        logger.error(f"Error cleaning up SAMSON positions: {e}")
        raise HTTPException(status_code=500, detail=str(e))
