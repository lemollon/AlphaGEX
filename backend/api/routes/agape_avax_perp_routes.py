"""
AGAPE-AVAX-PERP API Routes - AVAX Perpetual Contract Trading Bot endpoints.

AGAPE-AVAX-PERP trades AVAX perpetual contracts using crypto market
...
router = APIRouter(prefix="/api/agape-avax-perp", tags=["AGAPE-AVAX-PERP"])
...
get_agape_avax_perp_trader = None
try:
    from trading.agape_avax_perp.trader import AgapeAvaxPerpTrader, get_agape_avax_perp_trader, create_agape_avax_perp_trader
    AGAPE_AVAX_PERP_AVAILABLE = True
    logger.info("AGAPE-AVAX-PERP Routes: AgapeAvaxPerpTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-AVAX-PERP Routes: AgapeAvaxPerpTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-AVAX-PERP trader instance."""
    if not AGAPE_AVAX_PERP_AVAILABLE:
        return None
    trader = get_agape_avax_perp_trader()
    if trader is None:
        try:
            trader = create_agape_avax_perp_trader()
        except Exception as e:
            logger.error(f"AGAPE-AVAX-PERP Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-AVAX-PERP bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-AVAX-PERP trader not initialized",
            "message": "AGAPE-AVAX-PERP module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-AVAX-PERP status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-AVAX-PERP positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-AVAX-PERP not available"}
    ...
    try:
        ...
        return {
            ...
            "data": positions,
            "count": len(positions),
            "current_avax_price": current_price,
            "fetched_at": _format_ct(),
        }
    except Exception as e:
        logger.error(f"AGAPE-AVAX-PERP positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-AVAX-PERP closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_avax_perp_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        logger.error(f"AGAPE-AVAX-PERP equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_avax_perp_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            SELECT timestamp, equity, unrealized_pnl,
                   realized_pnl_cumulative, open_positions, avax_price
            FROM agape_avax_perp_equity_snapshots
        ...
        if open_rows:
            # Get current AVAX price from latest snapshot or trader
            current_avax_price = None
            if snapshots:
                current_avax_price = float(snapshots[-1][5]) if snapshots[-1][5] else None

            if not current_avax_price:
                # Try from trader
                trader = _get_trader()
                if trader:
                    try:
                        current_avax_price = trader.executor.get_current_price()
                    except Exception:
                        pass

            if current_avax_price:
                for pos_row in open_rows:
                    side = pos_row[1]
                    quantity = float(pos_row[2])
                    entry_price = float(pos_row[3])
                    direction = 1 if side == 'long' else -1
                    pnl = (current_avax_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        for snapshot in snapshots:
            ts, balance, snap_unrealized, snap_realized_cum, open_count, avax_price = snapshot
        ...
        logger.error(f"AGAPE-AVAX-PERP intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-AVAX-PERP performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-AVAX-PERP performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-AVAX-PERP activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-AVAX-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-AVAX-PERP logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-AVAX-PERP scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-AVAX-PERP scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    symbol: str = Query(default="AVAX", description="Crypto symbol"),
):
    """Get current crypto market microstructure snapshot for AVAX.
    ...
    try:
        ...
        logger.error(f"AGAPE-AVAX-PERP snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-AVAX-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-AVAX-PERP signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for AVAX perpetuals.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-AVAX-PERP.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-AVAX-PERP: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-AVAX-PERP uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        "Actual gamma exposure calculated from Deribit AVAX options. "
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-AVAX-PERP uses contrarian logic."
                    ),
...
async def get_margin_analysis():
    """Margin analysis for AGAPE-AVAX-PERP open positions.

    Returns margin usage, liquidation prices, effective leverage,
    funding cost projections, and per-position breakdown using
    AVAX-PERP perpetual contract margin mechanics.
    """
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-AVAX-PERP not available"}
    ...
    try:
        ...
        spec = PERPETUAL_MARGIN_SPECS.get("AVAX-PERP", {})
        positions = trader.db.get_open_positions()
        try:
            current_price = trader.executor.get_current_price()
        except Exception:
        ...
        starting_capital = getattr(trader.config, "starting_capital", 2500.0)
        ...
        summary["spec"] = {
            "name": spec.get("name", "AVAX Perpetual"),
            "default_leverage": spec.get("default_leverage", 10),
            "max_leverage": spec.get("max_leverage", 50),
            "maintenance_margin_rate": spec.get("maintenance_margin_rate", 0.004),
            "funding_interval_hours": spec.get("funding_interval_hours", 8),
        }
        ...
        logger.error(f"AGAPE-AVAX-PERP margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-AVAX-PERP trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-AVAX-PERP not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-AVAX-PERP enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-AVAX-PERP trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-AVAX-PERP not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-AVAX-PERP disabled"}


@router.post("/force-close-all")
async def force_close_all(reason: str = "MANUAL_CLOSE", limit: int = 0):
    """Force-close open AGAPE-AVAX-PERP positions at current mark.
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for AVAX charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("AVAX"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-AVAX-PERP chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for AVAX.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("AVAX")
        if not snap:
        ...
        logger.error(f"AGAPE-AVAX-PERP brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
