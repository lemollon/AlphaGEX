...
get_agape_shib_futures_trader = None
try:
    from trading.agape_shib_futures.trader import AgapeShibFuturesTrader, get_agape_shib_futures_trader, create_agape_shib_futures_trader
    AGAPE_SHIB_FUTURES_AVAILABLE = True
    logger.info("AGAPE-SHIB-FUTURES Routes: AgapeShibFuturesTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-SHIB-FUTURES Routes: AgapeShibFuturesTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-SHIB-FUTURES trader instance."""
    if not AGAPE_SHIB_FUTURES_AVAILABLE:
        return None
    trader = get_agape_shib_futures_trader()
    if trader is None:
        try:
            trader = create_agape_shib_futures_trader()
        except Exception as e:
            logger.error(f"AGAPE-SHIB-FUTURES Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-SHIB-FUTURES bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-SHIB-FUTURES trader not initialized",
            "message": "AGAPE-SHIB-FUTURES module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-SHIB-FUTURES positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-SHIB-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_shib_futures_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_shib_futures_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            ...
            FROM agape_shib_futures_equity_snapshots
        ...
        logger.error(f"AGAPE-SHIB-FUTURES intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-SHIB-FUTURES performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-SHIB-FUTURES activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-SHIB-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-SHIB-FUTURES scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-SHIB-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for SHIB futures.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-SHIB-FUTURES.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-SHIB-FUTURES: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-SHIB-FUTURES uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-SHIB-FUTURES uses contrarian logic."
                    ),
...
async def get_margin_analysis():
    """Margin analysis for AGAPE-SHIB-FUTURES open positions."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-SHIB-FUTURES not available"}
    ...
    try:
        ...
        spec = PERPETUAL_MARGIN_SPECS.get("1000SHIB-FUT", {})
        positions = trader.db.get_open_positions()
        try:
            current_price = trader.executor.get_current_price()
        except Exception:
        ...
        summary["spec"] = {
            "name": spec.get("name", "SHIB Futures"),
        ...
        logger.error(f"AGAPE-SHIB-FUTURES margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-SHIB-FUTURES trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-SHIB-FUTURES not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-SHIB-FUTURES enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-SHIB-FUTURES trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-SHIB-FUTURES not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-SHIB-FUTURES disabled"}


@router.post("/force-close-all")
async def force_close_all(reason: str = "MANUAL_CLOSE", limit: int = 0):
    """Force-close open AGAPE-SHIB-FUTURES positions at current mark.
...
async def get_chart_data():
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
# agape_perp_signal_briefs (shared with SHIB-PERP — same SHIB snapshot).
# This route never calls Claude.
# ------------------------------------------------------------------
@router.get("/brief")
async def get_signal_brief():
    ...
    try:
        ...
        logger.error(f"AGAPE-SHIB-FUTURES brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
