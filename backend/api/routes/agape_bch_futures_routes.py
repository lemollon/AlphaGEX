"""
AGAPE-BCH-FUTURES API Routes - BCH Futures Contract Trading Bot endpoints.

AGAPE-BCH-FUTURES trades BCH-FUT futures contracts using crypto market
...
router = APIRouter(prefix="/api/agape-bch-futures", tags=["AGAPE-BCH-FUTURES"])
...
get_agape_bch_futures_trader = None
try:
    from trading.agape_bch_futures.trader import AgapeBchFuturesTrader, get_agape_bch_futures_trader, create_agape_bch_futures_trader
    AGAPE_BCH_FUTURES_AVAILABLE = True
    logger.info("AGAPE-BCH-FUTURES Routes: AgapeBchFuturesTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-BCH-FUTURES Routes: AgapeBchFuturesTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-BCH-FUTURES trader instance."""
    if not AGAPE_BCH_FUTURES_AVAILABLE:
        return None
    trader = get_agape_bch_futures_trader()
    if trader is None:
        try:
            trader = create_agape_bch_futures_trader()
        except Exception as e:
            logger.error(f"AGAPE-BCH-FUTURES Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-BCH-FUTURES bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-BCH-FUTURES trader not initialized",
            "message": "AGAPE-BCH-FUTURES module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-BCH-FUTURES status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-BCH-FUTURES positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-BCH-FUTURES not available"}
    ...
    try:
        ...
        return {
            ...
            "data": positions,
            "count": len(positions),
            "current_bch_price": current_price,
            "fetched_at": _format_ct(),
        }
    except Exception as e:
        logger.error(f"AGAPE-BCH-FUTURES positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-BCH-FUTURES closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_bch_futures_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        logger.error(f"AGAPE-BCH-FUTURES equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_bch_futures_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            SELECT timestamp, equity, unrealized_pnl,
                   realized_pnl_cumulative, open_positions, bch_price
            FROM agape_bch_futures_equity_snapshots
        ...
        if open_rows:
            # Get current BCH price from latest snapshot or trader
            current_bch_price = None
            if snapshots:
                current_bch_price = float(snapshots[-1][5]) if snapshots[-1][5] else None

            if not current_bch_price:
                # Try from trader
                trader = _get_trader()
                if trader:
                    try:
                        current_bch_price = trader.executor.get_current_price()
                    except Exception:
                        pass

            if current_bch_price:
                for pos_row in open_rows:
                    side = pos_row[1]
                    quantity = float(pos_row[2])
                    entry_price = float(pos_row[3])
                    direction = 1 if side == 'long' else -1
                    pnl = (current_bch_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        for snapshot in snapshots:
            ts, balance, snap_unrealized, snap_realized_cum, open_count, bch_price = snapshot
        ...
        logger.error(f"AGAPE-BCH-FUTURES intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-BCH-FUTURES performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-BCH-FUTURES performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-BCH-FUTURES activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-BCH-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-BCH-FUTURES logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-BCH-FUTURES scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-BCH-FUTURES scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    symbol: str = Query(default="BCH", description="Crypto symbol"),
):
    """Get current crypto market microstructure snapshot for BCH.
    ...
    try:
        ...
        logger.error(f"AGAPE-BCH-FUTURES snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-BCH-FUTURES not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-BCH-FUTURES signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for BCH futures.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-BCH-FUTURES.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-BCH-FUTURES: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-BCH-FUTURES uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        "Actual gamma exposure calculated from Deribit BCH options. "
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-BCH-FUTURES uses contrarian logic."
                    ),
            ...
            "trade_instrument": {
                "symbol": "BCH-FUT (Futures)",
                "exchange": "Coinbase Derivatives FCM",
                "type": "Monthly Futures Contract",
                "contract_size": "1 contract = 25 units of BCH index = 25 BCH underlying",
...
async def get_margin_analysis():
    """Margin analysis for AGAPE-BCH-FUTURES open positions."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-BCH-FUTURES not available"}
    ...
    try:
        ...
        spec = PERPETUAL_MARGIN_SPECS.get("BCH-FUT", {})
        positions = trader.db.get_open_positions()
        try:
            current_price = trader.executor.get_current_price()
        except Exception:
        ...
        summary["spec"] = {
            "name": spec.get("name", "BCH Futures"),
        ...
        logger.error(f"AGAPE-BCH-FUTURES margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-BCH-FUTURES trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-BCH-FUTURES not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-BCH-FUTURES enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-BCH-FUTURES trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-BCH-FUTURES not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-BCH-FUTURES disabled"}


@router.post("/force-close-all")
async def force_close_all(reason: str = "MANUAL_CLOSE", limit: int = 0):
    """Force-close open AGAPE-BCH-FUTURES positions at current mark.
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for BCH charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("BCH"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-BCH-FUTURES chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for BCH.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("BCH")
        if not snap:
        ...
        logger.error(f"AGAPE-BCH-FUTURES brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
