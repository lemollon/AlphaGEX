"""
AGAPE-BTC API Routes - BTC Micro Futures Trading Bot endpoints.
...
router = APIRouter(prefix="/api/agape-btc", tags=["AGAPE-BTC"])
...
AgapeBtcTrader = None
get_agape_btc_trader = None
try:
    from trading.agape_btc.trader import AgapeBtcTrader, get_agape_btc_trader, create_agape_btc_trader
    AGAPE_BTC_AVAILABLE = True
    logger.info("AGAPE-BTC Routes: AgapeBtcTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-BTC Routes: AgapeBtcTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-BTC trader instance."""
    if not AGAPE_BTC_AVAILABLE:
        return None
    trader = get_agape_btc_trader()
    if trader is None:
        try:
            trader = create_agape_btc_trader()
        except Exception as e:
            logger.error(f"AGAPE-BTC Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-BTC bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-BTC trader not initialized",
            "message": "AGAPE-BTC module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-BTC positions with unrealized P&L.
    ...
    - Current unrealized P&L (contract_size=0.1 BTC for /MBT)
    """
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-BTC not available"}

    try:
        positions = trader.db.get_open_positions()
        current_price = trader.executor.get_current_price()

        # Add unrealized P&L - /MBT contract size is 0.1 BTC
        ...
        logger.error(f"AGAPE-BTC positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        # Get starting capital from config (key/value schema with agape_btc_ prefix)
        starting_capital = 5000.0
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_btc_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        logger.error(f"AGAPE-BTC equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_btc_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            ...
            FROM agape_btc_equity_snapshots
        ...
        # AGAPE-BTC: P&L = (current - entry) * 0.1 * contracts * direction (/MBT = 0.1 BTC)
        unrealized_pnl = 0.0
        ...
        if open_rows:
            ...
            if current_btc_price:
                for pos_row in open_rows:
                    ...
                    pnl = (current_btc_price - entry_price) * 0.1 * contracts * direction
                    unrealized_pnl += pnl
        ...
        logger.error(f"AGAPE-BTC intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-BTC performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-BTC scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    try:
        ...
        logger.error(f"AGAPE-BTC signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for BTC.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-BTC.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-BTC: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-BTC uses crypto market microstructure signals as equivalents "
                "to the GEX-based analysis used by equity bots (FORTRESS, WATCHTOWER, etc.) "
                "applied to Bitcoin via Micro Bitcoin Futures (/MBT)."
            ),
            "mappings": [
                {
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-BTC uses contrarian logic."
                    ),
            ...
            "trade_instrument": {
                ...
                "exchange": "CME Globex",
                "broker": "tastytrade",
                "contract_size": "0.1 BTC",
                "tick_value": "$0.50",
                "margin": "~$1,200-2,500 per contract",
            },
        },
    }
...
async def enable_bot():
    """Enable AGAPE-BTC trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-BTC not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-BTC enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-BTC trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-BTC not available")
    trader.disable()
    return {"success": True, "message": "AGAPE-BTC disabled"}
