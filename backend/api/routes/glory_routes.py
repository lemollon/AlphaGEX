"""
GLORY (Weekly Gamma) API Routes - Enhanced Version
======================================================

API endpoints for weekly options gamma visualization.
GLORY focuses on stocks/ETFs with weekly options (not 0DTE).

Named after the Samson of Watchfulness - watching longer-term gamma setups.

Enhanced Features (matching WATCHTOWER):
...
router = APIRouter(prefix="/api/glory", tags=["GLORY"])
...
def load_gamma_history(symbol: str):
    ...
    try:
        ...
        _history_loaded[symbol] = True
        logger.info(f"GLORY: Loaded gamma history for {symbol}")
...
def persist_alert(alert: Alert, symbol: str):
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES ('GLORY', %s, %s, %s, %s, %s, %s, %s, %s, %s)
...
def persist_danger_zone(symbol: str, strike: float, danger_type: str,
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES ('GLORY', %s, %s, %s, %s, %s, %s, %s, NOW())
    ...
    try:
        ...
        if status == 'SPIKE':
            cursor.execute("""
                ...
                WHERE system = 'GLORY' AND symbol = %s
...
def cleanup_old_data():
    ...
    try:
        ...
        if deleted > 0:
            logger.debug(f"GLORY: Cleaned up {deleted} old history entries")
    except Exception as e:
    ...
    try:
        ...
        if sandbox_key and sandbox_account:
            try:
                ...
                logger.warning(f"GLORY: Sandbox credentials failed: {e}")
        ...
        if prod_key and prod_account:
            try:
                ...
                logger.warning(f"GLORY: Production credentials failed: {e}")

        logger.error("GLORY: No valid Tradier credentials found")
        return None

    except ImportError as e:
        logger.error(f"GLORY: Failed to import unified_config: {e}")
        return None
    except Exception as e:
        logger.error(f"GLORY: Failed to get Tradier fetcher: {e}")
        return None
...
async def fetch_gamma_data(symbol: str, expiration: str) -> dict:
    ...
    if not tradier:
        logger.warning("GLORY: Tradier API not available")
        return {
            'symbol': symbol,
    ...
    try:
        ...
        if len(contracts) == 0:
            logger.warning(f"GLORY: No options data for {symbol} expiration {expiration}")
            return {
                'symbol': symbol,
        ...
        logger.error(f"Error fetching GLORY gamma data: {e}")
        import traceback
        traceback.print_exc()
        return {
            'symbol': symbol,
...
async def get_glory_gamma(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY gamma data: {e}")
...
async def get_glory_expirations(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY expirations: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_alerts(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY alerts: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_patterns(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY patterns: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        ...
        if conn:
            ...
            cursor.execute("""
                ...
                WHERE system = 'GLORY' AND symbol = %s AND trend_date = %s
        ...
        logger.error(f"Error getting GLORY strike trends: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_gamma_flips(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY gamma flips: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/danger-zones/log")
async def get_glory_danger_zone_logs(
    symbol: str = Query("AAPL", description="Stock/ETF symbol"),
    ...
    try:
        ...
        logger.error(f"Error getting GLORY danger zone logs: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_context(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY context: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_accuracy(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY accuracy: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_weekly_setups(
    ...
    """
    Weekly Setup Scanner for GLORY.
    ...
    try:
        ...
        logger.error(f"Error getting GLORY weekly setups: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_gamma_trend(
    ...
    """
    Multi-Day Gamma Trend for GLORY.
    ...
    try:
        ...
        logger.error(f"Error getting GLORY gamma trend: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_glory_opex_analysis(
    ...
    try:
        ...
        logger.error(f"Error getting GLORY OPEX analysis: {e}")
        raise HTTPException(status_code=500, detail=str(e))
