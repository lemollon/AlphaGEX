"""
AGAPE-XRP-PERP API Routes - XRP Perpetual Contract Trading Bot endpoints.

AGAPE-XRP-PERP trades XRP perpetual contracts using crypto market
...
router = APIRouter(prefix="/api/agape-xrp-perp", tags=["AGAPE-XRP-PERP"])
...
get_agape_xrp_perp_trader = None
try:
    from trading.agape_xrp_perp.trader import AgapeXrpPerpTrader, get_agape_xrp_perp_trader, create_agape_xrp_perp_trader
    AGAPE_XRP_PERP_AVAILABLE = True
    logger.info("AGAPE-XRP-PERP Routes: AgapeXrpPerpTrader loaded")
except ImportError as e:
    logger.warning(f"AGAPE-XRP-PERP Routes: AgapeXrpPerpTrader not available: {e}")
...
def _get_trader():
    """Get or lazily create the AGAPE-XRP-PERP trader instance."""
    if not AGAPE_XRP_PERP_AVAILABLE:
        return None
    trader = get_agape_xrp_perp_trader()
    if trader is None:
        try:
            trader = create_agape_xrp_perp_trader()
        except Exception as e:
            logger.error(f"AGAPE-XRP-PERP Routes: Failed to create trader: {e}")
    return trader
...
async def get_status():
    """Get AGAPE-XRP-PERP bot status, configuration, and open positions.
    ...
    if not trader:
        return {
            ...
            "reason": "AGAPE-XRP-PERP trader not initialized",
            "message": "AGAPE-XRP-PERP module not available",
        }
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP status error: {e}")
...
async def get_positions():
    """Get all open AGAPE-XRP-PERP positions with unrealized P&L.
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-XRP-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP positions error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_closed_trades(
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP closed trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_equity_curve(
    ...
    try:
        ...
        try:
            cursor.execute(
                "SELECT value FROM autonomous_config WHERE key = 'agape_xrp_perp_starting_capital'"
            )
            row = cursor.fetchone()
            if row and row[0]:
        ...
        logger.error(f"AGAPE-XRP-PERP equity curve error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        if conn:
            conn.close()
...
async def get_equity_curve_intraday(date: Optional[str] = None):
    ...
    try:
        ...
        # Get starting capital from config
        cursor.execute("SELECT value FROM autonomous_config WHERE key = 'agape_xrp_perp_starting_capital'")
        row = cursor.fetchone()
        if row and row[0]:
            try:
                starting_capital = float(row[0])
        ...
        cursor.execute("""
            ...
            FROM agape_xrp_perp_equity_snapshots
        ...
        if open_rows:
            ...
            if current_xrp_price:
                for pos_row in open_rows:
                    ...
                    pnl = (current_xrp_price - entry_price) * quantity * direction
                    unrealized_pnl += pnl
        ...
        logger.error(f"AGAPE-XRP-PERP intraday equity error: {e}")
        import traceback
        traceback.print_exc()
...
async def get_performance():
    """Get AGAPE-XRP-PERP performance statistics.
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP performance error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_logs(
    ...
    """Get AGAPE-XRP-PERP activity log."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "data": [], "message": "AGAPE-XRP-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP logs error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_scan_activity(
    ...
    """Get AGAPE-XRP-PERP scan history - every cycle is logged.
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP scan activity error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_crypto_snapshot(
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP snapshot error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def generate_signal():
    ...
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-XRP-PERP not available"}
    ...
    try:
        ...
        logger.error(f"AGAPE-XRP-PERP signal error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_gex_mapping():
    """Returns the GEX -> Crypto signal mapping reference for XRP perpetuals.

    Educational endpoint showing how equity GEX concepts translate
    to crypto market microstructure signals used by AGAPE-XRP-PERP.
    """
    return {
        "success": True,
        "data": {
            "title": "AGAPE-XRP-PERP: GEX -> Crypto Signal Mapping",
            "description": (
                "AGAPE-XRP-PERP uses crypto market microstructure signals as equivalents "
            ...
            "mappings": [
                ...
                },
                {
                    ...
                    "explanation": (
                        ...
                        "predicting reversals. AGAPE-XRP-PERP uses contrarian logic."
                    ),
...
async def get_margin_analysis():
    """Margin analysis for AGAPE-XRP-PERP open positions."""
    trader = _get_trader()
    if not trader:
        return {"success": False, "message": "AGAPE-XRP-PERP not available"}
    ...
    try:
        ...
        starting_capital = getattr(trader.config, "starting_capital", 9000.0)
        ...
        for pos in positions:
            ...
            result = MarginCalculator.calculate_perpetual_margin(
                ...
                maintenance_margin_rate=spec.get("maintenance_margin_rate", 0.008),
        ...
        summary["spec"] = {
            "name": spec.get("name", "XRP Perpetual"),
            "default_leverage": spec.get("default_leverage", 5),
            "max_leverage": spec.get("max_leverage", 25),
            "maintenance_margin_rate": spec.get("maintenance_margin_rate", 0.008),
            "funding_interval_hours": spec.get("funding_interval_hours", 8),
        }
        ...
        logger.error(f"AGAPE-XRP-PERP margin analysis error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def enable_bot():
    """Enable AGAPE-XRP-PERP trading."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-XRP-PERP not available")
    trader.enable()
    return {"success": True, "message": "AGAPE-XRP-PERP enabled"}


@router.post("/disable")
async def disable_bot():
    """Disable AGAPE-XRP-PERP trading (positions still managed)."""
    trader = _get_trader()
    if not trader:
        raise HTTPException(status_code=503, detail="AGAPE-XRP-PERP not available")
    trader.disable()
...
async def get_chart_data():
    """Return 30 days of h4 history (price + L/S + OI + funding) for XRP charts.
    ...
    try:
        ...
        return {"success": True, "data": _fetch("XRP"), "fetched_at": _format_ct()}
    except Exception as e:
        logger.error(f"AGAPE-XRP-PERP chart-data error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_signal_brief():
    """Return Claude-generated brief explaining what the bot sees for XRP.
    ...
    try:
        ...
        provider = get_crypto_data_provider()
        snap = provider.get_snapshot("XRP")
        if not snap:
        ...
        logger.error(f"AGAPE-XRP-PERP brief error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
