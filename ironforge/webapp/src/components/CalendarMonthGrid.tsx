...
/**
 * Day-of-news halt policy (effective 2026-05-06): a calendar day is in
 * "blackout" iff it IS the event date. The prior multi-day pre-event halt
 * was retired, so we no longer shade the runup days red.
 */
function dayInBlackout(iso: string, ev: CalendarEvent): boolean {
  return iso === ev.event_date
}
    ...
    <div className="bg-forge-card rounded-lg p-3">
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((c, i) => {
          ...
          if (blackout) bg = 'bg-red-800/40'

          const tooltip = blackout
            ? `${blackout.title}\nEvent: ${blackout.event_date} ${blackout.event_time_ct} CT\nResumes: ${new Date(blackout.halt_end_ts).toLocaleString('en-US', { timeZone: 'America/Chicago' })} CT\nSource: ${blackout.source}`
            : ''
