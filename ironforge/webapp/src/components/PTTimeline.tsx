 ...
 *   Morning   8:30 – 10:30   (120 min)
 *   Midday   10:30 – 1:00    (150 min)
 *   Afternoon 1:00 – 2:45    (105 min)
 *
 * Zones (CT) — SPARK (Apr 2026: morning extended to 12pm):
 *   Morning   8:30 – 12:00   (210 min)
 *   Midday   12:00 – 1:00    ( 60 min)
 *   Afternoon 1:00 – 2:45    (105 min)
 *
...
const MIDDAY_START_DEFAULT = 630  // 10:30 AM (FLAME / INFERNO)
const MIDDAY_START_SPARK = 720    // 12:00 PM (SPARK)
...
export default function PTTimeline({ bot }: { bot?: 'flame' | 'spark' | 'inferno' } = {}) {
  const MIDDAY_START = bot === 'spark' ? MIDDAY_START_SPARK : MIDDAY_START_DEFAULT
  const middayLabel =
    bot === 'spark' ? '12:00' : '10:30'
  ...
  return (
    <div className={`rounded-xl border border-forge-border bg-forge-card/60 px-4 py-3 ${grayed ? 'opacity-40' : ''}`}>
      ...
      <div className="relative h-4 text-[9px] text-forge-muted mt-0.5 font-mono">
        ...
        <span className="absolute -translate-x-1/2" style={{ left: pct(MIDDAY_START) }}>{middayLabel}</span>
