...
import { getCTNow, getCTMinutes, getMorningEndMinutes, isMarketOpen } from '@/lib/pt-tiers'

/**
 * Horizontal timeline showing the three PT zones with a real-time "you are here" marker.
 *
 * Zones (CT) — FLAME / default:
 *   Morning  50%  8:30 – 10:30
 *   Midday   30%  10:30 – 1:00
 *   PM       20%  1:00 – 2:45
 *   EOD            2:45 – 3:00
 *
 * SPARK MORNING is extended to 12:00 PM CT — see getMorningEndMinutes.
 */
...
export default function PTTimeline({ bot }: { bot?: 'flame' | 'spark' | 'inferno' }) {
  const MIDDAY_START = getMorningEndMinutes(bot)
  ...
  return (
    <div className={`rounded-xl border border-forge-border bg-forge-card/60 px-4 py-3 ${grayed ? 'opacity-40' : ''}`}>
      ...
      <div className="relative h-4 text-[9px] text-forge-muted mt-0.5 font-mono">
        ...
        <span className="absolute -translate-x-1/2" style={{ left: pct(MIDDAY_START) }}>{minutesToHHMM(MIDDAY_START)}</span>
