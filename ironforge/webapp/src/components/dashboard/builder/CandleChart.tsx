...
export default function CandleChart({
  ...
  const chartData = useMemo(() => {
    ...
    const bars = visibleCandles.map((c, i) => {
      ...
      return { x, centerX, bodyTop, bodyHeight, wickTop, wickBottom, color, volH, volY, volColor: isUp ? '#4ade8044' : '#f8717144', time: c.time }
    })
  ...
  if (strikes) {
    ...
    longPrices.forEach((p) => {
      if (p >= minPrice && p <= maxPrice) strikeLines.push({ y: pToY(p), color: '#4ade80', dash: '5,4', label: `$${p}` })
    })
