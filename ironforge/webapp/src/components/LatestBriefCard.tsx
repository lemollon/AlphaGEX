...
export default function LatestBriefCard({ bot }: { bot: 'flame' | 'spark' | 'inferno' | 'blaze' | 'flare' }) {
  ...
  // use it here and skip the forge_briefings (daily EOD) source.
  const { data: legacyData, error, isLoading } = useSWR<{ brief: Brief | null }>(
    `/api/${bot}/briefs/latest`,
    fetcher,
    { refreshInterval: BRIEF_REFRESH_MS },
  )
  const data: { brief: Brief | null } = legacyData ?? { brief: null }
