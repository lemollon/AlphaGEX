...
/**
 * Daily OHLC history via Tradier `/markets/history` (interval=daily).
 * Returns rows ascending by date as `{ date, close }`, closes > 0 only.
 * Used by FLARE's conviction signal to compute SMA20 / SMA50 of SPY closes.
 * `days` is the calendar lookback; ~100 covers ~70 trading days (enough for SMA50).
 */
export async function getDailyHistory(
  ...
  const data = await tradierGet('/markets/history', { symbol, interval: 'daily', start })
  let series = data?.history?.day
  if (!series) return []
  if (!Array.isArray(series)) series = [series]
  return series
    .map((d: any) => ({ date: String(d.date), close: parseFloat(d.close ?? '0') }))
    .filter((d: { date: string; close: number }) => Number.isFinite(d.close) && d.close > 0)
