...
export function dteMode(bot: string): string | null {
  ...
  // EBB (2026-08-16): both bots run the SPY 0DTE put credit spread, one strategy
  // at two clocks — FLAME 13:05 CT, SPARK 10:05 CT.
  if (bot === 'flame') return '0DTE'
  if (bot === 'spark') return '0DTE'
...
CREATE TABLE IF NOT EXISTS ${bot}_config (
  ...
  eod_cutoff_et TEXT DEFAULT '14:45',  -- Central time (legacy _et name is a misnomer); 2:45 PM CT EOD force-close
...
CREATE TABLE IF NOT EXISTS flare_daily_state (
  trade_date         DATE         PRIMARY KEY,
  wall_fade_count    INTEGER      NOT NULL DEFAULT 0,
  wall_break_count   INTEGER      NOT NULL DEFAULT 0,
  flip_cross_count   INTEGER      NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_flare_daily_state_date ON flare_daily_state(trade_date DESC);
`
...
async function ensureTablesOnce(): Promise<void> {
  ...
  try {
    ...
    for (const bot of ALL_BOTS) {
      ...
      // BLAZE/FLARE: directional debit-vertical columns (additive over the IC schema).
      if (bot === 'blaze' || bot === 'flare') {
        for (const col of [
    ...
    for (const [bot, dte, maxDT, maxPerDay] of [
      ...
      ['flare', '0DTE', 0, 0],    // FLARE: 0DTE directional debit verticals, no PDT cap
    ] as const) {
