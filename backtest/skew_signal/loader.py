...
Two access patterns:
  * load_chain_at_minute() — single (T, exp, M). Fast for single-day smoke tests.
  * load_day_chain() — ALL minutes for a (T, exp) day in one query. Used by the
    engine to avoid 266 round-trips per backtest day; cuts wall-clock by ~100x.
"""
...
def load_day_chain(
    ...
    trade_date: dt.date,
    expiration_date: dt.date,
) -> Dict[int, Dict[float, ChainBar]]:
    """Fetch ALL minute bars for a (trade_date, expiration) pair in one query.

    Returns {minute_idx: {strike: ChainBar}} where minute_idx is the offset
    in minutes from the day's first bar (so minute 5 = 09:35 ET).

    100x faster than calling load_chain_at_minute() per minute over a day:
    one chain query + one OI query covers all 391 bars.
    """
    chain_sql = """
        WITH first_bar AS (
            SELECT MIN(bar_time) AS t0
    ...
    by_minute_strike: Dict[int, Dict[float, dict]] = {}
    for minute_idx, strike, right, bid, ask in rows:
        m = int(minute_idx)
        k = float(strike)
        e = by_minute_strike.setdefault(m, {}).setdefault(k, {
            "call_bid": 0.0, "call_ask": 0.0,
            "put_bid": 0.0, "put_ask": 0.0,
        })
        bid_v = float(bid) if bid is not None else 0.0
    ...
    out: Dict[int, Dict[float, ChainBar]] = {}
    for m, strikes_q in by_minute_strike.items():
        per_strike: Dict[float, ChainBar] = {}
        for k, q in strikes_q.items():
            oi = oi_by_strike.get(k, {"call_oi": 0, "put_oi": 0})
            per_strike[k] = ChainBar(
                strike=k,
                call_bid=q["call_bid"], call_ask=q["call_ask"],
                put_bid=q["put_bid"], put_ask=q["put_ask"],
                call_oi=oi["call_oi"], put_oi=oi["put_oi"],
