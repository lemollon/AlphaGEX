"""
GEXIS Rate Limiter - Protect API from abuse and control costs.
...
class GEXISRateLimiter:
    """
    Rate limiter for GEXIS API requests.
...
gexis_rate_limiter = GEXISRateLimiter()
...
def rate_limited(endpoint: str):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            # Check rate limit
            allowed, retry_after, reason = gexis_rate_limiter.check_rate_limit(
                user_id, endpoint
            )
