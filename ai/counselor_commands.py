"""
COUNSELOR Commands - Extended command set for COUNSELOR.
...
def get_strategy_performance(days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        strategies = {}
        for bot in ["FORTRESS", "SOLOMON", "ANCHOR", "LAZARUS"]:
            try:
                c.execute("""
                    SELECT
                        COUNT(*) as trades,
...
def execute_strategy_performance_command(days: int = 30) -> Dict[str, Any]:
    ...
    cached = counselor_cache.get(cache_key)
    if cached:
        return cached
    ...
    # Cache for 5 minutes
    counselor_cache.set(cache_key, result, ttl=300)
...
def execute_suggestion_command() -> Dict[str, Any]:
    ...
    cached = counselor_cache.get("suggestion:current")
    if cached:
        return cached
    ...
    # Cache for 2 minutes
    counselor_cache.set("suggestion:current", result, ttl=120)
