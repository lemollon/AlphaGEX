"""
GEXIS Tracing and Observability - Enhanced telemetry for GEXIS operations.
...
class GEXISTracer:
    """
    Tracer for GEXIS operations with correlation ID support.

    Usage:
        tracer = GEXISTracer()

        # Context manager for tracing
        with tracer.trace("gexis.command.status") as span:
            span.set_attribute("command", "/status")
            result = execute_status_command()
            span.set_attribute("result_size", len(result))

        # Decorator for functions
        @tracer.traced("gexis.fetch_market_data")
        def fetch_market_data(symbol: str):
            return api.get_data(symbol)
    """

    def __init__(self, service_name: str = "gexis"):
        """
    ...
    def traced(
        ...
        Usage:
            @tracer.traced("gexis.command.status")
    ...
    def traced_async(
        ...
        Usage:
            @tracer.traced_async("gexis.command.status")
...
def trace_command(command_name: str):
    """Decorator for tracing GEXIS commands."""
    return gexis_tracer.traced(f"gexis.command.{command_name}")


def trace_command_async(command_name: str):
    """Async decorator for tracing GEXIS commands."""
    return gexis_tracer.traced_async(f"gexis.command.{command_name}")


def trace_tool(tool_name: str):
    """Decorator for tracing GEXIS tools."""
    return gexis_tracer.traced(f"gexis.tool.{tool_name}")


def trace_api_call(api_name: str):
    """Decorator for tracing external API calls."""
    return gexis_tracer.traced(f"gexis.api.{api_name}")
...
class RequestContext:
    """
    Context for a GEXIS request, providing correlation ID and trace context.
